...
<%@ page  import = "geso.traphaco.erp.beans.erp_chinhanh.IErp_chinhanhList" %>
<%@ page  import = "geso.traphaco.erp.beans.erp_chinhanh.imp.Erp_chinhanhList" %>
<%@ page  import = "geso.traphaco.center.util.*" %>
...
<% 	IErp_chinhanhList cnList = (IErp_chinhanhList)session.getAttribute("cnList");
	ResultSet Rscn = cnList.getRscn();
	 int[] quyen = new  int[5];
		quyen = util.Getquyen("Erp_ChiNhanhSvl","&view=TT&chixem=1",userId);
%>
			...
			<table width="100%" cellpadding="0" cellspacing="1">		
				...
				<tr>
					<td width="100%">
						...
						<%-- <%if(quyen[0]!=0 && cnList.getChixem().equals("0") ){ %> --%>
									...
									<%}%>
										...
										<td align = "center">
											<table border = 0 cellpadding="0" cellspacing="0">
												...
												<td>
													<%if(quyen[2]!=0  ){ %>
													<a href = "../../Erp_ChiNhanhUpdateSvl?userid=<%=userId%>&update=<%=Rscn.getString("id_cn")%>"><img title="Cập nhật" src="../images/Edit20.png" alt="Chinh sua" width="20" height="20" longdesc="Chinh sua" border = 0></A>&nbsp;
													<%} %>
												</td>
												<td>
												<%if(quyen[1]!=0  ){ %>
												<a href = ../../Erp_ChiNhanhSvl?userid=<%=userId%>&delete=<%=Rscn.getString("id_cn")%>><img title="Xóa" src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa Chi nhánh này ?')) return false;"></a>&nbsp;
												<%}%>
												</td>
												<td>
												<!-- QUYEN VIEW DLN --> 
