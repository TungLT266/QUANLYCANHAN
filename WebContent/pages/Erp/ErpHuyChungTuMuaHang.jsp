...
<% IErpHuychungtumuahangList obj = (IErpHuychungtumuahangList)session.getAttribute("obj"); %>
...
<% obj.setNextSplittings(); 
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[5];
		quyen = util.Getquyen("ErpHuychungtuMhSvl","",userId);
...
<body>
<form name="erpHctmhForm" method="post" action="../../ErpHuychungtuMhSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng > Nghiệp vụ khác > Hủy chứng từ
        </div>
            ...
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >								                          
                    ...
                    <TR>
                        ...
                        <TD class="plainlabel">
                            <input type="text"  
                                   id="sochungtu" name="sochungtu" value="<%= obj.getsochungtu() %>" maxlength="40" onchange="submitform()" />
                        </TD>
                    </TR>
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					...
					<%
                 		if(hctmhList != null)
                 		{
		                    ...
		                    <TD align="center" style="display: none"><%= hctmhList.getString("SOCHUNGTUPHATSINH") %></TD>	
		                    <TD align="center">
		                    	<%
		                    		String trangthai = "";
		                    ...
		                    <% if(tt.equals("0")){ %>
                                ...
                                <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%=hctmhList.getString("SOPHIEU")%>'
							       ...
							       border="0" onclick="if(!confirm('Bạn có muốn chốt nhận hàng này?')) {return false ;}else{ processing('<%="chotphieu"+hctmhList.getString("SOPHIEU")%>' , '../../ErpHuychungtuMhUpdateSvl?userId=<%=userId%>&update=<%= hctmhList.getString("SOPHIEU") %>');}"  >
								</A><%} %>
                                <%if(quyen[1]!=0){ %>
                                <A href = "../../ErpHuychungtuMhSvl?userId=<%=userId%>&delete=<%= hctmhList.getString("SOPHIEU") %> %>"><img src="../images/Delete20.png" alt="Xóa hủy chứng từ" title="Xóa hủy chứng từ" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa hủy chứng từ này?')) return false;"></A>									
		                    <%}} else {if(quyen[3]!=0){ %>
		                    	<A href = "../../ErpHuychungtuMhUpdateSvl?userId=<%=userId%>&display=<%= hctmhList.getString("SOPHIEU") %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                    	<% }} %>
		                    </TD>
		                </TR>
                     <% m++; } hctmhList.close(); } %>
