...
<%@ page import="geso.traphaco.erp.beans.giaidoansx.IErpGiaidoanSXList"%>
...
<%	
	...
	quyen = util.GetquyenNew("ErpGiaidoanSXSvl","",userId);
	IErpGiaidoanSXList obj = (IErpGiaidoanSXList)session.getAttribute("obj");
	ResultSet csxRs = obj.getGiaidoanRs();
...
<form name="khtt" method="post" action="../../ErpGiaidoanSXSvl">
            ...
            <TR>
                <TD align="left" class="tbnavigation">
                    <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
                        <TR height="22">
                            <TD align="left" colspan="2" class="tbnavigation">&nbsp;Dữ liệu nền > Sản xuất > Giai đoạn sản xuất  </TD>  
                                        ...
                                        <%}%>
                                                ...
                                                <% if( tt.equals("1") ) { %>
	                                                <% if(quyen[Utility.SUA]!=0){ %>
								                   			<A href = "../../ErpGiaidoanSXUpdateSvl?userId=<%=userId%>&update=<%= csxRs.getString("pk_seq") %>"><img title="Cập nhật" src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border = 0></A> &nbsp;
													<% }  %>
													<%if(quyen[Utility.XOA]!=0){ %>
														<A href = "../../ErpGiaidoanSXSvl?userId=<%=userId%>&delete=<%= csxRs.getString("pk_seq") %>"><img title="Xóa" src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn muốn xóa giai đoạn này?')) return false;"></A>&nbsp;
													<% }  %>
													<%if(quyen[Utility.XEM]!=0){ %>
														<A href = "../../ErpGiaidoanSXUpdateSvl?userId=<%=userId%>&display=<%= csxRs.getString("pk_seq") %>"><IMG title="Hiển thị" src="../images/Display20.png" alt="Hien thi" title="Hien thi" border=0></A>
													<% }  %>
												<% } else { %>
													<%if(quyen[Utility.SUA]!=0){ %>
														<A href = "../../ErpGiaidoanSXUpdateSvl?userId=<%=userId%>&update=<%= csxRs.getString("pk_seq") %>"><img title="Cập nhật" src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border = 0></A> &nbsp;
