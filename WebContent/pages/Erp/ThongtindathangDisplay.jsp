	...
	<TR>
				...
				<TR>
					<TD align="left" class="tbnavigation">
						<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
							<TR height="22">
								<TD align="left" colspan="2" class="tbnavigation">Dữ liệu nền > Kho vận > Thông tin lập kế hoạch > Cập nhật</TD>
						...
						<FIELDSET >
							...
							<TR>
								...
								<TD class="plainlabel" valign="middle" width="25%" >
									<select name="spId" style="width: 250px" disabled >
									<option value=""></option>
								<% while(spRs.next()){ 
							...
							<% if(obj.getIsMua().equals("1")){ %>
								...
								<input type="checkbox" name="isMua" value="1" checked disabled >&nbsp; Mua bên ngoài
							
							<% }else{ %>
							
								<input type="checkbox" name="isMua" value="1" disabled >&nbsp; Mua bên ngoài
					...
					<% if(obj.getIsMua().equals("1")){ %>				
							<TR>
								...
								<TD class="plainlabel" valign="middle" width="25%" colspan = 3 >
									<select name="nccId"  style="width: 250px" disabled >
										<option value=""></option>
								<% while(nccRs.next()){ 
							...
							<TR>
								...
								<TD class="plainlabel" valign="middle" >
									<input type="text" name="mou" value="<%= formatter.format(Double.parseDouble(obj.getMOU())) %>"  readonly >
								</TD>

								<TD class="plainlabel" valign="middle" width="120px">Thời gian giao hàng (ngày)</TD>
								<TD class="plainlabel" valign="middle" >
									<input type="text" name="leadtime" value="<%= obj.getLeadtime() %>"  readonly >
								</TD>
					...
					<% }else{ %>		
							<TR>
								...
								<TD class="plainlabel" valign="middle" colspan = 3>
									<input type="text" name="lotsize" value="<%= formatter.format(Double.parseDouble(obj.getLotsize())) %>" readonly >
								</TD>
							...
							<TR>
								...
								<TD class="plainlabel" valign="middle" width="25%" colspan = 3 >
									<select name="nmId"  style="width: 250px" >
										<option value=""></option>
						<% if(nmRs != null){
								try{
		...
		<%		try{
					...
					while(dtsxRs.next()){ %>
						...
						<% if(dtsxRs.getInt("CHON") > 0 & dtsxRs.getInt("LOAI") == 1){ %>
								<TD  valign="middle" >
									...
									<input type="checkbox" name="<%= "dt_" + dtsxRs.getString("ID") %>" value="1" checked disabled ><%= dtsxRs.getString("TEN")%>

								</TD>				
								<TD align = "center"><input type = "text" name = "<%= "tgsx_" + dtsxRs.getString("ID") %>" value = "<%= dtsxRs.getString("THOIGIAN")%>" style = "width:100;text-align:right" onKeyPress="return keypress(this);" readonly ></TD>
						<%}else{ %>							
			...
			<%				if(dtsxRs_ss.getInt("CHON") > 0 & dtsxRs_ss.getInt("LOAI") == 2){ %>
								...
								<TD  valign="middle" >
									<input type="hidden" name="DtsxIds" value="<%= dtsxRs_ss.getString("ID")%>"  >
									
									<input type="checkbox" name="<%= "dt_ss_" + dtsxRs_ss.getString("ID") %>" value="1" checked disabled ><%= dtsxRs_ss.getString("TEN")%>
								</TD>	
								<TD align = "center"><input type = "text" name = "<%= "tgsx_ss_" + dtsxRs_ss.getString("ID") %>" value = "<%= dtsxRs_ss.getString("THOIGIAN")%>" style = "width:100;text-align:right" onKeyPress="return keypress(this);" readonly></TD>
						
			<%				}else{ %>
								<TD>						
									<input type="checkbox" name="<%= "dt_ss_" + dtsxRs_ss.getString("ID") %>" value="1" disabled ><%= dtsxRs_ss.getString("TEN")%>
								</TD>
								</TD>	
								<TD align = "center"><input type = "text" name = "<%= "tgsx_ss_" + dtsxRs_ss.getString("ID") %>" value = "" style = "width:100;text-align:right" onKeyPress="return keypress(this);" readonly></TD>
			<%				} %>
				...
				<% } %>
							...
							<TR>
								...
								<TD class="plainlabel" valign="middle" colspan = 3 >
									<input type="text" name="cleanup" value="<%= obj.getCleanUp() %>" readonly>
								</TD>
