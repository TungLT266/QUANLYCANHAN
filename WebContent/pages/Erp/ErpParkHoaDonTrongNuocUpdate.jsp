...
<%@ page  import = "geso.traphaco.erp.beans.parktrongnuoc.*" %>
<%@ page  import = "geso.traphaco.erp.beans.parktrongnuoc.imp.*" %>
...
function DieuChinhTien(){
	...
	var thanhtienVAT = document.getElementsByName("thanhtienVAT");
	
	var tong =0;
	var tongVAT = 0;
	for(var i =0; i< thanhtien.length; i++){
		tong += parseInt(replaceAll(thanhtien.item(i).value,",",""));
	}
	
	for(var i =0; i< thanhtienVAT.length; i++){
		tongVAT += parseInt(replaceAll(thanhtienVAT.item(i).value,",",""));
	}
	
	
	// set tổng tiền và thuế giá trị gia tăng
	document.getElementById("thuegtgt").value= DinhDangDonGia(tongVAT);
	document.getElementById("tong").value= DinhDangDonGia(tong+ tongVAT);
	document.getElementById("sotienchuathue").value = DinhDangDonGia(tong);
	...
	for(var i = 0; i < idhangmua.length; i++)
	{
		...
		for(var j = 0; j < sodong; j++)
		{
			if(solo.item(j).value != "" && soluong.item(j).value != "")
			{
				...
				sln = sln + parseFloat(soluong.item(j).value.replace(/,/g,""));
		...
		thanhtieni = thanhtieni + Math.round(parseFloat(replaceAll(dongia.item(i).value, ",", ""))* sln);

		// chia 100 để ra phần trăm
		tienthuegtgti = Math.round(Math.round(thanhtieni *(parseFloat(replaceAll(vat.item(i).value, ",", "")))/100));
		
		tongtien = tongtien + thanhtieni;
		tienthuegtgt = tienthuegtgt + tienthuegtgti;
		thanhtien.item(i).value = DinhDangDonGia(thanhtieni.toFixed(0));
		thanhtienVAT.item(i).value = DinhDangDonGia(tienthuegtgti.toFixed(0));
	}
	
	// set tổng tiền và thuế giá trị gia tăng
	document.getElementById("thuegtgt").value= DinhDangDonGia(tienthuegtgt.toFixed(0));
	tong = parseFloat(tongtien) + parseFloat(tienthuegtgt);
	
	document.getElementById("tong").value= DinhDangDonGia(tong.toFixed(0));
	document.getElementById("sotienchuathue").value = DinhDangDonGia(tongtien.toFixed(0));
}
	...
	function Kiemtramoithu()
	{
		...
		for(var i=0; i< idhangmua.length; i++){
			if(soluongdanhan.item(i).value.trim().length >0){
				...
				console.log(" so luong dat : "+ soluongdat_);
				console.log(" so luong da nhan : "+ soluongdanhan_);
				console.log(" so luong nhan : "+ soluongnhan_);
				console.log(" so luong con : "+ a);
				console.log(" so luong xx : "+ (parseFloat(dungsai) * parseFloat(soluongdat_)/100));
...
/* 			var thuesuat = document.getElementById('thuesuat'+ i);
	...
	{
		var evalue= parseFloat (e.value.replace(/,/g,""));
		e.value = DinhDangDonGia( roundNumber (evalue,0));
	}
...
<form name="erpParkForm" method="post" action="../../ErpParkHoadontrongnuocUpdateSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng &gt; Mua hàng trong nước &gt; Hóa đơn trong nước &gt; Cập nhật
        </div>
    	...
    	<legend class="legendtitle"> Hóa đơn NCC</legend>
                   ...
                   <%} %> --%>
					...
					<TD class="plainlabel" valign="middle" > Số PO </TD>   
                      <TD class="plainlabel" valign="middle" colspan="3">
                      	<select name="poId" id="poId" onchange="changePO();" style="width: 400px;" multiple >
						<option value=""></option>
						<% if (poRs != null)
						{
							while (poRs.next())
							{
								...
								{
								%>
									<option value="<%=poRs.getString("pk_seq")%>" selected="selected"><%=poRs.getString("ten")%></option>
								<% } else { %>
					...
					<TR>
						...
						<TD class="plainlabel" width="15%">Số tiền chưa thuế</TD>
						<TD class="plainlabel" width="15%">
							<input type="text"  readonly name="sotienchuathue"  id="sotienchuathue" value="<%= formatter.format(Double.parseDouble(pBean.getSoTienChuaThue()))  %>" />
 						</TD>
					</TR>
					...
					<TR>
						...
						<TD class="plainlabel" width="15%">
							<input type="text" name="sohoadon" id="sohoadon" value="<%= pBean.getSoHoaDon() %>" />
 						</TD>
						
 						
 						<TD class="plainlabel" width="15%">Thuế giá trị gia tăng</TD>
						<TD class="plainlabel" width="15%">
							<input type="text"  readonly name="thuegtgt" id="thuegtgt" value="<%=formatter.format(Double.parseDouble(pBean.getThuegtgt()))  %>" />
 						</TD>
					...
					<TR>
						...
						<TD class="plainlabel" width="15%">
							<input type="text" name="kyhieuhoadon" id= "kyhieuhoadon" value="<%=pBean.getKyHieuHoaDon()  %>" />
 						</TD>
 						
 						<TD class="plainlabel" width="15%">Tổng</TD>
						<TD class="plainlabel" width="15%">
							<input type="text"  readonly name="tong" id="tong" value="<%= formatter.format(pBean.getTong()) %>" />
 						</TD>
					...
					<TR>
						...
						<TD class="plainlabel" width="15%" colspan="1">
							<input type="text"  name="ngayhoadon" id="ngayhoadon" value="<%= pBean.getNgayHoaDon() %>"  class="days"/>
 						</TD>
 						
 						<TD class="plainlabel" width="15%">Chênh lệch nguyên tệ</TD>
						<TD class="plainlabel" width="15%" >
							<input type="text" name="chenhlechhd" id="chenhlechhd"  value="<%=pBean.getChenhlechHd()%>"  />		 
						</TD>
					</TR>
            ...
            <TABLE class="tabledetail" width="100%" border="0" cellpadding="1" cellspacing="1" >
                <TR class="tbheader"> 
					...
					<TH align="center" width="9%">Mã sản phẩm</TH>
					...
					<TH align="center" width="9%">Số lượng đã nhận</TH>
					<TH align="center" width="10%">Số lượng HĐ</TH>
					<TH align="center" width="11%">Thành tiền</TH>
					<TH align="center" width="10%">Thành tiền VAT</TH>     	 	
                </TR>
                	...
                	for(int i = 0; i < spList.size(); i++)
                	{
                		ISanpham sp = spList.get(i);
	               		...
	               		<tr>
           	 				...
           	 				<td align="left">
           	 					<input type="hidden" style="width: 100%; text-align: left;" value="<%= sp.getDvdl() %>" 
           	 					name= "dvdl" readonly="readonly" >
           	 				...
           	 				<td align="right">
           	 					<input type="text" style="width: 100%; text-align: right;" value="<%= sp.getDongia() %>" name= "dongiaMua" readonly="readonly" >
           	 				</td>
           	 				<td align="right">
           	 					<input type="text" style="width: 100%; text-align: right;" value="<%= formatter2.format( Double.parseDouble(sp.getVat().replaceAll(",",""))) %>" name= "vat" readonly="readonly" >
           	 				</td>
           	 				<td align="right">
           	 					<input type="text" style="width: 100%; text-align: right;" value="<%= DinhDang.dinhdangkho( Double.parseDouble(sp.getSoluongDaNhan().replaceAll(",",""))) %>" name= "soluongdanhan" readonly="readonly" >
           	 				</td>
           	 				<td align="right">
           	 					<input type="text" style="width: 70%; text-align: right;" value="<%= sp.getSoluongnhan() %>" name= "soluongnhan" 
           	 					readonly = "readonly" onKeyPress = "return keypress(event);" onchange="DinhDang();">
           	 					
           	 					<input type="hidden"  value="<%= sp.getSoluongMaxNhan() %>" name= "soluongMAXnhan">
           	 					
           	 					<a href="" id="<%= sp.getId()+ sp.getMuahang_fk()+i %>" rel="subcontent<%= i %>">
				                    ...
				                    <table width="90%" align="center">
				                        ...
				                        <%
				                        	...
				                        	if(spDetailList.size() > 0)
				                        	{
				                        		...
				                        		%>
				                        			<tr>
							                            <td>
							                            	<input type="text" size="100px" name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".solo" %>" value="<%= spDetail.getSolo() %>" onchange="replaces()"/>
							                            </td>
							                            <td>
							                            	<input type="text" size="100px"  name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".soluong" %>" onkeyup="Lamtronso(this)" value="<%= spDetail.getSoluong() %>" style="text-align: right;" onchange="replaces()"/></td>
							                            ...
							                            <td>
							                            	<input type="text" size="100px"  class="days" 
							                            		name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".ngaysanxuat" %>" value="<%= spDetail.getNgaySx() %>"  onchange="replaces()" /></td>
							                            <td>
							                            	<input type="text" size="100px"  class="days" 
							                            		name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".ngayhethan" %>" value="<%= spDetail.getNgayHethan() %>" readonly="readonly"   />
							                            </td>
							                        </tr>
				                        		<% stt++; }
				                        	} 
				                        ...
				                        <%
				                        	for(int k = 0; k < 10 - spDetailList.size(); k++)
				                        	{
				                        		...
				                        		<tr>
						                            <td>
						                            	<input type="text" size="100px" name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".solo" %>" value="" onchange="replaces()" /></td>
						                            <td>
						                            	<input type="text" size="100px"  name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".soluong" %>" onkeyup="Lamtronso(this)" value="" style="text-align: right;" onchange="replaces()"/></td>
						                            ...
						                            <td>
						                            	<input type="text" size="100px"  class="days" name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".ngaysanxuat" %>" 
						                            	  value=""  onChange="replaces()" />
						                            </td>
						                            <td>
						                            	<input type="text" size="100px"  name="<%= sp.getId() +"."+sp.getMuahang_fk()+i+ ".ngayhethan" %>"  class="days" 
           	 				...
           	 				<td align="right">
           	 					<input type="text" style="width: 100%; text-align: right;"  onchange ="DieuChinhTien();"
           	 					value="<%= formatter2.format( Double.parseDouble(sp.getthanhtien().replaceAll(",",""))) %>" name= "thanhtien"  >
           	 				</td>
           	 				<td align="right"><input type="text"
								style="width: 100%; text-align: right;" name="thanhtienVAT" onchange ="DieuChinhTien();"
								value="<%= formatter2.format( Double.parseDouble(sp.getThanhtienVATVND().replaceAll(",","")))%>" 
							></td>
