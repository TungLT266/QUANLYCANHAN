			...
			if(idsp.item(i).value != "" || masp.item(i).value != "")
			{
				for(var k = 0;k <sodong ;k++)
		 		{
		 			...
		 			{
		 				if((idsp[i].value == idsp[k].value) && idsp[k].value.length !=0    && document.getElementById("loaihh").value != 2 ) // neu la chi phi dich vu dc trung ngay nhan
	 					{
				...
				if(soluong.item(i).value != "")
				{
					if(dongia.item(i).value != "")
					{
						...
						var tt = 0;
						var tt1 = parseFloat(slg) *parseFloat(replaceAll(dgia,",",""));	
						var ttavat1 = parseFloat(tt1) + parseFloat(tt1) * parseFloat(tx) / 100;
						
						donGiaUSD.item(i).value = parseFloat(dgia) * parseFloat(tiGiaNguyenTe.value);
						tt = parseFloat(slg) * parseFloat(donGiaUSD.item(i).value);
						ttgoc += Math.round( (parseFloat(slg) * parseFloat(dgia)) * (1 + parseFloat(tx) / 100) );
...
/* 			else
	...
	{    
	 ...
	 {	
		 ...
		 for(var i=0;i<ptThanhToan.length;i++ ){
			 if(ptThanhToan.item(i).value.trim().length>0){
				 tongPtThanhToan = parseInt(tongPtThanhToan) + parseInt(ptThanhToan.item(i).value);
		 ...
		 if (tongPtThanhToan > 100){
			error.value="Tổng phần trăm trong thời hạn thanh toán lớn hơn 100%. Vui lòng nhập lại thời hạn thanh toán .";
			alert ("Tổng phần trăm trong thời hạn thanh toán lớn hơn 100% ");
			return false;
		 }
		 if (tongPtThanhToan < 100){
			 error.value="Tổng phần trăm trong thời hạn thanh toán nhỏ hơn 100%. Vui lòng nhập lại thời hạn thanh toán .";
			 alert("Tổng phần trăm trong thời hạn thanh toán nhỏ hơn 100%. ");
			return false;
		 }
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="dmhForm" method="post" action="../../ErpDonmuahangTSCCUpdateSvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left" class="tbnavigation"> Quản lý mua hàng > Mua hàng VT/CPDV/TSCĐ/CCDC > Đơn mua hàng > Cập nhật</div>
								...
								<TD class="plainlabel"  colspan="3">
									...
									<a href="" id="thoiHanThanhToan" rel="thThanhToan">
                          			...
                          			<DIV id="thThanhToan" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B;
				                             background-color: white; width: 450px; min-height:150px; overflow:auto; padding: 4px;">
				                    	<table width="100%" align="center">
				                        <tr>
		                            	...
		                            	<%
		                            		...
		                            		if(dong == 0 && dmhBean.getNgayThanhToanArr() != null)
		                            		{
		                            			String[] ngayThanhToanArr = dmhBean.getNgayThanhToanArr();
		                            			String[] ptThanhToanArr = dmhBean.getPtThanhToanArr();
		                            			String[] soTienThanhToanArr = dmhBean.getSoTienThanhToanArr();
		                            			while (dong < ngayThanhToanArr.length)
		                            			{ %>
		                            				<Tr>
					                            		<td><input type="text" class="days" name="ngayThanhToan" value="<%= ngayThanhToanArr[dong] %>" style="width: 150px" /></td>
					                            		<td><input type="text" name="ptThanhToan" value="<%= ptThanhToanArr[dong] %>" style="width: 150px;text-align: right;" /></td>
					                            		<td><input type="text" name="soTienThanhToan" value="<%= soTienThanhToanArr[dong] %>" style="width: 150px;text-align: right;" readonly/></td>
					                            	</Tr>
		                            				<% dong++;
		                            			}
		                            		}
						...
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1" cellspacing="1">
							<TR class="tbheader">
								...
								<% if ( dmhBean.getLoaihanghoa().trim().equals("0") ) { %>
									...
									<TH align="center" width="20%">Tên hàng mua</TH>
								...
								<% if (dmhBean.getLoai().equals("2") && dmhBean.getLoaihanghoa().trim().equals("0") )
								   { %>
									<TH align="center" width="7%" style="display: none">Kho nhận</TH>
								<% } else { %>
									<TH align="center" width="7%" style="display: none" >Kho nhận</TH>
								<% } %>
								
								<%if(dmhBean.getLoaihanghoa().equals("2")){ %>
								<TH align="center" width="7%" >Sản phẩm</TH>
								<%}else{ %>
								<TH align="center" width="7%" >Tên in PO</TH>
								<%} %>
							</TR>
								...
								if(spList.size() > 0) { 
									for(int i = 0; i < spList.size(); i++) { 
										...
										<tr>
											...
											<TD align="center" width="5%">
												<input type="text" value="<%= sp.getSoluong() %>" name="soluong" style="width: 100%; text-align: right;" onkeyup="FormatNumber(this)"> 
											...
											<% if (dmhBean.getNguonGocHH().equals("NN"))
											  { %>
													<TD align="center" width="7%" style="display: none">
							...
							<% for(int i = count; i < 40; i++) { %>
								...
								<tr>
									...
									<TD align="center" width="7%">
										<input type="text" value="" name="thuexuat" id="thuexuat<%=i%>" style="width: 100%; text-align: right;" readonly = readonly > 
									</TD>
