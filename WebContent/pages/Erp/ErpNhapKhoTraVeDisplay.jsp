...
<%@ page import="geso.traphaco.erp.beans.yeucauchuyenkho.imp.*"%>
<%@ page import="java.sql.ResultSet"%>
<%@ page import="java.util.Hashtable"%>
<%@ page import="java.util.List"%>
<%
	IErpNhapkhotrave lsxBean = (IErpNhapkhotrave)session.getAttribute("lsxBean");
	...
	String[] spTienVat = lsxBean.getSpTienVat();
			...
			if( spMa.item(i).value != '' )
			{
				...
				for(var j = 0; j < soluongCT.length; j++)
				{
					if(soluongCT.item(j).value != "")
					{
						...
						var _soluong = soluongCT.item(j).value.replace(/,/g,"");
						var tienBVAT = Math.round( parseFloat(_soluong) * parseFloat(_dongia) );
						
						tongtienBVAT = parseFloat(tongtienBVAT) + parseFloat(tienBVAT);
						
						//alert( 'Ma: ' + spMa.item(i).value + ' - TIEN: ' + tienBVAT );
					}
				}
				
				soluong.item(i).value = totalLUONG;
				//thanhtien.item(i).value = DinhDangTien ( tongtienBVAT );
				
				if( _tienVat == '' )
					_tienVat = Math.round( parseFloat(tienBVAT) * parseFloat(_vat) / 100.0 );
				
				thanhtien.item(i).value = DinhDangTien ( parseFloat(tongtienBVAT) + parseFloat(_tienVat) );
				
				_tongtienBVAT = parseFloat(_tongtienBVAT) + parseFloat(tongtienBVAT);
				_tongtienVAT = parseFloat(_tongtienVAT) + parseFloat(_tienVat);
				_tongtienAVAT = parseFloat(_tongtienVAT) + parseFloat(_tongtienBVAT);
...
<form name="hctmhForm" method="post" action="../../ErpNhapkhotraveUpdateSvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left" class="tbnavigation">
					Quản lý tồn kho > Chức năng > Nhập kho hàng trả về > Hiển thị
				</div>
			...
			<div align="left" id="button"
				style="width: 100%; height: 32px; padding: 0px; float: none" class="tbdarkrow">
					<A href="../../ErpNhapkhotraveSvl?userId=<%=userId%>"> 
				...
				<fieldset>
					...
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							<TR>
								<TD width="120px" class="plainlabel" valign="top">Ngày nhập kho </TD>
								<TD width="250px"  class="plainlabel"  >
									<input type="text" class="days" readonly="readonly" name="tungay" value="<%= lsxBean.getTungay() %>" />
								</TD>
							
								<TD class="plainlabel" valign="top">Ngày trả </TD>
								<TD  class="plainlabel" valign="top" >
									<input type="text" readonly="readonly" name="denngay" value="<%= lsxBean.getDenngay() %>" />
								</TD>
							</TR>
							<TR>
			                ...
			                <TR>
			                    ...
			                    <TD class="plainlabel" colspan="3"  >
			                    	<select name = "khonhapId" style="width: 800px;" onchange="submitform();" class="select2"  >
			                    		<option value=""> </option>
			                        	<%
			                        		if(khonhapRs != null)
			                        		{
							...
							<TR>
								...
								<TD class="plainlabel" >
									<input type="text" id="tongtienBVAT" value="" style="text-align: right;" readonly="readonly" /> VNĐ
								</TD>
								<TD class="plainlabel" >Tổng tiền VAT</TD>
								<TD class="plainlabel" >
									<input type="text" id="tongtienVAT" value="" style="text-align: right;" readonly="readonly" /> VNĐ
								</TD>
							</TR>
							<TR>
								<TD class="plainlabel" >Tổng tiền AVAT</TD>
								<TD class="plainlabel" colspan="3" >
									<input type="text" id="tongtienAVAT" value="" style="text-align: right;" readonly="readonly" /> VNĐ
								</TD>
							</TR>
							<TR>
						...
						<table cellpadding="0px" cellspacing="1px" width="100%">
							...
							<tr>
								...
								<td >
									<input type="text" name="vat" value="<%= spVat[i] %>" style="width: 100%; text-align: right; " readonly="readonly" >
								</td>
								<td >
									<input type="text" name="tienVat" value="<%= spTienVat[i] %>" style="width: 100%; text-align: right; " readonly="readonly" >
								</td>
								<td >
								...
								<td >
									...
									<DIV id="subcontent_<%=count%>" style="position: absolute; visibility: hidden; border: 9px solid #80CB9B; background-color: white; 
											width: 900px; max-height: 500px; overflow: auto; padding: 4px;">
										
										<table width="890px" align="center">
											<tr>
												<th width="60px" style="font-size: 11px">Số lượng</th>
												<th width="80px" style="font-size: 11px">Số lô</th>
												<th width="80px" style="font-size: 11px">Ngày hết hạn</th>
					                            ...
					                            <th width="80px" style="font-size: 11px">Hàm lượng</th>
												...
												<%
						                        	...
						                        	if(keys != null)
						                        	{
						                        		...
						                        		while( keys.hasMoreElements()  )
						                        		{
						                        			...
						                        			String[] arr = Utility.mySplit(key, "__");
						                        			String temID = spMa[i];
												...
												<tr>
													<td  >
														<input type="text" style="width: 100%; text-align: right;" name="<%= temID %>_spSOLUONG" value="<%= _soluong %>" readonly="readonly">
													</td>
													<td >
														<input type="text" style="width: 100%; text-align: center;" name="<%= temID %>_spSOLO" value="<%= arr[1] %>" readonly="readonly" >
													</td>
													<td  >
														<input type="text" style="width: 100%; text-align: center;" name="<%= temID %>_spNGAYHETHAN" value="<%= arr[2] %>" readonly="readonly" >
													</td>
													
													<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spMAME" value="<%= arr[3] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spMATHUNG" value="<%= arr[4] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;" name="<%= temID %>_spVitri" onkeyup="ajax_showOptions(this,'layvitriNHANHANG',event)" AUTOCOMPLETE="off" value="<%= arr[5] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spMAPHIEU" value="<%= arr[6] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spPhieudt" value="<%= arr[7] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spPhieueo" value="<%= arr[8] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spMarq" value="<%= arr[9] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spHamluong" value="<%= arr[10] %>" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;" name="<%= temID %>_spHamam" value="<%= arr[11] %>" >
			                        				</td>
