...
System.out.println("::: AJAX: lay thong tin " +(String) session.getAttribute("khonhanid"));
	...
	{		
	   	...
	   	if( loadvitriNHAN_KHONHAN.equals("1") )
		{
			...
			if( session.getAttribute("khonhanid") != null )
				khonhanid = session.getAttribute("khonhanid").toString();
			
			command = "select pk_seq, MA + ', ' + TEN as TEN from ERP_BIN " + 
					  " where KHOTT_FK = '" + khonhanid + "' and trangthai = '1' and timkiem like  '%"+query+"%' ";
		}
	   	else
	   	{
			command = " select a.ma, a.ten, b.donvi, isnull(a.hansudung, 0) as hansudung, isnull(c.thuexuat, 0) as thuexuat, ISNULL( ( select sum(available) from ERP_KHOTT_SANPHAM where KHOTT_FK = '100001' and sanpham_fk = a.pk_seq ), 0 ) as avai " +
					  "	from ERP_SANPHAM a inner join DONVIDOLUONG b on a.dvdl_fk = b.pk_seq inner join NGANHHANG c on a.nganhhang_fk = c.pk_seq where a.timkiem like  '%"+query+"%' ";
	   	}
