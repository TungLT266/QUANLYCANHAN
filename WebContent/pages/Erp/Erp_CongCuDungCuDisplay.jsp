...
<%
	...
	if (!util.check(userId, userTen, sum)) {
		...
		System.out.println("");
		IErp_CongCuDungCu obj = (Erp_CongCuDungCu) session
				.getAttribute("obj");
		...
		ResultSet dcNguyenGiaRs  = obj.getDieuChuyenCCDCRs();
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="ccdcForm" method="post"
		action="../../Erp_CongCuDungCuUpdateSvl">
		<input type="hidden" name="Id" value='<%=obj.getId()%>'> <input
			...
			name="action" type="hidden" value="save"> <INPUT
			name="khauhao" type="hidden" value="1"> <INPUT name="id"
			type="hidden" value="<%=obj.getId()%>">
				...
				<TD colspan="4" align='left' valign='top' bgcolor="#ffffff">
								...
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr height="22">
										<TD align="left" colspan="2" class="tbnavigation">Quản lý
											Chi phí trả trước &gt; Chi phí trả trước &gt; Hiển thị</TD>
								...
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<TR class="tbdarkrow">
										<TD width="30" align="left"><A
											href="/TraphacoHYERP/Erp_CongCuDungCuSvl"> <img
					...
					<TABLE width="100%" cellspacing="0" cellpadding="6">
						<tr>
							<TD align="left" colspan="4" class="legendtitle">
								<FIELDSET>
									...
									<textarea name="dataerror" id="error" style="width: 100%"
						...
						<tr>
							<TD align="left" colspan="4" class="legendtitle">
								<FIELDSET>
									<LEGEND class="legendtitle">Thông tin Chi phí trả
										trước </LEGEND>
									<TABLE width="100%" cellspacing="0" cellpadding="6">
		
										<tr>
											<td class="plainlabel" width="15%">Loại Chi phí trả
												trước</td>
											<td colspan="3" class="plainlabel"><select name="lccdcId"onchange="submitform()">
													<option value=""></option>
													<%
														if (RsLccdc != null) {
																while (RsLccdc.next()) {
																	if (obj.getLccdcId()
																			.equals(RsLccdc.getString("pk_seq"))) {
													%>
													<option selected="selected"
														value="<%=RsLccdc.getString("pk_seq")%>"><%=RsLccdc.getString("ten")%></option>
													<%
														} else {
													%>
										...
										<TR>
											<TD width="15%" class="plainlabel">Mã <FONT
												class="erroralert">*</FONT>
											</TD>
											<TD class="plainlabel"><INPUT type="text" name="ma"
												id="ma" size="40" type="text" value="<%=obj.getMa()%>"
												readonly>
											</TD>

											<TD width="15%" class="plainlabel">Diễn giải <font
												class="erroralert">*</font>
											</TD>

											<TD class="plainlabel"><input type="text"
												name="diengiai" id="diengiai"
												value="<%=obj.getDiengiai()%>" readonly>
											</TD>
										...
										<TR>
											...
											<TD class="plainlabel"><INPUT type="text" name="soluong"
												id="soluong" value="<%=obj.getSoLuong()%>"
												onkeypress="return keypress(event);" readonly="readonly">
											</TD>

											<TD class="plainlabel">Đơn vị tính</TD>
											<TD class="plainlabel"><INPUT type="text" name="dvt"
												id="dvt" value="<%=obj.getDvt()%>" readonly="readonly">
											</TD>
										</TR>
										<TR>
											<TD width="15%" class="plainlabel">Đơn giá</TD>
											<TD class="plainlabel"><input type="text" name="dongia"
												id="dongia"
												<%System.out.println("don gia: " + obj.getDonGia());
				System.out.println("don gia: "
						+ Double.parseDouble(obj.getDonGia()));%>
												value="<%=formatter.format(Double.parseDouble(obj.getDonGia()))%>"
												onkeypress="return keypress(event);" readonly="readonly">
											</TD>
											...
											<TD class="plainlabel"><INPUT type="text"
												...
												value="<%=formatter.format(Double.parseDouble(obj
						.getThanhTien()))%>"
												readonly="readonly">
											</TD>
										...
										<TR>
											<TD width="15%" class="plainlabel">Số tháng khấu hao <font
												class="erroralert">*</font>
											</TD>

											<TD class="plainlabel"><input type="text" name="stkh"
												id="stkh" value="<%=obj.getSothangKH()%>"
												onkeypress="return keypress(event);" maxlength="4" readonly>
											</TD>

											<TD width="15%" class="plainlabel">Ngày bắt đầu khấu hao
												<font class="erroralert">*</font></TD>

											<TD class="plainlabel"><input class="days" type="text"
												name="tbdkh" id="tbdkh" value="<%=obj.getThangbatdauKH()%>"
												maxlength="10" readonly>
											</TD>

										</TR>
										<TR>
											<TD class="plainlabel">Phòng ban<FONT class="erroralert">
													*</FONT>
											</TD>

											<TD class="plainlabel" colspan=3><select name="dvthId"
												id="dvthId" style="width: 200px">
													<option value=""></option>
													<%
														if (DvthList != null)
																while (DvthList.next()) {
																	if (obj.getDvthId()
																			.equals(DvthList.getString("PK_SEQ"))) {
													%>
                     ...
                     <DIV id="subcontentNG" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B; background-color: white; width: 450px; max-height:150px; overflow-y:scroll; padding: 4px;">
						...
						<TR>
							<td>
								...
								<ul class="tabs">
									<li><a href="#" class="legendtitle"> Khoản mục chi phí</a>
									</li>
									<li><a href="#" class="legendtitle">Phân bổ dự kiến</a>
									</li>
								</ul>
								...
								<div class="panes">
									<div align="left">
										<TABLE style="width: 30%" cellpadding="2px" cellspacing="1px"
											align="center">
											<TR>
												<TD class="legendtitle">Chọn khoản mục chi phí</TD>
											</TR>
										</TABLE>
										...
										<TABLE style="width: 30%" cellpadding="2px" cellspacing="1px"
											...
											<TR>
												<TD align="left" width="100%"><select
													style="width: 600px" name="khoanMucId"
													onchange="submitform();">
														<option value=""></option>
														<%
															if (kmcpRs != null) {
																		...
																		while (kmcpRs.next()) {
																			if (phanBo.getKhoanMucId().equals(
																					kmcpRs.getString("NCPID"))) {
														%>
														<option selected="selected"
															value="<%=kmcpRs.getString("NCPID")%>">
															<%=kmcpRs.getString("DIENGIAI")%>
														</option>
														<%
															} else {
														%>
														<option value="<%=kmcpRs.getString("NCPID")%>">
															<%=kmcpRs.getString("DIENGIAI")%>
														</option>
														<%
															}
												...
												<TD align="left" width="100%"><INPUT TYPE="text"
													name="phanTramKhauHao" value="<%=phanBo.getPhanTram()%>"
									...
									<div align="left">
										...
										<TABLE id="Master" class="tabledetail" width="100%" border="0"
											cellpadding="4" cellspacing="1">
											<TR class="tbheader">
												<TH class="tbheader">Tháng thứ</TH>
												<TH class="tbheader">Phân bổ dự kiến</TH>
												<TH class="tbheader">Phân bổ lũy kế dự kiến</TH>
												<TH class="tbheader">Giá trị còn lại dự kiến</TH>
												<TH class="tbheader">Phân bổ thực tế</TH>
												<TH class="tbheader">Phân bổ lũy kế thực tế</TH>
											...
											<TR class=<%=darkrow%>>
												...
												%>
												<TD align="center"><%=khRs.getString("THANG")%></TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("KHAUHAODUKIEN"))))%>
												</TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("KHAUHAOLUYKEDUKIEN"))))%>
												</TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("GIATRICONLAIDUKIEN"))))%>
												</TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("KHAUHAOTHUCTE"))))%>
												</TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("KHAUHAOLUYKETHUCTE"))))%>
												</TD>
												<TD align="center"><%=formatter.format(Math.round(Double
									.parseDouble(khRs
											.getString("GIATRICONLAITHUCTE"))))%>
												</TD>
											</TR>
