...
<% String userTen = (String) session.getAttribute("userTen");
	...
	ResultSet rsList = obj.getParkList();
...
String sum = (String) session.getAttribute("sum");
	...
	if(!util.check(userId, userTen, sum)){
		response.sendRedirect("/TraphacoHYERP/index.jsp");
		 ...
		 int[] quyen = new  int[5];
		 quyen = util.Getquyen("ErpParkHoadonSvl","",userId);
...
<body>
<form name="erpParkForm" method="post" action="../../ErpParkHoadonSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng > Mua hàng VT/CPDV/TSCĐ/CCDC > Hóa đơn NCC
        </div>
                ...
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >								                          
                    ...
                    <TR style=display:none>
                     ...
                     <TR>
                        ...
                        <TD class="plainlabel" valign="middle">
                           <select  id= "trangthai" name="trangthai" onChange="submitform()" style="width: 200px;">
	                    ...
	                    if(rsList!=null)
	                    {
	                    	while(rsList.next())
	                    	{
		                        ...
		                        <%} %>
		                    	    ...
		                    	    String trangthai = "";
		                            String style="";
		                    		String tt = rsList.getString("trangthai");
		                    		String htat = rsList.getString("DAHOANTAT");
		                    		String dtt = rsList.getString("DACOTHANHTOAN");
		                    		int danhanhang = Integer.parseInt(rsList.getString("DANHANHANG"));
		                    	...
		                    	%>
									...
									<TD align="center" <%= style %>><%= rsList.getString("prefix")+ rsList.getString("PK_SEQ") %></TD>
				                    <TD align="center" <%= style %>><%=  rsList.getString("NGAYGHINHAN") %></TD>
				                    <TD align="center" <%= style %>><%=  rsList.getString("sohoadon")%></TD>
				                    <TD align="left" <%= style %>><%=  rsList.getString("TENNHACUNGCAP") %></TD> 
		                    ...
		                    <TD align="center" <%= style %>><%= rsList.getString("NGAYTAO") %></TD>	
		                    <TD align="left" <%= style %>><%=  rsList.getString("TENNV") %></TD>
         					<TD align="center" <%= style %>><%=  rsList.getString("NGAYSUA") %></TD>
							<TD align="left" <%= style %>><%=  rsList.getString("TENNVS") %></TD> 
		                        
		                        
		                        <TD align="center"> 
		                    <% if(tt.equals("0")){ %>
		                   		 <%if(quyen[2]!=0){ %>
                                <A href = "../../ErpParkHoadonUpdateSvl?userId=<%=userId%>&update=<%= rsList.getString("PK_SEQ") %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                                	
                                <%} %>
                                <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%= rsList.getString("PK_SEQ") %>'
							       href=""><img
							       src="../images/Chot.png" alt="Chốt"
							       width="20" height="20" title="Chốt"
							       border="0" onclick="if(!confirm('Bạn có muốn chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+ rsList.getString("PK_SEQ") %>' , '../../ErpParkHoadonSvl?userId=<%=userId%>&chot=<%= rsList.getString("PK_SEQ")  %>');}"  >
								</A>											     
								<%} %>
                                <%if(quyen[1]!=0){ %>
                                     <A href = "../../ErpParkHoadonSvl?userId=<%=userId%>&delete=<%= rsList.getString("PK_SEQ")  %>&poId=<%= rsList.getString("PK_SEQ")  %>"><img src="../images/Delete20.png" alt="Xóa Chứng từ" title="Xóa Chứng từ" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa Chứng từ này?')) return false;"></A>
                                <%} %>									
		                    <%}else{ if(tt.equals("1")){ %>
		                    	<A href = "../../ErpParkHoadonUpdateSvl?userId=<%=userId%>&display=<%= rsList.getString("PK_SEQ")  %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;	
		                    	 <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%= rsList.getString("PK_SEQ") %>'
							       href=""><img
							      src="../images/unChot.png" alt="Bỏ chốt" title="Bỏ chốt"
									width="20" height="20" longdesc="Bỏ chốt"							
							       border="0" onclick="if(!confirm('Bạn có muốn bỏ chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+ rsList.getString("PK_SEQ") %>' , '../../ErpParkHoadonSvl?userId=<%=userId%>&unchot=<%= rsList.getString("PK_SEQ") %>');}"  >
								</A>
								<% } %>
								<%-- <%} if(danhanhang ==0) { %>
		                    	<A href="../../ErpNhanhang_GiaySvl?userId=<%=userId%>&convert=<%= rsList.getString("PK_SEQ")   %>&NCCId=<%= rsList.getString("NCCID")   %>&hoadonnccId=<%=  rsList.getString("HOADONID")   %>&loaihd=<%= rsList.getString("LOAIHD")  %>&loaihh=<%= rsList.getString("LOAIHH")   %>">
