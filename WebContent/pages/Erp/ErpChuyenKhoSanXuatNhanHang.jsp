...
<% ResultSet rskhu = lsxBean.getRsKhukhonhan(); %>
		...
		for(i = 0; i < masp.length; i++)
		{
			if(masp.item(i).value != "")
			{
				...
				if(pos > 0)
				{
					...
					solo.item(i).value = sp.substring(0, parseInt(sp.indexOf("] [")));
	...
	function DinhDangDonGia(num) 
	{
	 ...
	 {	
		 document.getElementById("btnSave").innerHTML = "<a href='#'><img src='../images/waiting.gif'  title='cho luu..' border='1' longdesc='cho luu..' style='border-style:outset'> Processing...</a>";
	 	 document.forms['hctmhForm'].action.value = 'nhanhang';
...
<input type="hidden" name="tungay" value='<%= obj.getTungayTao()%>'>
 
<input type="hidden" name="denngay" value='<%= obj.getDenngayTao()%>'>
<input type="hidden" name="trangthai1" value='<%= obj.getTrangthai()%>'>
 <input type="hidden" name="task" value='<%=lsxBean.gettask()%>'>
<input type="hidden" name="isnhanHang" value='<%=lsxBean.getIsnhanHang()%>'>
 
<input type="hidden" name="sophieu" value='<%= obj.getSophieu()%>'>
<input type="hidden" name="nguoitao" value='<%= obj.getNguoitao()%>'>
<input type="hidden" name="nguoisua" value='<%= obj.getNguoisua()%>'>
    ...
    <fieldset>
    	<legend class="legendtitle">Yêu cầu xuất kho </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">							
                ...
                <%} %>
                <% if(  lsxBean.getNdxId().equals("100025") || lsxBean.getNdxId().equals("100006") || lsxBean.getNdxId().equals("100010") || lsxBean.getNdxId().equals("100011") || lsxBean.getNdxId().equals("100026") ) { %>
	                <TR>
			...
			<tr class="tbheader">
				...
				<th align="center" width="15%">Số lượng nhận</th>
				...
				{ 
					...
					<tr>
						<td>
							...
							<input type="text" name="masp" value="<%= yeucau.getMa() %>" style="width: 100%" readonly="readonly"  onkeyup="ajax_showOptions(this,'donmuahang',event)" AUTOCOMPLETE="off" > 
						</td>
						...
						<td> <input type="text" name="soluongnhan" readonly="readonly" value="<%= yeucau.getSoluongNhan() %>" style="width: 100%; text-align: right; "  > </td>
						<td>
			                    ...
			                    <table width="100%" align="center">
			                        	...
			                        	if(spConList.size() > 0)
			                        	{
			                        		...
			                        		%>
			                        			...
			                        			<%if(lsxBean.getIsQuanlykhuvuc_khonhap().equals("1")){ %>
			                        				<td> 
			                        				<select id="<%= yeucau.getId() + "khuvuc_id" %>" name="<%= yeucau.getId() + "khuvuc_id" %>"  > 
			                        				<option> </option>
 														<%
 														...
 														if(rskhu!=null){
 															rskhu.beforeFirst();
 															while (rskhu.next()){
 																if(rskhu.getString("pk_seq").equals(spCon.getKhuId())){
 																	%>
 																	<option selected="selected" value="<%=rskhu.getString("pk_seq")%>"> <%=rskhu.getString("ten") %> </option>
 																	<%
 																}else{
 																	%>
 																	<option  value="<%=rskhu.getString("pk_seq")%>"> <%=rskhu.getString("ten") %> </option>
 																	<%
 																}		
 															}
 														}
						                            ...
						                            <td> 
						                            	...
						                            	<input name="<%= yeucau.getId() + "xk_id" %>" 
						                            	type="hidden" style="width: 100%" value="<%= spCon.getXk_Id() %>" readonly="readonly" /> 
						                            ...
						                            <td>
						                            	<input readonly="readonly" name="<%= yeucau.getId() + "soluongxuat" %>" 
