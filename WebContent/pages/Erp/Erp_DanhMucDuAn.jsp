...
%>
<%
	IErp_DanhMucDuAnList obj = (IErp_DanhMucDuAnList) session.getAttribute("obj");
	List<Erp_ListItem> xuatDungList = obj.getXuatDungList();
%>
<%
int[] quyen = new  int[5];
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="xdForm" method="post" action="../../Erp_DanhMucDuAnSvl">
						...
						<TR>
							<TD align="left" class="tbnavigation">
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<TR height="22">
										<TD align="left" colspan="2" class="tbnavigation">&nbsp;Dữ liệu nền &gt; Kế toán &gt; Danh mục dự án</TD>
											...
											<FIELDSET>
												...
												<TABLE class="tblight" width="100%" cellpadding="6" cellspacing="0">
													<TR>
														...
														<TD class="plainlabel" colspan=5><INPUT name="maCCDC" type="text" size="30" value='<%=obj.getMa()%>' onChange="submitform();"></TD>
														...
														<TD width="98%">
															<TABLE width="100%" border="0" cellspacing="1" cellpadding="2">
																...
																<%
																	if (xuatDungList != null) {
																			try {
																				...
																				for (Erp_ListItem xuatDung : xuatDungList) {
																					if (m % 2 != 0) {
																%>
																<TR class=<%=lightrow%>>
																	<%
																...
																<TR class=<%=darkrow%>>
																	...
																	<TD align="center"><%=xuatDung.getTenTrangThai()%></TD>
																	<TD align="left" >
																		<%= xuatDung.getNgayTao() %>
																	</TD>
																	<TD align="left" >
																		...
																		<TABLE>
																			<TR>
																				<TD>
																						...
																						<%if(quyen[2]!=0  && xuatDung.getTrangThai().trim().equals("0")){ %>
																							<A href="../../Erp_DanhMucDuAnUpdateSvl?userId=<%=userId%>&update=<%=xuatDung.getId()%>">
																							<img title="Cập nhật" src="../images/Edit20.png" alt="Cap nhat" width="20" height="20" longdesc="Cap nhat" border=0></A>&nbsp;
																						<%} %> 
 																						 <%if(quyen[1]!=0 && xuatDung.getTrangThai().trim().equals("0")){ %>
 																						<A href="../../Erp_DanhMucDuAnSvl?userId=<%=userId%>&delete=<%=xuatDung.getId()%>"> 
																						<img title="Xóa" src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa danh mục dự án này ?')) return false;"></A>&nbsp; 
 																						<%} %>  
																						<!-- QUYEN VIEW DLN -->
																						<%if(quyen[1]!=0 && (xuatDung.getTrangThai().trim().equals("1") || xuatDung.getTrangThai().trim().equals("2"))){ %>
																						<A href="../../Erp_DanhMucDuAnUpdateSvl?userId=<%=userId%>&display=<%=xuatDung.getId()%>">
																							<img title="Hiển thị" src="../images/Display20.png" alt="Hiển thị" width="20" height="20" longdesc="Hiển thị" border=0></A>
																						<%} %>
																						<%if(quyen[1]!=0 && (xuatDung.getTrangThai().trim().equals("0"))){ %>
																						<A href="../../Erp_DanhMucDuAnSvl?userId=<%=userId%>&chot=<%=xuatDung.getId()%>" onclick="return chot(<%=xuatDung.getId()%>);">
