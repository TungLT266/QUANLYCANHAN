...
<% ResultSet lspList = (ResultSet)obj.getLoaisanpham(); 
	...
	int[] quyen = new  int[5];
	quyen = util.Getquyen("ErpDonmuahangList_GiaySvl","&loai="+obj.getLoai(),userId);
...
<body>
<form name="erpDmhForm" method="post" action="../../ErpDonmuahangList_GiaySvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng > Mua hàng nhập khẩu > Đơn mua hàng
        </div>
                   		...
                   		if(dmhList != null)
                   		{
                   			...
                   			{  		
				                ...
				                <% if(tt.equals("0")){ %>
				               			 ...
				               			 <A href = "../../ErpDonmuahangUpdate_GiaySvl?userId=<%=userId%>&display=<%= dmhList.getString("dmhId") %>&duyet=<%= duyet %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;
				              			 <%if(quyen[0] != 0 ){ %>
				                		
		                               <A href = "../../ErpDonmuahangUpdate_GiaySvl?userId=<%=userId%>&update=<%= dmhList.getString("dmhId") %>&duyet=<%= duyet %>">
			                    	...
			                    	<%  
		                               ...
		                               <%if(quyen[3]!=0){ %>
			                                 <A id='chotphieu<%=dmhList.getString("dmhId")%>'
										      ...
										      border="0" onclick="if(!confirm('Bạn có muốn chốt đơn hàng này?')) {return false ;}else{ processing('<%="chotphieu"+dmhList.getString("dmhId")%>' , '../../ErpDonmuahangList_GiaySvl?userId=<%=userId%>&chot=<%= dmhList.getString("dmhId")%>&loaidh=<%= obj.getLoai() %>');}"  >
										    </A>
									    <%} %>
                                    ...
                                    <%if(quyen[2]!=0){ %>
                                		<A href = "../../ErpDonmuahangList_GiaySvl?userId=<%=userId%>&delete=<%= dmhList.getString("dmhId") %>&loaidh=<%= obj.getLoai() %>"><img src="../images/Delete20.png" width="20" height="20" border=0 
                                	...
                                	<%} %>
                                	 <%if(quyen[3]!=0){ %>
	                                	<%if(dmhList.getString("DUYET").equals("0")) { %>
	                                		<A href = "../../ErpDonmuahangList_GiaySvl?userId=<%=userId%>&hoantat=<%= dmhList.getString("dmhId") %>&loaidh=<%= obj.getLoai() %>"><IMG src="../images/hoantat.gif" alt="Hoàn tất đơn mua hàng" title="Hoàn tất đơn mua hàng" border=0 onclick="if(!confirm('Bạn có muốn hoàn tất đơn mua hàng này?')) return false;" ></A>&nbsp;
	                                	<%} %>
                                	<%} %>
		                            ...
		                            <%	String cohoadon = dmhList.getString("iscohoadon")==null?"0":dmhList.getString("iscohoadon");
		                               ...
		                               <%if(quyen[3]!=0){ %>
			                                 <A id='chotphieu<%=dmhList.getString("dmhId")%>'
										      ...
										      border="0" onclick="if(!confirm('Bạn có muốn bỏ chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+dmhList.getString("dmhId")%>' , '../../ErpDonmuahangList_GiaySvl?userId=<%=userId%>&unchot=<%= dmhList.getString("dmhId") %>&loaidh=<%= obj.getLoai() %>');}"  >
											</A>
									    <%} }}%>
                                	<%if(dmhList.getString("DUYET").equals("0")) { %>
                                		<A href = "../../ErpDonmuahangList_GiaySvl?userId=<%=userId%>&hoantat=<%= dmhList.getString("dmhId") %>&loaidh=<%= obj.getLoai() %>"> <IMG src="../images/hoantat.gif"  alt="Hoàn tất đơn mua hàng" title="Hoàn tất đơn mua hàng" border=0 onclick="if(!confirm('Bạn có muốn hoàn tất đơn mua hàng này?')) return false;"></A>&nbsp;
		                         ...
		                         %>		                         
		                            	<A href = "../../ErpDonmuahangUpdate_GiaySvl?userId=<%=userId%>&display=<%= dmhList.getString("dmhId") %>&duyet=<%= duyet %>">
