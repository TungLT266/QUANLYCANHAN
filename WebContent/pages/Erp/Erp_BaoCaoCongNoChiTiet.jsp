...
%>
<%
	IErp_BaoCaoCongNoChiTietList obj = (IErp_BaoCaoCongNoChiTietList) session.getAttribute("obj");
...
<%
int[] quyen = new  int[5];
quyen = util.Getquyen("Erp_BaoCaoCongNoChiTietSvl","",userId);
%>
...
var nhomkh = $('#nhomdoituong');
			...
			if (loaiDoiTuongNoId == "" && (nhomkh != null && nhomkh.val()==""))
			{
			...
			if (doiTuongNoId == "" && (nhomkh != null && nhomkh.val()==""))
			{
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="xdForm" method="post" action="../../Erp_BaoCaoCongNoChiTietSvl">
						...
						<TR>
							<TD align="left" class="tbnavigation">
								<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
									<TR height="22">
										<TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý kế toán > Báo cáo > Sổ chi tiết công nợ của 1 tài khoản</TD>
											...
											<FIELDSET>
												...
												<TABLE class="tblight" width="100%" cellpadding="6"
													...
													<TR style="display: none;">
														<TD width="19%" class="plainlabel">Mã chi phí trả
															trước</TD>
														<TD class="plainlabel" colspan=5><INPUT name="maCCDC"
														...
														<TD class="plainlabel" colspan=5><INPUT
															name="soChungTu" type="text" size="30" value='<%=""%>'
															onChange="submitform();">
														</TD>
													</TR>
													<TR>
														<TD width="19%" class="plainlabel">Từ ngày</TD>
														<TD class="plainlabel" ><INPUT name="tuNgay"
															id="tuNgay" class="days" type="text" size="30"
															value='<%=obj.getDieuKienLoc().getTuNgay()%>'
															readonly="readonly">
														</TD>

														<TD width="19%" class="plainlabel">Đến ngày</TD>
														<TD class="plainlabel" colspan=5><INPUT
															name="denNgay" id="denNgay" class="days" type="text"
															size="30" value='<%=obj.getDieuKienLoc().getDenNgay()%>'
															readonly="readonly">
														</TD>
													</TR>
													<!-- 													style="display: none;" -->
													<TR>
														...
														<TD class="plainlabel"><select name="congTyLId"
															style="width: 300px;">
													...
													<TR>
														...
														<TD class="plainlabel" <%if(obj.getDieuKienLoc().getLoaiNhom().trim().length() == 0) {%> colspan="7" <%}%>><select
															name="soHieuTaiKhoanNo" id="soHieuTaiKhoanNo"
															style="width: 300px;" onchange="changTK()">
																<option value=""></option>
																<%
													...
													<TR>
														...
														<TD class="plainlabel"><select
															name="loaiDoiTuongNoId" id="loaiDoiTuongNoId"
															onchange="submitform(0);" style="width: 300px;">
																<option value=""></option>
																<%
													...
													<TR>
														...
														<TD class="plainlabel">
															<select name="doiTuongNoId"  id="doiTuongNoId" style="width: 300px;">
																...
																%>
																	<option value="<%=item.getValue() %>" selected><%=item.getMaFast() + " - " +  item.getName() %></option>
																<%
																		} else {
																%>
																	<option value="<%=item.getValue() %>"><%=item.getMaFast() + " - " +  item.getName() %></option>
																<%
																	}
																%>
															</select>
													...
													<TR>
														<TD class="plainlabel" colspan="2"><a class="button2"
															...
															href="javascript:submitform(1);""> <img
																style="top: -4px;" src="../images/button.png" alt="">Tìm
																kiếm</a> &nbsp;&nbsp;&nbsp;&nbsp; <a class="button3"
															href="javascript:exportExcel()"><img
										...
										<TD width="100%">
											<FIELDSET>
												<LEGEND class="legendtitle">
													&nbsp;Sổ chi tiết công nợ của 1 tài khoản &nbsp;&nbsp; 
												</LEGEND>
												<TABLE width="100%">
													<TR>
														<TD width="98%">
															<TABLE width="100%" border="0" cellspacing="2" cellpadding="2">
																<TR class="tbheader" id="task_flyout">
																	<TH width="3%">STT</TH>
																	<TH width="5%">Số hóa đơn</TH>
																	<TH width="8%">Ngày ghi nhận</TH>
																	<TH width="8%">Ngày chứng từ</TH>
																	<TH width="15%">Diễn giải chứng từ</TH>
																	<TH width="8%">Tài khoản đối ứng</TH>
																	<TH width="8%">Phát sinh nợ</TH>
																	<TH width="8%">Phát sinh có</TH>
																	<TH width="10%">Loại chứng từ</TH>
																	<TH width="5%">Mã chứng từ</TH>
																</TR>
													<%
														if (bcRS != null) {
															try {
																...
																<TR class=<%=darkrow%>>
																	...
																	%>
																	<TD align="center" width="3%"><%= m + 1 %></TD>
																	<TD align="center" width="3%"><%= bcRS.getString(1) %></TD>
																	<TD align="center" width="8%"><%= bcRS.getString(2) %></TD>
																	<TD align="center" width="8%"><%= bcRS.getString(3) %></TD>
																	<TD align="right"  width="15%"><%= bcRS.getString(4)%></TD>
																	<TD align="center"  width="8%"><%= bcRS.getString(5)%></TD>
																	<TD align="right"  width="8%"><%= formatter.format(bcRS.getDouble(6))%></TD>
																	<TD align="right"   width="8%"><%= formatter.format(bcRS.getDouble(7))%></TD>
																	<TD align="left" width="10%"><%= bcRS.getString(8) %></TD>
																	<TD align="center" width="5%"><%= bcRS.getString(9) %></TD>
																</TR>
																<%
																	m++;
																}
