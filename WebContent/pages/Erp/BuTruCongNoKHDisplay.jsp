...
System.out.println("USE JSP: "+userId);
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	&nbsp;Quản lý công nợ &gt; Công nợ phải thu &gt; Bù trừ công nợ KH &gt; Hiển thị
        </div>
            ...
            <table width="100%" cellspacing="0" cellpadding="6px">                
                ...
                <TR>
                 ...
                 <TR>
        			...
        			<SELECT name="khchuyennoId" id="khchuyennoId" style = "width:200px" onChange = "submitform();">
   					<option value = "" >&nbsp;</option>
   					<%
   					if(KHChuyenNoList != null)
   					{
   						try
   						{
   							while(KHChuyenNoList.next())
   							{  
   								if( KHChuyenNoList.getString("pk_seq").equals(nvgnBean.getKHChuyenNoId())){ %>
   							<option value="<%= KHChuyenNoList.getString("pk_seq") %>" selected="selected" ><%= KHChuyenNoList.getString("MA") %> - <%= KHChuyenNoList.getString("nppTen")%></option>
   								<%}else { %>
   							<option value="<%= KHChuyenNoList.getString("pk_seq") %>" ><%= KHChuyenNoList.getString("MA") %> - <%= KHChuyenNoList.getString("nppTen") %></option>
   		 						<%} 
   							} 
   							KHChuyenNoList.close();
   						} 
   						catch(java.sql.SQLException ex){}
   					}
   					%>
                    ...
                    <TD class="plainlabel" valign="middle">
						<input type="text" id="tongchuyenno" value="<%=formatter.format( Double.parseDouble(nvgnBean.getTongchuyenno())) %>"  readonly="readonly" > 													 
					</TD>	
             ...
             <div id="xoanokh" align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
	             <table width="100%" border="0" cellspacing="1" cellpadding="1">
						...
						<TR>
							...
							<TD align="center">
								<input type="hidden" readonly="readonly" name="hdId" style="width: 100%" value="<%= hdId[i]%>" > 
								<input type="hidden" readonly="readonly" name="hdloai" style="width: 100%" value="<%= hdLoai[i] %>" >
								<input type="hidden" readonly="readonly" name="hdtigia" style="width: 100%" value="<%= hdTigia[i]%>" > 	 	
								<input type="text" readonly="readonly" name="hdten" style="width: 100%" value="<%= loaihd %>" > 						
							</TD>
							<TD align="center">
							...
							<TD align="center">
								<input type="text" readonly="readonly" name="hdsotiengoc" style="width: 100%" value="<%= formatter.format( Double.parseDouble(hdSotiengoc[i]))  %> " >
							</TD>
							<TD align="center">
								<input type="text" readonly="readonly" name="hdsotienphaixoano" style="width: 100%" value="<%= formatter.format( Double.parseDouble(hdSotienphaixoa[i])) %> " >
							</TD>																												
							<TD align="center">
								<input type="text"  name="hdsotienxoa" style="width: 100%" value="<%=formatter.format( Double.parseDouble( hdSotienxoa[i]))  %> " onChange="Change_NO_CO(100);" onkeypress="return keypress(event);" > 
							</TD>
							<TD align="center">
								<input type="text" readonly="readonly" name="hdsotienconlai" style="width: 100%" value="<%= formatter.format( Double.parseDouble(hdSotienconlai[i])) %>" > 
							</TD>
							<TD align="center">
							<% if (hdChon[i].equals(hdId[i] )){ %>
								<input name="chon" id="chon"<%=stt %>  value="<%= hdId[i] %>" type="checkbox"  checked  onChange="Change_NO_CO( <%= i  %> );" >
							<%}else{ %>											
								<input name="chon" id="chon"<%=stt %> type="checkbox" value="<%= hdId[i]%>"  onChange="Change_NO_CO( <%= i  %> );" >
							<%} %>
						</TD> 
	                    </TR> <%
	                    stt++; }} %>
			...
			<LEGEND class="legendtitle" style="color: black">Khách hàng nhận nợ</LEGEND>       
              <table width="100%" cellspacing="0" cellpadding="6px">
             	<TR>
        			...
        			<SELECT name="khnhannoId" id="khnhannoId" style = "width:200px" >
   					<option value = "" >&nbsp;</option>
   					<%
   					System.out.println("KHNHANNO:"+nvgnBean.getKHNhanNoId());
   					
   					if(KHNhanNoList != null)
   					{
   						try
   						{
   							while(KHNhanNoList.next())
   							{  
   								if( KHNhanNoList.getString("pk_seq").equals(nvgnBean.getKHNhanNoId())){ %>
   							<option value="<%= KHNhanNoList.getString("pk_seq") %>" selected="selected" ><%= KHNhanNoList.getString("MA") %> - <%= KHNhanNoList.getString("nppTen")%></option>
   								<%}else { %>
   							<option value="<%= KHNhanNoList.getString("pk_seq") %>" ><%= KHNhanNoList.getString("MA") %> - <%= KHNhanNoList.getString("nppTen") %></option>
   		 						<%} 
   							} 
   							KHNhanNoList.close();
   						} 
					...
					<TD class="plainlabel" valign="middle">Tổng </TD> 
                    <TD class="plainlabel" valign="middle">
						<input type="text" id="tongnhanno" value="<%=formatter.format( Double.parseDouble(nvgnBean.getTongnhanno())) %>"  readonly="readonly" > 													 
					</TD>
