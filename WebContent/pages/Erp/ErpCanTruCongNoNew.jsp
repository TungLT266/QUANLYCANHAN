...
<%@page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<% List<IHoadon> hoaDonKHList;   
	System.out.println("khachhnag jsp :"+ obj.getKhId());
	%>
	
	<%  if(trangThai.length()==0 || "0".equals(trangThai)){
			hoaDonKHList = obj.getListHoaDonKhachHang();
			System.out.println("truong hop 1");
		}else{
			System.out.println("truong hop 2");
			hoaDonKHList = obj.getListHoaDonKhachHang2();
		}%>
...
// 	tru dan KH
	...
	TinhTongCanTru("Ncc");
	TinhTongCanTru("KH"); */
	TinhTongTienHoaDon("KH");
	TinhTongTienHoaDon("Ncc");
}

function TinhTongTienHoaDon(loai)
{
	//
	...
	for(var i = 1; i<SelectedList.length + 1;i++)
		{
		 	  if(document.forms["cpk"]["select"+loai+"#"+i].checked)
			  {
			    ...
			    document.forms["cpk"]["xoaNo"+loai+"#"+i].value= giatri;
			     
			  }else
			  {
				  giatri =  document.forms["cpk"]["phaiXoaNo"+loai+"#"+i].value;
					 
				  var xn = document.forms["cpk"]["xoaNo"+loai+"#"+i].value; 
				  
				  document.forms["cpk"]["conLai"+loai+"#"+i].value = giatri - xn ;
				  
				  document.forms["cpk"]["select"+loai+"#"+i].checked = false;
			  }	  
		}	
	

	
	var xoaNoKHList = document.getElementsByName("xoaNo"+loai);	
	
	var tong = parseFloat("0");
	var giatri = 0;
	for(var i = 1; i<xoaNoKHList.length + 1;i++)
		{
			  giatri =  document.forms["cpk"]["xoaNo"+loai+"#"+i].value;
			  	while (giatri.indexOf(",")>=0) {
				  giatri = giatri.replace(",","");
				}
			  			 			  
			  var giatri1= document.forms["cpk"]["phaiXoaNo"+loai+"#"+i].value;
			  while (giatri1.indexOf(",")>=0) {
				  giatri1 = giatri1.replace(",","");
				}
			  
			  if(parseFloat(giatri1)>= parseFloat(giatri)){
				  //neu lon hon moi duoc xoa no		
					document.forms["cpk"]["xoaNo"+loai+"#"+i].value = formatCurrency(parseFloat(giatri));
			  		document.forms["cpk"]["conLai"+loai+"#"+i].value=formatCurrency(parseFloat(giatri1)-parseFloat(giatri));
			  }else{
	...
	// alert(tong);
	if(loai=="KH"){
		document.getElementById("tongCanTruKH").value =formatCurrency(tong);
	}else{
		document.getElementById("tongCanTruNcc").value =formatCurrency(tong);
	}
	var tongtienncc=document.getElementById("tongCanTruNcc").value;
	
	 while (tongtienncc.indexOf(",")>=0) {
		 tongtienncc = tongtienncc.replace(",","");
		}
	  
	
	  
	var tongtienkh=document.getElementById("tongCanTruKH").value;
	
	 while (tongtienkh.indexOf(",")>=0) {
		 tongtienkh = tongtienkh.replace(",","");
		}
	var condu=parseFloat(tongtienncc)-parseFloat(tongtienkh);
	document.getElementById("SoTienConDuCanTru").value =formatCurrency(condu);
	
}
 
function TinhTongKH(loai)
{
	var xoaNoKHList = document.getElementsByName("xoaNoKH"+loai);
	var tong = parseFloat("0");
	var giatri = 0;
	for(var i = 1; i<xoaNoKHList.length + 1;i++)
		{
		 
			  giatri =  document.forms["cpk"]["xoaNoKH"+loai+"#"+i].value;
			  	while (giatri.indexOf(",")>=0) {
				  giatri = giatri.replace(",","");
				}
...
<SCRIPT language="JavaScript" type="text/javascript">
	...
	function save()
	{
		...
		if(soChungTu.length==0)
			{
				setErrors("Vui lòng điền số chứng từ");
		    	return;
			}
		 var sotienTT = document.forms["cpk"].tongCanTruKH.value;
		 var SoTienConDuCanTru = document.forms["cpk"].SoTienConDuCanTru.value;

	    if(sotienTT==0)
	    	{
	    		setErrors("Bạn phải chọn Hóa đơn của khách hàng và nhà cung cấp để tạo cấn trừ công nợ!.");
		    	return;
	    	}
		 
	    if(SoTienConDuCanTru!=0)
    	{
    		setErrors("Số tiền cần trừ phải bằng nhau,vui lòng điều chỉnh lại!.");
	    	return;
    	}
						...
						<tr>
							<TD align="left" colspan="4" class="legendtitle">
								<FIELDSET>
									...
									<textarea name="dataerror"  id="dataerror" style="width:100%;margin:0px " readonly="readonly" rows="1">
									<%=obj.getMsg() %></textarea>
									
									<% System.out.print("query1242141:"+obj.getMsg()); %>
								</FIELDSET></TD>
						</tr>
											...
											<td colspan="4">
												...
												<table width="100%" cellpadding="0px" cellspacing="1px">
													...
													<input style="width: 100%;text-align: center;" name="rowIdNcc" type="text" readonly="readonly" value=<%=(i+1)%>>
													 ...
													 <input type="hidden" style="width: 100%;" value="<%= hoadon.getTigia() %>" name= "tigiaHoaDonNcc" readonly="readonly" > 													 
													</td>
													<td>
													  <input type="hidden" style="width: 100%;" value="<%= hoadon.getLoaihd1() %>" name= "loaiHoaDonNcc" readonly="readonly" >  
													  <input style="width: 100%;" name="tenLoaiHdNcc" id=<%="tenLoaiHdNcc#"+(i+1) %> type="text" readonly="readonly" value="<%=hoadon.getTenloaihd1() %>" >
													</td>
													...
													<td> <input style="width: 100%;text-align: right;" name="soTienNcc" id=<%="soTienNcc#"+(i+1) %> type="text" readonly="readonly" value="<%=hoadon.getSoTienGoc2()%>" > </td>
													<td> <input style="width: 100%;text-align: right;" name= "phaiXoaNoNcc" id=<%="phaiXoaNoNcc#"+(i+1) %> type="text" value="<%=hoadon.getSotienno()%>" readonly="readonly" >  </td>
													<td> <input style="width: 100%;text-align: right;" name= "xoaNoNcc" id=<%="xoaNoNcc#"+(i+1) %> type="text" value="<%=hoadon.getThanhtoan()%>" onchange="tinhToanChenhLech();"  > </td>
													...
													double thanhtoan=0;
													try{
														thanhtoan=Double.parseDouble(hoadon.getThanhtoan().replace(",",""));
													}catch(Exception err ){
														
													}
													double  Sotienno=0;
													try{
														Sotienno=Double.parseDouble(hoadon.getTongtiencoVAT().replace(",",""));
													}catch(Exception err ){
							...
							<TD height="100%" width="100%">
								<FIELDSET>
									...
									<TR>
											...
											<TD class="plainlabel" valign="middle">
									<%if("0".equals(trangThai)){ %>
											<input type="text" id="kh" name="kh" value = "<%=obj.getKhTen() %>" style="width: 400px" disabled="disabled">  
											<%}else{ %>
											<input type="text" id="kh" name="kh" value = "<%=obj.getKhTen() %>" style="width: 400px">  
											<%} %>
                        		...
                        		<input readonly="readonly" id = "tongCanTruKH" name= "tongCanTruKH"  type="text" style="width: 200px" value = "<%=formatter.format(Double.parseDouble(obj.getSoTienThanhToan())) %>">  
                        		Còn dư <input readonly="readonly" id = "SoTienConDuCanTru" name = "SoTienConDuCanTru" style="width: 200px" type="text" value = "0">
                        		</TD> 
										</TR>
										...
										<TR>
											<td colspan="4">
												...
												<table width="100%" cellpadding="0px" cellspacing="1px">
													...
													<td> <input style="width: 100%;text-align: right" name="xoaNoKH"  onchange="tinhToanChenhLech();" type="text" id = <%="xoaNoKH#"+(i+1) %> value = <%=hoadon.getThanhtoan()%>  ></td>
													...
													double thanhtoan=0;
													try{
														thanhtoan=Double.parseDouble(hoadon.getThanhtoan().replaceAll(",",""));
													}catch(Exception err ){
														
													}
													double  Sotienno=0;
													try{
														Sotienno=Double.parseDouble(hoadon.getSotienno().replaceAll(",",""));
													}catch(Exception err ){
					...
					<TABLE width="100%" border="0" cellpadding="0" cellspacing="1">
						<tr>
							<TD align="left" colspan="4" class="legendtitle">
								<FIELDSET>
									<LEGEND class="legendtitle">Thông báo </LEGEND>
									<textarea name="dataerror"  id="dataerror" style="width:100%;margin:0px " readonly="readonly" rows="1"><%=obj.getMsg() %></textarea>
						...
						<TR>
							<TD height="100%" width="100%">
								...
								<TR>
                        			...
                        			<TD class="plainlabel" valign="middle">Tổng </TD> 
                        			<TD class="plainlabel" valign="middle"><input type="hidden" id="tongNcc" value="0"  readonly="readonly" >
										<input type="text" id="tongCanTruNcc" value="<%=formatter.format(Double.parseDouble(obj.getSoTienThanhToan())) %>"  readonly="readonly" > 													 
									</TD>														
								</TR>							
										<TR>
											<td colspan="4">
												...
												<table width="100%" cellpadding="0px" cellspacing="1px">
													...
													double  Sotienno=0;
													try{
														Sotienno=Double.parseDouble(hoadon.getSotienno().replace(",",""));
													}catch(Exception err ){
							...
							<TD height="100%" width="100%">
								<FIELDSET>
									...
									<TD class="plainlabel" valign="middle">
		                     		...
		                     		<input type="hidden" id="isNPP" name="isNPP" value = "<%=obj.getisNPP() %>" style="width: 500px" >  
                        		</TD>
                        		<TD class="plainlabel" valign="middle">Tổng    </TD> 
                        		<TD class="plainlabel" valign="middle"> 
                        		
                        		<input readonly="readonly" id = "tongKH" type="hidden" value = "0">
                        		<input readonly="readonly" name= "tongCanTruKH" id = "tongCanTruKH" type="text" value = "0"> 
                        		
                        			 Còn lại 
                        			 	<input readonly="readonly"  name = "SoTienConDuCanTru"  id = "SoTienConDuCanTru" type="text" value = "0"> 
						                	...
						                	for(int i = 0; i < hoaDonKHList.size(); i++)
						                	{
						                		IHoadon hoadon = hoaDonKHList.get(i);
							               		%>
													...
													<td> <input style="width: 100%;text-align:right" name="phaiXoaNoKH" type="text" id = <%="phaiXoaNoKH#"+(i+1) %> value=<%= hoadon.getSotienno()%> readonly="readonly"></td>
													<td> <input style="width: 100%;text-align:right" name="xoaNoKH" type="text" id = <%="xoaNoKH#"+(i+1) %> value = <%=hoadon.getThanhtoan()%>   ></td>
													<td><input style="width: 100%;text-align:right" name="conlaiKH" type="text" id = <%="conLaiKH#"+(i+1) %> value=<%=hoadon.getConlai_CantruCN() %> readonly="readonly"> </td>
													<td>
													<%  
													double thanhtoan=0;
