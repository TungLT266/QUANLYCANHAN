...
<%  
	...
	if(!util.check(userId, userTen, sum)){
		response.sendRedirect("/TraphacoHYERP/index.jsp");
		 ...
		 int[] quyen = new  int[5];
		 quyen = util.Getquyen("ErpYeuCauChuyenKhoSvl","&task=chuyenNL",userId);
...
<head>
    ...
    <SCRIPT language="javascript" type="text/javascript">
	 ...
	 {   
	    ...
	    document.forms["erpLsxForm"].khonhanId.value = "";
	    console.log( "gia tri" + document.forms["erpLsxForm"].khonhanId.value  );
...
<body>
<form name="erpLsxForm" method="post" action="../../ErpYeuCauChuyenKhoSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý tồn kho > Chức năng  > <%=(obj.getIsnhanHang().equals("LSX")?"Phiếu yêu cầu ":"Yêu cầu chuyển kho")%> 
        </div>
            ...
            <%}
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    ...
	                    <TH align="center" width="10%">Ngày yêu cầu</TH>
	                 	<TH align="center" width="10%">Số hóa đơn</TH>  
	                    <TH align="center" width="12%">Kho nhận</TH> 
	                    ...
	                    <TH align="center" width="10%">Chi nhánh/ đối tác đặt</TH>
					...
					<%
                 		if(lsxList != null)
                 		{
							...
							<TD ><%= lsxList.getString("nppdat") %></TD>		
		                    <TD align="center"> 
		                  	  <% if(tt.equals("0")){ %> 
		                    
		                    	<%if(quyen[2]!=0){ %>
		                 			<A href = "../../ErpYeuCauChuyenKhoUpdateSvl?userId=<%=userId%>&copy=<%=lsxList .getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/copy20.png" alt="Copy" title="Copy" border=0></A>&nbsp;
                                	<A href = "../../ErpYeuCauChuyenKhoUpdateSvl?userId=<%=userId%>&update=<%=lsxList .getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                                <%} %>
                                 <%if(quyen[4]!=0){ %>
                                ...
                                <%} %>
                                 <%if(quyen[1]!=0){ %>
                                	<A href = "../../ErpYeuCauChuyenKhoSvl?userId=<%=userId%>&delete=<%= lsxList.getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><img src="../images/Delete20.png" alt="Xóa chuyển kho" title="Xóa chuyển kho" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa yêu cầu chuyển kho này không?')) return false;"></A>
                                <%} %>	
		                    ...
		                    <% } else  if(tt.equals("1")   ) { %> 
			                    	...
			                    	<%if(quyen[2]!=0){ %>
			                    		<A href = "../../ErpYeuCauChuyenKhoUpdateSvl?userId=<%=userId%>&copy=<%=lsxList .getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/copy20.png" alt="Copy" title="Copy" border=0></A>&nbsp;
	                                	<A href = "../../ErpYeuCauChuyenKhoSvl?userId=<%=userId%>&taochuyenkho=<%=lsxList .getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/nhanhang.png" alt="Chuyển kho" title="Chuyển kho" border=0></A>&nbsp;
	                                <%} %>
	                            
			                    	<A href = "../../ErpYeuCauChuyenKhoUpdateSvl?userId=<%=userId%>&display=<%= lsxList.getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" width="20" height="20" border=0></A>&nbsp;
					                <%  
		                       ...
		                       <% } else  if(  tt.equals("2")) { %> 
				                    	...
				                    	<A href = "../../ErpYeuCauChuyenKhoUpdateSvl?userId=<%=userId%>&display=<%= lsxList.getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" width="20" height="20" border=0></A>&nbsp;
				                    	<%if(quyen[2]!=0){ %>
				                    	 <A href = "../../ErpYeuCauChuyenKhoSvl?userId=<%=userId%>&hoantat=<%= lsxList.getString("PK_SEQ") %>&task=<%=obj.getIsnhanHang()%>"><img src="../images/hoantat.gif" alt="Hoàn tất chuyển kho" title="Hoàn tất chuyển kho" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn hoàn tất phiếu yêu cầu chuyển kho  này?')) return false;" ></A>&nbsp;
				                    	 <%} %>		
