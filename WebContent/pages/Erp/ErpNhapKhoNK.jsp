...
<% 	IErpNhapkhoNKList obj = (IErpNhapkhoNKList)session.getAttribute("obj");%>
<% ResultSet dvthList = (ResultSet)obj.getDvthList(); %>
<% ResultSet rsNK = (ResultSet)obj.getNkList() ;%>
...
<% obj.setNextSplittings();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[5];
		quyen = util.Getquyen("ErpNhapkhoNKSvl", "",userId);
%>
...
<script type="text/javascript">
	...
	<SCRIPT language="javascript" type="text/javascript">
	 ...
	 {   
	    ...
	    document.forms["erpNhForm"].sonhapkho.value = "";
...
<body>
	<form name="erpNhForm" method="post" action="../../ErpNhapkhoNKSvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 70%; padding: 5px; float: left" class="tbnavigation">Quản lý mua hàng > Mua hàng nhập khẩu > Nhập kho nhập khẩu</div>
			...
			<div id="cotent" style="width: 100%; float: none">
					...
					<fieldset style="margin-top: 5px">
						<legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
							<TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px">
								...
								<TR>
									<TD class="plainlabel" >Mã nhập kho</TD>
									<TD  class="plainlabel">
										<input type="text" id="sonhapkho" name="sonhapkho" value="<%= obj.getSoNK() %>" onchange="submitform()" />
									</TD>
				...
				<div align="left" style="width: 100%; float: none; clear: left; font-size: 0.7em">
					<fieldset>
						<legend>
							<span class="legendtitle"> Nhập kho nhập khẩu </span>&nbsp;&nbsp;
								<%-- <%if(quyen[0]!=0){ %> 
					...
					<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
						...
						<%
						    ...
						    if (rsNK != null)
							    while (rsNK.next()) {
							    		String msg= rsNK.getString("tooltip")==null?"":rsNK.getString("tooltip");
		                 				if((m % 2 ) == 0) {%>
										<TR onMouseover='ddrivetip("<%= msg %>" , 700)'
									onMouseout="hideddrivetip()" class='tbdarkrow'>
										<%}else{ %>									
										<TR onMouseover='ddrivetip("<%= msg %>" , 700)'
									onMouseout="hideddrivetip()" class='tblightrow'>
										<%} %>
										
										<TD align="center"><%= rsNK.getString("SOCHUNGTU") %></TD>
										<TD align="center"><%= rsNK.getString("NGAYNHAP") %></TD>
										<TD align="left"><%= rsNK.getString("NCCTEN") %></TD>
										<TD align="center"><%= rsNK.getString("POID") %></TD>
										<TD align="center">
										<%
				                        String trangthai = "";
										
				                        String tt = rsNK.getString("TRANGTHAI");
									...
									<TD align="center"><%= rsNK.getString("NGAYTAO") %></TD>
									<TD align="center"><%= rsNK.getString("NGUOITAO") %></TD>
									<TD align="center"><%= rsNK.getString("NGAYSUA") %></TD>
									<TD align="center"><%= rsNK.getString("NGUOISUA") %></TD>
									<TD align="center">
										<% if(tt.equals("0")){ %> 
										<%if(quyen[2]!=0){ %>
											<A href="../../ErpNhapkhoNKUpdateSvl?userId=<%=userId%>&update=<%= rsNK.getString("NKID") %>">
											<IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A><%} %>&nbsp; 
											
										<%if(quyen[4]!=0){ %>
											<A id='chotphieu<%= rsNK.getString("NKID") %>' href="../../ErpNhapkhoNKUpdateSvl?userId=<%=userId%>&chot=<%= rsNK.getString("NKID")%>&nccId=<%=rsNK.getString("NCCID") %> ">
									...
									<%if(quyen[1]!=0){ %>
										<A href="../../ErpNhapkhoNKSvl?userId=<%=userId%>&delete=<%= rsNK.getString("NKID") %>">
										<img src="../images/Delete20.png" alt="Xóa nhập kho" title="Xóa nhận hàng" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa nhập kho này?')) return false;"></A>
									<% } } else if(tt.equals("1")) 
									{  if(rsNK.getInt("CONXUATHANG") == 0 ) {%>
										<A href="../../ErpParkHoadonnhapkhauSvl?userId=<%=userId%>&convert=<%= rsNK.getString("NKID") %>&nccId=<%=rsNK.getString("NCCID") %>&poId=<%=rsNK.getString("NKID") %>">
										<IMG src="../images/Next.png" alt="Chuyển thành hóa đơn NCC" title="Chuyển thành hóa đơn NCC" border=0></A>&nbsp;&nbsp;
									<%} %>
										<A href="../../ErpNhapkhoNKUpdateSvl?userId=<%=userId%>&display=<%= rsNK.getString("NKID")  %>">
									...
									<%  
										...
										<% if(quyen[3]!=0){ %><A href="../../ErpNhapkhoNKUpdateSvl?userId=<%=userId%>&display=<%= rsNK.getString("NKID")  %>">
											<IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
										 <% }  %>
