...
<% obj.setNextSplittings();
   ResultSet rskhoNhan = (ResultSet) obj.getKhonhanRs();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[5];
		quyen = util.Getquyen("ErpNhanhangnhapkhau_GiaySvl", "&loai="+obj.getLoaimh(),userId);
%>
...
<body>
	<form name="erpNhForm" method="post" action="../../ErpNhanhangnhapkhau_GiaySvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			...
			<div id="cotent" style="width: 100%; float: none">
				...
				<!-- Phần hóa đơn mà chưa có nhận hàng, đang nằm chờ -->
				<div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
		        	<fieldset>
			                	...
			                	if(rsHoaDon !=null){
			                		while(rsHoaDon.next()){
										...
										<%} %>
						                    ...
						                    <TD align="center" >
						                    	<% if( daNhanHang== 0){ %>
						                    	<A href="../../ErpNhanhangnhapkhau_GiaySvl?userId=<%=userId%>&convert=<%= rsHoaDon.getString("PK_SEQ") %>&NCCId=<%= 
						...
						<%
								...
								for(int i =0; i < htList.size(); i ++)
									...
									<TD align="center">
										<% if(tt.equals("0")){ %> 
										<%if(quyen[2]!=0){ %>
											<A href="../../ErpNhanhangnhapkhauUpdate_GiaySvl?userId=<%=userId%>&update=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
											<IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A><%} %>&nbsp; 
										<%if(quyen[4]!=0){ %>
											<A id='chotphieu<%= ht.getId()%>' href="../../ErpNhanhangnhapkhau_GiaySvl?userId=<%=userId%>&chot=<%= ht.getId() %>&lhhId=<%= ht.getloaihanghoaId() %>&loai=<%= obj.getLoaimh()%> ">
									...
									<%if(quyen[1]!=0 && tt.equals("0")){ %>
										<A href="../../ErpNhanhangnhapkhau_GiaySvl?userId=<%=userId%>&delete=<%= ht.getId() %>&poId=<%= ht.getPoId() %>&loai=<%= obj.getLoaimh() %>">
									...
									<% } } else 
										{
											/* if(tt.equals("2")){ */
										%>
										<% if(quyen[3]!=0){ %><A href="../../ErpNhanhangnhapkhauUpdate_GiaySvl?userId=<%=userId%>&display=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
										...
										<% if(quyen[2]!=0 && DaRaHd==0 && tt.equals("1") ){ %>
											<A href="../../ErpNhanhangnhapkhauUpdate_GiaySvl?userId=<%=userId%>&suaSoHd=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
