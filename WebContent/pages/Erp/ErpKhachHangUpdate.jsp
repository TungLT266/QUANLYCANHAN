...
<script language="javascript" type="text/javascript">
	...
	function ChonCK() {
		document.forms['khBeanForm'].action.value = 'ChonCK';
		document.forms['khBeanForm'].submit();
	}
...
/* 	if(nganhang.value=="")
	...
	function Save() {
		...
		if(msg.value==''){
			congDonSPCungMa();
			document.forms['khBeanForm'].action.value = 'Update';
			document.forms['khBeanForm'].submit();
		}else
			{
			return;
	...
	function submitform()
	 {   
		var hanmucdh = document.getElementById("hanmucdh");
		if(hanmucdh == '') hanmucdh = '0';
		
		hanmucdh.value = DinhDangTien(hanmucdh);
						...
						<tr>
							<td align="left" class="tbnavigation">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr height="22">
										<TD align="left" colspan="2" class="tbnavigation">&nbsp;Dữ liệu nền &gt; Kinh doanh &gt; Khách hàng &gt; Cập nhật</TD>
							...
							<TD align="left" colspan="4" class="legendtitle">
								...
								<TD class="plainlabel">
										...
										<TR>
											...
											<td class="plainlabel"><input type="text" name="mst" id="mst" value="<%=khBean.getMST()%>"   /></td>
										...
										<tr>											
											...
											<TD class="plainlabel">
												...
												%> <input type="checkbox" name="trangthaiHD" value="1" checked="checked"> <%
 												...
 												%> <input type="checkbox" name="trangthaiHD" value="1"> <%
 													}
 												%>
											</TD>
										</TR>
...
<%// Bắt đầu Tab chiết khấu 
	...
	for(int i = 0; i < kbhId.length; i++){
		...
		chungloaiSP = (List<IErpKhachHang_ChungLoaiSP>)kbh_ckcl.get(kbhId[i]);
%>
				<TABLE width="100%" cellspacing="0" cellpadding="2">
						...
						<TD colspan="3">															
							<DIV id = "tabContaier">
								<DIV class="tbheader">
									...
									<div align ="left"><B><%= "Chiết khấu " + tenkbh %></B> </div>
	   	        					...
	   	        					<DIV id="<%= "tab1" + kbhId[i] %>" class="tabContents" >
										...
										<TR>
		                                	...
		                                	<TD class="plainlabel" align = "left">	                                		
		                                		<INPUT name="<%= "hanmuc" + kbhId[i] %>" id="hanmuc" type="text" value="<%= formatter.format(Double.parseDouble(hanmuc[i].replaceAll(",", ""))) %>" onkeypress="return keypress(event);" >
		                                		
		                                	</TD> 
		                                
		                                	<TD class="plainlabel"  align = "left">Thời hạn nợ</TD>
		                                	<TD class="plainlabel"  align = "left">
		                                		<INPUT name="<%= "thoihan" + kbhId[i] %>" id="thoihan" type="text" value="<%= thoihan[i].replaceAll(",", "") %>"  onkeypress="return keypress(event);" >
		                                	</TD>
								  		</TR>
										...
										<TR>
		                                	...
		                                	<TD class="plainlabel" align = "left">	                                		
		                                		<INPUT name="<%= "cktructiep" + kbhId[i] %>" id="cktructiep" type="text" value="<%= cktt[i].replaceAll(",", "") %>" onkeypress="return keypress(event);" >
		                                	</TD>
		                                
		                                	<TD class="plainlabel"  align = "left">% Ứng chiết khấu</TD>
		                                	<TD class="plainlabel"  align = "left">
		                                		<INPUT name="<%= "ungck" + kbhId[i] %>" id="ungck" type="text" value="<%= ungck[i].replaceAll(",", "") %>"  onkeypress="return keypress(event);" >
		                                	</TD>
								  		</TR>
										<TR class="tbheader" >
		     	   		 				...
		     	   		 				<%if(chonck[i].equals("1")) { %>
		     	   							<INPUT type="radio" name="<%= "chonchietkhau" + kbhId[i] %>" value="1" onclick="ChonCK();" checked>Chiết khấu theo sản phẩm &nbsp;
		     	   						<%} else { %>
		     	   							<INPUT type="radio" name="<%= "chonchietkhau" + kbhId[i] %>" value="1" onclick="ChonCK();">Chiết khấu theo sản phẩm &nbsp;
		     	   						<%} %>
		     	   			
		     	   							</TD>
		     	   							<TD class="plainlabel" id="menuchon" colspan = "3"  align = "left">
		     	   						<%if(chonck[i].equals("2")) { %>
		     	   							<INPUT type="radio" name="<%= "chonchietkhau" + kbhId[i] %>" value ="2" onclick="ChonCK();" checked>Chiết khấu theo chủng loại &nbsp;
		     	   						<%} else { %>
		     	   							<INPUT type="radio" name="<%= "chonchietkhau" + kbhId[i] %>" value ="2" onclick="ChonCK();">Chiết khấu theo chủng loại &nbsp;
		     	   						<%} %>
								...
								%> 
												<TR class= 'tblightrow' >
													<TD align="left" >
														...
														<input name= "<%= "maChungLoaiSP" + kbhId[i] %>" type="text" value="<%=clSPDetail.getMaChungLoai() %>" readonly style="width:100%">
													</TD>
													<TD align = "center" >
								    					<input name= "<%= "tenChungLoaiSP" + kbhId[i] %>" type="text" value="<%= clSPDetail.getTenChungLoai() %>"  readonly style="width:100%">
								    				</TD>							    
								    				<TD align = "center" >
