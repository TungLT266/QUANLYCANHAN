...
<% IErpDenghimuahangList obj = (IErpDenghimuahangList)session.getAttribute("obj"); %>
...
<% ResultSet lspList = (ResultSet)obj.getLoaisanpham(); 
	...
	int[] quyen = new  int[5];
	quyen = util.Getquyen("ErpDenghimuahangSvl","",userId);
...
<body>
<form name="erpDmhForm" method="post" action="../../ErpDenghimuahangSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng > Mua hàng VT/CPDV/TSCĐ/CCDC > Đề nghị mua hàng
        </div>
  	...
  	<div id="cotent" style="width:100%; float:none">
                ...
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >								                          
                    ...
                    <TR>
                        ...
                        <TD class="plainlabel" valign="middle" >
							...
							<% if(obj.getTrangthai().equals("0;0")){ %>
                            	<option value="0;0" SELECTED >Chưa duyệt(Chưa chốt)</option>
                            	<option value="0;1" >Chưa duyệt(Đã chốt)</option>
                            	<option value="1;1" >Đã duyệt(Đã chốt)</option>
                            	
                            	<option value="3; " >Đã xóa</option>
                            	<option value="4; " >Đã hủy</option>
                            <%}else if(obj.getTrangthai().equals("0;1")){ %>
                              	<option value="0;0"  >Chưa duyệt(Chưa chốt)</option>
                            	<option value="0;1" SELECTED >Chưa duyệt(Đã chốt)</option>
                            	<option value="1;1" >Đã duyệt(Đã chốt)</option>
                            	
                            	<option value="3; " >Đã xóa</option>
                            	<option value="4; " >Đã hủy</option>
                            <%}else if(obj.getTrangthai().equals("1;1")){ %>                          
                              	<option value="0;0"  >Chưa duyệt(Chưa chốt)</option>
                            	<option value="0;1" SELECTED >Chưa duyệt(Đã chốt)</option>
                            	<option value="1;1" >Đã duyệt(Đã chốt)</option>
                            	<option value="3; " >Đã xóa</option>
        ...
        <div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
        	<fieldset>
            	<legend><span class="legendtitle"> Đề nghị mua hàng </span>&nbsp;&nbsp;
            	<%if(quyen[0]!=0){ %>
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    ...
	                    <TH align="center" width="7%">Mã ĐNMH</TH>
...
//                    				}
		                        ...
		                        <%}else{ %>
		                          	<TR class='tblightrow'  >
		                        <%} %>
		                        	<%-- <TD align="center"><%= dmhList.getString("SOTHAMCHIEU") == null?"N/A":dmhList.getString("SOTHAMCHIEU") %></TD> --%>
				                    <TD align="center" onMouseover="ddrivetip('<%=msg %>', 300)"; onMouseout="hideddrivetip()"><%= dmhList.getString("sodenghi") %></TD>
				                    <TD align="center"><%= dmhList.getString("ngaydenghi") %></TD>
				                    	...
				                    	<%}
				                    		...
				                    		String trangthai = "";
					                    		...
					                    		if(tt.equals("0"))
					                    		{
					                    			...
					                    			if(dmhList.getString("ISTRUONGBPDUYET").equals("0"))
					                    				if(!dachot.equals("1")){
					                    					trangthai = "Chưa duyệt(Chưa chốt)";
					                    				}else{
		                		...
		                		<% if(tt.equals("0")){ %>
               			 			...
               			 			<A href = "../../ErpDenghimuahangUpdateSvl?userId=<%=userId%>&display=<%= dmhList.getString("dmhId") %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;   
			                    	<%  
			                    		//if( !dmhList.getString("DUYET").equals("0") && dachot.equals("0") ){
		                    			if(dachot.equals("0") ){
		                    			%>
		                    				<%if(quyen[0] != 0 ){ %>
		                               		<A href = "../../ErpDenghimuahangUpdateSvl?userId=<%=userId%>&update=<%= dmhList.getString("dmhId") %>">
		                                 	...
		                                 	<A id='chotphieu<%=dmhList.getString("dmhId")%>'
										      ...
										      border="0" onclick="if(!confirm('Bạn có muốn chốt đề nghị mua hàng này?')) {return false ;}else{ processing('<%="chotphieu"+dmhList.getString("dmhId")%>' , '../../ErpDenghimuahangSvl?userId=<%=userId%>&chot=<%= dmhList.getString("dmhId")%>');}"  >
										    </A>
									    	<%} %>
		                            		...
		                            		<% if(quyen[5]!=0){%>
		                    			 		<A id='chotphieu<%= dmhList.getString("dmhId") %>'
											      ...
											      src="../images/unChot.png" alt="Bỏ chốt" title="Bỏ chốt"
											       ...
											       border="0" onclick="if(!confirm('Bạn có muốn bỏ chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+dmhList.getString("dmhId")%>' , '../../ErpDenghimuahangSvl?userId=<%=userId%>&unchot=<%= dmhList.getString("dmhId") %>');}"  >
												</A>
											<%} %>
		                            	<%} %>
		                            
                                    	<%if(quyen[2]!=0){ %>
                                			<A href = "../../ErpDenghimuahangSvl?userId=<%=userId%>&delete=<%= dmhList.getString("dmhId") %>"><img src="../images/Delete20.png" width="20" height="20" border=0 
		                         ...
		                         <%} else { %>
		                            	...
		                            	<A href = "../../ErpDenghimuahangUpdateSvl?userId=<%=userId%>&display=<%= dmhList.getString("dmhId") %>">
