...
<script language="javascript" type="text/javascript">
	...
	function TraTienCT(stt)
	{
		 ...
		 tongcong = tinhTongTienCua1Cot(document.getElementsByName("tienthanhtoan"));
			tongcongVND = tinhTongTienCua1CotCoTiGia(document.getElementsByName("tienthanhtoan"), document.getElementsByName("tigiachungtu")); 
		if(tienteId == "100000")
		{
	...
	function TraTienHD(stt)
	{
		...
		//	 tienthanhtoan = '0';
		 ...
		 tongcong = 0; 
		 tongcongVND = 0;
		 tongcong = tinhTongTienCua1Cot(document.getElementsByName("thanhtoan"));
		 tongcongVND = tinhTongTienCua1CotCoTiGia(document.getElementsByName("thanhtoan"), document.getElementsByName("tigiaHD"));
	...
	function tinhTongTienCua1Cot(cotArray) {
		var tongTienCua1Cot = 0.0;
		for (var i = 0; i < cotArray.length; i++) {
			tongTienCua1Cot +=parseFloat(cotArray[i].value.trim().length > 0 ? parseFloat(cotArray[i].value.trim().replace(/,/g,"")) : 0.0);
		}
		return tongTienCua1Cot;
	}
	
	function tinhTongTienCua1CotCoTiGia(cotArray, tigia) {
		var tongTienCua1Cot = 0.0;
		for (var i = 0; i < cotArray.length; i++) {
			var tiGia = 1.0;
			if (tigia != null && tigia.length > 0) {
				tiGia = (tigia[i].value.trim().length > 0)? parseFloat(tigia[i].value.trim().replace(/,/g,"")) : 1.0;
			}
			tongTienCua1Cot += parseFloat(cotArray[i].value.trim().length > 0 ? parseFloat(cotArray[i].value.trim().replace(/,/g,""))*tiGia : 0.0);
			console.log(tongTienCua1Cot);
		}
		return tongTienCua1Cot;
	}
	 ...
	 function ThanhToan(n)
	 {
		 ...
		 if(tienteId == "100000")
			...
			else
			{
				...
				document.getElementById("chenhlech").value = DinhDangDonGia((parseFloat(sotienungtruoc) - tongtien).toFixed(2));
				 
				//document.getElementById("sotienungtruocVND").value = DinhDangTien(roundNumber(tongctttVND, 0));
				document.getElementById("tongthanhtoanVND").value = DinhDangTien(roundNumber(tongtienVND, 0));
				document.getElementById("chenhlechVND").value = DinhDangTien(roundNumber(parseFloat(sotienungtruocVND) - tongtienVND, 0)); 
			}
	...
	{    
	 ...
	 {	
		 ...
		 if(tienteId == "100000")
		{
			 if(document.getElementById("chenhlech").value < 0){
				 alert("Số tiền ứng trước phải lớn hơn số tiền thanh toán !!!");
				 return;
			 }
		}else{
			if(document.getElementById("chenhlechVND").value < 0){
				 alert("Số tiền ứng trước VNĐ phải lớn hơn số tiền thanh toán VNĐ !!!");
				 return;
			 }
		}
				...
				<TR>
					<TD align="left" class="tbnavigation">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
						  <tr height="22">
							 <TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý công nợ > Công nợ phải thu > Xóa nợ khách hàng > Tạo mới</TD> 
			...
			<TR ><TD >
					<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
						<TR class = "tbdarkrow">
						    ...
						    <div id="btnSave">
						    <A href="javascript:saveform()" ><IMG src="../images/Save30.png" title="Lưu lai" alt="Lưu lai" border = "1"  style="border-style:outset"></A>
						    </div>
						    </TD>
							<TD >&nbsp; </TD>						
						</TR>
		...
		<TABLE width="100%" border="0" cellpadding="0"  cellspacing="1" >
		  	<tr>
				<TD align="left" colspan="4" class="legendtitle">
    				...
    				<textarea name="dataerror"  style="width: 100% ;  font-weight:bold"  style="width: 100% ; color:#ffffff ; font-weight:bold" 
    				readonly="readonly" rows="1"><%= tthdBean.getMsg() %></textarea>
					</FIELDSET>
			   </TD>
			</TR>			
			<TR>
                        	...
                        	<%
                        		if(nppList != null)
                        		{
                        			...
                        			if( nppList.getString("pk_seq").equals(tthdBean.getNppId())){ %>
                        				<option value="<%= nppList.getString("pk_seq") %>" selected="selected" ><%= nppList.getString("MA") %> - <%= nppList.getString("nppTen") %></option>
                        			<%}else { %>
                        				<option value="<%= nppList.getString("pk_seq") %>" ><%= nppList.getString("MA") %> - <%= nppList.getString("nppTen") %></option>
                            ...
                            if(ctttList.size() > 0)
                            {
                             ...
                             <tr>
                              <td>
                               ...
                               <%} else if(hoadonTT.getLoaict().equals("3")) { %>
                               <input type="text" style="width: 100%" value="Chi phí khác"> 
                              <%}else if(hoadonTT.getLoaict().equals("12")) { %>
                              <input type="text" style="width: 100%" value="Đề nghị thanh toán"> 
                             <%}else{ %>
                		...
                		String loaichungtu = "";
	               		...
	               		<tr>
	               			<% if( hoadon.getLoaihd().equals("0")) loaichungtu = "Hóa đơn tài chính";
           	 					...
           	 					else if ( hoadon.getLoaihd().equals("6")) loaichungtu = "Thanh toán hóa đơn";
           	 				else if ( hoadon.getLoaihd().equals("5")) loaichungtu = "Tạm ứng";
           	 				%>
           	 				<td align="center">
