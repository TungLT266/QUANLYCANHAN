...
<% obj.setNextSplittings();
   ResultSet rskhoNhan = (ResultSet) obj.getKhonhanRs();
	...
	if(!util.check(userId, userTen, sum)){
		...
		quyen = util.GetquyenNew("ErpNhanhangsanxuatSvl", "",userId);
		System.out.println("không con gì: ");
%>
	...
	<SCRIPT language="javascript" type="text/javascript">
	 ...
	 {   
	  ...
	  /*   document.forms["erpNhForm"].ncc.value = ""; */
	    ...
	    document.forms["erpNhForm"].khonhanId.value = ""; 
	    document.forms["erpNhForm"].lsxid.value = ""; 
...
<body>
	<form name="erpNhForm" method="post" action="../../ErpNhanhangsanxuatSvl">
		<input type="hidden" name="userId" value="<%= userId %>"> 
		<input type="hidden" name="action" value="1"> 
		<input type="hidden" name="loaimh" value="<%= obj.getLoaimh() %>"> 
		<input type="hidden" name="crrApprSplitting" value="<%= obj.getCrrApprSplitting() %>">
		<input type="hidden" name="nxtApprSplitting" value="<%= obj.getNxtApprSplitting() %>"> 
		<input type="hidden" name="sopo" value="<%= obj.getSoPO() %>" >
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				...
				<div style="width: 70%; padding: 5px; float: left" class="tbnavigation">Quản lý cung ứng > Sản xuất > Nhận hàng sản xuất</div>
						...
						<legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
							<TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px">
								...
								<TR >
									...
									<TD class="plainlabel" valign="middle">
										<input type="text" name="lsxid" value="<%= obj.getLsxId()%>" onchange="submitform()">
									</TD>
								</TR>
							...
							<TR>
		                  	...
		                  	<tr>
					            ...
					            <TD width="150px" class="plainlabel" valign="top">Kho nhận hàng đạt:</TD>
			                		...
			                		while(rsHoaDon.next()){
										...
										<%} %>
						                    ...
						                    <TD align="center" >
						                    	...
						                    	<A href="../../ErpNhanhangsanxuatSvl?userId=<%=userId%>&convert=<%= rsHoaDon.getString("PK_SEQ") %>&NCCId=<%= 
				...
				<div align="left" style="width: 100%; float: none; clear: left; font-size: 0.7em">
					...
					<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
						<TR class="tbheader">
							...
							<TH align="center" width="7%">Số lệnh sản xuất</TH>
								...
								for(int i =0; i < htList.size(); i ++)
									{
										...
										<TD align="center"><%= ht.getMasanpham() %></TD>
										<TD align="center"><%= ht.getTensanpham() %></TD>
									...
									<TD align="center">
										...
										<%if(quyen[Utility.SUA]!=0){ %>
											<A href="../../ErpNhanhangsanxuatUpdateSvl?userId=<%=userId%>&update=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
											<IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A><%} %>&nbsp; 
										<%if(quyen[Utility.CHOT]!=0){ %>
											<A id='chotphieu<%= ht.getId()%>' href="../../ErpNhanhangsanxuatSvl?userId=<%=userId%>&chot=<%= ht.getId() %>&lhhId=<%= ht.getloaihanghoaId() %>&loai=<%= obj.getLoaimh()%> ">
									...
									<%if(quyen[Utility.XOA]!=0 && tt.equals("0")){ %>
										<A href="../../ErpNhanhangsanxuatSvl?userId=<%=userId%>&delete=<%= ht.getId() %>&poId=<%= ht.getPoId() %>&loai=<%= obj.getLoaimh() %>">
									...
									<% } } else 
										...
										<% if(quyen[Utility.XEM]!=0){ %><A href="../../ErpNhanhangsanxuatUpdateSvl?userId=<%=userId%>&display=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
...
try{
	if(htList!=null) htList.clear();
//	if(dvthList!=null) dvthList.close();
	if(nhanhangList!=null) nhanhangList.close();
