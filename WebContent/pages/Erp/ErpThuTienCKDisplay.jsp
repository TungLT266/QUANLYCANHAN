...
<%
	IErpThutienCK tthdBean = (IErpThutienCK) session
			.getAttribute("tthdBean");
%>
<%
...
<%
	...
	ResultSet DoituongkhacRs = tthdBean.getDoituongkhacList();
%>
<%
...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
		var nppTen =  document.getElementsByName("npp");
		var nppId =  document.getElementsByName("nppId");
	
			if(nppTen.item(0).value != "" )
			{
				var temp = nppTen.item(0).value;
				var pos = parseInt(temp.indexOf(" - "));
				if(pos > 0)
				{
					/// PK_SEQ VÀ nppTEN phân cách bằng ' - ', mã và tên phân cách bằng ' -- '
					nppId.item(0).value = temp.substring(0, pos);
					nppTen.item(0).value = temp.substring(pos+3, temp.length);
				}
			}
			else
			{
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="tthdForm" method="post" action="../../ErpThutienUpdateSvl">
		<input type="hidden" name="userId" value='<%=userId%>'> <input
			type="hidden" name="id" value='<%=tthdBean.getId()%>'> <input
			type="hidden" name="action" value='1'>

		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left"
					class="tbnavigation">Quản lý công nợ &gt; Công nợ phải thu
					&gt; Thu tiền chuyển khoản &gt; Hiển thị</div>
			...
			<div align="left" id="button"
				...
				<A href="javascript: goBack();"><img src="../images/Back30.png"
					alt="Quay ve" title="Quay ve" border="1" longdesc="Quay ve"
					style="border-style: outset">
				</A> <A
					href="../../ErpThutienPdfSvl?userId=<%=tthdBean.getUserId()%>&id=<%=tthdBean.getId()%>">
					<IMG src="../images/Printer30.png" title="In phieu" alt="In phieu"
			...
			<div align="left" style="width: 100% %; float: none; clear: left">
				<fieldset>
					...
					<textarea name="dataerror" rows="1" readonly="readonly"
						style="width: 100%"><%=tthdBean.getMsg()%></textarea>
					<%
						tthdBean.setMsg("");
					%>
				</fieldset>
				...
				<fieldset>
					...
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="4" cellspacing="0" border="0">
							<TR>
								<TD width="170px" class="plainlabel" valign="top">Ngày
									chứng từ</TD>
								<TD class="plainlabel" width="300px" valign="top"><input
									type="text" class="days" id="ngaychungtu" name="ngaychungtu"
									value="<%=tthdBean.getNgaychungtu()%>" readonly />
								</TD>
								<TD width="170px" class="plainlabel" valign="top">Ngày ghi
									sổ</TD>
								<TD class="plainlabel" valign="top" colspan=3><input
									type="text" class="days" id="ngayghiso" name="ngayghiso"
									value="<%=tthdBean.getNgayghiso()%>" readonly />
								</TD>
							</TR>
							...
							<TR>
								...
								<TD class="plainlabel" colspan="5"><input type="text"
									name="nguoinoptien" value="<%=tthdBean.getNguoinoptien()%>"
									style="width: 200px"></TD>
							</TR>

							<TR>
								<TD class="plainlabel">Nội dung thanh toán</TD>
								<TD class="plainlabel" colspan="5"><select name="noidungId"
									id="noidungId" style="width: 200px">
										<option value=""></option>
										<%
											if (ndList != null) {
												try {
													while (ndList.next()) {
														if (ndList.getString("pk_seq").equals(
																tthdBean.getNoidungId())) {
										%>
										<option value="<%=ndList.getString("pk_seq")%>"
											selected="selected"><%=ndList.getString("ten")%></option>
										<%
											} else {
										%>
							...
							<TR>
								<TD class="plainlabel">Số chứng từ</TD>
								 <TD class="plainlabel">
			                        <input type="text" name="soChungTu_Chu" id="soChungTu_Chu" value="<%= tthdBean.getSoChungTu_Chu() %>" onKeyPress = "return validateSpecialCharacters(event);" readonly="readonly" onchange="upperLetters(this);" style="width: 100px;">
			                        <input type="text" name="soChungTu_So" id="soChungTu_So" value="<%= tthdBean.getSoChungTu_So() %>" style="width: 50px;" onkeypress="return keypress(event);" readonly="readonly">
			                    </TD> 
								<TD class="plainlabel">Tiền tệ</TD>
								<TD class="plainlabel" style="width: 100px"><select
									name="tienteId" id="tienteId">
										<%
											if (tienteList != null) {
												try {
													while (tienteList.next()) {
														if (tienteList.getString("pk_seq").equals(
																tthdBean.getTienteId())) {
										%>
										<option value="<%=tienteList.getString("pk_seq")%>"
											selected="selected"><%=tienteList.getString("ten")%></option>
										<%
											} else {
										%>
								...
								<TD class="plainlabel" style="width: 50px" align="right">Tỉ
									giá</TD>
								<TD class="plainlabel"><input type="text" name="tigia"
									Id="tigia"
									value="<%=formatter.format(Double.parseDouble(tthdBean.getTigia()
					.replaceAll(",", "")))%>"
									style="width: 100px; text-align: right;" readonly></TD>
							...
							<TR>
								...
								<TD class="plainlabel">Khách hàng /Nhóm khách hàng <%
									if (nkhtt.equals("1")) {
								%>
									<input type="checkbox" name="nhomkhtt" value="1"
									checked="checked" onChange="submitform();"> <%
 	} else {
 %>
									<input type="checkbox" name="nhomkhtt" value="0"
									onChange="submitform();"> <%
 	}
 %>
								</TD>
								...
								<TD colspan=5 class="plainlabel"><select
									name="nhomkhttId" id="nhomkhttId" style="width: 300px"
											...
											if (nhomkhttList != null) {
														try {
															while (nhomkhttList.next()) {
																if (nhomkhttList.getString("pk_seq").equals(
																		tthdBean.getNhomkhttId())) {
										%>
										<option value="<%=nhomkhttList.getString("pk_seq")%>"
											selected="selected"><%=nhomkhttList.getString("ten")%></option>
										<%
											} else {
										%>
								...
								%>
								<TD class="plainlabel">
			                     <input type="text" id="npp" name="npp" value = "<%= tthdBean.getKh_npp_Ten() %>" style="width: 300px"  >  
			                     <input type="hidden" id="nppId" name="nppId" onChange="ChangeTienTe();" value = "<%= tthdBean.getnppIdgoc() %>" style="width: 300px"  >
			                     </TD>

								<TD class="plainlabel">Kênh bán hàng</TD>
								<TD class="plainlabel" colspan="3"><select name="kbhId"
									id="kbhId" style="width: 200px">
										<%
											if (kbhList != null) {
														try {
															while (kbhList.next()) {
																if (kbhList.getString("pk_seq").equals(
																		tthdBean.getKbhId())) {
										%>
										<option value="<%=kbhList.getString("pk_seq")%>"
											selected="selected"><%=kbhList.getString("Ten")%></option>
										<%
											} else {
										%>
							...
							<TR>
							    ...
							    <TD class="plainlabel" colspan="<%=tthdBean.getIsChuyenCN().equals("1") ? "1" : "5"%>">
									<%
									...
									%> <input type="checkbox"
									name="ischuyenCN" value="1" checked="checked"
									onChange="submitform();"> <%
 	} else {
 %> <input
									type="checkbox" name="ischuyenCN" value="0"
									onChange="submitform();"> <%
 	}
 %>
								</TD>
								...
								<TD class="plainlabel" colspan="5">
									...
									%> <input
									type="checkbox" name="isthuhoCN" id="isthuhoCN" value="1"
									checked="checked" onChange="submitform();"> <%
 	} else {
 %>
									<input type="checkbox" name="isthuhoCN" id="isthuhoCN"
									value="0" onChange="submitform();"> <%
 	}
 %>
								</TD>
								<%
							...
							<TR>
								...
								<TD class="plainlabel" colspan="5"><select
									name="chinhanhId" id="chinhanhId" style="width: 300px"
									onChange="ChangeTienTe();">
											...
											if (ChinhanhRs != null) {
														try {
															while (ChinhanhRs.next()) {
																if (ChinhanhRs.getString("pk_seq").equals(
																		tthdBean.getNPPChinhanhId())) {
										%>
										<option value="<%=ChinhanhRs.getString("pk_seq")%>"
											selected="selected"><%=ChinhanhRs.getString("nppTen")%></option>
										<%
											} else {
										%>
							...
							<%
// 								if (!(tthdBean.getIsChuyenCN().equals("1") && tthdBean.getIsthuhoCN().equals("0"))) {
							%>
							<TR>
								<TD class="plainlabel">Khách hàng </TD>
				      			<TD class="plainlabel" colspan="5" >
						                     <input type="text" id="npp" name="npp" value = "<%= tthdBean.getKh_npp_Ten() %>" style="width: 300px"  >  
						                     <input type="hidden" id="nppId" onChange="ChangeTienTe();" name="nppId" value = "<%= tthdBean.getNppId() %>" style="width: 300px"  >
						        </TD>    
							</TR>
							<%
// 								}
...
<TR>   
	<%if( !(tthdBean.getDoiTuongDinhKhoan().equals("")) ) { %>
	 ...
	 <%}%> 				          		  					                      
							...
							<TR>
								...
								%>
								<TD width="140px" class="plainlabel" valign="top">Số tiền
									thực thu (ngoại tệ)</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									id="thuduocNT" name="thuduocNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
							.getThuduocNT().replaceAll(",", "")))%>"
									style="text-align: right;" onchange="TinhChenhLech();"
								...
								<TD class="plainlabel" valign="top"
									<%if (tthdBean.getTienteId().equals("100000")) {%> colspan=5
									<%} else {%> colspan=3 <%}%>><input type="text"
									id="thuduoc" name="thuduoc"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getThuduoc().replaceAll(",", "")))%>"
							...
							<TR>
								...
								<TD class="plainlabel" valign="top"><select
									name="doituongtamung" id="doituongtamung" style="width: 200px"
									onChange="ChangeTienTe();">
								...
								<TD class="plainlabel" colspan="3"><select name="nccId"
									id="nccId" style="width: 200px" onChange="ChangeTienTe();">
										<option value=""></option>
										<%
											if (nccList != null) {
														try {
															while (nccList.next()) {
																if (nccList.getString("pk_seq").equals(
																		tthdBean.getNccId())) {
										%>
										<option value="<%=nccList.getString("pk_seq")%>"
											selected="selected"><%= nccList.getString("ma")%> - <%=nccList.getString("ten")%></option>
										<%
											} else {
										%>
										<option value="<%=nccList.getString("pk_seq")%>"><%= nccList.getString("ma")%> - <%=nccList.getString("ten")%></option>
										<%
											}
															}
															nccList.close();
								...
								<TD class="plainlabel" colspan="3"><select name="nvtuId"
									id="nvtuId" style="width: 200px" onChange="ChangeTienTe();">
										<option value=""></option>
										<%
											if (nvtutList != null) {
														try {
															while (nvtutList.next()) {
																if (nvtutList.getString("pk_seq").equals(
																		tthdBean.getNvtuId())) {
										%>
										<option value="<%=nvtutList.getString("pk_seq")%>"
											selected="selected"><%= nvtutList.getString("ma")%> - <%=nvtutList.getString("ten")%></option>
										<%
											} else {
										%>
										<option value="<%=nvtutList.getString("pk_seq")%>"><%= nvtutList.getString("ma")%> - <%=nvtutList.getString("ten")%></option>
										<%
											}
															}
															nvtutList.close();
							...
							<%} else if(tthdBean.getNoidungId().equals("100006")){ %> <!-- Thu tiền khách hàng quầy --> 
       		<TR>
 				 <TD class="plainlabel" >Chuyển xuống chi nhánh </TD>
               <TD class="plainlabel" colspan="<%=tthdBean.getIsChuyenCN().equals("1") ? "1" : "5"%>">
                  <%
                  ...
                  if (ischuyenCN.equals("1")){ %>
					 <input type="checkbox" name="ischuyenCN" value="1" checked="checked" disabled  onChange="submitform();"> 
					<%} else  { %> 
						<input type="checkbox" name="ischuyenCN" value="0" disabled  onChange="checkThuHoCN(); submitform();"> 
					<%} %>
               </TD>
               <% if(tthdBean.getIsChuyenCN().equals("1")) { // HIỆN CHI NHÁNH CHO HO CHỌN %>
             	<!-- Không cần hiển thị Thu hộ hóa đơn CN - Mặc định không tích chọn-->
             <!--   <TD class="plainlabel" >Thu hộ hóa đơn CN </TD> -->
                   <TD class="plainlabel" colspan="6" >
                   	<input type="hidden" name ="isthuhoCN" id ="isthuhoCN" value= "0">
                      <%-- <%
                      String isthuhoCN = tthdBean.getIsthuhoCN();	 
                 
                      if (isthuhoCN.equals("1")){ %>
						 <input type="checkbox" name="isthuhoCN" id="isthuhoCN" value="1" checked="checked" onChange="submitform();"> 
						<%} else  { %> 
							<input type="checkbox" name="isthuhoCN" id="isthuhoCN" value="0" onChange="submitform();"> 
						<%} %> --%>
                   </TD>
               <%} %>    
           </TR>
            ...
            <TR>
               	...
               	<TD class="plainlabel">Quầy / Trung tâm PP </TD>
     				<TD class="plainlabel" colspan="5">                        
           ...
           <%} %>
                <%} %>
							...
							<TR>
								...
								<TD class="plainlabel" colspan=5><select name="htttId"
									id="htttId" style="width: 200px" onChange="ChangeTienTe();">
										<!-- <option value=""> </option> -->
										<%
											if (htttList != null) {
												try {
													while (htttList.next()) {
														if (htttList.getString("pk_seq").equals(
																tthdBean.getHtttId())) {
										%>
										<option value="<%=htttList.getString("pk_seq")%>"
											selected="selected"><%=htttList.getString("ten")%></option>
										<%
											} else {
										%>
							...
							<TR>
								...
								<TD class="plainlabel" colspan="5"><select
									name="sotaikhoan" id="sotaikhoan" style="width: 600px">

										<%
											if (sotkRs != null) {
													try {
														while (sotkRs.next()) {
															if (sotkRs.getString("SOTAIKHOAN").equals(
																	tthdBean.getSotaikhoan())) {
										%>
										<option value="<%=sotkRs.getString("SOTAIKHOAN")%>"
											selected="selected"><%=sotkRs.getString("TAIKHOAN")%></option>
										<%
											} else {
										%>
							...
							<%
								if ((tthdBean.getNoidungId().equals("100004") && !tthdBean
										.getTienteId().equals("100000"))
										|| (tthdBean.getNoidungId().equals("100000") && !tthdBean
												.getTienteId().equals("100000"))
										|| (tthdBean.getNoidungId().equals("100003") && !tthdBean
												.getTienteId().equals("100000"))) { // Thu tien bán hàng/ Thu hoi tam ung va tien ngoai te
							%>
							<TR>
								<TD class="plainlabel">Số tiền hóa đơn (ngoại tệ)</TD>
								<TD class="plainlabel"><input type="text"
									style="text-align: right;" name="sotienthanhtoanNT"
									id="sotienthanhtoanNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
						.getSotienttNT()))%>"
								...
								<TD class="plainlabel" colspan=3><input type="text"
									style="text-align: right;" name="sotienthanhtoan"
									id="sotienthanhtoan"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getSotientt().replaceAll(",", "")))%>"
									AUTOCOMPLETE="off" readonly></TD>
						...
						.getThuduocNT()))%>"
								...
								<TD width="140px" class="plainlabel" valign="top">Số tiền
									thực thu (VNĐ)</TD>
								<TD class="plainlabel" valign="top" colspan=3><input
									type="text" id="thuduoc" name="thuduoc"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getThuduoc()))%>"
									style="text-align: right;" readonly /> <input type="hidden"
									id="cpnganhangNT" name="cpnganhangNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
						.getChiphinganhangNT()))%>"
									style="text-align: right;" readonly /> <input type="hidden"
									id="cpnganhang" name="cpnganhang"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getChiphinganhang()))%>"
									style="text-align: right;" readonly /></TD>
							</TR>
							...
							%>
							<TR>
								<TD width="140px" class="plainlabel" valign="top">Chiết
									khấu thanh toán (ngoại tệ)</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									name="chietkhauNT" id="chietkhauNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
							.getChietkhauNT().replace(",", "")))%>"
									style="text-align: right;" onchange="TinhChenhLech();"
									onKeyPress="return keypress(event);"></TD>

								<TD class="plainlabel" valign="top">Chiết khấu thanh toán
									(VNĐ)</TD>
								<TD class="plainlabel" colspan="3" valign="top"><input
									type="text" name="chietkhau" id="chietkhau"
									value="<%=formatter.format(Double.parseDouble(tthdBean
							.getChietkhau().replace(",", "")))%>"
							...
							<TR>
								...
								<TD class="plainlabel"><input type="text" name="ghichu"
									value="<%=tthdBean.getNoidungtt()%>" style="width: 300px"
									readonly></TD>

								<%
									if (!tthdBean.getNoidungId().equals("100004")) {
								%>
								<TD width="140px" class="plainlabel" valign="top">Chênh
									lệch (VNĐ)</TD>
								<TD class="plainlabel" valign="top" colspan="3"><input
									type="text" id="chenhlech" name="chenhlech"
									value="<%=formatter.format(Double.parseDouble(tthdBean
							.getChenhlech().replaceAll(",", "")))%>"
									style="text-align: right;" readonly />
								</TD>
								<%
									} else {
							...
							<%
								} else if ((tthdBean.getNoidungId().equals("100004") && tthdBean
										.getTienteId().equals("100000"))
										|| (tthdBean.getNoidungId().equals("100000") && tthdBean
												.getTienteId().equals("100000"))
										|| (tthdBean.getNoidungId().equals("100003") && tthdBean
												.getTienteId().equals("100000"))) { //Thu tien ban hang/ Thu hoi tam ung va VND
							%>
							<TR>
								<TD class="plainlabel">Số tiền hóa đơn (VNĐ)</TD>
								<TD class="plainlabel" colspan=5><input type="text"
						...
						.getSotientt().replaceAll(",", "")))%>"
							...
							<TR>
								<TD width="140px" class="plainlabel" valign="top">Số tiền
									thực thu (VNĐ)</TD>
								<TD class="plainlabel" valign="top" colspan=5><input
									type="text" id="thuduoc" name="thuduoc"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getThuduoc()))%>"
									style="text-align: right;" readonly /> <input type="hidden"
						...
						.getChenhlech()))%>"
							...
							<%
								} else if (tthdBean.getNoidungId().equals("100001")
										&& !tthdBean.getTienteId().equals("100000")) { //Thu tien khach hang ung truoc va ngoai te
							%>
							<TR>
								<TD width="140px" class="plainlabel" valign="top">Số tiền
									thực thu (ngoại tệ)</TD>
						...
						.getThuduoc()))%>"
									...
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
						.getChiphinganhangNT()))%>"
									style="text-align: right;" readonly /> <input type="text"
									id="cpnganhang" name="cpnganhang"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getChiphinganhang()))%>"
									style="text-align: right;" readonly /></TD>
							...
							<TR>
								<TD width="140px" class="plainlabel" valign="top">Tổng tiền
									trả trước (ngoại tệ)</TD>
								<TD class="plainlabel" valign="top"><input type="text"
									id="tongNT" name="tongNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
						.getTongNT()))%>"
									style="text-align: right;" readonly />
								</TD>
								...
								<TD width="140px" class="plainlabel" valign="top">Tổng tiền
									trả trước (VNĐ)</TD>
								<TD class="plainlabel" valign="top" colspan=3><input
									type="text" id="tongVND" name="tongVND"
									value="<%=formatter.format(Double.parseDouble(tthdBean
						.getTongVND()))%>"
									style="text-align: right;" readonly /></TD>
							...
							<TR>
								...
								<TD class="plainlabel">Diễn giải chứng từ</TD>
								<TD class="plainlabel" colspan="5"><input type="text"
									name="ghichu" value="<%=tthdBean.getNoidungtt()%>"
							...
							<%
										...
										& tthdBean.getTienteId().equals("100000")) { //Thu tien khach hang ung truoc va VND
							%>
							<TR>
								<TD width="140px" class="plainlabel" valign="top">Số tiền
									thực thu (VNĐ)</TD>
						...
						.getTongVND()))%>"
							...
							<%
								} else if (tthdBean.getNoidungId().equals("100002")
										&& !tthdBean.getTienteId().equals("100000")) { // THU TIỀN KHÁC VÀ NGOẠI TỆ
							%>
							<TR>
                    ...
                    <TD class="plainlabel" colspan = "5">                   
                        <input type="text" name="nguoinoptien" value="<%= tthdBean.getNguoinoptien() %>" style = "width:200px"> 
							...
							<TR>
								...
								<TD class="plainlabel" colspan="5"><input type="text"
									name="diachi" value="<%=tthdBean.getDiachi()%>"
									style="width: 300px"> <%
 	if (tthdBean.getHtttId().equals("100001")) {
 %>

									<input type="hidden" id="cpnganhangNT" name="cpnganhangNT"
									value="<%=formatterNT.format(Double.parseDouble(tthdBean
							.getChiphinganhangNT().replaceAll(",", "")))%>"
									style="text-align: right;" onKeyPress="return keypress(event);" />

									<input type="hidden" id="cpnganhang" name="cpnganhang"
									value="<%=formatter.format(Double.parseDouble(tthdBean
							.getChiphinganhang().replaceAll(",", "")))%>"
									style="text-align: right;" readonly /> <%
 	}
 %>
								</TD>
						...
						<%}else  if(tthdBean.getNoidungId().equals("100006") && !tthdBean.getTienteId().equals("100000")){ //Thu tien khach hang ung truoc va ngoai te  %>
                <TR>
            	...
            	<TR>
                    <TD width="140px" class="plainlabel" valign="top" style="display: none;">Tổng tiền trả trước (ngoại tệ)</TD>
                    ...
                    <TD width="140px" class="plainlabel" valign="top" style="display: none;">Tổng tiền trả trước (VNĐ)</TD>
                    <TD class="plainlabel" valign="top" colspan = 3 style="display: none;">
			...
			<%}else  if(tthdBean.getNoidungId().equals("100006") && tthdBean.getTienteId().equals("100000")){ //Thu tien khach hang ung truoc va VND  %>
                <TR>	                
	                 <TD width="140px" class="plainlabel" valign="top">Số tiền thực thu (VNĐ)</TD>
                    ...
                    <TD class="plainlabel" colspan = "5">                   
                        <input type="text" name="ghichu" value="<%= tthdBean.getNoidungtt() %>" style = "width:300px"> 
							...
							<%
										...
										&& tthdBean.getTienteId().equals("100000")) { // THU KHÁC VÀ VND
							%>
 ...
 %>
							...
							<TR>
								...
								<TD class="plainlabel" colspan="5"><input type="text"
									name="ctkemtheo" value="<%=tthdBean.getChungTuKemTheo()%>"
									style="width: 300px"></TD>
							</TR>
						</TABLE>
					...
					<%
						if (tthdBean.getNoidungId().equals("100002")) {
					%>
					<hr>
					<div align="left" style="width: 100%; float: none; clear: none;"
						...
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1"
							cellspacing="1">
							<tr class="tbheader">
								<th width="30%">Số hiệu tài khoản</th>
								<th width="35%">Đối tượng</th>
							...
							<%
								int j = 0;
									int count = 0;
									if (dinhkhoancoList.size() > 0) {
										System.out.println("dinhkhoancoList.size(): "
												+ dinhkhoancoList.size());
										for (j = 0; j < dinhkhoancoList.size(); j++) {
											IDinhkhoanco dkc = dinhkhoancoList.get(j);
											ResultSet doituongrs = null;
							%>
							<tr>
								<td><select name="taikhoanId" id="taikhoanId"
											...
											try {
															...
															if (RSTaiKhoan != null) {
																while (RSTaiKhoan.next()) {
																	...
																	if (dkc.getTaikhoanId().equals(
																			RSTaiKhoan.getString("pk_seq"))) {
										%>
										<option value="<%=RSTaiKhoan
												.getString("pk_seq")%>" selected><%=RSTaiKhoan
												.getString("sohieutk")%>
										</option>
										<%
											} else {
								...
								<td><input type="hidden" style="width: 100%"
									name="doituongdinhkhoan" id="doituongdinhkhoan"
									value="<%=dkc.getDoituongdinhkhoan()%>" /> <%
 	            if (dkc.getDoituongdinhkhoan().equals("1"))
 					doituongrs = SanphamRs;
 				else if (dkc.getDoituongdinhkhoan().equals("2"))
 					doituongrs = NganhangRs;
 				else if (dkc.getDoituongdinhkhoan().equals("3"))
 					doituongrs = NccRs;
 				else if (dkc.getDoituongdinhkhoan().equals("4"))
 					doituongrs = TaisanRs;
 				else if (dkc.getDoituongdinhkhoan().equals("5"))
 					doituongrs = KhachhangRs;
 				else if (dkc.getDoituongdinhkhoan().equals("6"))
 					doituongrs = NhanvienRs;
									
 				else if (dkc.getDoituongdinhkhoan().equals("8"))
 					doituongrs = DoituongkhacRs;
                   %> 
 
                     <%	if (dkc.getDoituongdinhkhoan().trim().length() > 0 && !dkc.getDoituongdinhkhoan().equals("0")) {%>
									<select name="doituongId" id="doituongId" style="width: 100%">
										<option value=""></option>
										<%
											try {
												doituongrs.beforeFirst();
												if (doituongrs != null) {
												  while (doituongrs.next()) {
																		if (dkc.getDoituongId().equals(
																				doituongrs.getString("pk_seq"))) {
										%>
										<option value="<%=doituongrs.getString("pk_seq")%>" selected><%= doituongrs.getString("ma")%> - <%=doituongrs.getString("ten")%></option>
										<%} else {%>
										<option value="<%=doituongrs.getString("pk_seq")%>"><%= doituongrs.getString("ma")%> - <%=doituongrs.getString("ten")%></option>
										<%}}
																}
															} catch (java.sql.SQLException e) {
															}
								 ...
								 <td>
                                    <%if (Double.parseDouble(dkc.getSotienNT().replaceAll(",", "")) > 0 ){ %>
                                 	<input type="text" style="width: 100%" name="sotienco" id = "sotienco" value="<%=dkc.getSotienNT() %>" align="left"  onchange="TinhTienDKC();" />
                                  <%}else { %>
                                    <input type="text" style="width: 100%" name="sotienco" id = "sotienco" value="<%=dkc.getSotien() %>" align="left"  onchange="TinhTienDKC();" />
                                  <%} %>
                                 </td>
							...
							<tr>
								<td><select name="taikhoanId" id="taikhoanId"
									onchange="submit();" style="width: 100%">
										...
										%>
										<option value=""></option>
										<option value="<%=RSTaiKhoan.getString("pk_seq")%>"><%=RSTaiKhoan.getString("sohieutk")%>
										</option>
										<%
											}
														}
								...
								<td><input type="hidden" style="width: 100%"
									name="doituongdinhkhoan" id="doituongdinhkhoan" value="" /> <select
									name="doituongId" id="doituongId" style="width: 100%">
										<option value=""></option>
								</select></td>

								<td><input type="text" style="width: 100%" name="sotienco"
									id="sotienco" value="" style="text-align: left"
									onchange="TinhTienDKC();" />
							</tr>
							<%
								}
							%>
					...
					<%
						if (tthdBean.getNoidungId().equals("100000")
								|| tthdBean.getNoidungId().equals("100003")
								|| tthdBean.getNoidungId().equals("100004")) {
					%>
					<hr>
					...
					<div align="left" style="width: 100%; float: none; clear: none;"
						...
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1"
							...
							<TR>
								...
								<TH align="center" width="10%"></TH>
								<TH align="center" width="10%"></TH>
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totaltongtien2" id="totaltongtien2" readonly="readonly">
								</TH>
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totaltongtien" id="totaltongtien" readonly="readonly">
								</TH>
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totalthanhtoan" id="totalthanhtoan" readonly="readonly">
								</TH>
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totalconlai" id="totalconlai" readonly="readonly">
								</TH>
							...
							<TR>
								...
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totaltongtienNT" id="totaltongtienNT" readonly="readonly">
								</TH>
								<TH align="center" width="10%"><input type="text"
									style="width: 100%; text-align: right;" value="0"
									name="totalthanhtoan" id="totalthanhtoan" readonly="readonly">
							...
							<tr>
								...
								<td align="center"><input type="hidden"
									style="width: 100%;" value="<%=hoadon.getLoaihd()%>"
									name="loaihd"> <input type="text"
									style="width: 100%;" value="<%=hoadon.getTenloaihd()%>"
									name="tenhd" readonly="readonly">
								<td align="center"><input type="text" style="width: 100%;"
									value="<%=hoadon.getMahopdong()%>" name="mahd"
								...
								<% } %>
								<td align="center"><input type="hidden"
									style="width: 100%;" value="<%=hoadon.getId()%>" name="idHd">
									<input type="text" style="width: 100%;"
									value="<%=hoadon.getKyhieu()%>" name="kyhieuhd"
									readonly="readonly"></td>
								<td align="center"><input type="text"
									style="width: 100%; text-align: center;"
									value="<%=hoadon.getSo()%>" name="sohd" readonly="readonly">

								</td>
								<td align="center"><input type="text"
									style="width: 100%; text-align: center;"
									value="<%=hoadon.getNgay()%>" name="ngayhd"
									readonly="readonly"></td>

								<td align="center"><input type="text"
									style="width: 100%; text-align: center;"
									value="<%=hoadon.getNgaydenhanTT()%>" name="ngaydenhanTT"
									id="ngaydenhanTT<%=i%>" readonly="readonly"></td>
								...
								<td align="center"><input type="text"
									...
									value="<%=formatter.format(Double.parseDouble(hoadon
								.getSotiengoc().replaceAll(",", "")))%>"
									name="stgoc" id="stgoc<%=i%>" readonly="readonly"></td>
								<%
									} else {
								%>
								<td align="center"><input type="text"
									style="width: 100%; text-align: right;"
									value="<%=formatterNT.format(Double.parseDouble(hoadon
								.getSotiengoc().replaceAll(",", "")))%>"
									name="stgoc" id="stgoc<%=i%>" readonly="readonly"></td>
								<%
									}
								...
								<td align="center"><input type="text"
									style="width: 100%; text-align: right;"
									value="<%=formatter.format(Double.parseDouble(hoadon
							.getTongtiencoVAT().replaceAll(",", "")))%>"
									name="avat" id="avat<%=i%>" readonly="readonly"></td>
								...
								<td align="center"><input type="text"
									style="width: 100%; text-align: right;"
									value="<%=formatterNT.format(Double.parseDouble(hoadon
								.getSotienNT().replaceAll(",", "")))%>"
									name="sotienNT" id="sotienNT<%=i%>" readonly="readonly">
								</td>

								<td align="center"><input type="text"
									style="width: 100%; text-align: right;"
									value="<%=formatterNT.format(Double.parseDouble(hoadon
								.getThanhtoan().replaceAll(",", "")))%>"
									name="thanhtoan" id="thanhtoan<%=i%>" readonly="readonly">
								</td>
								<%
									} else {
								%>
								...
								<td align="center"><input type="text"
									style="width: 100%; text-align: right;"
									value="<%=hoadon.getConlai()%>" name="conlai"
									id="conlai<%=i%>" readonly="readonly"></td>
								<td align="center">
									<%
										if (hoadon.getConlai().equals("0")) {
									%> <input type="checkbox"
									value="<%=hoadon.getId()%>" name="trahet" id="trahet<%=i%>"
									checked="checked" disabled> <%
 	} else {
 %> <input
									type="checkbox" value="<%=hoadon.getId()%>" name="trahet"
									id="trahet<%=i%>" disabled> <%
 	}
 %>
								</td>
							</tr>
...
<script type="text/javascript">
	jQuery(function()
	{		
		$("#npp").autocomplete("ErpKhachHangHOList.jsp");
	});	
	ThanhToan(100);
</script>
...
<%
	...
	if (dinhkhoancoList != null)
		dinhkhoancoList.clear();
	if (SanphamRs != null)
		SanphamRs.close();
	if (NganhangRs != null)
		NganhangRs.close();
	...
	if (DoituongkhacRs != null)
		DoituongkhacRs.close();
