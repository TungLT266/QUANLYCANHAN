...
<% 	
List<Erp_Item> loaiThanhToanList = tthdBean.getLoaiThanhToanList();
List<Erp_Item> doiTuongKhacList = tthdBean.getDoiTuongKhacList();
...
String[] diaChiHd = tthdBean.getDiaChiHd();
if(diaChiHd == null){
...
String[] ttcps = tthdBean.getTtcpIds();	
if(ttcps == null){
	ttcps = new String[count];
	for (int i = 0; i < count; i++){
		ttcps[i] = "";
	}
}
...
function ThanhToan(n)
{
	...
	var trahet = document.getElementsByName(
);
...
console.log("size tra het: " + thanhtoan.length);
	...
	{    
	 ...
	 {	
		 ...
		 document.getElementById("btnSave").innerHTML = "<a href='#'><img src='../images/waiting.gif'  title='cho luu..' border='1' longdesc='cho luu..' style='border-style:outset'> Processing...</a>";
	 	 document.forms['tthdForm'].action.value='saveTiGia';
	     document.forms['tthdForm'].submit();
	 }
...
<input type="hidden" name="daDuyet" value='<%= tthdBean.getDaDuyet() %>'>
<input type="hidden" name="trangThai" value='<%= tthdBean.getTrangThai() %>'>
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
        ...
        <%if (tthdBean.getDaDuyet() == 1 && tthdBean.getTrangThai() == 0) {%>
        <span id="btnSave">
        ...
        <%--   <A href="../../ErpUynhiemchiPdfSvl?userId=<%= tthdBean.getUserId() %>&id=<%= tthdBean.getId() %>&httt=<%= tthdBean.getHtttId() %>&inchitiet=0" >
	        ...
	        <A href="../../ErpUyNhiemChiExcelSvl?userId=<%= tthdBean.getUserId() %>&id=<%= tthdBean.getId() %>&httt=<%= tthdBean.getHtttId() %>&inchitiet=0" >
	        <IMG src="../images/Printer30.png" title="In phieu" alt="In phieu dieu chuyen tien" border ="1px" style="border-style:outset"></A>
	        
	        <A href="../../ErpUyNhiemChiExcelSvl?userId=<%= tthdBean.getUserId() %>&id=<%= tthdBean.getId() %>&httt=<%= tthdBean.getHtttId() %>&inchitiet=2" >
	        <IMG src="../images/excel30.gif" title="In phieu" alt="In phieu nop tien ngan sach" border ="1px" style="border-style:outset"></A>
	        
	        <A href="../../ErpUynhiemchiPdfSvl?userId=<%= tthdBean.getUserId() %>&id=<%= tthdBean.getId() %>&httt=<%= tthdBean.getHtttId() %>&inchitiet=0" >
	        <IMG src="../images/pdfblue.jpg" title="In phieu dieu chuyen tien pdf" alt="In phieu dieu chuyen tien pdf" border ="1px" style="border-style:outset; width: 30px; height: 30px "></A>
    </div>
    	...
    	<legend class="legendtitle"> Ủy nhiệm chi </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0" border = '0'>		
            	...
            	<TR>
                    ...
                    <TD width="20%" class="plainlabel" valign="top">Loại thanh toán </TD>
                    <TD class="plainlabel" valign="top" >
                   		 <input type="hidden" name="doituongthanhtoan" id="doituongthanhtoan" value='<%= tthdBean.getDoiTuongChiPhiKhac() %>'>
                    	<select name="doituongthanhtoan" id="doituongthanhtoan" style = "width:200px" disabled="disabled">					              	
                    	 <% if (loaiThanhToanList != null)
						{
                    		 System.out.println("tthdBean.getDoiTuongChiPhiKhac(): " + tthdBean.getDoiTuongChiPhiKhac());
							for (Erp_Item item : loaiThanhToanList)
							{
								if (item.getValue().equals(tthdBean.getDoiTuongChiPhiKhac()))
								{
								%>
                     ...
                     <input type="hidden" name="thanhtoantuTV" id="thanhtoantuTV" value='<%= tthdBean.getCheckThanhtoantuTV() %>'>
                    <%if(Double.parseDouble(tthdBean.getCheckThanhtoantuTV()) == 1){ %>
                    	<input type="checkbox" id="thanhtoantuTV" name ="thanhtoantuTV" value= "1" checked="checked" disabled="disabled">
                    <%}else{ %>
                    	<input type="checkbox" id="thanhtoantuTV" name ="thanhtoantuTV" value= "1" disabled="disabled">
                    <%} %>
                    </TD>
                 ...
                 <TD width="20%" class="plainlabel" valign="top">Ngày chứng từ</TD>
                    ...
                    <TD  class="plainlabel" valign="top">
                    	<input type="text"  class="days" id="ngayghinhan" name="ngayghinhan" value="<%= tthdBean.getNgayghinhan() %>" 
                    		maxlength="10"  onChange = "ChangeTienTe();" readonly="readonly"/>
                    </TD>
                    
                    <TD class="plainlabel">Số chứng từ</TD>
                      <TD class="plainlabel">
                        <input type="text" readonly="readonly" name="soChungTu_Chu" id="soChungTu_Chu" value="<%= tthdBean.getSoChungTu_Chu() %>" onKeyPress = "return validateSpecialCharacters(event);" onchange="upperLetters(this);" style="width: 100px;">
                        <input type="text" name="soChungTu_So" id="soChungTu_So" value="<%= tthdBean.getSoChungTu_So() %>" style="width: 100px;">
                    </TD>
                </TR>  
	        <%if(tthdBean.getDoiTuongChiPhiKhac().equals("1")){ %>
	                <TR>
	                    <TD class="plainlabel">Nhà cung cấp / Nhóm NCC
	                    	<input type="hidden" name="nhomncccn" id="nhomncccn" value='<%= tthdBean.getNhomNCCCN() %>'>
	                    	<% if(tthdBean.getNhomNCCCN().equals("1")) { %>
									<input type="checkbox" id="nhomncccn" name="nhomncccn" value="1" checked = "checked" disabled="disabled">
									<% } else {  %>
										<input type="checkbox" id="nhomncccn" name="nhomncccn" value="1" disabled="disabled">
									<% } %>
	                    </TD>
	                    <% if(tthdBean.getNhomNCCCN().equals("1")) { %>
	                    <TD class="plainlabel" colspan="3">
	                    	<input type="hidden" name="nhomnccId" id="nhomnccId" value='<%= tthdBean.getNhomNCCCNId() %>'>
		                  <select name="nhomnccId" id="nhomnccId" style="width: 300px" disabled="disabled">
                        	<option value=""> </option>
                        	<%
                        		if(nhomnccList != null)
                        		{
	                    ...
	                    <%}else{ %>
	                    <TD class="plainlabel" colspan="3">
	                    	<input type="hidden" name="nccId" id="nccId" value='<%= tthdBean.getNccId() %>'>
		                  <select name="nccId" id="nccId" style="width: 300px" disabled="disabled">
                        	<option value=""> </option>
                        	<%
                        		if(nccList != null)
                        		{
	         ...
	         <%}else if(tthdBean.getDoiTuongChiPhiKhac().equals("0")){ // LOAITT: KHÁCH HÀNG TRẢ TRƯỚC >> LOAD NV %>
						<TR>
						 <TD  class="plainlabel"> Tên nhân viên</TD>						        
					      <TD class="plainlabel" colspan="3">
					      <input type="hidden" name="NhanvienId" id="NhanvienId" value='<%= tthdBean.getNhanVienId() %>'>
		                  <select name="NhanvienId" id="NhanvienId" style="width: 300px" disabled="disabled">
                        	<option value=""> </option>
                        	<%
                        		if(nvList != null)
                        		{
			...
			<%} else if(tthdBean.getDoiTuongChiPhiKhac().equals("2")){ // LOAITT: KHÁCH HÀNG TRẢ TRƯỚC >> LOAD KH %>
						...
						<TD class="plainlabel" > Tên khách hàng</TD>
							<TD class="plainlabel" colspan="3">
							<input type="hidden" name="khid" id="khid" value='<%= tthdBean.getKhachhangId() %>'>
		                  <select name="khid" id="khid" style="width: 300px" disabled="disabled">
                        	<option value=""> </option>
                        	<%
                        		if(khList != null)
                        		{
			...
			<%} else if (tthdBean.getDoiTuongChiPhiKhac().equals("4")) { %>
					     <TR>
					          ...
					          <input  type="text" name="bpTen" id="bpTen" value="<%= tthdBean.getBophanTen()%>"  style="width:400px" readonly="readonly">
			...
			<%} else if (tthdBean.getDoiTuongChiPhiKhac().equals("6")) { %>
                <TR>
		          	<TD width="20%" class="plainlabel" valign="top">Đối tượng khác</TD>
			          <TD colspan="4" class="plainlabel" valign="top">
					...
					{ 
						...
						{
							if (item.getValue().equals(tthdBean.getDoiTuongKhacId()))
							{
							%>
								<option value="<%=item.getValue()%>" selected="selected"><%=item.getName()%></option>
							<% } else { %>
			...
			<%} else if (tthdBean.getDoiTuongChiPhiKhac().equals("7")) { %>
                <TR>
		          	<TD width="20%" class="plainlabel" valign="top">Chi nhánh</TD>
			          <TD colspan="4" class="plainlabel" valign="top">
			          <select name="khachHang_NPP_FK" id="khachHang_NPP_FK" style="width: 200px; border-radius:4px; height: 20px;" onChange = "submitform();" >
					<% if (tthdBean.getNppList() != null)
					{
						for (Erp_Item item : tthdBean.getNppList())
						{
							if (item.getValue().equals(tthdBean.getKhachHang_NPP_FK()))
							{
							%>
								<option value="<%=item.getValue()%>" selected="selected"><%=item.getName()%></option>
							<% } else { %>
                ...
                <TR>
                   <TD class="plainlabel">Hình thức thanh toán</TD>               
                    <TD colspan="6" class="plainlabel">
                    	<input type="hidden" name="htttId" id="htttId" value='<%= tthdBean.getHtttId() %>'>
                        <select name="htttId" id="htttId" style="width:200px" disabled="disabled">
                        	<option value=""> </option>
                        	<%
                        		if(htttList != null)
                        		{
           ...
           <%
                ...
                <TR>
                  ...
                  <%} %>
                     <TD class="plainlabel" colspan = 4> 
						<input type="hidden" name="sotaikhoan" id="sotaikhoan" value='<%= tthdBean.getSotaikhoan() %>'>                   
                        <select name="sotaikhoan" id="sotaikhoan" style="width: 300px" disabled="disabled">
                        	<OPTION VALUE = "">&nbsp;</OPTION>
                        	<%
                        		if(sotkRs != null)
                        		{
            ...
            <% }%>
            	...
            	<TR>
                    ...
                    <TD class="plainlabel" style = "width:100px">
                    	<input type="hidden" name="tienteId" id="tienteId" value='<%= tthdBean.getTienteId() %>'>
                        <select name="tienteId" id="tienteId" disabled="disabled" style = "width:200px;">
                        	<%
                        		if(tienteList != null)
                        		{
                        			try
           		...
           		<% if(tthdBean.getTienteId().equals("100000")){ %>
                      	<input type="text" name="tigia" Id="tigia" value="<%= formatter1.format(Double.parseDouble(tthdBean.getTigia().replaceAll(",","")))  %>" style = "width:100px;text-align: right;"  readonly>
           		<%}else{ %>
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
						...
						<%if (tthdBean.getDaDuyet() == 1 && tthdBean.getTrangThai() == 0){ %>                  
           				<input type="text" name="tigia" Id="tigia" value="<%= formatter1.format(Double.parseDouble(tthdBean.getTigia().replaceAll(",","")))  %>" style = "width:100px;text-align: right;"  onchange="ThanhToan(100);" onKeyPress = "return keypress(event);">
           				<%}else{ %>
           				<input type="text" name="tigia" Id="tigia" value="<%= formatter1.format(Double.parseDouble(tthdBean.getTigia().replaceAll(",","")))  %>" style = "width:100px;text-align: right;" readonly="readonly">
           				<%} %>
           		<%   }else{ %>
           				<%if (tthdBean.getDaDuyet() == 1 && tthdBean.getTrangThai() == 0){ %> 
           				...
           				<%}else{ %>
           				<input type="text" name="tigia" Id="tigia" value="<%= formatter2.format(Double.parseDouble(tthdBean.getTigia().replaceAll(",","")))  %>" style = "width:100px;text-align: right;" readonly="readonly">
           				<%} %>
           		<%   } %>
           		<%} %>
                      </TD> 
...
<%// THANH TOAN HOA DON DUNG NGOAI TE %>
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
                       ...
                       <input type="text" style="text-align: right;" name="sotienHDNT" id = "sotienHDNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getSotienHDNT().replaceAll(",",""))) %>" readonly="readonly">
            <%	 }else{ %>
                       <input type="text" style="text-align: right;" name="sotienHDNT" id = "sotienHDNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getSotienHDNT().replaceAll(",",""))) %>" readonly="readonly">                       
            <%   } %>
                ...
                <TR>
                    ...
                    <TD class="plainlabel" colspan = 5>Trích phí bằng VNĐ
                    	<input type="hidden" name="trichphi" id="trichphi" value='<%= tthdBean.getTrichphi() %>'>
						<input type="checkbox" id="trichphi" name="trichphi" value="1" disabled="disabled">
	                </TD>
	               <%}else{ %> 
                    <TD class="plainlabel">Trích phí bằng VNĐ
                    	<input type="hidden" name="trichphi" id="trichphi" value='<%= tthdBean.getTrichphi() %>'>
						<input type="checkbox" id="trichphi" name="trichphi" value="1" checked = "checked" disabled="disabled">
	                </TD>
					<TD class="plainlabel">Tài khoản trích phí (VND) </TD>				
					<TD class="plainlabel" colspan = 3>
                     	<input type="hidden" name="sotaikhoan_tp" id="sotaikhoan_tp" value='<%= tthdBean.getSotaikhoan_tp() %>'>
                        <select name="sotaikhoan_tp" id="sotaikhoan_tp" style="width: 500px" disabled="disabled">
                        		<OPTION VALUE = "">&nbsp;</OPTION>
                        	<%
                        		if(sotkRs_tp != null)
                        		{
...
<%// THANH TOAN HOA DON DUNG NGOAI TE - TRICH PHI BANG NGOAI TE
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
                    	...
                    	<input type="text" style="text-align: right;" name="pnganhangNT" id = "pnganhangNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getPhinganhangNT().replaceAll(",",""))) %>" readonly="readonly">
			<%	 }else{ %>
						<input type="text" style="text-align: right;" name="pnganhangNT" id = "pnganhangNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getPhinganhangNT().replaceAll(",",""))) %>" readonly="readonly">
			<%   } %>                    
                     </TD> 
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
						...
						||  tthdBean.getLoaiThanhToan().equals("0")){ %>                    
                         ...
                         <input type="text" style="text-align: right;" name="vatNT" id = "vatNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getThueVATNT().replaceAll(",",""))) %>" readonly="readonly"> 
            <%	 }else{ %>
                   		<input type="text" style="text-align: right;" name="vatNT" id = "vatNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getThueVATNT().replaceAll(",",""))) %>" readonly="readonly">
            <%   } %>
                     </TD> 
							...
							<TABLE width="100%" align="center">
        						<TR class="tbheader">
									...
									<TH width="100px" align = "center">Tên NCC </TH>
               						<TH align="center" width="200px">Địa chỉ</TH>
					      		...
					      		<TR>
									...
									<TD>
										<input type="text" name = "mauHD_VAT" id = "mauHD_VAT" value = "<%= tthdBean.getMauhoadon() %>" readonly="readonly" style="width: 100%" >
									</TD>
									<TD>
									...
									<TD>
										<input type="text" name = "nghangTen" id = "nghangTen" value = "<%= tthdBean.getTenNCC_VAT() %>" style="width: 100%" readonly="readonly" >
            	...
            	<TR>
                    ...
                    <TD class="plainlabel">
                         <input type="text" style="text-align: right;" name="tongtienNT" id = "tongtienNT" value="<%= formatter1.format(Double.parseDouble(tthdBean.getSotienttNT().replaceAll(",",""))) %> "  readonly="readonly"> 
                     </TD> 
            	...
            	<TR>
                    ...
                    <TD  class="plainlabel" style = "width:350px" >
                        <input type="text" name="noidungthanhtoan" value="<%= tthdBean.getNoidungtt() %>" readonly="readonly"> 
                     </TD> 
...
<%// THANH TOAN HOP DONG DUNG NGOAI TE - TRICH PHI VNĐ
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
                         ...
                         <input type="text" style="text-align: right;" name="pnganhangVND" id = "pnganhangVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getPhinganhang().replaceAll(",",""))) %>" readonly="readonly"> 
            <%	 }else{ %>
                   		<input type="text" style="text-align: right;" name="pnganhangVND" id = "pnganhangVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getPhinganhang().replaceAll(",",""))) %>" readonly="readonly">
            <%   } %>
                     </TD> 
			...
			<%   if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
					 ...
					 || (Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) > 2014)
                         ...
                         <input type="text" style="text-align: right;" name="vatVND" id = "vatVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getThueVAT().replaceAll(",",""))) %>" readonly="readonly"> 
            <%   }else{ %>
            	         <input type="text" style="text-align: right;" name="vatVND" id = "vatVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getThueVAT().replaceAll(",",""))) %>" readonly="readonly">
            <%   } %>
                    </TD> 
							...
							<TABLE width="100%" align="center">
        						<TR class="tbheader">
					      		...
					      		<TR>
									...
									<TD>
										<input type="text" name = "kyhieu_VAT" id = "kyhieu_VAT" value = "<%= tthdBean.getKyhieu() %>" style="width: 100%" readonly="readonly">
									</TD>
											      		
									<TD>
										<input type="text" name = "sohd_VAT" id = "sohd_VAT" value = "<%= tthdBean.getSohoadon() %>" style="width: 100%" readonly="readonly">
									</TD>
									...
									<TD>
										<input type="text" name= "tienhang_VAT" id= "tienhang_VAT" value = "<%= formatter1.format(Double.parseDouble(tthdBean.getTienhang().replaceAll(",",""))) %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);" readonly="readonly">
									</TD>

									<TD>
										<input type="text" name= "thuesuat_VAT" id= "thuesuat_VAT" value = "<%= formatter1.format(Double.parseDouble(tthdBean.getThuesuat().replaceAll(",",""))) %>" style="width: 100%;text-align: right" onChange = "javascript:tinhthue();" onKeyPress = "return keypress(event);" >
									</TD>
									
									<TD>
										<input type="text" name= "tienthue_VAT" id= "tienthue_VAT" value = "<%= formatter1.format(Double.parseDouble(tthdBean.getTienthue().replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
									</TD>
								</TR>
							</TABLE>
						    <DIV align="right">
            	...
            	<TR>
                    ...
                    <TD class="plainlabel" style = "width:150px">Tổng tiền thanh toán (VNĐ)</TD>
                    <TD class="plainlabel" colspan = 3>
                         <input type="text" style="text-align: right;" name="tongtienVND" id = "tongtienVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getSotientt().replaceAll(",",""))) %>" readonly="readonly"> 
                     </TD> 
                </TR>
...
<%// THANH TOAN HOA DON VNĐ			%>
	<%}else{ %>
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
                         ...
                         <input type="text" style="text-align: right;" name="sotienHDVND" id = "sotienHDVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getSotienHD().replaceAll(",",""))) %>" readonly="readonly">                    
                <%	 }else{ %> 
			...
			<%  	 if(((Integer.parseInt(tthdBean.getNgayghinhan().substring(0, 4)) == 2014 & Integer.parseInt(tthdBean.getNgayghinhan().substring(5, 7)) < 4)
				...
				<TR style="display: none;">
					...
					<TD align = "left" class="plainlabel" colspan = 3>	                   
                         <input type="text" style="text-align: right;width:181px" name="vatVND" id = "vatVND" value="<%= formatter2.format(Double.parseDouble(tthdBean.getThueVAT().replaceAll(",",""))) %>" readonly="readonly">
                     </TD> 
				<%} else  { %>
                   <TD class="plainlabel" >Thuế VAT (VND) </TD>
					<TD align = "left" class="plainlabel" colspan = 3>
							...
							<TABLE width="100%" align="center">
        						<TR class="tbheader">
					      		...
					      		<TR>
									...
									<TD>
										<input type="text" name = "mauHD_VAT" id = "mauHD_VAT" value = "<%= tthdBean.getMauhoadon() %>" style="width: 100%" readonly="readonly">
									</TD>
									<TD>
										<input type="text" name = "kyhieu_VAT" id = "kyhieu_VAT" value = "<%= tthdBean.getKyhieu() %>" style="width: 100%" readonly="readonly">
									</TD>
									...
									<TD>
										<input type="hidden" name = "nhId_VAT" id = "nhId_VAT" value = "<%= tthdBean.getNhId_VAT() %>" style="width: 100%" readonly="readonly">
										<input type="hidden" name = "cnId_VAT" id = "cnId_VAT" value = "<%= tthdBean.getCnId_VAT() %>" style="width: 100%" readonly="readonly">
									...
									<TD>
										<input type="text" name= "tienhang_VAT" id= "tienhang_VAT" value = "<%= formatter2.format(Double.parseDouble(tthdBean.getTienhang().replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
									</TD>

									<TD>
										<input type="text" name= "thuesuat_VAT" id= "thuesuat_VAT" value = "<%= formatter2.format(Double.parseDouble(tthdBean.getThuesuat().replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
									</TD>

									<TD>
										<input type="text" name= "tienthue_VAT" id= "tienthue_VAT" value = "<%= formatter2.format(Double.parseDouble(tthdBean.getTienthue().replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
									</TD>
								</TR>
							</TABLE>
						    <DIV align="right">
	        ...
	        <% if(tthdBean.getTienteId().equals("100000")){ // Don vi tien la VND %>
                ...
                <%} %>
                	...
                	<TH align="center" width="10%">Số dư HĐ (đã có VAT)</TH>
               	 	...
               	 	<TH align="center" width="20%">Địa chỉ</TH>
                	...
                	for(int i = 0; i < hoadonList.size(); i++)
                	{
                		IHoadon hoadon = hoadonList.get(i);
						  ...
						  <%if(tthdBean.getDoiTuongChiPhiKhac().equals("1")) { %>	                
							...
							<td align="center">
           	 					<input type="text" style="width: 100%; text-align: right;" value="<%= formatter1.format(Double.parseDouble(hoadon.getSoTienGoc2().replaceAll(",",""))) %>" name= "sotiengoc" id="sotiengoc<%= i %>" readonly="readonly" >
           	 				</td>
           	 			...
           	 			<%} %>
           	 				...
           	 				<%if(hoadon.getLoaihd1().equals("6")){ %>
           	 					<input type="text" style="width: 100%; text-align: right;" readonly="readonly" value="<%= formatter1.format(Double.parseDouble(hoadon.getThanhtoan().replaceAll(",",""))) %>" name= "thanhtoan" id="thanhtoan<%= i %>" readonly="readonly">
           	 				<%} else { %>					
           	 					<input type="text" style="width: 100%; text-align: right;" value="<%= formatter1.format(Double.parseDouble(hoadon.getThanhtoan().replaceAll(",",""))) %>" name= "thanhtoan" id="thanhtoan<%= i %>" readonly="readonly">
           	 			    <%} %>
           	 				</td>
           	 				<td align="center">
						...
						<% if(!tthdBean.getTienteId().equals("100000")){ // Ngoại tệ%>           	 				
           	 			...
           	 			<%} %>
           	 				...
           	 				<td align="center"> 
           	 					<input type="text" name = "diaChiHd" id="diaChiHd_<%= i %>" value="<%=hoadon.getDiaChi()%>" readonly="readonly" >
           	           	 	</td>
           	 				<td align="center">
           	 					<input type="hidden" name="trahet" id="trahet" value='<%= hoadon.getId() %>'>
           	 				<% System.out.println("stt: " + i);	if(hoadon.getConlai().equals("0")){ %>
           	 					<input type="checkbox" value="<%= hoadon.getId() %>" name = "trahet" id="trahet<%= i %>" checked="checked" disabled="disabled">
           	 				<%} else { %>
           	 					<input type="checkbox"  value="<%= hoadon.getId() %>" name = "trahet" id="trahet<%= i %>" disabled="disabled">
           	 				<%} %>
           	 				</td>
           	 			</tr>
           	 	<%} %>
			...
			<%} %>
    			...
    			<tr class="tbheader">
    				...
    				<th align="center" width = "150px">Đối tượng/TT chi phí </th>
    				<th align="center" width = "150px">Khoản mục chi phí</th>
    				...
    				<th align="center" width = "150px">Sản phẩm </th>
					<TH align="center" width = "200px">Mẫu hóa đơn</TH>	
    				<TH align="center" width = "200px">Ký hiệu</TH>	
    				<TH align="center" width = "200px">Số hóa đơn</TH>
    	         	<TH width = "200px" align = "center">Ngày hóa đơn </TH>
    				<TH width = "200px" align = "center">Tên NCC </TH>
    				<TH width = "200px" align = "center">Địa chỉ </TH>
    				<TH width = "200px" align = "center">MST </TH>	
    				<TH width = "200px" align = "center">Diễn giải </TH>
    				<TH width = "200px" align = "center">Tiền hàng </TH>
    				<TH width = "200px" align = "center">Thuế suất(%) </TH>
    				<TH width = "200px" align = "center">Tiền thuế </TH>
    			</tr>
    		...
    		for(int i=0; i < count;i++){ 
    			...
    			<TR>
    				<!-- TÀI KHOẢN  -->
                		<TD  align="left" >  
                			<input type="hidden" name="TaiKhoanId" id="TaiKhoanId_<%=i %>" value='<%= TaiKhoanId[i] %>'>
                    		<select name="TaiKhoanId" id="TaiKhoanId_<%=i %>" style="width: 150px;" disabled="disabled">
    							<option value=""></option>
                    		<%			                       		
    				...
    				<!-- ĐỐI TƯỢNG  -->						 
                	<TD align="center"  >    
                		<input type="hidden" name="dcIds_<%=i %>" id="dcIds_<%=i %>" value="<%= dcIds[i] + "_" + loais[i]%>">                   		
                		<select  name="dcIds_<%=i %>" id="dcIds_<%=i %>" style="width: 150px;" disabled="disabled">
    			   			<option   value="">&nbsp;</option>
    						<%		
    						  String selected = "";
    				...
    				<!-- TRUNG TÂM CHI PHÍ  -->						 
            	<TD align="center"  >                       		
            		<select  name="ttcps_<%=i %>" id="ttcps_<%=i %>" style="width: 150px;" disabled="disabled">
			   			<option   value="">&nbsp;</option>
						<%		
						  String selected1 = "";
						  ttcpRs = tthdBean.getTrungTamChiPhiRs(TaiKhoanId[i]);
						  if(ttcpRs != null){
								while(ttcpRs.next()){ 
									if(ttcpRs.getString("PK_SEQ").equals(ttcps[i])){
										selected1 = "selected";
									}else{
										selected1 = "";	
									}
								%>											
								<option   <%=selected1 %> value="<%=ttcpRs.getString("PK_SEQ")%>"> <%=ttcpRs.getString("MA") %> --<%= ttcpRs.getString("TEN") %> </option>															   			
    				...
    				<!--PHÒNG BAN  -->
    				<TD  align="left" >  
    					<input type="hidden" name="PhongbanId" id="PhongbanId_<%=i %>" value='<%= PhongbanId[i] %>'>
            			<select name="PhongbanId" id="PhongbanId_<%=i %>" style="width: 150px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    			  ...
    			  <!-- KÊNH BÁN HÀNG  -->
    				<TD  align="left" >  
    					<input type="hidden" name="KenhId" id="KenhId_<%=i %>" value='<%= kenhBhId[i]%>'>
            			<select name="KenhId" id="KenhId_<%=i %>" style="width: 80px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    				...
    				<!-- MÃ VỤ VIỆC  -->
    				<TD  align="left" >  
    					<input type="hidden" name="mavvId" id="mavvId_<%=i %>" value='<%= MavvId[i]%>'>
            			<select name="mavvId" id="mavvId_<%=i %>" style="width: 80px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    				...
    				<!-- ĐỊA BÀN  -->
    				<TD  align="left" >  
    					<input type="hidden" name="""" id="diabanId_<%=i %>" value='<%= DiaBanId[i]%>'>
            			<select name="diabanId" id="diabanId_<%=i %>" style="width: 80px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    				 ...
    				 <!-- TỈNH THÀNH  -->
    				<TD  align="left" >  
    					<input type="hidden" name="TinhthanhId" id="TinhthanhId_<%=i %>" value='<%= TinhThanhId[i]%>'>
            			<select  name="TinhthanhId" id="TinhthanhId_<%=i %>" style="width: 100px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    				 ...
    				 <!-- BỆNH VIỆN  -->
    				<TD  align="left" >  
    					<input type="hidden" name="BenhvienId" id="BenhvienId_<%=i %>"" value='<%= BenhvienId[i]%>'>
            			<select  name="BenhvienId" id="BenhvienId_<%=i %>" style="width: 100px;" disabled="disabled">
    						<option value=""></option>
                			<%	
    				 ...
    				 <!-- SẢN PHẨM  -->
    				<TD  align="left" >  
    					<input type="hidden" name="SanphamId" id="SanphamId_<%=i %>" value='<%= SanPhamId[i]%>'>
            			<select name="SanphamId" id="SanphamId_<%=i %>" style="width: 150px;" disabled="disabled">
    						<option value=""></option>
                			<%	
				 ...
				 <TD>
					<input type="text" name = "mauHoaDon_VAT_<%=i %>" class="uppercase"  value = "<%=Mauhd[i] %>" style="width: 100%" readonly="readonly">
				</TD>  
				 <TD>
					<input type="text" name = "kyhieu_VAT_<%=i %>" class="uppercase"  value = "<%= Kyhieuhd[i] %>" style="width: 100%" readonly="readonly">
				</TD>        
    				<TD>
    					<input type="text" name = "sohd_VAT_<%=i %>"  value = "<%= Sohd[i] %>" style="width: 100%" readonly="readonly">
    				</TD>
    				...
    				<TD>											
    					<input type="text" name = "tenNCC_VAT_<%=i %>"  value = "<%= TenNCChd[i] %>" style="width: 100%" readonly="readonly">
    				</TD>

				<TD>											
					<input type="text" name = "diaChiNCC_<%=i %>"  value = "<%= diaChiHd[i] %>" style="width: 100%" readonly="readonly">
				</TD>
    				<TD>
    					<input type="text" name= "mst_VAT_<%=i %>" value = "<%= MSThd[i] %>" style="width: 100%" readonly="readonly">
    				</TD>
    				
	    			<TD>
						<input type="text" name= "diengiai_VAT_<%=i %>" value = "<%= Diengiaihd[i] %>" style="width: 100%" readonly="readonly">
					</TD>

    				<TD>
    					<%if(Tienhanghd[i]!=null && Tienhanghd[i].trim().length()>0) {%>
    						<input type="text" name= "tienhang_VAT_<%=i %>" id= "tienhang_VAT_<%=i %>" value = "<%= formatter1.format(Double.parseDouble(Tienhanghd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} else { %>
    						<input type="text" name= "tienhang_VAT_<%=i %>" id= "tienhang_VAT_<%=i %>" value = "<%= Tienhanghd[i] %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} %>
    				</TD>

    				<TD>
    					<%if(Thuesuathd[i]!=null && Thuesuathd[i].trim().length()>0) {%>								
    						<input type="text" name= "thuesuat_VAT_<%=i %>"  id = "thuesuat_VAT_<%=i %>"  value = "<%= formatter1.format(Double.parseDouble(Thuesuathd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} else { %>
    						<input type="text" name= "thuesuat_VAT_<%=i %>"  id = "thuesuat_VAT_<%=i %>" value = "<%= Thuesuathd[i] %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} %>
    				</TD>

    				<TD>
    					<%if(Tienthuehd[i]!=null && Tienthuehd[i].trim().length()>0) {%>	
    						<input type="text" name= "tienthue_VAT_<%=i %>"  id= "tienthue_VAT_<%=i %>" value = "<%= formatter1.format(Double.parseDouble(Tienthuehd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} else { %>
    						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>"  value = "<%= Tienthuehd[i] %>" style="width: 100%;text-align: right" readonly="readonly">
    					<%} %>
    				</TD>  		                    
    			</TR>
         <% }
...
<!--     			Tạm dừng ở đây -->
            	...
            	%>
            		<TD  align="left" >
            		<input type="hidden" name="TaiKhoanId" id="TaiKhoanId_<%=i %>" value='<%= tthdBean.getDaDuyet() %>'>
    				<select style="width: 150px;"   name="TaiKhoanId" id="TaiKhoanId_<%=i %>" disabled="disabled">
    					<option value=""></option>
            		<%	                        		
				...
				<TD>											
					<input type="text" name = "diaChiNCC_<%=i %>"  value = "" style="width: 100%" readonly="readonly">
				</TD>
    				<TD>
    				...
    				<TD>
    					<%if(Tienhanghd[i]!=null && Tienhanghd[i].trim().length()>0) {%>
    						<input type="text" name= "tienhang_VAT_<%=i %>" id= "tienhang_VAT_<%=i %>" value = "<%= formatter1.format(Double.parseDouble(Tienhanghd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" onkeyup = "javascript:tinhthue_loaikhac_new();" onKeyPress = "return keypress(event);">
    					<%} else { %>
    				...
    				<TD>
    					<%if(Thuesuathd[i]!=null && Thuesuathd[i].trim().length()>0) {%>								
    						<input type="text" name= "thuesuat_VAT_<%=i %>" id= "thuesuat_VAT_<%=i %>"  value = "<%= formatter1.format(Double.parseDouble(Thuesuathd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" onkeyup = "javascript:tinhthue_loaikhac_new();" onKeyPress = "return keypress(event);">
    					<%} else { %>
    				...
    				<TD>
    					<%if(Tienthuehd[i]!=null && Tienthuehd[i].trim().length()>0) {%>	
    						<input type="text" name= "tienthue_VAT_<%=i %>" id= "tienthue_VAT_<%=i %>"  value = "<%= formatter1.format(Double.parseDouble(Tienthuehd[i].replaceAll(",",""))) %>" style="width: 100%;text-align: right" onKeyPress = "return keypress(event);">
    					<%} else { %>
