...
<%
	IErpKhuVucKho kvkList = (IErpKhuVucKho) session.getAttribute("kvkList");
	ResultSet rsKVK = kvkList.getKhuVucKhoRs();
	ResultSet rsKho=kvkList.getKhoRs();
	 int[] quyen = new  int[5];
	 quyen = util.Getquyen("ErpKhuVucKhoSvl","&loaiNPP=0&isKH=0",userId);	
...
<body leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0" onload="thongbao()">
	<form name="kvkListForm" method="post" action="../../ErpKhuVucKhoSvl">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%">
						...
						<tr>
							<td align="left">
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr height="22">
										<td align="left" colspan="2" class="tbnavigation">&nbsp;Dữ liệu nền > Kho vận > Khai báo khu vực kho</td>
										...
										<td width="100%" align="left">
											<fieldset>
												...
												<table width="100%" cellpadding="6" cellspacing="0">
													<tr>
														<td width="10%" class="plainlabel">Mã khu vực </td>
														<td class="plainlabel"><input type="text" name ="makhu" value="<%= kvkList.getMa() %>" onchange="Submit()" />
													
													</tr>
													<tr>
														<td width="10%" class="plainlabel">Tên khu vực</td>
													<td class="plainlabel"><input type="text" name ="tenkhu" value="<%= kvkList.getDienGiai() %>" onchange="Submit()" />
													</tr>
													<tr>
														<td width="10%" class="plainlabel">Thuộc kho</td>
														<TD class="plainlabel"><select name="makho" onchange="Submit()" >
																<option value=""></option>
																<%if (rsKho != null)
				while (rsKho.next()) {
			...
			if (rsKVK != null)
				while (rsKVK.next()) {
					String trangthai = rsKVK.getString("TrangThai");
					...
					if (m % 2 != 0) {%>
													...
													<tr class=<%=darkrow%>>
														<%}%>
														<td align="center"><%=rsKVK.getString("Ma")%></td>
														<td align="left"><%=rsKVK.getString("DienGiai")%></td>
														<td align="center"><%=rsKVK.getString("Kho")%></td>
														<td align="center"><%=trangthai%></td>
														<td align="center"><%=rsKVK.getString("NgayTao")%></td>
														<td align="center"><%=rsKVK.getString("NguoiTao")%></td>
														<td align="center"><%=rsKVK.getString("NgaySua")%></td>
														<td align="center"><%=rsKVK.getString("NguoiSua")%></td>
														<td align="center">
															<table border=0 cellpadding="0" cellspacing="0">
																<tr>
																	<td>
																	<%if(quyen[2]!=0){ %>
																	<a href="../../ErpKhuVucKhoUpdateSvl?userid=<%=userId%>&update=<%=rsKVK.getString("PK_SEQ")%>"><img
																			src="../images/Edit20.png" alt="Chinh sua" width="20" height="20" longdesc="Chinh sua" border=0></A>&nbsp;
																	<%} %>
																			</td>
																	<td>
																	<%if(quyen[1]!=0){ %>
																	<a href="../../ErpKhuVucKhoSvl?userid=<%=userId%>&delete=<%=rsKVK.getString("PK_SEQ")%>"><img
																			src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0
																			onclick="if(!confirm('Bạn muốn xóa khu vực này?')) return false;"> </a>&nbsp;
																			<%} %>
																			</td>
																	<td>
																	<!-- QUYEN VIEW DLN --> 
																	<a href="../../ErpKhuVucKhoUpdateSvl?userid=<%=userId%>&display=<%=rsKVK.getString("PK_SEQ")%>"><img src="../images/Display20.png" title="Hiển thị" alt="Hien thi" width="20" height="20" longdesc="Hien thi" border = 0></A>
																	<!-- END QUYEN VIEW DLN --> 
																			</td>
																</tr>
															</table>
