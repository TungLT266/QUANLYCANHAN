...
<% obj.setNextSplittings();
   ResultSet rskhoNhan = (ResultSet) obj.getKhonhanRs();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[5];
		quyen = util.Getquyen("ErpNhanhang_GiaySvl", "&loai=2",userId);
%>
...
<body>
	<form name="erpNhForm" method="post" action="../../ErpNhanhang_GiaySvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			...
			<%-- <%System.out.println("loai dmh: "+obj.getLoaimh() ); %>
			...
			<div style="width: 70%; padding: 5px; float: left" class="tbnavigation">Quản lý mua hàng > Mua hàng VT/CPDV/TSCĐ/CCDC > Nhận hàng</div>
			                	...
			                	if(rsHoaDon !=null){
			                		while(rsHoaDon.next()){
										...
										<%} %>
						                    ...
						                    <TD align="center" ><%=rsHoaDon.getString("PREFIX") +rsHoaDon.getString("PK_SEQ")  %></TD>
						                    ...
						                    <TD align="center" >
						                    	<% if( daNhanHang== 0){ %>
						                    	<A href="../../ErpNhanhang_GiaySvl?userId=<%=userId%>&convert=<%= rsHoaDon.getString("PK_SEQ") %>&NCCId=<%= 
						                    		rsHoaDon.getString("NCCID") %>&hoadonnccId=<%= rsHoaDon.getString("HDNCCID") %>
						                    		&loaihd=<%= rsHoaDon.getString("LOAIHD") %>&loaihh=<%= rsHoaDon.getString("LOAIHH") %>">
								...
								for(int i =0; i < htList.size(); i ++)
									...
									<TD align="center">
										<% if(tt.equals("0")){ %> 
											...
											<%if(quyen[2]!=0){ %>
												<A href="../../ErpNhanhangUpdate_GiaySvl?userId=<%=userId%>&update=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
												<IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A><%} %>&nbsp; 
											
											
											<%if(quyen[4]!=0){ %>
												<A id='chotphieu<%= ht.getId()%>' href="../../ErpNhanhang_GiaySvl?userId=<%=userId%>&chot=<%= ht.getId() %>&lhhId=<%= ht.getloaihanghoaId() %>&loai=<%= obj.getLoaimh()%> ">
									...
									<% if(quyen[3]!=0){ %><A href="../../ErpNhanhangUpdate_GiaySvl?userId=<%=userId%>&display=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>"> <IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
										<% }%>
									
									<% System.out.println("trangthai: " +tt);%>
									<% System.out.println("quyen: "+quyen[0] + "," +quyen[1] + ","+quyen[2] + ","+quyen[3] + ","+quyen[4] + ","+quyen[5]);%>
									
									<%if(quyen[1]!=0){ %>
										<A href="../../ErpNhanhang_GiaySvl?userId=<%=userId%>&delete=<%= ht.getId() %>&poId=<%= ht.getPoId() %>&loai=<%= obj.getLoaimh() %>">
									...
									<% } } else 
										...
										%>
											<%-- 
											 <A id='chotphieu<%=nhanhangList.getString("nhId")%>'
												       href=""><img
												      src="../images/unChot.png" alt="Bỏ chốt" title="Bỏ chốt"
														width="20" height="20" longdesc="Bỏ chốt"							
												       border="0" onclick="if(!confirm('Bạn có muốn bỏ chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+nhanhangList.getString("nhId")%>' , '../../ErpNhanhang_GiaySvl?userId=<%=userId%>&unchot=<%=nhanhangList.getString("nhId")%>');}"  >
													</A> --%>
											<%-- 	<%} %> --%>
										...
										<% if(quyen[2]!=0 && DaRaHd==0 && tt.equals("1") ){ %>
											<A href="../../ErpNhanhangUpdate_GiaySvl?userId=<%=userId%>&suaSoHd=<%= ht.getId() %>&loai=<%= obj.getLoaimh() %>">
