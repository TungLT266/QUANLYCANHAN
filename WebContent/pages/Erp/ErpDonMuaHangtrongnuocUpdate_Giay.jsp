...
function replaces()
{
			...
			if(soluong.item(i).value != "")
			{
				if(dongia.item(i).value != "")
				{
					...
					if(document.getElementById("AUSDVAT") && document.getElementById("BUSDVAT")){
						...
						/* tien nguyen te */
						tt1 = Math.round(parseFloat(slg) *parseFloat(replaceAll(dgia,",","")));	
					} else {
					...
					var ttavat1 =Math.round( parseFloat(tt1)) +Math.round( parseFloat(tt1) * parseFloat(tx) / 100 );
					...
					avat_nguyente=avat_nguyente+tt1;
					bvat_nguyente=bvat_nguyente+ttavat1;
				}
			}
	...
	if(document.getElementById("AUSDVAT") && document.getElementById("BUSDVAT")){
		...
		document.getElementById("AUSDVAT").value= parseFloat( Math.round(avat_nguyente) * 100)/100 ;
		document.getElementById("BUSDVAT").value= parseFloat( Math.round(bvat_nguyente) * 100)/100 ;	
	}
	...
	function ThayDoiDonViTinh(element)
	{
	 ...
	 function changeGiaCong()
	 {
		document.forms['dmhForm'].nccId.value= "";
		document.forms['dmhForm'].action.value='checkedGiaCong';
		document.forms["dmhForm"].submit();
	...
	<form name="dmhForm" method="post" action="../../ErpDonmuahangtrongnuocUpdate_GiaySvl">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left" class="tbnavigation"> Quản lý mua hàng > Mua hàng trong nước > Đơn mua hàng > Cập nhật</div>
						...
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							...
							<TD class="plainlabel">Thời hạn thanh toán </TD>
								...
								<td class="plainlabel" valign="middle">
									<% if(dmhBean.getIsGiaCong().equals("1")) {%>
										 <input type="checkbox" name="isgiacong" value="1" checked onChange="changeGiaCong();"><br>
									<%} else { %>
										 <input type="checkbox" name="isgiacong" value="1"  onChange="changeGiaCong();"><br>
									<%} %>
		                            	...
		                            	<%
		                            		...
		                            		if(nguoiduyetRs != null)
		                            		{
		                            			while (nguoiduyetRs.next())
		                            			{
		                            				%>
		                            				<Tr>
					                            		<td><input type="text" value="<%= nguoiduyetRs.getString("DIENGIAI") %>" style="width: 130px" />
					                            			<input type="hidden" name="duyetIds" value="<%= nguoiduyetRs.getString("NHANVIEN_FK") %>" style="width: 130px" />
					                            		</td>
						...
						<TABLE class="tabledetail" width="100%" border="0" cellpadding="1" cellspacing="1">
									...
									for(int i = 0; i < spList.size(); i++) { 
										...
										<tr>
											...
											<TD align="center" width="6%">
												...
												<select style="width: 100%" name="donvitinh" id="<%= "donvitinh_"+i %>" 
											...
											<% if (dmhBean.getLoai().equals("1") ){ // trong nước %> 
												<input type="text" value="<%= sp.getDongia() %>" name="dongia" style="width: 100%; text-align: right;" onkeyup="FormatNumber4sole(this)" >
											<%} else {%>
											...
											<% System.out.println("aaaaaaaaa"+sp.getNgaynhandukien());%>
											<TD align="center" width="7%">
												<input type="text" value="<%= sp.getNgaynhandukien() %>"  name="ngaynhandukien" id="ngaynhandukien" style="width: 100%; text-align: right;" class="days" > 
											</TD>
							...
							<% for(int i = count; i < 40; i++) { %>
								...
								<tr>
									...
									<TD align="center" width="7%">
										<input type="text" value="" onkeyup="Lamtronso(this)" name="soluong" style="width: 100%; text-align: right;" onkeyup="FormatNumber(this)" > 
										<input type="hidden" value="" name="soluongOld" style="width: 100%; text-align: right;" > 
									</TD>
									<TD align="center" width="8%">
											
											<% if (dmhBean.getLoai().equals("1") ){ // trong nước %> 
												<input type="text" value="" name="dongia" style="width: 100%; text-align: right;" onkeyup="FormatNumber4sole(this)" >
											<%} else {%>
									...
									<TD align="center" width="7%">
										<input type="checkbox" value="1" name="mienthue" id="mienthue<%=i%>" style="width: 100%; text-align: right;" onclick="tinhthuexuat('mienthue<%=i%>','thuexuat<%=i%>');" > 
									</TD>
									<TD align="center" width="8%">
