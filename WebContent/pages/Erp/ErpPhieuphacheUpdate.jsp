...
if(!util.check(userId, userTen, sum)){
	...
	ResultSet NgaychungtuRs = obj.getNgaychungtuRs();
						...
						<FIELDSET>
							...
							<TABLE border="0" width="100%" cellpadding="6" cellspacing="0">
                          		<TR>
									...
									<TD class="plainlabel" valign="middle">
										<%if(obj.getNgaychungtu().length() > 0){ %>
											<select name="ngaychungtu" disabled style="width: 300px" class="select2">
										<%} else { %>
											<select name="ngaychungtu" style="width: 300px" class="select2">
										<%} %>
			                        		<option value=""></option>
			                        		<%if(NgaychungtuRs != null){
												while(NgaychungtuRs.next()){ 
													if(NgaychungtuRs.getString("pk_seq").equals(obj.getNgaychungtu())){ %>
														<option value="<%=NgaychungtuRs.getString("pk_seq") %>" selected><%=NgaychungtuRs.getString("ngayphache") %></option>
													<%} else { %>
														<option value="<%= NgaychungtuRs.getString("pk_seq") %>" ><%=NgaychungtuRs.getString("ngayphache") %></option>
													<%}
												}
											} %>
			                        	</select>
									</TD>
									<TD width="15%" class="plainlabel">Sản phẩm <FONT class="erroralert">*</FONT></TD>
									<TD class="plainlabel">
										<select disabled name="sanpham" id="sanpham" style="width: 300px" class="select2">
			                        		<option value=""></option>
			                        		<%if(SanphamRs != null){
	                          	...
	                          	<TR>
									...
									<TD class="plainlabel">
										<%if(obj.getNgaychungtu().length() > 0){ %>
											<select disabled name="kehoach" id="kehoach" style="width: 300px" class="select2">
										<%} else { %>
			                  	...
			                  	<TR>
									...
									<TD class="plainlabel">
										...
										<select name="tailieuphache" disabled id="tailieuphache" style="width: 300px" class="select2" onchange="submitform();">
			                        		<option value=""></option>
			                  	...
			                  	<TR>
									...
									<TD class="plainlabel">
										<select disabled name="khonhap" id="khonhap" style="width: 300px" class="select2">
			                        		<option value=""></option>
			                        		<%if(KhoRs != null){
			                        			KhoRs.beforeFirst();
												while(KhoRs.next()){
										...
										<a href="" id="thongso" rel="subcontent">
			           	 				...
			           	 				<DIV id="subcontent" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B; background-color: white; width: 700px; max-height:290px; overflow:auto; padding: 4px;">
						                        ...
						                        for(int i = 0; i < ThongTinNhapKhoList.size(); i++){
							                        ...
							                        <tr>
							                        	<td><input readonly type="text" name="solo" value="<%=ThongTinNhapKho.getSolo() %>" style="width: 100%"></td>
							                        	<td><input readonly type="text" name="vitri" value="<%=ThongTinNhapKho.getVitri() %>" style="width: 100%"></td>
							                        	<td><input readonly type="text" name="sothung" value="<%=ThongTinNhapKho.getSothung() %>" style="width: 100%"></td>
							                        	<td><input readonly type="text" name="some" value="<%=ThongTinNhapKho.getSome() %>" style="width: 100%"></td>
							                        	<td><input readonly type="text" name="soluongchitiet" value="<%=ThongTinNhapKho.getSoluongchitiet() %>" style="width: 100%; text-align: right;" onchange="tinhsoluongnhap();"></td>
							                        </tr>
						                        <%} %>
			           	 					</table>
			           	 					<div align="right">
			                  	...
			                  	<TR> 
	                          		<TD class="plainlabel" valign="middle" width="15%">Trạng thái </TD>   
			                        <TD class="plainlabel" valign="middle" colspan="3">
										...
										for(int i = 0; i < sanphamList.size(); i++){
											...
											<tr>
												...
												<td>
													...
													<select disabled name="khoxuat" style="width: 250px" class="select2" onchange="submitform();">
						                        		<option value=""></option>
						                        		<%if(KhoRs != null){
						                        			KhoRs.beforeFirst();
															while(KhoRs.next()){
													...
													<a href="" id="thongso_<%=i %>" rel="subcontent_<%=i %>">
						           	 				...
						           	 				<DIV id="subcontent_<%=i %>" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B; background-color: white; width: 1000px; max-height:500px; overflow:auto; padding: 4px;">
						           	 					<table width="95%" align="center">
						           	 						...
						           	 						<tr>
									                            <th width="10%" style="font-size: 11px">Số lô</th>
									                            <th width="8%" style="font-size: 11px">Ngày hết hạn</th>
									                            <th width="8%" style="font-size: 11px">Ngày nhập kho</th>
									                            <th width="8%" style="font-size: 11px">Mã mẻ</th>
									                            <th width="8%" style="font-size: 11px">Mã thùng</th>
									                            <th width="10%" style="font-size: 11px">Vị trí</th>
									                            <th width="8%" style="font-size: 11px">Mã phiếu</th>
									                            <th width="8%" style="font-size: 11px">Phiếu ĐT</th>
									                            <th width="8%" style="font-size: 11px">Phiếu EO</th>
									                            <th width="8%" style="font-size: 11px">Nhà sản xuất</th>
									                        ...
									                        for(int j = 0; j < SpChitietList.size(); j++){
									                        	...
									                        	<tr>
									                        		<td>
									                        			<input type="hidden" readonly name="khott_sp_ct_<%=i %>" value="<%=SpChitiet.getKhoSpCtId() %>">
									                        		...
									                        		<td><input type="text" readonly name="vitri_<%=i %>" value="<%=SpChitiet.getVitri() %>" style="width: 100%"></td>
									                        		...
									                        		<td><input type="text" readonly name="nhasanxuat_<%=i %>" value="<%=SpChitiet.getNhasanxuat() %>" style="width: 100%"></td>
