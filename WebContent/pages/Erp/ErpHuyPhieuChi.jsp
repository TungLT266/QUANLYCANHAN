...
<% IErpHuyphieuchiList obj = (IErpHuyphieuchiList)session.getAttribute("obj"); %>
...
<% obj.setNextSplittings(); 
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[5];
		quyen = util.Getquyen("ErpHuyphieuchiSvl","",userId);
...
<body>
<form name="erpHctmhForm" method="post" action="../../ErpHuyphieuchiSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý công nợ > Công nợ phải trả > Hủy chứng từ 
        </div>
		                    ...
		                    <% if(tt.equals("0")){ %>
                                ...
                                <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%= ht.getId()%>'
							       ...
							       border="0" onclick="if(!confirm('Bạn có muốn chốt hủy phiếu này?')) {return false ;}else{ processing('<%="chotphieu"+ht.getId()%>' , '../../ErpHuyphieuchiSvl?userId=<%=userId%>&chot=<%= ht.getId() %>&loaict=<%=ht.getloaichungtu() %>&soct=<%=ht.getSoChungTuGoc()%>');}"  >
								</A><%} %>
								<A href = "../../ErpHuyphieuchiUpdateSvl?userId=<%=userId%>&update=<%= ht.getId() %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>
                                
                               <% } else {if(quyen[3]!=0){ %>
		                    	<A href = "../../ErpHuyphieuchiUpdateSvl?userId=<%=userId%>&display=<%= ht.getId() %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
