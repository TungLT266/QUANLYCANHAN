...
%>
<% ResultSet rsSP= obj.getKho();%> 
<% ResultSet rsTP= obj.getThanhpham();%> 
<% ResultSet rsSolo= obj.getSolo();%>  
<%  
...
String sum = (String) session.getAttribute("sum");
	...
	if(!util.check(userId, userTen, sum)){
		response.sendRedirect("/TraphacoHYERP/index.jsp");
		 ...
		 int[] quyen = new  int[5];
		 quyen = util.Getquyen("ErpParkHoadontrongnuocSvl","",userId);
...
<script type="text/javascript">
    ...
    <SCRIPT language="javascript" type="text/javascript">
	 ...
	 {   
	    ...
	    document.forms["erpParkForm"].ngaytao.value = "";
	    
	    document.forms["erpParkForm"].tungay.value = "";
	    document.forms["erpParkForm"].denngay.value = "";
	    document.forms["erpParkForm"].kho.value = "";
	    
	    document.forms["erpParkForm"].solo.value = "";
	    document.forms["erpParkForm"].thanhpham.value = "";
...
<body>
<form name="erpParkForm" method="post" action="../../ErpParkHoadontrongnuocSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý mua hàng &gt; Mua hàng trong nước &gt; Hóa đơn trong nước
        </div>
            ...
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >								                          
                    ...
                    <TR>
                        ...
                        <TD class="plainlabel">
                            <input type="text" class="days" 
                                   id="tungay" name="tungay" value="<%= obj.getTungay()%>" onchange="submitform()" />
                        </TD>
                      <TD class="plainlabel" width="15%">Đến ngày </TD>
                        <TD class="plainlabel">
                            <input type="text" class="days" 
                                   id="denngay" name="denngay" value="<%= obj.getDenngay() %>" onchange="submitform()" />
                        </TD>
                    </TR>
                    ...
                    <TR>
                        <TD class="plainlabel" width="15%">Số nhập kho trong nước </TD>
                        <TD class="plainlabel">
                        ...
                        <TD class="plainlabel" valign="top" >
			                    	<select name="kho" id="kho" onchange="submitform();"  style="width: 200px;">
			                        	...
			                        	<%
			                        		if(rsSP != null)
			                        		{
			                        			...
			                        			while(rsSP.next())
			                        			{  
			                        			if( rsSP.getString("pk_seq").equals(obj.getKhoId())){ %>
			                        				<option value="<%= rsSP.getString("pk_seq") %>" selected="selected" ><%=rsSP.getString("ten") %></option>
			                        			<%}else { %>
			                        				<option value="<%= rsSP.getString("pk_seq") %>" ><%=rsSP.getString("ten") %></option>
			                        		 <% } } rsSP.close();
			                        		}
			                        	%>
			                        </select>
                     ...
                     <TR>
                        <TD class="plainlabel" width="15%">Ngày tạo </TD>
                        <TD class="plainlabel">
                            <input type="text" class="days" 
                                   id="ngaytao" name="ngaytao" value="<%= obj.getNgaytao() %>" onchange="submitform()" />
                        </TD>
                    ...
                    <TR>
                     ...
                     <TR>
                        <TD class="plainlabel" width="15%">Thành phẩm</TD>
                         <TD class="plainlabel" valign="top" >
			                    	<select name="thanhpham" id="thanhpham" onchange="submitform();"  style="width: 200px;">
			                    		<option value=""> Tất cả</option>
			                        	<%
			                        		if(rsTP != null)
			                        		{
			                        			...
			                        			while(rsTP.next())
			                        			{  
			                        			if( rsTP.getString("diengiai_chuoi").equals(obj.getThanhphamId())){ %>
			                        				<option value="<%= rsTP.getString("diengiai_chuoi") %>" selected="selected" ><%=rsTP.getString("diengiai_chuoi") %></option>
			                        			<%}else { %>
			                        				<option value="<%= rsTP.getString("diengiai_chuoi") %>" ><%=rsTP.getString("diengiai_chuoi") %></option>
			                        		 <% } } rsTP.close();
			                        		}
			                        	%>
			                        </select> 
                        ...
                        <TD class="plainlabel" valign="middle" >Số lô</TD>
                         <TD class="plainlabel" valign="top" >
			                     	<select name="solo" id="solo" onchange="submitform();"  style="width: 200px;">
			                    		<option value=""> Tất cả</option>
			                        	<%
			                        		if(rsSolo != null)
			                        		{
			                        			...
			                        			while(rsSolo.next())
			                        			{  
			                        			if( rsSolo.getString("diengiai").equals(obj.getSoloId())){ %>
			                        				<option value="<%= rsSolo.getString("diengiai") %>" selected="selected" ><%=rsSolo.getString("diengiai") %></option>
			                        			<%}else { %>
			                        				<option value="<%= rsSolo.getString("diengiai") %>" ><%=rsSolo.getString("diengiai") %></option>
			                        		 <% } } rsSolo.close();
			                        		}
			                        	%>
			                        </select> 
                    ...
                    <TR style=display:none>
                        ...
                        <TD class="plainlabel" valign="middle">
                        <% System.out.println("Trạng thái = " + obj.getTrangthai()); %>
								 ...
								 <%}else if(obj.getTrangthai().trim().equals("3")) {  %>
									<option value=""> </option>
								 	<option value="0">Chưa chốt</option>
								  	...
								  	<option value="4">Hoàn tất</option>
								  	<option value="5">Đã hủy</option>
								  <%}else if(obj.getTrangthai().trim().equals("4")) {  %>
									<option value=""> </option>
								 	<option value="0">Chưa chốt</option>
								  	<option value="1" >Đã chốt</option>
								  	<option value="2" >Đã duyệt</option>
								  	...
								  	<option value="5">Đã hủy</option>
								  <%}else if(obj.getTrangthai().trim().equals("5")) {  %>
								  	<option value=""> </option>
								 	<option value="0">Chưa chốt</option>
								  	<option value="1" >Đã chốt</option>
								  	<option value="2" >Đã duyệt</option>
						...
						{ 		
                    		...
                    		else if( (tt.equals("2") && htat.equals("0")) || tt.equals("3"))
                    		{
		                        ...
		                        <%} %>
									...
									<TD align="center" <%= style %> ><%= ht.getPREFIX()+ ht.getId() %></TD>
				                    <TD align="center" <%= style %>><%= ht.getNgayghinhan() %></TD>
				                    <TD align="center" <%= style %>><%= ht.getGhichu() %></TD>
				                    <TD align="center" <%= style %>><%= ht.getSohoadon() %></TD>
				                    <TD align="left" <%= style %>><%= ht.getTenNCC() %></TD> 	
		                    ...
		                    <TD align="center" <%= style %>>
 					     	...
 					     	<TD align="center" <%= style %>><%= ht.getNgaytao() %></TD>	
		                    <TD align="left" <%= style %>><%=  ht.getNguoitao() %></TD>
         					<TD align="center" <%= style %>><%=  ht.getNgaysua()  %></TD>
							<TD align="left"<%= style %>><%= ht.getNguoisua() %></TD> 
									
		                    <TD align="center"> 
		                    <% if(tt.equals("0")){ %>
		                   		 <%if(quyen[2]!=0){ %>
                                <A href = "../../ErpParkHoadontrongnuocUpdateSvl?userId=<%=userId%>&update=<%= ht.getId()%>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                                ...
                                <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%= ht.getId()%>'
							       ...
							       border="0" onclick="if(!confirm('Bạn có muốn chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+ ht.getId()%>' , '../../ErpParkHoadontrongnuocSvl?userId=<%=userId%>&chot=<%= ht.getId() %>');}"  >
								</A>											     
								<%} %>
                                <%if(quyen[1]!=0){ %>
                                     <A href = "../../ErpParkHoadontrongnuocSvl?userId=<%=userId%>&delete=<%= ht.getId() %>&poId=<%= ht.getId() %>"><img src="../images/Delete20.png" alt="Xóa Chứng từ" title="Xóa Chứng từ" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa Chứng từ này?')) return false;"></A>
                                <%} %>									
		                    <%}else{ if(tt.equals("1")){ %>
		                    	<A href = "../../ErpParkHoadontrongnuocUpdateSvl?userId=<%=userId%>&display=<%= ht.getId() %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;	
		                    	 <%if(quyen[4]!=0){ %>
                                 <A id='chotphieu<%= ht.getId()%>'
							       href=""><img
							      src="../images/unChot.png" alt="Bỏ chốt" title="Bỏ chốt"
									width="20" height="20" longdesc="Bỏ chốt"							
							       border="0" onclick="if(!confirm('Bạn có muốn bỏ chốt ghi nhận này?')) {return false ;}else{ processing('<%="chotphieu"+ ht.getId()%>' , '../../ErpParkHoadontrongnuocSvl?userId=<%=userId%>&unchot=<%= ht.getId()%>');}"  >
								</A>
								<%} %>
		                    	 
		                    <%-- <% if(danhanhang ==0) { %>
		                    	<A href="../../ErpNhanhangtrongnuoc_GiaySvl?userId=<%=userId%>&convert=<%= ht.getId()  %>&NCCId=<%= ht.getNCCId()  %>&hoadonnccId=<%= ht.getHoadonId()  %>&loaihd=<%= ht.getLoaiHd()  %>">
