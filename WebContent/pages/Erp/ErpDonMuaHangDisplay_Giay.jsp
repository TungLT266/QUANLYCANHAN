				...
				if(soluong.item(i).value != "")
				{
					if(dongia.item(i).value != "")
					{
						...
						if(document.getElementById("AUSDVAT") && document.getElementById("BUSDVAT")){
							...
							donGiaUSD.item(i).value = (temp_x.toFixed(0));
							tt = parseFloat(slg) * parseFloat(donGiaUSD.item(i).value);
							
							
							/* tien nguyen te */
							tt1 = parseFloat(slg) *parseFloat(replaceAll(dgia,",",""));	
	...
	<form name="dmhForm" method="post" action="">
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left" class="tbnavigation"> Quản lý mua hàng > Mua hàng nhập khẩu > Đơn mua hàng > Hiển thị</div>
			...
			<div align="left" id="button" style="width: 100%; height: 32px; padding: 0px; float: none" class="tbdarkrow">
				...
				<%-- <% if(dmhBean.getCanDuyet().equals("0")) { %> --%>
					<A href="../../ErpDonmuahangUpdate_GiaySvl?userId=<%=userId %>&dmhId=<%= dmhBean.getId() %>&task=print" >
						...
						<A href="../../InDonDatHang?userId=<%=userId %>&Id=<%= dmhBean.getId() %>&loai=indonhang" >
						<img src="../images/pdf.jpg" alt="Print"  title="Print Don Dat Hang" border="1" longdesc="Print" style="border-style:outset;width:30px; height:30px"></A>
						
						<!-- khi la hop dong moi cho in phu luc hop dong -->
						<% if (dmhBean.getLoaiDMH_NK().equals("0")) { %>
						<A href="../../ErpInPhuLucHopDongSvl?userId=<%=userId %>&Id=<%= dmhBean.getId()%>" >
						<img src="../images/pdfblue.jpg" alt="Print"  title="Print Phu Luc Hop Dong" border="1" longdesc="Print" style="border-style:outset; width:30px; height:30px"> </A>
						<%}%>
	           	 					...
	           	 					<div style='display:none'>
               							<DIV id="ndThongtinhdngoai" style='padding:0px 5px; background:#fff;'>
                            				...
                            				<tr>
                            					...
                            					<td colspan = 2>
                            						<INPUT type="text" name="sohopdong" id="sohopdong" style="width: 200px" value='<%=dmhBean.getSohopdong() %>'>&nbsp;
                            					</td>
                            				</tr>
                            				<%-- <tr>
								...
								if(spList.size() > 0) { 
									for(int i = 0; i < spList.size(); i++) { 
										...
										<tr>
											...
											<TD align="center" width="8%">
												<input type="hidden" value="<%= sp.getThuexuat() %>" name="thuexuat" style="width: 100%; text-align: right;" readonly = readonly>
