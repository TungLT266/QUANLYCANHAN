...
<%@page import="geso.traphaco.erp.beans.baocaosudungccdc.*" %>
...
<%
	...
	IErp_BaoCaoSuDungCCDC obj = (IErp_BaoCaoSuDungCCDC) session.getAttribute("obj");
	
	ResultSet nhomCCDCRs= obj.getNhomCCDCRs();
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="frm" method="post" action="../../BoBaoCaoSuDungCCDC">
		<input type="hidden" name="action" value='1'> 
		<input type="hidden" name="userId" value='<%=userId%>'>
		<div id="main" style="width: 99%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left"
					class="tbnavigation">Quản lý tài sản >  Bộ báo cáo sử dụng CPPB </div>
			...
			<div align="left"
				...
				<fieldset>
					<legend class="legendtitle">Bộ báo cáo sử dụng CPPB
					</legend>
							...
							<TABLE width="70%" cellpadding="6" cellspacing="0">
								...
								<TD class="plainlabel" colspan="3">
									...
									<% if(obj.getLoaiBaoCao().equals("1")) { %>
										<option value="1" selected="selected">Báo cáo tăng CPPB</option>
										<option value="2">Báo cáo giảm CPPB</option>
										<option value="3">Báo cáo KH CPPB theo đơn vị</option>
<!-- 										<option value="4">Báo cáo phân bổ KH</option> -->
									<% } else if(obj.getLoaiBaoCao().equals("2")){ %>
										<option value="1">Báo cáo tăng CPPB</option>
										<option value="2"  selected="selected">Báo cáo giảm CPPB</option>
										<option value="3">Báo cáo KH CPPB theo đơn vị</option>
<!-- 										<option value="4">Báo cáo phân bổ KH</option> -->
									<% } else  if(obj.getLoaiBaoCao().equals("3")){  %> 
										<option value="1">Báo cáo tăng CPPB</option>
										<option value="2">Báo cáo giảm CPPB</option>
										<option value="3" selected="selected">Báo cáo KH CPPB theo đơn vị</option>
<!-- 										<option value="4">Báo cáo phân bổ KH</option> -->
									<% } 
...
<%-- 										<option value="1">Báo cáo tăng CPPB</option>
										<option value="2">Báo cáo giảm CPPB</option>
										<option value="3">Báo cáo KH CPPB theo đơn vị</option>
										<option value="4" selected="selected">Báo cáo phân bổ KH</option>
									<% }%> --%>
									</select>
								</TR>
								...
								<% } %>
								<TR>
								<TD class="plainlabel"> Loại CPPB</TD>
									<TD class="plainlabel" colspan="3">
										<select name="nhomCCDC" id="nhomCCDC" style="width: 200px" >
											<option value=""></option>
											<% if (nhomCCDCRs != null)
											   {
												while (nhomCCDCRs.next())
												{
														if (nhomCCDCRs.getString("pk_seq").equals(obj.getNhomCCDCId()))
														{ %>
														<option value="<%=nhomCCDCRs.getString("pk_seq")%>" selected="selected"><%=nhomCCDCRs.getString("ten")%></option>
											<% } else { %>
												<option value="<%=nhomCCDCRs.getString("pk_seq")%>"><%=nhomCCDCRs.getString("ten")%></option>
											<% } } nhomCCDCRs.close(); } %>
										</select>			
									</TD>
								</TR>
