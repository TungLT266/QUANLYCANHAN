...
<%
	...
	List<Erp_Item> loaiThanhToanList = dmhBean.getDoiTuongKhacList();
...
{ 		
	...
	var boolThem=document.getElementById("boolThem");
 ...
 */					
	...
	for(var j = 0; j < stt.length; j++ )
	{
	   ...
	   {    
	     ...
	     var diaChiNCC = document.getElementById("diaChiNCC" +j +k);
		...
		if((mausoHD.item(k).value!=""||kyhieu.item(k).value!="")&&tenncc.item(k).value==""&&tenNCC!=null)	
		{
			tenncc.item(k).value=tenNCC.value;
			mst.item(k).value=MST.value;
			diaChiNCC.item(k).value=diachi.value;
	 ...
	 function tinhthue(isSum, row, detail)
	 	{
			...
			var tongcong = document.getElementsByName("thanhtiensauthue");
				console.log("stt leng: "+ stt.length);
...
//	 			tongcong.item(i).value = DinhDangTien(tongtien);
	...
	function upperLetters(element,i)
	{
...
<BODY leftmargin="0" bottommargin="0" topmargin="0" rightmargin="0">
	<form name="dmhForm" method="post" action="../../ErpPhieuThanhToanUpdateSvl">
		...
		<input type="hidden" name="boolThem" value='<%=dmhBean.getThem()%>'>
					...
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							...
							<input type="hidden" name="dvthId" id="dvthId" value='<%=dmhBean.getDvthId()%>'>	
									<select name="dvthId" id="dvthId" style="width: 300px" onchange="changePhongBan();" disabled="disabled">
										<% if (dvthList != null)
										{
											while (dvthList.next())
											{
						...
						<tbody id="nguoidenghithanhtoan" style="display: none">
							<TR class="plainlabel">							
								...
								<TD colspan="3">
									...
									<input TYPE="HIDDEN" name="nguoidenghi" id="nguoidenghi" style="width: 400px" value="<%=(dmhBean.getNguoidenghithanhtoan() != null && !dmhBean.getNguoidenghithanhtoan().equals("NULL")) ? dmhBean.getNguoidenghithanhtoan() : ""%>">
									<input TYPE="text" name="nguoidenghiTen" id="nguoidenghiTen" style="width: 400px" value="<%=(dmhBean.getNguoidenghithanhtoan() != null && !dmhBean.getNguoidenghithanhtoan().equals("NULL")) ? dmhBean.getNguoidenghithanhtoanTen() : ""%>">
...
<!-- 								</td> -->
						...
						<tbody id="khoanmucchi" style="display: none">
							...
							<TR class="xoatrang">
								...
								<TD class="plainlabel"><input type="text" name="BVAT" id="BVAT" value="<%=formatter.format(Double.parseDouble(dmhBean.getTongtienchuaVat()))%>" style="text-align: right; border-radius:4px; height: 20px;"
									readonly="readonly"> </TD>
								<TD class="plainlabel">VAT</TD>
								<TD class="plainlabel"><input type="text" name="VAT" id="VAT" value="<%=formatter.format(Double.parseDouble(dmhBean.getVat()))%>" style="text-align: right; border-radius:4px; height: 20px;"
...
<!-- 								</TD> -->
							...
							<TR>
								...
								<TD class="plainlabel xoatrang"><input type="text" name="CONLAI" id="CONLAI" value="<%=formatter.format(Double.parseDouble(dmhBean.getTongtienconlai()))%>" style="text-align: right; border-radius:4px; height: 20px;"
									readonly="readonly"> 
								</TD>
...
<%-- 												      			<input type="text" name= <%= "mst" + i %> id = <%= "mst" + i + dem %>  value = "<%=hd.getMasothue() %>" style="width: 100%; border-radius:4px; height: 20px;"    onkeyup="ajax_showOptions(this,'masothue',event)" AUTOCOMPLETE="off" > --%>
												      		...
												      		<td>
												      			<input type="text" name=<%= "tienhang" + i %> value = "<%= formatter.format(Double.parseDouble(hd.getTienhang().replace(",", ""))) %>" style="width: 100%; border-radius:4px; height: 20px; text-align: right; " onkeypress="return keypress(event);" onChange="tinhthue(true, <%=i%>, <%=dem%>);">
												      		</td>

												      		<td>
												      			<input type="text" name=<%= "thuesuat" + i  %> value = "<%= formatter.format(Double.parseDouble(hd.getThuesuat().replace(",", ""))) %>" style="width: 100%; border-radius:4px; height: 20px; text-align: right;" onkeypress="return keypress(event);" onChange="tinhthue(true, <%=i%>, <%=dem%>);">
												      			<input type="hidden" name=<%= "thuesuat_goc" + i  %> value = "<%= hd.getThuesuat() %>" style="width: 100%" >
												      		</td>

												      		<td>
												      			<input type="text" name=<%= "thue" + i %> value = "<%= formatter.format(Double.parseDouble(hd.getTienthue().replace(",", ""))) %>"  style="width: 100%; border-radius:4px; height: 20px; text-align: right;" onkeypress="return keypress(event);" onChange="tinhTienThueTuDo(this,<%= i %>)" >
												      		</td>

												      		<td>
												      			<input type="text" name=<%= "cong" + i %> value = "<%= formatter.format(Double.parseDouble(hd.getTienhang().replace(",", "")) + Double.parseDouble(hd.getTienthue().replace(",", ""))) %>" style="width: 100%; border-radius:4px; height: 20px; text-align: right;" readonly>
												      		</td>
											      ...
											      // Thêm 5 dòng trống để có thể thêm hóa đơn	 
										      		...
										      		<tr>	
											      		<td>
											      			<input type="text" name=<%= "mausoHD" + i %> ID=<%= "mausoHD" + i +"_"+dem %>  value ="" style="width: 100%; border-radius:4px; height: 20px;"  onchange="upperLetters(this,<%= i %>);">
											      				<script type="text/javascript">
																	jQuery(function()
																		{		
																			$("#mausoHD<%="" + i +"_"+ dem %>").autocomplete("MAUHOADON_DNTT.jsp?");
											      		...
											      		<%-- 	<input type="hidden" name=<%= "mausoHD" + i %> value ="" style="width: 100%; border-radius:4px; height: 20px;" > --%>											      		
											      			<input type="text" name=<%= "kyhieu" + i %> id=<%= "kyhieu" + i+"_" +dem%> value = "" style="width: 100%; border-radius:4px; height: 20px;"  onchange="upperLetters(this,<%= i %>);">
											      						      		<script type="text/javascript">
																	jQuery(function()
																		{		
																			$("#kyhieu<%="" + i +"_"+ dem %>").autocomplete("KYHIEU_DNTT.jsp?");
							                        		...
							                        		if(sanPhamKhoList != null)
							                        		{
							                    				...
							                    				<tr align="center" >
																	<td>
																		...
																		<input type="hidden" name="<%= "spId" + i + t %>" style="width: 100%; border-radius:4px; height: 20px;" value="<%= item.getSpId() %>"  >
							...
							<% for(int i = count; i < 10+soDongThem; i++) { %>
							 	<tr>
											...
											<%-- <input type="hidden" name="stt" value = "<%= i %>" >
												...
												<input type="hidden" name="stt" value = "<%= i %>" >
												<input  type="text" name="stt_1" value="<%= i + 1 %>" style="text-align: center; border-radius:4px; height: 20px;" readonly="readonly">
