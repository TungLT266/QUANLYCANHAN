	...
	<form name="nhForm" method="post"
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			...
			<div align="left" id="button"
				...
				class="tbdarkrow">
				<A href="../../ErpNhanhangnhapkhau_GiaySvl?userId=<%= userId %>&loai=0"> <img
					src="../images/Back30.png" alt="Quay ve" title="Quay ve"
					border="1px" longdesc="Quay ve" style="border-style: outset">
				</A> 
				
				<A href="../../ErpInBienBanGiaoNhanKiemNhapHangHoaPdfSvl?userId=<%=userId %>&display=<%= nhBean.getId()%>&loai=<%=nhBean.getLoaimh() %>">
						<img src="../images/Printer30.png" alt="In biên bản giao nhận và kiểm nhập hàng hóa"  title="In biên bản giao nhận và kiểm nhập hàng hóa" border="1" longdesc="Print" style="border-style:outset"></A>
			</div>
					...
					<div style="float: none; width: 100%" align="left">
										...
										<% if (nccList != null)
						{
							...
							<TR>
								...
								<TD class="plainlabel" valign="top">Số tờ khai</TD>
								<TD class="plainlabel" valign="top" ><input readonly
									type="text" value="<%= nhBean.getSoToKhai() %>" />
								</TD>
									...
									<%-- <select style="width: 100%" name="dvdl" disabled id="<%= "donvitinh_"+i %>" style=" width: 200px">
												...
												if (listDonvi.size() > 0)
												{
														for (int j = 0; j < listDonvi.size(); j++)
														{
															...
															if(donvi.getId().equals(sp.getDvdl())){
																donvitinh=donvi.getDonvi();
															}
															 
														
												  } }  %>
								
												<input  style="width: 100%" name="dvdl" disabled id="<%= "donvitinh_"+i %>" style=" width: 200px" readonly="readonly" 	
												 value="<%=donvitinh%>">	
								...
								<td align="left">
									...
									<DIV id="subcontent<%= i %>"
										style="position: absolute; visibility: hidden; border: 9px solid #80CB9B; 
											background-color: white; width: 850px; padding: 4px; height: 400px;z-index:20;overflow-y: scroll;">
										...
										<table width="80%" align="center" class="detail">
											...
											<% DetailMeSp temp = sp.getListDetailMeSp().get(j); %>
												<tr>
													...
													value="<%= temp.getMaThung() %>" readonly="readonly"></td>
													...
													<td>
														<% if(sp.getIsKiemDinh().equals("0")){ %>
															<select name= "<%= sp.getId() + "." + sp.getMuahang_fk() +i+ ".khuvucX" %>"  style="width: 270px;">
														...
														<%} else { %>
															<% System.out.println("Size:"+ listKhuVucKhoCXL.size()); %>
															<select name= "<%= sp.getId() + "." + sp.getMuahang_fk() +i+ ".khuvucX" %>"  style="width: 270px;">
													...
													<td><input type="text" size="100px" 
													name="<%= sp.getId() + "." + sp.getMuahang_fk() +i+ ".meX" %>"
													value="<%= temp.getMe() %>" ></td>
													<td><input type="text" size="100px"  
													name="<%=sp.getId() + "." + sp.getMuahang_fk() +i+ ".nsxtenX"%>"
													value="<%=temp.getNSXTen()%>" 
													readonly="readonly" />
													<input type="hidden" size="100px"  
