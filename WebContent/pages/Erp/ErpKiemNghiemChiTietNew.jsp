...
<script type="text/javascript">
	...
	function tinhTienTienThamSo(k){
		let pp_kyhieu = _.map( $('input[name=pp_kyhieu'+k+']'), x => x.value )  ;
		let pp_congthuc = $('input[name=pp_congthuc'+k+']');
		let pp_size = _.toInteger( $('#pp_size' + (k-1)).val());
		let congthuc = _.filter(pp_congthuc, x =>!_.isEmpty (x.value)) ;
		
		for(j = 0; j < pp_size;j++){
			_.forEach(congthuc, (e, t1)=> {
				let mau = _.map( $('input[name=pp_mau' + (k) + '_'+(j+1)+']') , x => x.value ) ;
				let index = _.findIndex(pp_congthuc, x => x == e );
				let ct = rep3(e.value, pp_kyhieu, mau);
				$('input[name=pp_mau' + (k) + '_'+(j+1)+']')[index].value = DinhDangN( parseFloat(eval(ct)), 3) ;
			});
			let pp_min = $('input[name=pp_min'+k+']');
			let pp_max = $('input[name=pp_max'+k+']');
			let pp_avg = $('input[name=pp_avg'+k+']');
			let pp_sum = $('input[name=pp_sum'+k+']');
			
			let pp_ismin = $('input[name=pp_ismin'+k+']');
			let pp_ismax = $('input[name=pp_ismax'+k+']');
			let pp_isavg = $('input[name=pp_isavg'+k+']');
			let pp_issum = $('input[name=pp_issum'+k+']');
			
			_.forEach(pp_congthuc, (e1, t2)=> {
				try{
					if(JSON.parse(pp_ismin[t2].value)){
						pp_min[t2].value = DinhDangN( _.min(_.map($('.pp_mau'+k+'_' + (t2+1)), x => parseFloat(x.value))),3);
					}
					if(JSON.parse(pp_ismax[t2].value)){
						pp_max[t2].value = DinhDangN( _.max(_.map($('.pp_mau'+k+'_' + (t2+1)), x => parseFloat(x.value))),3);
					}
					if(JSON.parse(pp_isavg[t2].value)){
						pp_avg[t2].value = DinhDangN( _.sum(_.map($('.pp_mau'+k+'_' + (t2+1)), x => parseFloat(x.value)))/ _.size(_.map($('.pp_mau'+k+'_' + (t2+1)), x => parseFloat(x.value))),3);
					}
					if(JSON.parse(pp_issum[t2].value)){
						pp_sum[t2].value = DinhDangN( _.sum(_.map($('.pp_mau'+k+'_' + (t2+1)), x => parseFloat(x.value))),3);
					}
				} catch(e){
					console.log(e);
				}
	...
	function tinhCongThuc(){
		...
		for(m = 0; m < pp_size.length; m++){
			tinhTienTienThamSo(m+1);	
		} 
	}
	
	let rep3 = (str, arr1, arr2) => {
		for(i = 0; i < _.size(arr2);  i++){
			str = str.replace(new RegExp(arr1[i], 'g'),_isNullOrNumeric(arr2[i]) );  
		}
		return str;
	}
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 60%; padding: 5px; float: left"
					...
					Kiểm nghiệm chi tiết &gt; Tạo mới</div>
					...
					<div style="float: none; width: 100%" align="left">
						<TABLE width="100%" cellpadding="4" cellspacing="0">
							...
							<TR>
								...
								<TD class="plainlabel" width="30%"><input type="time"
			...
			%>
				...
				<div class="panel1" style="overflow-x: scroll;width: 1150px;">
					<TABLE class="tabledetail" style="" border="0" cellpadding="1"
						cellspacing="1">
							<tr class="tbheader">
								<th align="center" width="40px">STT</th>
								<th align="center" width="40px">Ký hiệu</th> 
								
								<th align="center" width="200px">Tham số</th> 
								<th align="center" width="40px">ĐVT</th>
								<th align="center" width="100px">Công thức</th> 
								<% for(int i = 0; i < pp.get__sizePhuongPhap(); i++) {%>
									<th align="center" width="90px">Mẫu <%=i+1 %></th>
								<%} %>
								<th align="center" width="90px">Min</th> 
								<th align="center" width="90px">Max</th> 
								<th align="center" width="90px">Avg</th> 
								<th align="center" width="90px">Sum</th> 
							</tr>
							...
							<tr >
								<td align="center" width="40px">
									<input type="text" style="width: 40px; text-align: center;" value="<%=pkd.getSott() %>"   name= "sott<%=j+1 %>"  readonly="readonly">
									...
									<input type="hidden" style="width: 80px; text-align: center;" value="<%=pkd.isMax() %>"   name= "pp_ismax<%=j+1 %>"  readonly="readonly">
									
									<input type="hidden" style="width: 80px; text-align: center;" value="<%=pkd.isMin() %>"   name= "pp_ismin<%=j+1 %>"  readonly="readonly">
									<input type="hidden" style="width: 80px; text-align: center;" value="<%=pkd.isAvg() %>"   name= "pp_isavg<%=j+1 %>"  readonly="readonly">
									<input type="hidden" style="width: 80px; text-align: center;" value="<%=pkd.isSum() %>"   name= "pp_issum<%=j+1 %>"  readonly="readonly">
									
									
								</td>
								<td align="center" width="40px">
									<input type="text" style="width: 40px; text-align: center;" value="<%=pkd.getKyHieu() %>"  name= "pp_kyhieu<%=j+1 %>" readonly="readonly">
								</td>
								
								
								
								<td align="center" width="200px">
									<input type="text" style="width: 200px; text-align: left;" value="<%=pkd.getTenThamSo() %>" name= "pp_tenthamso<%=j+1 %>" readonly="readonly">
								</td>
								<td align="center" width="40px">
									<input type="text" style="width: 40px; text-align: center;" value="<%=pkd.getDonViTinh() %>"  name= "pp_donvitinh<%=j+1 %>" readonly="readonly">
								</td>
								
								<td align="center" width="100px">
									<input type="text" style="width: 100px; text-align: left;" value="<%=pkd.getCongThuc() %>" name= "pp_congthuc<%=j+1 %>" readonly="readonly">
								</td>
								
								<% if(!pkd.isDanhGia()) {%>
									<% if(pkd.isGroup()) {%>
										<td align="center" width="<%=(pp.get__sizePhuongPhap() + 4 ) * (90  + 4) %>px" colspan="<%=pp.get__sizePhuongPhap() + 4%>">
											<input type="text" style="width: <%=(pp.get__sizePhuongPhap() + 4 )* ( 90 + 4 ) %>px; text-align: right;" onchange="DinhDangNSoLe(this, 3); tinhCongThuc(); "  value=""  name= "pp_group<%=j+1 %>" >
										</td>
									<%} else{ %>
										<% for(int i = 0; i <  pkd.getThamSoMau().size(); i ++) {%>
										<td align="center" width="90px">
											<input type="text" style="width: 90px; text-align: right;" onchange="DinhDangNSoLe(this, 3); tinhCongThuc(); " <% if(pkd.isCongThuc()) {%> id="pp_mau_group<%=j+1 %>_<%=i+1 %>"
											 readonly="readonly" style="background-color: gray;" <% }%> value="<%=format.format(Double.parseDouble(pkd.getThamSoMau().get(i))) %>"  class="pp_mau<%=j+1 %>_<%=k+1 %>" name= "pp_mau<%=j+1 %>_<%=i+1 %>" >
										</td>
										<%} %>
										...
										<% for(int i = pkd.getThamSoMau().size(); i < pp.get__sizePhuongPhap(); i++) {%>
											<td align="center" width="90px">
												<input type="text" style="width: 90px; text-align: right;"  onchange="DinhDangNSoLe(this, 3); tinhCongThuc();" <% if(pkd.isCongThuc()) {%> id="pp_mau_group<%=j+1 %>_<%=i+1 %>" 
												readonly="readonly" style="background-color: gray;" <% }%> value="0" class="pp_mau<%=j+1 %>_<%=k+1 %>" name= "pp_mau<%=j+1 %>_<%=i+1 %>"   >
											</td>
										<%} %>
										<td align="center" width="90px">
											<input type="text" style="width: 90px; text-align: right;<% if(pkd.isMin()) { %> color:red; font-weight:bold <%} %>" value="<%=0 %>" name= "pp_min<%=j+1 %>" readonly="readonly">
										</td>
										<td align="center" width="90px">
											<input type="text" style="width: 90px; text-align: right;<% if(pkd.isMax()) { %> color:red; font-weight:bold <%} %>" value="<%=0 %>" name= "pp_max<%=j+1 %>" readonly="readonly">
										</td>
										<td align="center" width="90px">
											<input type="text" style="width: 90px; text-align: right;<% if(pkd.isAvg()) { %> color:red; font-weight:bold <%} %>" value="<%=0 %>" name= "pp_avg<%=j+1 %>" readonly="readonly">
										</td>
										<td align="center" width="90px">
											<input type="text" style="width: 90px; text-align: right;<% if(pkd.isSum()) { %> color:red; font-weight:bold <%} %>"  value="<%=0 %>" name= "pp_sum<%=j+1 %>" readonly="readonly">
										</td>
									<%} %>
								<%} else { %>
									...
									<% for(int i = 0; i <  pkd.getThamSoMau().size(); i ++) {%>
									<td align="center" width="90px">
										<input type="checkbox" style="width: 90px; text-align: right;" <%if(pkd.getThamSoMau().get(i).equals("1")) {%> checked="checked" <%} %> onchange="danhGiaMucDo(<%=k %>, <%=j+1 %>, <%=i+1 %>);"  value="1" id= "pp_danhgia<%=j+1 %>_<%=i+1 %>_<%=k %>" name= "pp_danhgia<%=j+1 %>_<%=i+1 %>"   />
