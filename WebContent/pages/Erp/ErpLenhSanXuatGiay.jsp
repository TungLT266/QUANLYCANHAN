...
<%  
	...
	if(!util.check(userId, userTen, sum)){
		response.sendRedirect("/TueLinh/index.jsp");
		 ...
		 quyen = util.GetquyenNew("ErpLenhsanxuatgiaySvl","&loaiNPP=0&isKH=0",userId);
...
<head>
    ...
    <SCRIPT language="javascript" type="text/javascript">
	 ...
	 function clearform()
	 {
	    ...
	    document.forms["erpLsxForm"].soLSX1.value = "";
...
<body>
<form name="erpLsxForm" method="post" action="../../ErpLenhsanxuatgiaySvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:70%; padding:5px; float:left" class="tbnavigation">
        	Quản lý cung ứng > Sản xuất > Lệnh sản xuất
        </div>
            ...
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >								                          
                     ...
                     <TR>
                        ...
                        <TD class="plainlabel"  valign="middle">
                           <select name="trangthai" onchange="submitform();" > 
                           		<option value="">  </option>
                   		    	<%  
 								String[] mang=new String[]{"0","2","3","4","5","6","7"};
 								String[] mangten=new String[]{"Chưa kích hoạt", 
                  ...
                  <TR>
                        ...
                        <TD class="plainlabel"  >Số LSX </TD>                        
                         <TD class="plainlabel" width="200px">
                            <input type="text"   onChange="submitform();"  id="soLSX1" name="soLSX1" value="<%=obj.getSoLSX() %>" />
                        </TD>
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    <TH align="center" width="10%">Số phiếu </TH>
	              	 	 <TH align="center" width="7%">Số LSX </TH> 
	                   
	                    <TH align="center" width="25%">Sản phẩm</TH>
					...
					<%
						int count = 1;
                 		if(lsxList != null)
                 		{
		                    ...
		                    <% } else { %>
		                    	<TD align="center" style="color: red;"><%= lsxList.getString("pk_seq") + (lsxList.getString("diengiai").length() >0? " ("+lsxList.getString("diengiai")+")" :""  ) %></TD>
			                 <%--  <TD align="center"><%= lsxList.getString("ngaybatdau") %></TD> --%>
		                    <% } %>
		                    
		                 	   <TD align="center" ><%= lsxList.getString("solenhsanxuat") %></TD>	 
		                   ...
		                   <%--  <TD align="center"><%= lsxList.getString("congdoan") %></TD>	 --%>					                                    
		                    ...
		                    <% if( Float.parseFloat(tt) < 1){ %>
		                    	<%if(quyen[Utility.SUA]!=0){ %>
                                	 <A href = "../../ErpLenhsanxuatgiayUpdateSvl?userId=<%=userId%>&update=<%=lsxList .getString("PK_SEQ") %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                                <%} %>
	                              <%if(quyen[Utility.XOA]!=0){ %>
                               	  <A href = "../../ErpLenhsanxuatgiaySvl?userId=<%=userId%>&delete=<%= lsxList.getString("PK_SEQ") %>"><img src="../images/Delete20.png" alt="Xóa lệnh sản xuất" title="Xóa lệnh sản xuất" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa lệnh sản xuất này?')) return false;"></A>									
		                    	  <%} %>
		                    	  	 <% if(lsxList.getString("duyet").equals("1")) {%>
		                    	  	  <%if(quyen[Utility.CHOT]!=0){ %>
	                       			 	<A href = "../../ErpLenhsanxuatgiaySvl?userId=<%=userId%>&kichhoat=<%= lsxList.getString("PK_SEQ") %>"><img src="../images/Chot.png" alt="Kích hoạt lệnh" title="Kích hoạt lệnh sản xuất" width="20" height="25" border=0 onclick="if(!confirm('Bạn có muốn kích hoạt lệnh sản xuất này?')) return false;"></A>
	                       			  <%}%>
	                       			 <%}%>
			                    
		                    <%} else if(Float.parseFloat(tt)>= 1 &&  Float.parseFloat(tt)<6 ){ %>
		                    	<%if( Float.parseFloat(tt)== 1 ){%>
		                    	 <%if(quyen[Utility.CHOT]!=0){ %>
			                    	<A href = "../../ErpLenhsanxuatgiaySvl?userId=<%=userId%>&kichhoat=<%= lsxList.getString("PK_SEQ") %>"><img src="../images/Chot.png" alt="Kích hoạt lệnh" title="Kích hoạt lệnh sản xuất" width="20" height="25" border=0 onclick="if(!confirm('Bạn có muốn kích hoạt lệnh sản xuất này?')) return false;"></A>
			                    <%}%>
		                    	...
		                    	if(quyen[Utility.SUA]!=0){ %>
                                 ...
                                 <A href = "../../ErpLenhsanxuatgiayUpdateSvl?userId=<%=userId%>&display=<%= lsxList.getString("PK_SEQ") %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>&nbsp;	
                                 <%}%>
                                <%if(quyen[Utility.CHOT]!=0){ %>
                                 <A href = "../../ErpLenhsanxuatgiaySvl?userId=<%=userId%>&hoantat=<%= lsxList.getString("PK_SEQ") %>"><img src="../images/hoantat.gif" alt="Hoàn tất lệnh sản xuất" title="Hoàn tất lệnh sản xuất" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn hoàn tất lệnh sản xuất này?')) return false;"></A>
                               <%}%>
                                <%
		                    	}
		                   else {  %>
