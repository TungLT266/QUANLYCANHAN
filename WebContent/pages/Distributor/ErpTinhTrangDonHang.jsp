...
NumberFormat formatter = new DecimalFormat("#,###,###");
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[6];
		quyen = util.Getquyen("ErpTinhtrangdonhangSvl","",userId);
		
		ResultSet rsnvbanhang=obj.getRsnvbanhang();
		ResultSet rskhoid=obj.getRskhoid();
		ResultSet rscongty=obj.getRscongty();
...
<head>
	...
	<script type="text/javascript">
		...
		function AutoLoad()
		{
			var lanchay = document.getElementById("lanchay").value;
			
			//alert('Lan chay: ' + lanchay)
			if( lanchay == '0' ) //Vừa mở trang thì không chạy
				document.getElementById("lanchay").value = "1";
			else
			{
				if( lanchay = '1' )
					document.forms["ckParkForm"].submit();
			...
			setTimeout(AutoLoad, 15000); //15s
		}
		
		function AnHienTimKiem()
		{
			var hientimkiem = document.getElementById("hientimkiem").value;
			...
			document.getElementById("hientimkiem").value = hientimkiem;
...
<body>
<form name="ckParkForm" method="post" action="../../ErpTinhtrangdonhangSvl">
<input type="hidden" name="userId" value="<%= userId %>"> 
<input type="hidden" name="action" value="1"> 
<input type="hidden" name="msg" value='<%= obj.getMsg() %>'> 
<input type="hidden" name="nppId" value='<%= obj.getNppId() %>'> 
<input type="hidden" name="currentPage" value="<%= obj.getCurrentPage() %>">
<input type="hidden" name="lanchay" id="lanchay" value='0'> 
<input type="hidden" name="hientimkiem" id="hientimkiem" value='<%= obj.getHientimkiem() %>'> 
		...
		<div id="main" style="width: 100%; padding-left: 2px">
			<div align="left" id="header" style="width: 100%; float: none">
				<div style="width: 55%; padding: 5px; float: left" class="tbnavigation">
					Quản lý bán hàng > Bán hàng > Tình trạng đơn hàng
				</div>
			...
			<div id="cotent" style="width: 100%; float: none">
				<div align="left"
					...
					<fieldset style="margin-top: 5px">
						<legend class="legendtitle"> 
							<a class="button3" href="javascript:AnHienTimKiem();"> 
										Tiêu chí tìm kiếm ( Ẩn / Hiện )&nbsp;&nbsp; </a>
						</legend>
						<div id='divTIMKIEM' style="width: 100%; <%= obj.getHientimkiem().equals("1") ? "" : " display:none; " %> " >
						<TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px"  >
							<TR>
								...
								<TD class="plainlabel" width="100px">Ngày nợ</TD>
								<TD class="plainlabel"><input type="text" 
									name="ngayno" value="<%= obj.getNgayno() %>" maxlength="10"
									onchange="submitform();" /></TD>
							...
							<TR>
								<TD class="plainlabel" width="100px">Loại đơn hàng </TD>
								<TD class="plainlabel">
								...
								<select name="trangthai" class="select2" style="width: 200px" onchange="submitform();">
									...
									<% if( obj.getTrangthai().equals("0") ) { %>
										<option value= "0" selected="selected" >Mới tạo</option>
									<% } else { %>
										<option value= "0" >Mới tạo</option>
									<% } %>
									<% if( obj.getTrangthai().equals("6") ) { %>
										<option value= "6" selected="selected" >Chờ xử lý</option>
									<% } else { %>
										<option value= "6" >Chờ xử lý</option>
									<% } %>
									<% if( obj.getTrangthai().equals("1") ) { %>
										<option value= "1" selected="selected" >Đã duyệt SS</option>
									<% } else { %>
										<option value= "1" >Đã duyệt SS</option>
									<% } %>
									<% if( obj.getTrangthai().equals("2") ) { %>
										<option value= "2" selected="selected" >Đã duyệt CS</option>
									<% } else { %>
										<option value= "2" >Đã duyệt CS</option>
									<% } %>
									<% if( obj.getTrangthai().equals("3") ) { %>
										<option value= "3" selected="selected" >Đã in HĐ</option>
									<% } else { %>
										<option value= "3" >Đã in HĐ</option>
									<% } %>
									<%-- <% if( obj.getTrangthai().equals("4") ) { %>
										<option value= "4" selected="selected" >Đã ra PGH</option>
									<% } else { %>
										<option value= "4" >Đã ra PGH</option>
									<% } %> --%>
									<% if( obj.getTrangthai().equals("7") ) { %>
										<option value= "7" selected="selected" >Đã ra SXH</option>
									<% } else { %>
										<option value= "7" >Đã ra SXH</option>
									<% } %>
									<%--  <% if( obj.getTrangthai().equals("6") ) { %>
										<option value= "6" selected="selected" >Đã ra SXH</option>
									<% } else { %>
										<option value= "6" >Đã ra SXH</option>
									<% } %>  --%>
							...
							<TD  class="plainlabel" ><INPUT name="soHopDong" type="text" size="30" value = '<%= obj.getSoHopDong()%>' onChange = "submitform();"></TD>
								
								<TD  class="plainlabel">NVBH</TD>
							<TD  class="plainlabel" >
				...
				<div align="left"
					...
					<fieldset>
						<legend>
							<span class="legendtitle">Theo dõi tình trạng đơn hàng</span>&nbsp;&nbsp;
						</legend>
						...
						<thead>
							<TR class="tbheader">
								...
								<TH align="center" style="width: 10%">Ngày chứng từ</TH>
								<TH align="center" style="width: 10%;">Mã đối tượng</TH>
								<TH align="center" style="width: 15%;">Đối tượng</TH>
								<TH align="center" style="width: 15%">Người tạo</TH>
								<TH align="center" style="width: 8%">TG xử lý <br />( giờ )</TH>
								
								<TH align="center" style="width: 5%">Mới tạo</TH>
								<TH align="center" style="width: 5%">Chờ xử lý</TH>
								<TH align="center" style="width: 5%">Đã hủy</TH>
								<TH align="center" style="width: 5%">SS duyệt</TH>
								<TH align="center" style="width: 5%">CS duyệt</TH>
								<!-- <TH align="center" style="width: 5%">Lệnh xuất hàng</TH> -->
								<TH align="center" style="width: 5%">Hóa đơn</TH>
								<TH align="center" style="width: 5%">Xuất kho</TH>
							</TR>
							</thead>
	                <tbody>
							<%
                 		...
                 		if(nhapkhoRs != null)
                 		{
                 			...
                 			{  	
                 				String sohd = nhapkhoRs.getString("hopdong_fk").equals("-1") ? "" : nhapkhoRs.getString("hopdong_fk");
								...
								String donhangmuon = nhapkhoRs.getString("donhangmuon");
								String trangthai = "";
	                    		String tt = nhapkhoRs.getString("trangthai");
	                    		
	                    		int daraPGH = nhapkhoRs.getInt("daraPGH");
	                    		int daraHOADON = nhapkhoRs.getInt("daraHOADON");
	                    		int cs_duyet = nhapkhoRs.getInt("CS_DUYET");
	                    		int ss_duyet = nhapkhoRs.getInt("SS_DUYET");
	                    		int mohoadon = nhapkhoRs.getInt("mohoadon");
	                    		...
	                    		else
	                    		{
		                    		...
		                    		else if( daraPGH > 0 || nhapkhoRs.getString("CS_DUYET").equals("1") )
		                    			_daraPGH = "X";
		                    		else if ( ss_duyet > 0 )
		                    			_ss_duyet = "X";
		                    		else if( tt.equals("0") )
								...
								<%} %>
								<TD align="center" onMouseover="ddrivetip('<%=msg %>', 450)"; onMouseout="hideddrivetip()"><%= nhapkhoRs.getString("machungtu") %></TD>
								<TD align="center"><%= nhapkhoRs.getString("ngaydonhang").substring(0, 19) %></TD>
								<TD ><%= nhapkhoRs.getString("nppMA") %></TD>
								<TD ><%= nhapkhoRs.getString("nppTEN") %></TD>
								<TD><%= nhapkhoRs.getString("nguoitao") %></TD>
								<%	
									String tg = "";
									double tgxuly = nhapkhoRs.getString("thoigianxuly") == null ? 0 :  nhapkhoRs.getDouble("thoigianxuly");

								  	if(tgxuly > 0 )
								  	{
									  	int h = (int)( tgxuly / 3600 );
									  	double giayCONLAI = tgxuly - ( h * 3600 );
	
									  	int p =  (int)( giayCONLAI / 60 );
									  	int g = (int) ( giayCONLAI - ( p * 60 ) );
								...
								<TD align="center"><%= _moitao %> </TD>
								<TD align="center"><%= _mohoadon %> </TD>
								<TD align="center"><%= _dahuy %> </TD>
								<TD align="center"><%= _ss_duyet %> </TD>
								<TD align="center"><%= _cs_duyet %> </TD>
								<TD align="center"><%= _daraHOADON %> </TD>
								<TD align="center"><%= _daraPGH %> </TD>
