...
<% IErpNhaphangtraveList obj = (IErpNhaphangtraveList)session.getAttribute("obj"); %>
...
<% obj.setNextSplittings();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[6];
		quyen = util.Getquyen("ErpNhaphangtraveSvl","",userId);
%>
...
<body>
<form name="ckParkForm" method="post" action="../../ErpNhaphangtraveSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:55%; padding:5px; float:left" class="tbnavigation">
				Quản lý bán hàng > Nghiệp vụ khác > Nhập hàng trả về
        </div>
  	...
  	<div id="cotent" style="width:100%; float:none">
        ...
        <div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
        	<fieldset>
            	<legend><span class="legendtitle">Nhập hàng trả về </span>&nbsp;&nbsp;
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <TD align="center"><%= nhapkhoRs.getString("NGAYNHAN") %></TD>
		                    <TD align="left"><%= nhapkhoRs.getString("khachhang") %></TD>  
		                    	 <TD align="center">
		                    	<%
		                    		String trangthai = "";
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                <A href = "../../ErpNhaphangtraveUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                              <% } %>
                                
                                <%if(quyen[Utility.CHOT]!=0){ %>
                                <A href = "../../ErpNhaphangtraveSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%= obj.getLoaidonhang() %>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn chốt Trả hàng này?')) return false;"></A>&nbsp;
                                 <% } %>
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                              	<A href = "../../ErpNhaphangtraveSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%= obj.getLoaidonhang() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa Trả hàng này?')) return false;"></A>
                              	<% } %>
                              										
		                    <%} else{ %>
		                    
		                    <%if(quyen[Utility.XEM]!=0){ %>
		                    	<A href = "../../ErpNhaphangtraveUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                   <% } %>
		                   
		                     <%-- <% if(tt.equals("2") && (quyen[Utility.XOA]!=0)){ %> 
                              	<A href = "../../ErpNhaphangtraveSvl?userId=<%=userId%>&UnChot=<%= nhapkhoRs.getString("PK_SEQ") %>"><img src="../images/unChot.png" alt="Mở Chốt" title="Mở Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn mở Trả hàng về NCC này?')) return false;"></A>
                              	 <% } %> --%>
