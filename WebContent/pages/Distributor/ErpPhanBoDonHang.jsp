...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:55%; padding:5px; float:left" class="tbnavigation">
        	...
        	&nbsp;Quản lý bán hàng > Bán hàng > Phân bổ đơn hàng
        ...
        <div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
        	<fieldset>
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    <TH align="center" style="width: 10%" >Mã chứng từ</TH>
	                    <TH align="center" style="width: 8%" >Ngày phân bổ</TH>
	                    <TH align="center" style="width: 18%" >Ghi chú</TH>
	                    <TH align="center" style="width: 20%" >Giám sát</TH>
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <TD align="center"><%= nhapkhoRs.getString("ngayphanbo") %></TD>
		                    <TD ><%= nhapkhoRs.getString("ghichu") %></TD>  
		                    <TD ><%= nhapkhoRs.getString("giamsat") %></TD>  
		                    <TD align="center">
		                    	<%
		                    		String trangthai = "";
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                <A href = "../../ErpPhanbodonhangUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
 							<% } %>     
 							                           
                             <%if(quyen[Utility.CHOT]!=0){ %>
                                <A href = "../../ErpPhanbodonhangSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn gửi SMS của TDV này không?')) return false;"></A>&nbsp;
							<% } %>                                     
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                              	<A href = "../../ErpPhanbodonhangSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa gửi SMS này?')) return false;"></A>
                              	<% } %>
                              	                   									
		                    <%} else{ %>
		                    
		                    		<%if(quyen[Utility.XEM]!=0){ %>
		                    		<A href = "../../ErpPhanbodonhangUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                    		<% } %>
		                    		
		                    		<%if(quyen[Utility.XOA]!=0){ %>
	                              	<A href = "../../ErpPhanbodonhangSvl?userId=<%=userId%>&deleteSAUKHIGUI=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa gửi SMS này?')) return false;"></A>
	                              	<% } %>
