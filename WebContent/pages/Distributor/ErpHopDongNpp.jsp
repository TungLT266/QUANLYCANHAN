...
<% obj.setNextSplittings();
	...
	if(!util.check(userId, userTen, sum)){
		...
		quyen = util.GetquyenNew("ErpHopdongNppSvl","",userId);
%>
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:55%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > Bán hàng thầu > Hợp đồng
            ...
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >
                	 <TR>
                        ...
                        <TD class="plainlabel" width="100px">Trình dược viên </TD>
                        <TD class="plainlabel">
	                        	...
	                        	<%
	                        		if(ddkdRs != null)
	                        		{
	                        		 ...
	                        		 <% } 	                        				                        				                        				                        			
	                        			...
	                        			} ddkdRs.close();} catch(Exception ex){}
	                        		}
	                        	%>
	                    	</select>	
                     ...
                     <TR>
                        ...
                        <TD class="plainlabel">
                     	...
                     	<TD class="plainlabel" valign="middle">Hiệu lực</TD>
                           ...
                           <select name="hieuLuc" class="select2" style="width: 200px"  onchange="submitform();"  >                                                                                                    		   	
								<% if (obj.getHieuLuc().equals("0")){%>
								  	<option value="0" selected>Còn hiệu lực</option>
								  	<option value="1">Hết hiệu lực</option>
								  	<option value="2" >Tất cả</option>								  	
								  	<option value="" ></option>
								<%}else if(obj.getHieuLuc().equals("1")) {%>
								 	<option value="0" >Còn hiệu lực</option>
								  	<option value="1" selected>Hết hiệu lực</option>
								  	<option value="2" >Tất cả</option>								  	
								  	<option value="" ></option>
								<%}else if(obj.getHieuLuc().equals("2")) {%>
								 	<option value="0" >Còn hiệu lực</option>
								  	<option value="1" >Hết hiệu lực</option>
					...
					<TD class="plainlabel" ><INPUT name="mahd" type="text" size="30" value = '<%= obj.getMaHD() %>' onChange = "submitform();"></TD>
					...
					<TD width="100px;" class="plainlabel" >Loại hợp đồng</TD>
                        <TD class="plainlabel">
                            <select name = "loaihdId" class="select2" style="width: 200px" onchange="submitform();" >
	                    		<option value="">All </option>
	                        	<% if(obj.getLoaiHD().equals("0")) { %>
	                        		<option value="0" selected="selected" >INS</option>
	                        	<% } else { %>
	                        		<option value="0"  >INS</option>
	                        	<% } %>
	                        	<% if(obj.getLoaiHD().equals("1")) { %>
	                        		<option value="1" selected="selected" >Phụ lục</option>
	                        	<% } else { %>
	                        		<option value="1"  >Phụ lục</option>
	                        	<% } %>
	                        	<% if(obj.getLoaiHD().equals("2")) { %>
	                        		<option value="2" selected="selected" >CLC</option>
	                        	<% } else { %>
	                        		<option value="2"  >CLC</option>
	                        	<% } %>
	                        	<% if(obj.getLoaiHD().equals("3")) { %>
                    ...
                    <tr>
                        <td colspan="6" class="plainlabel">
                            ...
                            <a class="button2" href="javascript:excelform()">
                                <img style="top: -4px;" src="../images/button.png" alt="">XNT thầu</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;   
                            <% } %>                                                   
                        </td> 
                    </tr>        					
                </TABLE>                      
        	...
        	<fieldset>
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    ...
	                    <TH align="center" style="width: 8%" >Từ ngày</TH>
	                    <TH align="center" style="width: 8%" >Đến ngày</TH>
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                	<A href = "../../ErpHopdongNppUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                               <% } %>
                                
                                <%if(quyen[Utility.CHOT]!=0&& !tt.equals("4")){ %>
    	                            <A href = "../../ErpHopdongNppSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%= obj.getLoaidonhang() %>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn chốt hợp đồng này?')) return false;"></A>&nbsp;
	                            <% } %>
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                                	<A href = "../../ErpHopdongNppSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%= obj.getLoaidonhang() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa hợp đồng này?')) return false;"></A>
                              	<% } %>	
                              									
		                    <%} else{ %>
		                    
			                    	<%if(quyen[Utility.XEM]!=0){ %>
			                    		<A href = "../../ErpHopdongNppUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
			                    	<% } %>	
			                    	
			                    	<% if( ( tt.equals("1") || tt.equals("2") ) && !loaihd.equals("1") ) { %>
			                    		
			                    		<%-- <% if(!loaihd.equals("3")){ %> --%>
			                    		<%if(quyen[Utility.CHUYEN] != 0  && nhapkhoRs.getString("hieuluc").equals("1") ){ %>
			                    			<A href = "../../ErpHopdongNppSvl?userId=<%=userId%>&convert=<%= nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Next.png" alt="Chuyển thành đơn hàng" title="Chuyển thành đơn hàng" width="20px" border=0 onclick="if(!confirm('Bạn có muốn chuyển thành đơn hàng?')) return false;" ></A>&nbsp;
			                    		<%} %>
			                    		<%-- <% } else { %>
			                    	...
			                    	<%if( quyen[Utility.CHOT] !=0 ){ %>
    	                            	<A href = "../../ErpHopdongNppSvl?userId=<%=userId%>&mochot=<%= nhapkhoRs.getString("PK_SEQ") %>"><img src="../images/unChot.png" alt="Mở Chốt" title="Mở Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn mở chốt hợp đồng này?')) return false;"></A>&nbsp;
	                            	<% } %>
