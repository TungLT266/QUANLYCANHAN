...
<%
	...
	if( dvkdId.trim().length() > 0 && kbhId.trim().length() > 0 && nppId.trim().length() > 0 )
	{
		...
		{	
			 ...
			 String command = "";
			 command =  "select (select kho.available from nhapp_kho kho where kho.sanpham_fk=a.pk_seq and kho.KHO_FK= "+khonhapId+" and NPP_FK='"+nppId+"' and kho.NHOMKENH_FK="+kbhId+" )as soluongton, " + 
					 ...
					 "					and BGMUANPP_FK in ( select top(1) PK_SEQ from BANGGIAMUANPP bg inner join BANGGIAMUANPP_NPP bg_npp on bg.PK_SEQ = bg_npp.BANGGIAMUANPP_FK where bg.TRANGTHAI = '1' and bg_npp.NPP_FK = '" + nppId + "' and bg.DVKD_FK = '" + dvkdId + "' and  bg.NHOMKENH_FK = '" + kbhId + "'  order by bg.TUNGAY desc ) ), 0) as giamua, c.thuexuat   " + 
