...
<% obj.setNextSplittings();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[6];
		quyen = util.Getquyen("ErpGuiSMSTDVSvl","",userId);
...
<body>
<form name="ckParkForm" method="post" action="../../ErpGuiSMSTDVSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:55%; padding:5px; float:left" class="tbnavigation">
        	...
        	&nbsp;Quản lý bán hàng > Bán hàng > Gửi SMS TDV tỉnh
  	...
  	<div id="cotent" style="width:100%; float:none">
        ...
        <fieldset style="margin-top:5px" >
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >
                           ...
                           <select name="trangthai" onchange="submitform();"  >
								...
								<%}else if(obj.getTrangthai().equals("1")) {%>
								 	<option value="0" >Chưa gửi</option>
								  	<option value="1" selected>Đã gửi</option>
								  	<option value="" >ALL</option>
								<%} else  {%>
								 	<option value="0" >Chưa gửi</option>
								  	<option value="1">Đã gửi</option>
                   ...
                   <TR>
                        <TD class="plainlabel" width="130px">Mã chứng từ</TD>
                        <TD class="plainlabel" width="250px" >
                            <input type="text" name="machungtu" value="<%= obj.getMachungtu() %>" maxlength="15" onchange="submitform();" />
                        </TD>
                        <TD class="plainlabel" width="130px">Mã đơn hàng</TD>
                        <TD class="plainlabel" width="250px" >
                            <input type="text" name="donhangid" value="<%= obj.getDonhangid() %>" maxlength="15" onchange="submitform();" />
                        </TD>
                    </TR>                          
        ...
        <div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
        	<fieldset>
            	<legend><span class="legendtitle"> Gửi SMS TDV tỉnh </span>&nbsp;&nbsp;
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    ...
	                    <TH align="center" style="width: 8%" >Ngày giao</TH>
	                    <TH align="center" style="width: 20%" >Ghi chú</TH>
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                <A href = "../../ErpGuiSMSTDVUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
 							<% } %>     
 							                           
                             <%if(quyen[Utility.CHOT]!=0){ %>
                                <A href = "../../ErpGuiSMSTDVSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn gửi SMS của TDV này không?')) return false;"></A>&nbsp;
							<% } %>                                     
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                              	<A href = "../../ErpGuiSMSTDVSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa gửi SMS này?')) return false;"></A>
                              	<% } %>
                              	                   									
		                    <%} else{ %>
		                    
		                    		<%if(quyen[Utility.XEM]!=0){ %>
		                    		<A href = "../../ErpGuiSMSTDVUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                    		<% } %>
		                    		
		                    		<%if( quyen[Utility.XOA] != 0 && tt.equals("1") ){ %>
	                              	<A href = "../../ErpGuiSMSTDVSvl?userId=<%=userId%>&deleteSAUKHIGUI=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa gửi SMS này?')) return false;"></A>
	                              	<% } %>
