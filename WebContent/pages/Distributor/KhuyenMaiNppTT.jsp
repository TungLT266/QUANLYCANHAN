...
<%@ page  import = "geso.traphaco.distributor.beans.ctkhuyenmai.*" %>
<%@ page  import = "geso.traphaco.distributor.beans.ctkhuyenmai.imp.*" %>
<%@ page  import = "geso.traphaco.distributor.beans.dieukienkhuyenmai.IDieukienkhuyenmai" %>
<%@ page  import = "geso.traphaco.distributor.beans.dieukienkhuyenmai.imp.*" %>
<%@ page  import = "geso.traphaco.distributor.beans.trakhuyenmai.ITrakhuyenmai" %>
<%@ page  import = "geso.traphaco.distributor.beans.trakhuyenmai.imp.*" %>
...
<% Hashtable<String, Float> sanpham_soluong = xlkm.getHashA(); %>
	...
	<script type="text/javascript">
  		...
  		function checkSpIds() //kiem tra ma san pham tra km da duoc chon so luong hay chua
		{
  		...
  		function Auto(checkId)
  		{
  			...
  			var idTkmSELECTED = idTlg.substring(idTlg.indexOf(".") + 1, idTlg.length );
  			...
  			if(chkId.checked)
			{
  				...
  				//
  				//alert('TU CHON - CTKM: ' + idTkm + ' -- TRA KM SELECTED: ' + idTkmSELECTED );
  				
  				var chon = document.getElementsByName(idTkm + ".chon");
  				if( chon != null )
  				{
  					for( j = 0; j < chon.length; j++ )
  	  				{
  	  					if(chon.item(j).value == idTkmSELECTED )
  	  						chon.item(j).checked = true;
  	  				}
  				}
  		...
  		//check sẵn các nút có tự động chọn cho người dùng
  		function TuDongChon_ALL()
  		{
  			var schemeList = document.getElementsByName("schemeList");
  			for(ii = 0; ii < schemeList.length; ii++ )
  			{
  				var ctkmId = schemeList.item(ii).value;
  				var tkmLIST = document.getElementsByName(ctkmId + ".trakmId");
  				
  				for(jj = 0; jj < tkmLIST.length; jj++ )
  				{
  					//var checkId = '100130.100120.auto';
  					var checkId = ctkmId + '.' + tkmLIST.item(jj).value + '.auto';
  		  			
  		  			//alert('id chon: ' + checkId + ' i: ' + i + ' j: ' + j + ' ctkm: ' + schemeList.length + ' tkm: ' + tkmLIST.length );
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr height="25px">
		<TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý bán hàng > Đơn hàng bán > Áp khuyễn mãi > Điều chỉnh</TD>
		<TD colspan="2" align="right" class="tbnavigation">Chào mừng bạn  <%= userId %> &nbsp;</TD>
	</tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<TR class="tbdarkrow">
		<TD align="left" width="30"><A href = "../../ErpDonhangNppETCUpdateSvl?userId=<%= userId %>&id=<%= xlkm.getIdDonhang() %>"><img src="../images/Back30.png" alt="Quay ve"  title="Quay ve" border="1" longdesc="Quay ve" style="border-style:outset"></A></TD>
...
<fieldset>
    ...
    <table class="tabledetail" id="content" width="100%" border="0" cellspacing="1" cellpadding="2">
        ...
        for(int i = 0; i < listCTKM.size(); i++)
		{
			...
			long tongtienTraKM = 0;	//GIÁ TRỊ TỔNG
			tongtienTraKM = ctkm.getTongGiaTriTraKM();
				...
				{	
						...
						if(k == 0)
						{
							if(j == 0){ %> 
					        	...
					        	<td rowspan="<%= rowSpan %>" align="center">
					        		<input type="checkbox" name="schemeSTT" value="<%= ctkm.getId() %>"  />
					        		<%= ctkm.getscheme() %>
					        		...
					        		<input type="hidden" name="schemeList_DUNGCHUNG" value="<%= ctkm.getSchemeDungChung() %>" />
					            ...
					            <td rowspan="<%= spList.size() %>" align="center" style="display: none" >
					            	...
					            	<input type="hidden" name="schemeDKKM" value="<%= dkkm.getId() %>" />
					            </td> 
           						...
           						<% if((dkkm.getTongtien()) > 0 || (dkkm.getTongluong() > 0)){ %>
       								<td align="center">
       									<%if(dkkm.getTongtien() > 0){ %>
       										<input type="text" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" name="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" size="5" value="<%= sp.getSoluongcan() %>"  readonly="readonly"/>
       									<%}else{ %>
       										<input type="text" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" name="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" size="5" value="<%= sp.getSoluongcan() %>"  readonly="readonly" />
       									<%} %>
       									
       									<input type="hidden" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".hidden" %>" value="<%= sp.getSoluongcan() %>" />
       									<input type="hidden" name="<%= ctkm.getId() + "." + dkkm.getId() + ".masanpham" %>" size="5" value="<%= sp.getMasp() %>" />
       								</td>
	       						<%}else{ %>
	       							<td align="center">
           							...
           							{ 
	      								Hashtable<String, Integer> spSl = trakm2.getSanpham_Soluong();
           								...
           								<% if(ctkm.getTra_OR()){ %>
           									...
           									<input class="addspeech" type="hidden" name="<%= ctkm.getId() + ".soxuat" %>" value="<%= ctkm.getSoxuatKM() %>" title="<%= "Nhập số xuất trả khuyến mại: " + trakm2.getDiengiai()%>" 
		           										style="width: 30px;" onkeyup="CapNhat_SoLuongTra(<%= ctkm.getId() %> , <%= trakm2.getId() %>, <%= count %>);" readonly="readonly" >
           								...
           								%>			
   											<p><span onMouseover="ddrivetip('<%= sp_tensp.get(key) + " - Giá " + sp_dgia.get(key) %>', 300)"; onMouseout="hideddrivetip()" style="font-size:10pt;"><%= sp_tensp.get(key) %></span> &nbsp;
   											<input type="text" value="" style="text-align:right; width: 50px" name="<%= ctkm.getId() + "." + trakm2.getId() + ".soluong" %>" onkeyup="checkSlgTraKm('<%= ctkm.getId() + "." + trakm2.getId() + ".soluong" %>')"></p>
	           						...
	           						{ %>
        									...
        									<% if( trakm2.getType() == 1 ){ %>
	           									...
	           									<%} else { %>
	           										<input class="addspeech" type="radio" name="<%= ctkm.getId() + ".chon" %>" value="<%= trakm2.getId() %>" 
         												...
         												<span style="font-weight: bold;" > Tổng tiền KM <%= trakm2.getTongtien() %> </span>
	           									<%} %>
	           									<input type="hidden" name="<%= ctkm.getId() + ".soxuat" %>" value="<%= ctkm.getSoxuatKM() %>" >
	           									<input type="hidden" name="<%= ctkm.getId() + "." + trakm2.getId() + ".tonggiatriTKM_CK" %>" id="<%= ctkm.getId() + "." + trakm2.getId() + ".tonggiatriTKM_CK" %>" value="<%= ctkm.getTongGiaTriTraKM_TheoTRAKM( trakm2.getId() ) %>" />
           								...
           								<%} else { if(trakm2.getHinhthuc() != 2 ){ %>
	           								<%
	           									...
	           									int xuatMACDINH = 0;
	           									//if( count == 0 )
	           										xuatMACDINH = ctkm.getSoxuatKM();
	           								%>
	           									<fieldset>
		           								<legend>
		           								<% if( ctkm.getTra_OR() ){ %>
		           									<%-- <input class="addspeech" type="radio" name="<%= ctkm.getId() + ".chon" %>" value="<%= trakm2.getId() %>" title="<%= "Chọn trả khuyến mại: " + trakm2.getId() + " - " + trakm2.getDiengiai()%>"> --%>
		           									<input class="addspeech" type="text" name="<%= ctkm.getId() + ".soxuat" %>" value="<%= xuatMACDINH %>" title="<%= "Nhập số xuất trả khuyến mại: " + trakm2.getDiengiai()%>" 
		           										style="width: 30px;" onkeyup="CapNhat_SoLuongTra(<%= ctkm.getId() %> , <%= trakm2.getId() %>, <%= count %>);" >
		           									<b> Trả sản phẩm cố định </b>
		           								<%} else { %>
		           									<a class="addspeech" href="" title="<%= trakm2.getDiengiai()%>" onclick="return false;">
		           									<b> Trả sản phẩm cố định </b></a>
		           									<input class="addspeech" type="hidden" name="<%= ctkm.getId() + ".soxuat" %>" value="<%= xuatMACDINH %>" title="<%= "Nhập số xuất trả khuyến mại: " + trakm2.getDiengiai()%>" 
		           										style="width: 30px;" onkeyup="CapNhat_SoLuongTra(<%= ctkm.getId() %> , <%= trakm2.getId() %>, <%= count %>);" >
		           								<%} %>
		           								</legend>
		           								...
		           								<% while(keysFix.hasMoreElements())
		           								{
		   											...
		   											<span onMouseover="ddrivetip('<%= sp_tenspFix.get(key) %>', 300)"; onMouseout="hideddrivetip()" style="color: black; font-weight: normal;" >
		   												<%= sp_tenspFix.get(key) %></span> &nbsp;
		   											<input type="text"   name="<%= ctkm.getId() + "." + trakm2.getId() + "." + key %>" value="<%= Math.round(spSlFix.get(key) * xuatMACDINH) %>" style="text-align:right; width: 50px" readonly="readonly" >
		   											<input type="hidden" name="<%= ctkm.getId() + "." + trakm2.getId() + ".masanphamFIX" %>" value="<%= key %>" >
		   											<input type="hidden" name="<%= ctkm.getId() + "." + trakm2.getId() + ".slsanphamFIX" %>" value="<%= spSlFix.get(key) %>" >
		   											</p>
			           							<%}%>        								     								
		           								</fieldset>	
           								<% } } } %>
					...
					<% } } else { %>
            				...
            				<% if( (dkkm.getTongtien()) > 0 || (dkkm.getTongluong() > 0)){ %>
   								<td align="center">
  									<%if(dkkm.getTongtien() > 0){ %>
  										<input type="text" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" size="5" value="<%= sp.getSoluongcan() %>" name="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" readonly="readonly" />
  									<%}else{ %>
  										<input type="text" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" size="5" value="<%= sp.getSoluongcan() %>" name="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".sudung" %>" readonly="readonly" />
  									<%} %>
  									<input type="hidden" id="<%= ctkm.getId() + "." + dkkm.getId() + "." + sp.getMasp() + ".hidden" %>" value="<%= sp.getSoluongcan() %>" />
    ...
    %>
    	<input type="hidden" name="sanphamList" value="<%= key + "--" + Float.toString(sanpham_soluong.get(key)) %>" >
    <%}%>
...
<script type="text/javascript">	
	...
	function CapNhat_SoLuongTra( ctkmId, trakmId, pos )
	{
		// ctkm.getId() + ".tongsoxuat"
		var tongsoxuat = document.getElementById(ctkmId + ".tongsoxuat").value;
		var soxuat = document.getElementsByName(ctkmId + '.soxuat' );
		
		var soxuatDASUDUNG = 0;
		for( i = 0; i < soxuat.length; i++ )
		{
			if( i != pos )
			{
				if( soxuat.item(i).value != '' )
					soxuatDASUDUNG += parseFloat( soxuat.item(i).value );
			}
		}
		
		var xuatCONLAI = parseFloat( tongsoxuat) - parseFloat( soxuatDASUDUNG );
		if( parseFloat(soxuat.item(pos).value) > xuatCONLAI )
		{
			alert('Số xuất khuyến mại không được vượt quá tổng số xuất còn lại ( ' + xuatCONLAI + ' )');
			soxuat.item(pos).value = xuatCONLAI;
			return;
		}
		
		var masanphamFIX = document.getElementsByName(ctkmId + '.' + trakmId + '.masanphamFIX' );
		var slsanphamFIX = document.getElementsByName(ctkmId + '.' + trakmId + '.slsanphamFIX' );
		
		//B1 Cập nhật lại số lượng trả theo số xuất
		
		//alert('So xuat: ' + parseFloat( soxuat.item(0).value ));
		if( masanphamFIX != null )
		{
			for( i = 0; i < masanphamFIX.length; i++ )
			{
				//alert('So luong FIX: ' + parseFloat( slsanphamFIX.item(i).value ));
				
				var slNhap = document.getElementsByName(ctkmId + '.' + trakmId + '.' + masanphamFIX.item(i).value );
				slNhap.item(0).value = parseFloat( slsanphamFIX.item(i).value ) * parseFloat( soxuat.item(pos).value );
				
			}
		}
		
		//B2 Phân bổ lại số xuất còn lại chỉ áp dụng với ô đầu tiên
		/* if( pos == 0 )
		{
			soxuatDASUDUNG = 0;
			for( i = 0; i < soxuat.length; i++ )
			...
			if( soxuatDASUDUNG < tongsoxuat )
			{
				soxuat.item(1).value = ( tongsoxuat - soxuatDASUDUNG );
				
				alert( parseFloat ( slsanphamFIX.item( 1 ).value ) * parseFloat( soxuat.item( 1 ).value ) );
				
				//var slNhap2 = document.getElementsByName(ctkmId + '.' + trakmId + '.' + masanphamFIX.item( pos + 1 ).value );
				//slNhap2.item( pos + 1 ).value = parseFloat( slsanphamFIX.item( pos + 1 ).value ) * parseFloat( soxuat.item( pos + 1 ).value );
			}
		} */
