...
<%@ page  import = "geso.traphaco.distributor.beans.hangtralai.imp.*" %>
<%@ page  import = "geso.traphaco.distributor.beans.hangtralai.*" %>
...
<% 
	...
	ResultSet dhid=lsxBean.getLoaihdRs();
	ResultSet sprs=lsxBean.getSanphamRs();
	ResultSet infors=lsxBean.getInfohd();
...
<div id="main" style="width:100%; padding-left:2px">
    ...
    <fieldset>
    	<legend class="legendtitle"> Nhập hàng trả lại  </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0" style="background-color: #C5E8CD !important">							
               <TR>
                ...
                <TR>
                    <TD class="plainlabel"  > Loại trả hàng </TD>
                   <TD class="plainlabel"    >
                   			<select name = "loaitra" class="select2" style="width: 200px"   onchange="submitform();"  >
                    		<option value="" <%if(lsxBean.getLoaitra().equals("")){ %>selected="selected" <%} %> ></option>
                    		<option value="0" <%if(lsxBean.getLoaitra().equals("0")){ %>selected="selected" <%} %>>Sản phẩm</option>
                    		<option value="1" <%if(lsxBean.getLoaitra().equals("1")){ %>selected="selected" <%} %>>Hóa đơn</option>
                    		<option value="2" <%if(lsxBean.getLoaitra().equals("2")){ %>selected="selected" <%} %> >Đơn hàng mượn</option>
                    		
                    	</select>
                   </TD>
                   
                   <%if(!lsxBean.getLoaihd().equals("")){ %>
                ...
                <%if(!lsxBean.getLoaitra().equals("")) {%>
                 <TR>
                 
                   <TD class="plainlabel"  > Chọn Loại </TD>
                   <TD class="plainlabel"    >
                   			<select name = "loaihd" class="select2" style="width: 200px"  onchange="submitform();"  >
                    		<option value="" <%if(lsxBean.getLoaihd().equals("")){ %>selected="selected" <%} %>></option>
                    		<option value="0"  <%if(lsxBean.getLoaihd().equals("0")){ %>selected="selected" <%} %> >OTC</option>
                    		<option value="1" <%if(lsxBean.getLoaihd().equals("1")){ %>selected="selected" <%} %>  >ETC</option>    		
                    	</select>
                   </TD>
                   <%if(!lsxBean.getLoaihd().equals(""))  {
                	   if(dhid!=null)
                   ...
                   if(lsxBean.getLoaitra().equals("1")){
                   %>
                  <TD class="plainlabel"  > Chọn Hóa đơn </TD>
                   <TD class="plainlabel"    >
                   			<select name = "donhangid" class="select2" style="width: 200px" onchange="submitform();"  >
                    		<option value=""></option>
                    		<%while(dhid.next()){ %>
                                 		<option value="<%=dhid.getString("pk_seq") %>"<% if(dhid.getString("pk_seq").equals(lsxBean.getDhid())){%> selected="selected" <% }%>><%=dhid.getString("pk_seq") %></option>      		<%} %>         		
                    	</select>
                   </TD>
                 <%}else{ %>
                  <TD class="plainlabel"  > Chọn Đơn hàng </TD>
                   <TD class="plainlabel"    >
                   			<select name = "donhangid" class="select2" style="width: 200px" onchange="submitform();" >
                   				<option value=""></option>
                   				<%while(dhid.next()){ %>
                ...
                <TR>
                 ...
                 <%if(infors!=null){ if(infors.next()) {%>
                 <TR>
                    <TD class="plainlabel" >khách hàng</TD>
                    <TD  class="plainlabel" >
                    	<input type="text"   name="khachhang" value="<%=infors.getString("ten")%>"/>
                    	<input type="hidden"   name="makhachhang" value="<%=infors.getString("makh")%>"/>
                    	</TD>
                    
                    <TD class="plainlabel" > Ký Hiệu  </TD>
                    	<TD class="plainlabel" >
                    	<input type="text"  name="kyhieu" value="<%= infors.getString("kyhieu")%>"/>
                    ...
                    <TD  class="plainlabel" >
                    	<input type="text"   name="sohoadon" value="<%= infors.getString("sohoadon")  %>"/></TD>
            ...
            <%} }%>    
			 ...
			 <table cellpadding="0px" cellspacing="1px" width="100%">
				...
				<td > <input type="text" name="spMa" value="<%=sprs.getString("ma")  %>" style="width: 100%; text-align: left;"  <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %> > </td>
				<td > <input type="text" name="spTen" value="<%= sprs.getString("ten") %>" style="width: 100%; text-align: left;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %>  > </td>
				<td > <input type="text" name="donvi" value="<%= sprs.getString("donvi")  %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %>  > </td>
				<td > <input type="text" name="soluong" value="<%=  sprs.getString("soluong") %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %> > </td>
				<td > <input type="text" name="dongia" value="<%=  sprs.getString("dongia") %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %>  > </td>
				<td > <input type="text" name="solo" value="<%=  sprs.getString("solo") %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %> > </td>
				<td > <input type="text" name="ngayhethan" value="<%=  sprs.getString("ngayhethan") %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %> > </td>
				<td > <input type="text" name="thanhtien" value="<%=  sprs.getString("thanhtien") %>" style="width: 100%; text-align: center;" <%if(!lsxBean.getLoaitra().equals("0")){ %> readonly="readonly"<%} %> > </td>
				 <td align="center">
