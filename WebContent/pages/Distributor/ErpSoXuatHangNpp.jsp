...
<% obj.setNextSplittings();
	...
	if(!util.check(userId, userTen, sum)){
		...
		int[] quyen = new  int[6];
		quyen = util.Getquyen("ErpSoxuathangNppSvl","&loai=1",userId);
		ResultSet rskhoid=obj.getRskhoid();
		ResultSet rsnvbanhang=obj.getRsnvbanhang();
		ResultSet nguoigiaors = obj.getNVgiaonhanRs();
%>
...
// 	    document.forms["ckParkForm"].nppId.value = "";
	 ...
	 function xuatExcel()
	 {
	 	document.forms['ckParkForm'].action.value= 'toExcel';
	 	document.forms['ckParkForm'].submit();
	 }
	</SCRIPT>
...
<body>
<form name="ckParkForm" method="post" action="../../ErpSoxuathangNppSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:55%; padding:5px; float:left" class="tbnavigation">
        	...
        	&nbsp;Quản lý bán hàng > Bán hàng > Sổ xuất hàng
  	...
  	<div id="cotent" style="width:100%; float:none">
                ...
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >
                    ...
                    <TR>
                        ...
                        <TD class="plainlabel">
	                    	...
	                    	<input type="text" name = "khId" onchange="submitform();" >
                        </TD>
                     ...
                     <TR>
                        ...
                        <TD class="plainlabel">
                            <select name="nguoigiao" class="select2" style="width: 200px" onchange="submitform();" >
								<option value="" >ALL</option>
									<%if(nguoigiaors!=null)
										{ while (nguoigiaors.next()){%>
										<option value="<%=nguoigiaors.getString("pk_seq")%>" <%if(nguoigiaors.getString("pk_Seq").equals(obj.getNguoigiao())){ %> selected="selected" <%} %>><%=nguoigiaors.getString("ten") %></option>
										<%}} %>
							</select>
                        </TD> 
                    ...
                    <tr>
                        <td colspan="4" class="plainlabel">
                            ...
                            <a class="button2" href="javascript:clearform()">
                                ...
                                <A href="javascript: xuatExcel()" ><IMG src="../images/excel.gif" title="Xuất file EXCEL" alt="Xuất file EXCEL" border = "1"  style="border-style:outset" width="30px" height="30px" >Xuất file excel</A>
                        </td>
        ...
        <div align="left" style="width:100%; float:none; clear:left; font-size:0.7em">
        	<fieldset>
            	<legend><span class="legendtitle"> Sổ xuất hàng </span>&nbsp;&nbsp;
            	...
            	<TABLE width="100%" border="0" cellspacing="1" cellpadding="4">
					<TR class="tbheader">
	                    ...
	                    <TH align="center" style="width: 15%" >Số hóa đơn</TH>
	                    <TH align="center" style="width: 8%" >Trạng thái</TH>
	                    <TH align="center" style="width: 10%" >Người giao</TH>
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <TD ><%= nhapkhoRs.getString("nppTEN") == null ? "" : nhapkhoRs.getString("nppTEN") %></TD>  
		                    ...
		                    <TD style="width: 100px;" ><%= nhapkhoRs.getString("ddhIds") %></TD>  
					     	...
					     	<TD align="center"><%= nhapkhoRs.getString("NVGN") %></TD>	
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                <A href = "../../ErpSoxuathangNppUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
 							<% } %>     
 							                           
                             <%if(quyen[Utility.CHOT]!=0){ %>
                                <A href = "../../ErpSoxuathangNppSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn chốt yêu cầu xuất kho này?')) return false;"></A>&nbsp;
							<% } %>                                     
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                              	<A href = "../../ErpSoxuathangNppSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa yêu cầu xuất kho này?')) return false;"></A>
                              	<% } %>
                              	                   									
		                    <%} else{ %>
		                    
		                    		<%if(quyen[Utility.XEM]!=0){ %>
		                    		<A href = "../../ErpSoxuathangNppUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                    		<% } %>
		                    		
		                    		<%if( quyen[Utility.HUYCHOT]!=0 && tt.equals("2") ){ %>
	                              		<A href = "../../ErpSoxuathangNppSvl?userId=<%=userId%>&mochot=<%= nhapkhoRs.getString("PK_SEQ") %>&nppId=<%= obj.getNppId() %>&loai=<%= obj.getPhanloai() %>"><img src="../images/unChot.png" alt="Mở chốt" title="Mở chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn mở chốt sổ xuất hàng này?')) return false;"></A>
	                              	 <% } %>
	                              	 
		                    		<%if(quyen[Utility.XEM]!=0){ %>
		                    		<A href = "../../SoxuathangInSvl?userId=<%=userId%>&id=<%= nhapkhoRs.getString("PK_SEQ") %>"><IMG src="../images/Printer30.png" alt="In" title="In" border=0></A>
		                    		<% } %>
