<%@page import="oracle.sql.LxMetaData"%>
<%@page import="geso.traphaco.center.util.Utility"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@page import="java.util.ArrayList"%>
<%@page import="java.sql.SQLException"%>
<%@ page  import = "geso.traphaco.distributor.beans.thanhlyhopdong.*" %>
<%@ page  import = "geso.traphaco.distributor.beans.thanhlyhopdong.imp.*" %>
...
<% IErpThanhlyhopdongNpp lsxBean = (IErpThanhlyhopdongNpp)session.getAttribute("lsxBean"); %>
<% ResultSet dvkdRs = lsxBean.getDvkdRs(); %>
<% ResultSet MahopdongRs = lsxBean.getMahdRs(); %>
...
<% 
	...
	String[] Mahoadon = lsxBean.getMahoadon(); 
...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
	 ...
	 { 
		...
		(
			{
		    url: "../../ErpThanhlyhopdongNppSvl?type=GetDonGia&spMa=" + spMa.item(pos).value + "&dvdlId=" + dvdlId.item(pos).value + "&nppId=" + nppId+"&kbhId="+kbhId+"&dvkdId="+dvkdId+""  ,
		    type : 'GET',
		    dataType: 'json',
	 ...
	 function submitform()
	 { 
		 var ngaytl = document.getElementsByName("ngaytl").value;
		 var ngaytlkt = document.getElementsByName("ngaytlkt").value; 
		 var ngaybthd = document.getElementsByName("hopdong_tungay").value;
		 var ngaykthd = document.getElementsByName("hopdong_denngay").value;
...
<form name="hctmhForm" method="post" action="../../ErpThanhlyhopdongNppUpdateSvl">
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > Bán hàng thầu >Thanh lý hợp đồng > Tạo mới
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
    	<A href= "../../ErpThanhlyhopdongNppSvl?userId=<%= userId %>&loaidonhang=<%= lsxBean.getLoaidonhang() %>" >
    ...
    <fieldset>
    	<legend class="legendtitle">Đơn đặt hàng </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">	
            	<TR>
            	 <TR>
                	<TD class="plainlabel" >Loại thanh lý</TD>
                    <TD class="plainlabel"   >
                    	<select name = "loaithanhly"   id="loaithanhly"  class="select2" style="width: 200px" onchange ="submitform();" >
                        	<%
                        	String loai[] ={"Theo từng tháng","Toàn bộ"};
                        			...
                        			while(i < loai.length)
                        			{  
                        			if(i == lsxBean.getLoaithanhly()){ %>
                        				<option value="<%= i%>" selected="selected" ><%= loai[i] %></option>
                        			<%}else { %>
                        				<option value="<%= i%>" ><%= loai[i] %></option>
                        		 <% }
                        			i++;
                ...
                <TR>
                	<TD class="plainlabel" style="display: none" >Giám sát </TD>
                ...
                <TR >
                    ...
                    <Td  class="plainlabel">
                   <select name = "Mahd"   id="Mahd"  class="select2" onchange = "submitform();" style="width: 200px"  >
                   <option value="" selected="selected" ></option>
                        	<%
                        		if(MahopdongRs != null)
                        		{
                        			try
                        			{
                        				System.out.println("Page mahopdong"+lsxBean.getMahopdong());
                        			while(MahopdongRs.next())
                        			{  
                        			if( MahopdongRs.getString("idhd").equals(lsxBean.getMahopdong())){ %>
                        				<option value="<%= MahopdongRs.getString("idhd") %>" selected="selected" ><%= MahopdongRs.getString("mahd") %></option>
                        			<%}else { %>
                        				<option value="<%= MahopdongRs.getString("idhd") %>" ><%= MahopdongRs.getString("mahd") %></option>
                        		 <% } } MahopdongRs.close();} catch(SQLException ex){}
                        		}
                        	%>
                    	</select>
                    </Td>
                    
                    <TD class="plainlabel" style="display: none" >Kho đặt hàng</TD>
                    <TD class="plainlabel"  style="display: none"  >
                 ...
                 <TR>
                    <TD width="150px" class="plainlabel" > Ngày bắt đầu </TD>
                ...
                <% if(lsxBean.getLoaithanhly() == 0)
                {%>
                
                <TR>
                    <TD width="150px" class="plainlabel" >Ngày thanh lý </TD>
                    <TD width="300px"  class="plainlabel"  >
                    	<input type="text" class="days" readonly="readonly"   id="ngaytl" name="ngaytl" value="<%= lsxBean.getNgaythanhly() %>"/>
                    </TD>
                    
                    <TD width="120px" class="plainlabel" >Ngày kết thúc</TD>
                    <TD class="plainlabel" >
                    	<input type="text" class="days" readonly="readonly"   id="ngaytlkt"  name="ngaytlkt" onchange = "submitform();" value="<%= lsxBean.getNgaythanhlykt() %>"/>
                    </TD>
                </TR>
                <%} %>
                <% if (!lsxBean.getLoaidonhang().equals("3")) { %>
                <TR>
                    ...
                    <TD class="plainlabel" <%= ( !lsxBean.getLoaidonhang().equals("1") )  ? " colspan='3' " : "" %>  >
                    	<select name = "khId" id="khId"  class="select2" style="width: 200px" onchange="submitform();" readonly = "readonly">
                    		<option value=""> </option>
                        	<%
                        		if(nppRs != null)
                        		{
			...
			<hr />
				<table cellpadding="0px" cellspacing="1px" width="100%">
					<tr class="tbheader">
						...
						<th align ="center" width="9%" >Mã hóa đơn </th>
						...
						<th align="center" width="8%" >Đơn giá<br />Sau VAT</th>
						<th align="center" width="8%" >Chiết khấu</th>
						<th align="center" width="7%" >% VAT</th>
					...
					<%
						...
						if(spMa != null)
						{
							...
							<tr>
								...
								<td>	<input type="text" name="mahoadon" readonly="readonly" value="<%= Mahoadon[i] %>" style="width: 100%"   ></td> 
								<td>
									<input type="text" name="spMa" readonly="readonly" value="<%= spMa[i] %>" style="width: 100%"  onkeyup="ajax_showOptions(this,'nhapkho',event)" AUTOCOMPLETE="off"  > 
								...
								<td> 
									<input type="text" readonly="readonly" name="soluong" value="<%= spSoluong[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
								</td>
								<td> <input type="text" readonly="readonly" name="dongia" value="<%= spGianhap[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : " " %> onkeyup="CapNhatSAUVAT(<%= count %>);"  > </td>
								<td> <input type="text" readonly="readonly" name="dongiaSAUVAT" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : " " %> onkeyup="CapNhatVAT(<%= count %>);" > </td>
								<td> <input type="text" readonly="readonly" name="chietkhau" value="<%= spChietkhau[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : "" %> > </td>
								<td> <input type="text" readonly="readonly" name="spvat" value="<%= spVat[i] %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" readonly="readonly"  <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : "" %> > </td>
								<td> <input type="text" readonly="readonly" name="thanhtien" value="" style="width: 100%; text-align: right;" readonly="readonly" > </td>
<%-- 								<td> <input type="text" readonly="readonly" name="tungay"  value="<%= spTungay[i] %>" style="width: 100%; text-align: center;" <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : " class='days' " %> > </td> --%>
<%-- 								<td> <input type="text" readonly="readonly" name="denngay" value="<%= spDenngay[i] %>" style="width: 100%; text-align: center;" <%= lsxBean.getLoaidonhang().equals("1") ? " readonly='readonly' " : " class='days' " %> > </td> --%>
							</tr>	
					...
					<% for(int i = count; i < 100; i++) { %>
						...
						<tr>
							<td align="center"> <%= i+1 %> </td>
							<td>	<input type="text" name="mahoadon" readonly="readonly" value="" style="width: 100%"   ></td> 
							<td>
