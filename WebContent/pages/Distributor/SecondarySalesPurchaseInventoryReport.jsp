...
<script language="javascript" type="text/javascript">	
	...
	function viewform() {
		...
		document.forms['frm'].action.value= 'xem';
		document.forms["frm"].submit();
		reset();
	}
	...
	function spcheckedAll(){
		var chkAll = document.getElementById("checkSp");
	 	var khoId = document.getElementsByName("spId");
		 console.log("vao day");
		 if(chkAll.checked)
		 {
			 for(i = 0; i < khoId.length; i++)
				...
				<fieldset>
					...
					<div style="width: 100%; float: none" align="left">
						<div style="width: 100%; float: none" align="left"
							...
							<TABLE width="77%" cellpadding="6" cellspacing="0">
								<TR>
								...
								<TR>
									...
									<%-- <TD class="plainlabel">
		                        		...
		                        		<img style="top: -4px;" src="../images/vitriluu.png" title="Danh sách kho"></a>
				                		<div style='display:none'>
			                        	<div id='splist' style='padding:0px 5px; background:#fff;'>
			                        		<table cellpadding="4px" cellspacing="2px" width="100%" align="center">
			                        			<tr class="tbheader">
			                        				<th align="center">Mã SP</th>
			                        				<th align="center">Tên SP</th>
			                        				<th align="center">Chọn <input type="checkbox" id="checkSp" onchange="spcheckedAll();" ></th>
			                        			</tr>
		                        				<% if(sanpham != null){
		                        					while(sanpham.next()){%>
				                        			<tr>
				                        				<td class="plainlabel" align="left"><%= sanpham.getString("ma") %></td>
			                        					<td class="plainlabel" align="left"><%= sanpham.getString("ten") %></td>
			                        					<td class="plainlabel">
			                        					<% if(obj.getsanphamId().contains(sanpham.getString("pk_seq"))) { %>
					                    						<input type="checkbox" id="spId" name="spId" value="<%= sanpham.getString("pk_seq") %>" checked="checked" >
					                    					<% } else { %>
					                    						<input type="checkbox" id="spId" name="spId" value="<%= sanpham.getString("pk_seq") %>"  >
					                    					<% } %>
			                        					</td>
			                        				</tr>
		                        				<%} }%>
									...
									<TD class="plainlabel"><select class="select2" name="spId" id="spId" style="width: 400px;" multiple='multiple' >
										<option value="" ></option>
										<%
										if (sanpham != null)
											while (sanpham.next()) {
								...
								<%-- <TR>
                           		...
                           		<TR>
                           			<TD class="plainlabel" >
                           			 <%if(obj.getChiTietXNT_Lo().trim().equals("1")){ %>
							  				<input onclick="capnhat_check1()" name="layctnxsolo" id="layctnxsolo" checked="checked" value="1"  type="checkbox" /> Xem chi tiết 			
							  		 <%}else{ %>
							  				<input name="layctnxsolo"  onclick="capnhat_check1()" id="layctnxsolo" value="1"  type="checkbox" /> Xem chi tiết
							  		  <%} %>
							  		   
                           			</TD>
                           			<TD class="plainlabel"  >
                           			<%if(obj.getTrangthai().trim().equals("1")){ %>
							  				<input name="laytien" checked="checked" value="1"  type="checkbox" /> Lấy tiền 
							  				<%}else{ %>
							  				<input name="laytien" value="1"  type="checkbox" />Lấy tiền 
							  				<%} %>	
                           			</TD>
                           			<TD class="plainlabel"  >
                           			<%if(obj.getHangdiduong().trim().equals("1")){ %>
							  				<input name="hangdiduong"  checked="checked" value="1"  type="checkbox" /> Lấy hàng đi đường 
							  				<%}else{ %>
							  				<input name="hangdiduong" value="1"  type="checkbox" />Lấy hàng đi đường
							  				<%} %>	
                           			</TD>
                           		...
                           		<tr>
                           			<td colspan="4">
                           			  <%if(obj.getChiTietXNT_Lo_Tong().trim().equals("1")){ %>
							  				<input onclick="capnhat_check()" name="layctnxsolo_tong" id="layctnxsolo_tong"  checked="checked" value="1"  type="checkbox" /> Xem tổng quát theo kho 			
							  		 <%}else{ %>
							  				<input onclick="capnhat_check()"  name="layctnxsolo_tong" id="layctnxsolo_tong"  value="1"  type="checkbox" /> Xem tổng quát theo kho
							  		  <%} %>
								...
								<TR>
									...
									<a class="button"
										href="javascript:viewform();"> <img style="top: -4px;"
											src="../images/button.png" alt=""> Xem báo cáo </a>
											</div>
									</td>
								</TR>
									...
									<TABLE width="100%" border="0" cellspacing="1" cellpadding="4"
										...
										<tbody id="ShowFields">
											<tr class="tbheader">
												...
												<th align="center">Số lô</th>
												<th align="center">Ngày hết hạn</th>
												<th align="center">Đầu kỳ</th>
												<th align="center">Nhập trong kỳ</th>
												<th align="center">Xuất trong kỳ</th>
												<th align="center">Cuối kỳ</th>
											</tr>
											<% 
										if(xntRs != null)
										{
											try{
												...
												while (xntRs.next()){
													...
													<%} else { %>
														<TR class= <%= darkrow%> >
													<%}%>							
														<TD align="center"><%=xntRs.getString("Mã số") %></TD>
														<TD align="left"><%=xntRs.getString("Tên sản phẩm") %> </TD>
														<TD align="center"><%=xntRs.getString("Đvt") %> </TD>
														<TD align="center"><%=xntRs.getString("Số lô") %> </TD>
														<TD align="center"><%=xntRs.getString("Ngày hết hạn") %> </TD>
														<TD align="right"><%=xntRs.getString("SL đầu kỳ") %> </TD>
														<TD align="right"><%=xntRs.getString("SL nhập") %> </TD>
														<TD align="right"><%=xntRs.getString("SL xuất") %> </TD>
														<TD align="right"><%=xntRs.getString("SL cuối kỳ") %> </TD>
