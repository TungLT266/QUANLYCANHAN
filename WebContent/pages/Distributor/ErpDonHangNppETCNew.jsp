...
<% ResultSet ddkdSPRs = lsxBean.getDdkdSPRs(); %>
		...
		for(i = 0; i < spMa.length; i++)
		{
			if(spMa.item(i).value != "")
			{
				...
				if(pos > 0)
				{
					...
					dongiaDACOVAT.item(i).value = Math.round( parseFloat( dongia.item(i).value ) * ( 1 + parseFloat( _ptVAT ) / 100.0 ) );
	...
	function TinhTien()
	 {
		...
		var tongtienSAUVAT = parseFloat(totalTT_sauVAT) - parseFloat( sotienGIAM );
	 ...
	 { 
		...
		(
			{
		    url: "../../ErpDonhangNppETCSvl?type=GetDonGia&spMa=" + spMa.item(pos).value + "&dvdlId=" + dvdlId.item(pos).value + "&nppId=" + nppId+"&kbhId="+kbhId+"&dvkdId="+dvkdId+""  ,
		    type : 'GET',
		    dataType: 'json',
	 ...
	 function changeLOAIDONHANG()
	 {
		 document.forms['hctmhForm'].action.value='changeLOAIDONHANG';
	     document.forms["hctmhForm"].submit();
	 }
...
function keyCode(event, stt) {
	...
	//Đặ vào ô đơn vị, web mới hiểu là foucus ô tiếp theo
	if( x == 9 )
	{
		//alert('TAB EVENT ' + stt);
		spMa.item(stt).value = spMa.item(stt).value.trim();
		donvi.item(stt).focus();
		//soluong.item(stt).focus();
	}  	
}

function keyCode2(event, stt) {
	...
	if( x == 9 )
	{
		soluong.item(stt).value = soluong.item(stt).value.trim();
		scheme.item(stt).focus();
	}  
...
function TaoMoi()
{
	document.forms['hctmhForm'].action.value='taomoi';
    document.forms["hctmhForm"].submit();
}
...
<input type="hidden" name="msgCHUYENSALES" id="msgCHUYENSALES" value='<%= lsxBean.getMsg() %>'>

<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
			Quản lý bán hàng > Bán hàng > Đơn hàng bán > Tạo mới
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
    	<%-- <A href= "../../ErpDonhangNppETCSvl?userId=<%= userId %>&loaidonhang=<%= lsxBean.getLoaidonhang() %>" > --%>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">
            	...
            	<TR>
                    ...
                    <TD class="plainlabel" <%= !lsxBean.getLoaidonhang().equals("2") ? "" : " colspan='3' " %> >
                    	<input type="text" <%= lsxBean.getLoaidonhang().equals("1") ? " class='days' " : " " %> readonly="readonly"   id="hopdong_tungay" name="hopdong_tungay" value="<%= lsxBean.getTungay() %>"/> 
                    	<%-- <input type="text" <%= lsxBean.getLoaidonhang().equals("1") ? " class='days' " : " class='days' " %> readonly="readonly"   id="hopdong_tungay" name="hopdong_tungay" value="<%= lsxBean.getTungay() %>"/> --%>
                    </TD>
            	...
            	<TR > 
                    ...
                    <TD class="plainlabel"  >
                    	<select name = "khKGId"  id="khKGId"  class="select2" style="width: 400px"  >
                    		<option value="" ></option>
                        	<%
                        		if(kyguiRs != null)
                        		{
                ...
                <TR>
                    ...
                    <TD class="plainlabel"    >
                    	<select name = "ddkdId"  class="select2" style="width: 300px" onchange="submitform();"  >
                    		<!-- <option value=""  > </option> -->
                        	<%
                        		if(ddkdRs != null)
                        		{
                ...
                <TR style="display: none;" >
                    ...
                    <TD class="plainlabel" >Giá trị voucher </TD>
                    <TD class="plainlabel" colspan="3" >
			           	 	...
			           	 	<DIV id="subcontentCN" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B;
				                    ...
				                    <table width="95%" align="center">
				                        <tr>
				                            ...
				                            <th width="200px" style="font-size: 12px; text-align: center;">Số ngày nợ xa nhất</th>
                	...
                	<TD class="plainlabel" >
                		...
                		<% if( !lsxBean.getLoaidonhang().equals("0") && lsxBean.getCapduyet().equals("CS") ) { %>
	                		<% if(lsxBean.getTratichluy().equals("1")) { %>
	                    		<input type="checkbox" name="tratichluy" value="1" checked="checked"  > Trả tích lũy
	                   		<% } else { %>
	                   			<input type="checkbox" name="tratichluy" value="1"  > Trả tích lũy
	                   		<% } %>
                   		<% } %>
                ...
                <% if( ( lsxBean.getLoaidonhang().equals("1") || lsxBean.getLoaidonhang().equals("2") ) && lsxBean.getIsKhuyenmai().equals("0")  ) { %>
                <TR >
				...
				<TR >
				   ...
				   <div id="btnApKhuyenMai">
				  		<a class="button2" href="javascript:Apkhuyenmai()">
							<img style="top: -4px;" src="../images/New.png" alt="">Lưu & Áp chiết khấu </a>
					</div>									
				  </TD>
				</TR>
				<% } %>
			...
			<hr />
				<table cellpadding="0px" cellspacing="1px" width="100%">
					<tr class="tbheader">
						...
						<% } else { %>
							<!-- <th align="center" width="1%" colspan="2" style="display: none;" >Ngày giao</th> -->
						<% } %>
					</tr>
					...
					<% } else { %>
						<th align="center" width="1%"  style="display: none;" >Từ ngày</th>
						<th align="center" width="1%" style="display: none;" >Đến ngày</th>
					<% } %>
					</tr>
					...
					<%
						...
						if(spMa != null)
						{
							for(int i = 0; i < spMa.length; i++)
							{
								...
								if( spDagiao != null )
								{
									if( spDagiao[i] != null && Double.parseDouble(spDagiao[i]) > 0 )
										style = " class='myClass' ";
								}
							%>
							...
							<tr <%= style %>  >
								<td >
									<input type="text" name="spMa" value="<%= spMa[i] %>" style="width: 100%"  onkeyup="ajax_showOptions(this,'nhapkho',event)" AUTOCOMPLETE="off" onkeydown="keyCode(event, <%= i %>)"  > 
									...
									<%if(spSoluonton!=null){ %>
								<input type="hidden" name="soluonggoc" value="<%=spSoluonton[i]  %>"  > 
								<%} %>
								</td>
								...
								<td>
									<input type="text" name="soluong" value="<%= spSoluong[i].trim() %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onkeydown="keyCode2(event, <%= i %>)" > 
									<input type="hidden" name="soluongDAGIAO" value="<%= spDagiao[i].trim() %>" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
								</td>
								<td> 
										...
										<select name = "spTDV"  style="width: 100%"  >
				                        	...
				                        	<%
				                        		if(ddkdSPRs != null)
				                        		{
				                        			try
				                        			{
				                        				ddkdSPRs.beforeFirst();	
				                        			while(ddkdSPRs.next())
				                        			{  
				                        			if( ddkdSPRs.getString("pk_seq").equals( spTDV[i] )){ %>
				                        				<option value="<%= ddkdSPRs.getString("pk_seq") %>" selected="selected" ><%= ddkdSPRs.getString("ten") %></option>
				                        			<%}else { %>
				                        				<option value="<%= ddkdSPRs.getString("pk_seq") %>" ><%= ddkdSPRs.getString("ten") %></option>
				                        		 <% } } } catch(SQLException ex){}
				                        		}
				                        	%>
			                    		</select>
					...
					<% for(int i = count; i < 30; i++) { System.out.println("::: LOAI DON HANG: " + lsxBean.getLoaidonhang()); %>
						
						<tr>
							<td>
								<input type="text" name="spMa" value="" style="width: 100%"  onkeyup="ajax_showOptions(this,'nhapkho',event)" AUTOCOMPLETE="off" onkeydown="keyCode(event, <%= i %>)" > 
							...
							<td> 
								<input type="text" name="soluong" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onkeydown="keyCode2(event, <%= i %>)" > 
								<input type="hidden" name="soluongDAGIAO" value="0" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
							</td>
							<td> 
								<input type="text" name="dongiaDACOVAT" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getChophepsuagia().equals("0") ? " readonly='readonly' " : "  " %> >
								<input type="hidden" name="dongiaGOC" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getChophepsuagia().equals("0") ? " readonly='readonly' " : "  " %> > 
								
								<input type="hidden" name="soluongton" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
							 	<input style="display:none;" type="text" name="soluongtongoc" value="0" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" > 
							 </td>
							 
							<td style="display: none;" > 
								<input type="text" name="dongia" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" <%= lsxBean.getChophepsuagia().equals("0") ? " readonly='readonly' " : "  " %> > 
							</td>
							<td style="display: none;" > <input type="text" name="ptchietkhauBHKM" value="" style="width: 100%; text-align: right;"  > </td>
							...
							<td style="display: none;" > <input type="text" name="spvat" value="" style="width: 100%; text-align: right;" onkeypress="return keypress(event);" readonly="readonly" > </td>
							...
							<% if(lsxBean.getLoaidonhang().equals("1")) { %>	
								...
								<td style="display: none;"> <input type="text" name="tungay" class="days" value="" style="width: 100%; text-align: center;" readonly="readonly"  > </td>
								<td style="display: none;"> <input type="text" name="denngay" class="days" value="" style="width: 100%; text-align: center;" readonly="readonly" > </td>
									...
									<select name = "spTDV"  style="width: 100%"  >
			                        	...
			                        	<%
			                        		if(ddkdSPRs != null)
			                        		{
			                        			...
			                        			while(ddkdSPRs.next())
			                        			{  
			                        			if( ddkdSPRs.getString("pk_seq").equals( "" )){ %>
			                        				<option value="<%= ddkdSPRs.getString("pk_seq") %>" selected="selected" ><%= ddkdSPRs.getString("ten") %></option>
			                        			<%}else { %>
			                        				<option value="<%= ddkdSPRs.getString("pk_seq") %>" ><%= ddkdSPRs.getString("ten") %></option>
			                        		 <% } } } catch( Exception ex){ System.out.println("::: LOI: " + ex.getMessage() ); }
			                        		}
			                        	%>
		                    		</select>
								</td>
								<td><input type="text" name="scheme" value="" style="width: 100%; color: red; " readonly="readonly" > </td>
							<% } else if(lsxBean.getLoaidonhang().equals("2")) { %>
								<td> 
									<input type="text" name="scheme" value="" style="width: 100%; color: red; " readonly="readonly" > 
									<input type="hidden" name="spTDV" value="<%= lsxBean.getDdkdId() %>"  > 
								</td>
							<% } else { %>
								<td style="display: none;"> <input type="text" name="tungay" class="days" value="" style="width: 100%; text-align: center; display: none; " readonly="readonly"  > </td>
								<td style="display: none;"> 
									<input type="text" name="denngay" class="days" value="" style="width: 100%; text-align: center; display: none; " readonly="readonly" > 
									<input type="hidden" name="spTDV" value="<%= lsxBean.getDdkdId() %>"  > 
...
<script type="text/javascript">
	...
	function checkCHUYENSALE()
	{
		var msg = document.getElementById("msgCHUYENSALES").value;
		
		if( msg != '' )
		{
			var r = confirm(msg + '\n Bạn có muốn tiếp tục tạo đơn hàng?' );
			if (r == true) {
...
<%  if(khonhapRs != null) khonhapRs.close(); %>
