...
<SCRIPT language="javascript" type="text/javascript">
	...
	{				
		...
		var maspCK_TDV = document.getElementsByName("maSanphamTDV");
		var tenspCK_TDV = document.getElementsByName("tenSanphamTDV");
		var dvtCK_TDV = document.getElementsByName("dvSanphamTDV");

		if( maspCK_TDV != null )
		{
			for(i = 0; i < maspCK_TDV.length; i++)
			{
				if(maspCK_TDV.item(i).value != "")
				{
					var sp = maspCK_TDV.item(i).value;
					var pos = parseInt(sp.indexOf(" - "));
			
					if(pos > 0)
					{
						maspCK_TDV.item(i).value = sp.substring(0, pos);
						sp = sp.substr(parseInt(sp.indexOf(" - ")) + 3);
				
						pos = parseInt(sp.indexOf(" - "));
						
						tenspCK_TDV.item(i).value = sp.substring(0, pos);
						
						sp = sp.substr(parseInt(sp.indexOf(" - ")) + 3);
				
						dvtCK_TDV.item(i).value = sp.substring(0, parseInt(sp.indexOf("] [")));
						sp = sp.substr(parseInt(sp.indexOf("] [")) + 3);
					}
				}
				else
				{
					maspCK_TDV.item(i).value = "";
					tenspCK_TDV.item(i).value = "";
					dvtCK_TDV.item(i).value = "";
				}
			}
		}
				                  		...
				                  		<div class="panes">
											...
											<tr>
												...
												<TD >
													...
													<%-- <%= util.getSelectBox(ctvRs, "max-height:100px;overflow:auto;", "ctvId", "", khBean.getCTVId(), "PK_SEQ", "TEN", "0", true, true) %> --%>
													
													<div style="width: 100%; overflow: auto; max-height: 60px; " >
														<table >
															<% if(ctvRs != null ) { 
																while( ctvRs.next() )
																{ %>
																	<tr>
																		<td><%= ctvRs.getString("TEN") %></td>
																	</tr>
																<% }
																ctvRs.close();
															} %>
											...
											<div>
							<FIELDSET>
							<LEGEND class="legendtitle">Thông tin giao hàng </LEGEND>
												...
												<TABLE border="0" width="100%" cellpadding="6" cellspacing="0">
													...
													<!-- THÔNG TIN ĐẶT HÀNG -->
																			<FIELDSET>
							<LEGEND class="legendtitle">Thông tin đặt hàng </LEGEND>
										<!-- 	<div> -->
