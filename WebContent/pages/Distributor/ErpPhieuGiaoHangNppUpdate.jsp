...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
	 ...
	 {	
		 ...
		 {
			 if( parseFloat( soluongDAT.item(i).value ) != parseFloat( soluong.item(i).value ) )
			 {
				 coguiHANG = 1;
			 }
		 }
			
		 //alert('Co giao hang gui: ' + coguiHANG );
		 if( coguiHANG >= 1 )
		 {
			 if(!confirm("Bạn có muốn tiếp tục nhập hàng gửi không?"))
				 return;
			 else
			 {
				 var ngaygiaohangGUI = document.getElementById("ngaygiaohangGUI").value;
				 if(ngaygiaohangGUI == '')
				 {
					 alert('Vui lòng nhập ngày giao hàng gửi');
					 return;
				 }
			 }
		 }
	...
	function CapNhatSoLuong(element, temId, pos)
	{
		element.value = DinhDangTien(element.value);
		
		var spSOLUONG = document.getElementsByName(temId + "_spSOLUONG");
		var soluong = document.getElementsByName("soluong");
		var soluong2 = document.getElementsByName("soluong2");
		...
		var totalSOLUONG = 0;
		for(var i = 0; i < spSOLUONG.length; i++ )
		{
			if( spSOLUONG.item(i).value != '' )
			{
				totalSOLUONG += parseFloat(spSOLUONG.item(i).value.replace(/,/g,""));
			}
		}
		//alert('SO LUONG: ' + totalSOLUONG );
		
		soluong.item(pos).value = DinhDangTien(totalSOLUONG);
		soluong2.item(pos).value = DinhDangTien(totalSOLUONG);
	}
...
<form name="hctmhForm" method="post" action="../../ErpPhieugiaohangNppUpdateSvl">
<input type="hidden" name="userId" value='<%= userId %>'>
<input type="hidden" name="action" value='1'>
<input type="hidden" name="nppId" value='<%= lsxBean.getNppId() %>'>
<input type="hidden" name="id" value='<%= lsxBean.getId() %>'>
<input type="hidden" name="khachhangkgId" value='<%= lsxBean.getKhachhangKGId() %>'>

<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > Bán hàng > Phiếu giao hàng > Cập nhật
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
    	<A href= "../../ErpPhieugiaohangNppSvl?userId=<%= userId %>&loai=<%= lsxBean.getXuatcho() %>" >
    ...
    <div align="left" style="width:100%; float:none; clear:left">
    <fieldset>
    	<legend class="legendtitle">Phiếu giao hàng </legend>
        	<div style="float:none; width:100%" align="left">
            <TABLE width="100%" cellpadding="4" cellspacing="0">							
                <TR>
                    <TD width="130px" class="plainlabel" >Ngày giao </TD>
                    <TD class="plainlabel"  >
                    ...
                    <TD class="plainlabel"  >
                    	<%-- <input type="text" name="xuatchoId" value="<%= lsxBean.getXuatcho() %>" > --%>
                ...
                <TR>
                    ...
                    <TD class="plainlabel" style="color: red;" >Ngày giao hàng gửi</TD>
                    <TD class="plainlabel" >
                    	<input type="text" class="days" readonly="readonly"  name="ngaygiaohangGUI" id='ngaygiaohangGUI' value="<%= lsxBean.getNgaygiaohanggui() %>"/>
                    </TD>
                </TR>
			...
			<table cellpadding="0px" cellspacing="1px" width="100%">
				<tr class="tbheader">
					...
					<th align="center" width="12%" >Số lượng đặt</th>
					<th align="center" width="10%" >Tồn kho</th>
					<th align="center" width="10%" >Đã xuất</th>
					<th align="center" width="11%" >Thực giao</th>
				...
				<%
					...
					if(spMa != null)
					{
						...
						<tr <%= style %> >
							...
							<td>
								<input type="text" name="soluongDAT" value="<%= spSoluongdat[i] %>" style="width: 70%; text-align: right;" readonly="readonly">							
								
								<a href="" id="schemeDAT_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>" rel="subcontentDAT_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>">
		           	 				&nbsp; <img alt="Số lô đã xuất" src="../images/vitriluu.png"></a>
		           	 		
	           	 		 		<DIV id="subcontentDAT_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B;
				                    ...
				                    <table width="95%" align="center">
		                            	...
		                            	<%
		                            		ResultSet spOldRs = lsxBean.getSoloTheoSpOLD(spId[i], spSoluong[i], spSCheme[i].trim() );
			                        		if(spOldRs != null)
			                        		{
			                        			while(spOldRs.next())
			                        			{
			                        			...
			                        			<tr>
			                        				<td>          				
			                        					<input type="text" style="width: 100%;text-align: right;"  value="<%= spOldRs.getString("SOLUONG")  %>" readonly="readonly" >
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;"  value="<%= spOldRs.getString("SOLO") %>" readonly="readonly">
			                        				</td>
			                        				<td>
			                        					<input type="text" style="width: 100%;text-align: center;"  value="<%= spOldRs.getString("NGAYHETHAN") %>" readonly="readonly">
			                        				</td>
			                        			</tr>
				                     ...
				                     <div align="right">
				                     	...
				                     	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				                     	<a href="javascript:dropdowncontent.hidediv('subcontentDAT_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>')" > Đóng lại </a>
				                     </div>
					            </DIV> 
					            
					            <script type="text/javascript">
					            	dropdowncontent.init('schemeDAT_<%= spId[i] %>_<%= spLoai[i] %>_<%= spSCheme[i] %>', "left-top", 300, "click");
					            </script>
							...
							<td>
								<input type="text" name="soluong" value="<%= spSoluong[i] %>" style="width: 70%; text-align: right;" onkeypress="return keypress(event); " onkeyup="Dinhdang(this);" >							
								...
								<a href="" id="scheme_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>" rel="subcontent_<%= spId[i] + "_" + spLoai[i] + "_" + spSCheme[i] %>">
				                    ...
				                    <table width="95%" align="center">
		                            	...
		                            	<%
		                            		ResultSet spRs = lsxBean.getSoloTheoSp(spId[i], spSoluong[i], spSCheme[i].trim() );
			                        		if(spRs != null)
			                        		{
			                        			while(spRs.next())
			                        			{
			                        			...
			                        			<tr>
			                        				<td>
			                        				<% if(sanpham_soluong.get(spId[i] + "__" + spLoai[i] + "__" + spSCheme[i].trim() + "__" + spRs.getString("SOLO") + "__" + spRs.getString("NGAYHETHAN") ) != null ) { %>
			                        					<input type="text" style="width: 100%;text-align: right;" name="<%= temID %>_spSOLUONG" value="<%= formater.format(Double.parseDouble( sanpham_soluong.get( spId[i] + "__" + spLoai[i] + "__" + spSCheme[i].trim() + "__" + spRs.getString("SOLO") + "__" + spRs.getString("NGAYHETHAN") ))) %>" onkeyup="CapNhatSoLuong(this, '<%= temID %>', <%= i %> );" >
			                        				<% } else { %>
			                        					<input type="text" style="width: 100%;text-align: right;" name="<%= temID %>_spSOLUONG" value="<%= tudeXUAT  %>" onkeyup="CapNhatSoLuong(this, '<%= temID %>', <%= i %> );" >
			                        				<% } %>
			                        				</td>
			                        				<td>
