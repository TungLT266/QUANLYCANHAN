...
<script>
    ...
    {
    	<% for( int i = 0; i < 50; i++ ) { %>
    	$(".chitiet<%= i %>").colorbox({width:"100%", inline:true, href:"#chitiet<%= i %>"});
...
<script language="javascript" type="text/javascript">
	...
	function replaces()
	{
		...
		var is_khongthue = document.getElementsByName("is_khongthue");
		var i;
		for(i = 0; i < spMa.length; i++)
		{
			if(spMa.item(i).value != "")
			{
				...
				if(pos > 0)
				{
					...
					sp= sp.substr(parseInt(sp.indexOf(" [")) + 2);
					is_khongthue.item(i).value = sp.substring(0, parseInt(sp.indexOf("]")));
					if(is_khongthue.item(i).value =="1")
						{
						document.getElementById("is_khongthue_"+i).checked = true;
						}
				}
			}
			else
			{
				...
				spvat.item(i).value = "";
				document.getElementById("is_khongthue_"+i).checked = false;
				document.getElementsByName("is_khongthue").item(i).value = "0";
			}
		}	
	...
	function changeCheck()
	{
		...
		for(var i = 0; i < spMa.length; i++)
		{
			...
			{
				var is_khongthue1 = document.getElementById("is_khongthue_"+i);
				if(is_khongthue1.checked)
				{
					thueVAT.item(i).value  = "0";
					is_khongthue.item(i).value  = "1";
				}else
				{
					is_khongthue.item(i).value  = "0";
					}
			}
		}
	}
	...
	function TinhTien()
			...
			if(spMa.item(i).value != "" && dongia.item(i).value != "" && soluong.item(i).value != "" )
			{
				...
				for(var j = 0; j < soluongCT.length; j++)
				{
					...
					if(slct != "")
					{
						...
						tienthue = parseFloat(tienthue) +  Math.round(Math.round(parseFloat(dongia.item(i).value.replace(/,/g,""))*parseFloat(slct))  * parseFloat(_thueVAT) / 100);
					}
				}
				
				var tt = parseFloat(tienchitiet);
				
				//tienthue = Math.round ( parseFloat(tt) * parseFloat(_thueVAT) / 100 );
	 ...
	 function submitformSP(stt)
	 { 
		 document.forms['hctmhForm'].action.value='submitSP';
		 document.forms['hctmhForm'].sttupdate.value=stt;
	     document.forms["hctmhForm"].submit();
	 }
...
<input type="hidden" name="ETC" value='<%= lsxBean.getETC() %>'>
<input type="hidden" name="sttupdate" value=''>
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > <%= lsxBean.getLoaidonhang().equals("2") ? "Đơn hàng bán" : "Bán nguyên vật liệu" %> > Tạo mới
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
    	<A href= "../../ErpDondathangSvl?userId=<%= userId %>&loaidonhang=<%= lsxBean.getLoaidonhang() %>&ETC=<%= lsxBean.getETC() %>" >
    ...
    <div align="left" style="width:100%; float:none; clear:left">
    <fieldset>
    	<legend class="legendtitle"><%= lsxBean.getLoaidonhang().equals("2") ? "Đơn hàng XNK" : "Bán nguyên vật liệu" %></legend>
        	
            <TABLE width="100%" cellpadding="4" cellspacing="0">						
                <TR>
                    <TD width="130px" class="plainlabel" valign="top">Ngày đơn hàng </TD>
                    <TD width="250px"  class="plainlabel" valign="top" >
                    	<input type="text" class="days" readonly="readonly"  name="ngaychuyen" id="ngaychuyen" value="<%= lsxBean.getNgayyeucau() %>"  onchange="submitform();"/>
                    </TD>
                ...
                <tr style = "display:none;">
                	<TD class="plainlabel" valign="top">Danh mục KH </TD>
                    <TD class="plainlabel" valign="top" colspan="3" >
                    	<select name="danhmucKH" onchange="submitform();" class="select2" style="width: 200px"  >
                    		<% if( lsxBean.getDanhmucKH().equals("0") ) { %>
                    			<option value="0" selected="selected" >Xuất khẩu</option>
                    		<% } else { %>
                    			<option value="0" >Xuất khẩu</option>
                    		<% } %>
                    		<% if( lsxBean.getDanhmucKH().equals("1") ) { %>
                    			<option value="1" selected="selected" >Kế hoạch</option>
                    		<% } else { %>
                    			<option value="1" >Kế hoạch</option>
                    		<% } %>
                    		<% if( lsxBean.getDanhmucKH().equals("2") ) { %>
                    	...
                    	<select name = "ddkdId"  class="select2" style="width: 200px"  >
                        	...
                        	<%
                        		if(ddkdRs != null)
                        		{
                        			try
                        			{
                        				System.out.println("lsxBean.getDdkdId()1: "+ lsxBean.getDdkdId());
                        			while(ddkdRs.next())
                        			{  
                ...
                <TR>
                	<TD class="plainlabel" valign="top">Khách hàng </TD>
                    <TD class="plainlabel" valign="top"  >
			...
			<table cellpadding="0px" cellspacing="1px" width="100%">
					<tr class="tbheader">
						...
						<th align="center" width="6%" >Đơn vị</th>
						<th align="center" width="10%" >Số lượng</th>
						<th align="center" width="6%" >Đơn giá</th>
					...
					<%
						...
						if(spMa != null)
						{
							...
							<tr>
								...
								<td> 
									<input type="text" name="soluong" value="<%= spSoluong[i] %>" style="width: 70%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onchange="submitformSP(<%= i %>);" > 
									
									<% if( spSoluong[i].trim().length() > 0 ) { %>
									<a class="chitiet<%= i %>" href="#" >
                        			<img style="top: -4px;" src="../images/vitriluu.png" title="Lịch sử bán hàng">
	                        		</a>
	                       		 	<div style='display:none'>
		                        	<div id='chitiet<%= i %>' style='padding:0px 5px; background:#fff; '>
			                            	...
			                            	<%
				                        		...
				                        		if(spRs != null)
				                        		{
				                        			while(spRs.next())
				                        			{
				                        				...
				                        				//String key2 = spMa[i] + "__" + spRs.getString("SOLO") + "__" + spRs.getString("NGAYHETHAN") + "__" + spRs.getString("VITRI") + "__" + spRs.getString("MATHUNG") + "__" + spRs.getString("MAME")+ "__" + spRs.getString("MAPHIEU");
				                        				...
				                        				System.out.println("sanpham_soluongDAXUAT.get( key2 ):"+sanpham_soluongDAXUAT.get( key2 ));
								...
								<td>
								 <input type="text" name="spvat" value="<%= spVat[i] %>" style="width: 50%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onchange = "changevat(<%=i %>);" <%= readonly ? "readonly" : "" %> > 
								 <%if(spMa[i].trim().length() >0  && spiskhongthue[i].equals("1")){ %>
							<input type="checkbox" checked="checked" name="is_khongthue1" id = "is_khongthue_<%=i %>" value="<%= spiskhongthue[i] %>" style="width: 20%" onchange = "changeCheck();" <%= readonly ? "disabled" : "" %> >
							<%}else{ %>
							<input type="checkbox" name="is_khongthue1" id = "is_khongthue_<%=i %>" value="<%= spiskhongthue[i] %>" style="width: 20%" onchange = "changeCheck();" <%= readonly ? "disabled" : "" %> >
							<%} %>
					...
					<% for(int i = count; i < 50; i++) { %>
						...
						<tr>
							...
							<td> 
								<input type="text" name="soluong" value="" style="width: 50%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onchange="submitformSP(<%= i %>);" > 
								<a href="javascript:void(0);" >&nbsp; <img alt="Chọn số lô" src="../images/vitriluu.png"></a>
							</td>
							...
							<td>
							 <input type="text" name="spvat" value="" style="width: 50%; text-align: right;" onkeypress="return keypress(event);" onkeyup="Dinhdang(this);" onchange = "changevat(<%=i %>);"  > 
							  <input type="checkbox" name="is_khongthue1"  id = "is_khongthue_<%=i %>" value="" style="width: 20%" onchange = "changeCheck();" >
							<input type="hidden"  name="is_khongthue" value="<%=0 %>">
							 </td>
							<td> <input type="text" name="thanhtien" value="" style="width: 100%; text-align: right;" readonly="readonly" > </td>
...
<script src="../scripts/loadingv2.js"></script>
</BODY>
</html>