                    ...
                    <TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
                        <TR height="22">
                        	<% if(obj.getPhanloai().equals("0")) { %>
                           	...
                           	<% } else if(obj.getPhanloai().equals("3")) { %>
                           		<TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý khuyến mại > Tích lũy theo giai đoạn </TD> 
                           	<% } else { %>
                           		<TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý trưng bày > Khai báo > Chương trình trưng bày </TD> 
                           	<% } %>
            ...
            <TR>
                ...
                <TR>
                    <TD width="100%">
                        <FIELDSET>
                        <% if(obj.getPhanloai().equals("0")) { %>
                            <LEGEND class="legendtitle">&nbsp;Khuyến mại tích lũy &nbsp;&nbsp;	
                        <% } else if(obj.getPhanloai().equals("1")) { %>
                        	<LEGEND class="legendtitle">&nbsp;Khuyến mại tích điểm &nbsp;&nbsp;
                        <% } else if(obj.getPhanloai().equals("3")) { %>
                        	<LEGEND class="legendtitle">&nbsp;Tích lũy theo giai đoạn &nbsp;&nbsp;
                        <% } else { %>
                        	<LEGEND class="legendtitle">&nbsp;Chương trình trưng bày &nbsp;&nbsp;
                        <% } %>   
                              <%if(quyen[0]!=0){ %>                      
                                    ...
                                    while ( tieuchiSKU.next()){
                                        ...
                                        <%}%>
                                                ...
                                                <% if( tieuchiSKU.getString("trangthai").trim().equals("0") ) { %>
					                                ...
					                                <A href = "../../TieuchithuongTLUpdateSvl?userId=<%=userId%>&update=<%= tieuchiSKU.getString("pk_seq") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Edit20.png" alt="Cap nhat" title="Cap nhat" border=0></A>&nbsp;
					                               <%} %>
					                               	<%if(quyen[Utility.CHOT]!=0) {%>	
					                                <A href = "../../TieuchithuongTLSvl?userId=<%=userId%>&duyet=<%= tieuchiSKU.getString("pk_seq") %>&phanloai=<%= obj.getPhanloai() %>"><img src="../images/Chot.png" alt="Duyet" width="20" height="20" longdesc="Duyet" border=0 onclick="if(!confirm('Bạn có muốn duyệt chỉ tiêu này?')) return false;"></A>&nbsp;
					                                <%} %>
					                                <%if(quyen[Utility.XOA]!=0) {%>	
					                                <A href = "../../TieuchithuongTLSvl?userId=<%=userId%>&delete=<%= tieuchiSKU.getString("pk_seq") %>&phanloai=<%= obj.getPhanloai() %>"><img src="../images/Delete20.png" alt="Xoa" width="20" height="20" longdesc="Xoa" border=0 onclick="if(!confirm('Bạn có muốn xóa chỉ tiêu này?')) return false;"></A>									
							                    	<%} %>
							                    <%} else { %>
							                    	<%if(quyen[Utility.XEM]!=0) {%>
							                    	<A href = "../../TieuchithuongTLUpdateSvl?userId=<%=userId%>&display=<%= tieuchiSKU.getString("pk_seq") %>&phanloai=<%= obj.getPhanloai() %>"><IMG src="../images/Display20.png" alt="Hien thi" title="Hien thi" border=0></A>&nbsp;
							                    	<%} %>
							                    <%} %>
							                    </TD>
                                            </TR>
