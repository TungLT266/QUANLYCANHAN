...
<%@ page  import = "geso.traphaco.center.beans.dieuchinhtonkho.*" %>
...
<% obj.setNextSplittings();
	...
	String url="";
	url = util.getUrl("ErpChuyenkhoTTSvl","&loaidonhang="+obj.getLoaidonhang()+"");
            ...
            <legend class="legendtitle"> Tiêu chí tìm kiếm</legend>
                <TABLE width="100%" cellpadding="6px" cellspacing="0px" style="margin-top: 5px " >
                	 <TR>
   					...
   					<TR>
	   					<TD class="plainlabel" width="150px">Chi nhánh / Đối tác</TD>
					...
					<%
                 		if(nhapkhoRs != null)
                 		{
		                    ...
		                    <%if(quyen[Utility.SUA]!=0){ %>
                                <A href = "../../ErpChuyenkhoTTUpdateSvl?userId=<%=userId%>&update=<%=nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%=nhapkhoRs.getString("loaidonhang")%>"><IMG src="../images/Edit20.png" alt="Cập nhật" title="Cập nhật" border=0></A>&nbsp;
                              <% } %>
                                
                                <%if(quyen[Utility.CHOT]!=0){ %>
                                <A href = "../../ErpChuyenkhoTTSvl?userId=<%=userId%>&chot=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%=nhapkhoRs.getString("loaidonhang")%>"><img src="../images/Chot.png" alt="Chốt" title="Chốt" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn chốt xuất chuyển hàng này?')) return false;"></A>&nbsp;
                                 <% } %>
                                
                                <%if(quyen[Utility.XOA]!=0){ %>
                              	<A href = "../../ErpChuyenkhoTTSvl?userId=<%=userId%>&delete=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%=nhapkhoRs.getString("loaidonhang")%>"><img src="../images/Delete20.png" alt="Xóa" title="Xóa" width="20" height="20" border=0 onclick="if(!confirm('Bạn có muốn xóa xuất chuyển hàng này?')) return false;"></A>
                              	<% } %>
                              										
		                    <%} else{ %>
		                    
		                    <%if(quyen[Utility.XEM]!=0){ %>
		                    	<A href = "../../ErpChuyenkhoTTUpdateSvl?userId=<%=userId%>&display=<%= nhapkhoRs.getString("PK_SEQ") %>&loaidonhang=<%=nhapkhoRs.getString("loaidonhang")%>"><IMG src="../images/Display20.png" alt="Hiển thị" title="Hiển thị" border=0></A>
		                   <% } %>
