...
<%@ page  import = "geso.traphaco.center.beans.hoadontaichinh.imp.*" %>
...
<% 
	...
	String[] tichluy_tienvat = lsxBean.getTichluy_tienvat();
...
<script language="javascript" type="text/javascript">
	...
	function SuaTienThue(stt)
	{
		...
		if( soluong.item(stt).value != '' && thanhtien.item(stt).value != '' )
		{
			...
			thanhtien.item(stt).value = ( Math.round( _soluong * _dongia ) - parseFloat(_ck) ) + parseFloat(_vat);
		}
	 ...
	 {	
			...
			for(var i = 0; i < spMa.length; i++)
			{
				if(spMa.item(i).value != "" && dongia.item(i).value != "" && soluong.item(i).value != "" )
				{
					...
					var soluongCT = document.getElementsByName(spMa.item(i).value+ "_spSOLUONG_"+i);
					var tienchitiet =0;
					var tienthue = 0;
					for(var j = 0; j < soluongCT.length; j++)
					{
						...
						if(slct != "")
						{
							...
							tienthue = parseFloat(tienthue) +  Math.round((Math.round(parseFloat(dongia.item(i).value.replace(/,/g,""))*parseFloat(slct))  * parseFloat ( spVat.item(i).value.replace(/,/g,"") ) / 100)- parseFloat(ck_sp));
						}
					}
					
					var tt = parseFloat(tienchitiet);
					
					
					//var tt = Math.round(parseFloat(dongia.item(i).value.replace(/,/g,"")) * parseFloat(soluong.item(i).value.replace(/,/g,"")));
					var tt_coCK = parseFloat(tienchitiet) - parseFloat(ck_sp);
...
<div id="main" style="width:100%; padding-left:2px">
	<div align="left" id="header" style="width:100%; float:none">
    	<div style="width:60%; padding:5px; float:left" class="tbnavigation">
        	Quản lý bán hàng > Chức năng > Xuất hóa đơn > Hiển Thị
        </div>
  	...
  	<div align="left" id="button" style="width:100%; height:32px; padding:0px; float:none" class="tbdarkrow">
        ...
        <%} %>
        	...
        	<%-- <A href= "../../ErpInHoaDonGiaTriGiaTangPdfSvl?userId=<%=userId%>&display=<%= lsxBean.getId() %>" >
        	
        	<img src="../images/excel.gif" alt="xuất hóa đơn giá trị gia tăng"  title="xuất hóa đơn giá trị gia tăng" border="1px" longdesc="Xuáº¥t hÃ³a Ä‘Æ¡n giÃ¡ trá»‹ gia trÄƒng" style="border-style:outset; width: 30px; height: 30px"></A>
         --%>
    	...
    	<legend class="legendtitle">Xuất hóa đơn </legend>
            ...
            <TABLE width="100%" cellpadding="4" cellspacing="0">							
                <TR>
                    ...
                    <TD class="plainlabel" valign="top">
                    	<input type="text" class="days" readonly="readonly"  name="ngayghinhan" value="<%= lsxBean.getNgayxuatHD() %>"/>
                    </TD>
                </TR>
                ...
                <TR>
                 ...
                 <TR>
                   ...
                   <TD class="plainlabel">
                    ...
                    <TD class="plainlabel" >
                    	<select name = "nppId" class="select2" style="width: 400px" onchange="submitform();" >
                    		<option value=""> </option>
                        	<%
                        		if(nppRs != null)
                        		{
                ...
                <TR>
                    ...
                    <TD class="plainlabel"  style = "display:none;" >
                    	<select name = "khId" class="select2" style="width: 400px" disabled="disabled" >
                    		<option value=""> </option>
                        	<%
                        		if(kbhRs != null)
                        		{
                 ...
                 <TR>
                   ...
                   <TR>
                    ...
                    <TD class="plainlabel" colspan="3" >
                    	<input readonly="readonly" type="text"  name="pl" value="<%= lsxBean.getPhanloai() %>" style="width: 500px" />
                    </TD>
                </TR>
                <TR>
				...
				<%
					...
					if(spMa != null)
					{
						...
						<tr>
								...
								<input type="text" name="soluong" value="<%= formatter.format(Double.parseDouble(spSoluong[i])) %>" style="width: 70%; text-align: right;" readonly="readonly">
									<a href="" id="scheme_<%= spMa[i] + "__" + spSCheme[i] %>" rel="subcontent_<%= spMa[i] + "__" + spSCheme[i] %>">
		           	 		 		...
		           	 		 		<DIV id="subcontent_<%= spMa[i] + "__" + spSCheme[i] %>" style="position:absolute; visibility: hidden; border: 9px solid #80CB9B;
					                             background-color: white; width: 680px; max-height:300px; overflow:auto; padding: 2px;">
					                    <table width="98%" align="center">
					                        ...
					                        <tr>
					                            ...
					                            <th width="200px" style="font-size: 11px">Vị trí</th>
					                            <th width="50px" style="font-size: 11px">Mã thùng</th>
												<th width="50px" style="font-size: 11px">Mã mẻ</th>
			                            	...
			                            	<%
				                        		...
				                        		if(spRs != null)
				                        		{
				                        			while(spRs.next())
				                        			{
				                        				...
				                        				String temID = spId[i] + "__" + spSCheme[i] + "__" + spRs.getString("SOLO") + "__" + spRs.getString("NGAYHETHAN") + "__" + spRs.getString("VITRI") + "__" + spRs.getString("MATHUNG") + "__" + spRs.getString("MAME") + "__" + spRs.getString("MAPHIEU") + "__" + spRs.getString("MARQ")+ "__" + spRs.getString("NSX")+ "__" + spRs.getString("NGAYNHAPKHO");
				                        				
				                        				System.out.println("::: KEY JSP: " + temID);
				                        			%>
				                        			
				                        			<tr>
				                        				<td>
				                        				<% if(sanpham_soluong.get( temID ) != null ) { %>
				                        					<input type="text" style="width: 100%;text-align: right;" name="<%= spMa[i] %>_spSOLUONG_<%=i %>" value="<%= formater.format(Double.parseDouble( sanpham_soluong.get( temID ))) %>" onkeyup="CheckSoLuong_DeXuat(this);" >
				                        				<% } else { %>
				                        					<input type="text" style="width: 100%;text-align: right;" name="<%= spMa[i] %>_spSOLUONG_<%=i %>" value="<%= tudeXUAT  %>" onkeyup="CheckSoLuong_DeXuat(this);" >
				                        				<% } %>
				                        				</td>
				                        				<td>
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spSOLO<%=i %>" value="<%= spRs.getString("SOLO") %>" readonly="readonly">
				                        				</td>
				                        				<td>
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spNGAYHETHAN<%=i %>" value="<%= spRs.getString("NGAYHETHAN") %>" readonly="readonly">
				                        				</td>
				                        				<td>
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spNGAYNHAPKHO<%=i %>" value="<%= spRs.getString("NGAYNHAPKHO") %>" readonly="readonly">
				                        				</td>
				                        				<td>
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spVITRI<%=i %>" value="<%= spRs.getString("VITRI") %>" readonly="readonly">
				                        				</td>
				                        				<td >
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spMATHUNG<%=i %>" value="<%= spRs.getString("MATHUNG") %>" readonly="readonly">
				                        				</td>
				                        				<td >
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spMAME<%=i %>" value="<%= spRs.getString("MAME") %>" readonly="readonly">
				                        				</td>
				                        				<td >
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spMAPHIEU<%=i %>" value="<%= spRs.getString("MAPHIEU") %>" readonly="readonly">
				                        				</td>
				                        				<td >
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spMARQ<%=i %>" value="<%= spRs.getString("MARQ") %>" readonly="readonly">
				                        				</td>
				                        				<td >
				                        					<input type="text" style="width: 100%;text-align: center;" name="<%= spMa[i] %>_spNSX<%=i %>" value="<%= spRs.getString("NSX") %>" readonly="readonly">
				                        				</td>
				                        				<td>
				                        					<input type="text" style="width: 100%;text-align: right;" name="<%= spMa[i] %>_spTONKHO<%=i %>" value="<%= formater.format(spRs.getDouble("AVAILABLE")) %>" readonly="readonly">
				                        				</td>
				                        			</tr>
							...
							<td>
								<input type="text" name="spVat" value="<%= formatter.format(Double.parseDouble(spVat[i]))%>" style="width: 50%; text-align: right;" readonly="readonly">	
								<%if( spiskhongthue[i].equals("1")){ %>
					...
					{ 
						for(int i = 0; i < tichluy_scheme.length; i++)
						{
							...
							if(lsxBean.getNOIBO().equals("1"))
							{
								 _tienthue= Double.parseDouble(tichluy_tienvat[i].replaceAll(",", ""));
							}
							else
							{
								 _tienthue = _vat * _chietkhau / 100;
							}
							...
							<TR class= 'tblightrow'>
							    <TD align = "right" colspan="4"  ><input type="text" value="<%= tichluy_scheme[i] %>" style="width:100%; text-align: right;" disabled="disabled"></TD>
							    ...
							    <TD align = "center" ><input type="text" value="0"  style="width:100%; text-align: right;" disabled="disabled"  ></TD>
							    <TD align = "center" >
