...
<%@ page  import = "geso.traphaco.center.beans.tieuchithuong.ITieuchithuong" %>
...
<% 	ITieuchithuong tctBean = (ITieuchithuong)session.getAttribute("tctBean"); 
	...
	ResultSet rshtbh = tctBean.getHtbhRs();
	...
	ResultSet rsCty=tctBean.getCongtyRs();
%>
...
{	
	...
	document.tctForm.action.value='hienthi';
...
<input type="hidden" name="htbhId" value='<%=tctBean.gethethongBH() %>'>
				...
				<TR>
					<TD align="left" class="tbnavigation">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
						  <tr height="22">
							 <TD align="left" colspan="2" class="tbnavigation">&nbsp;Quản lý chỉ tiêu  &gt; Khai báo &gt; Công thức thưởng &gt; Hiển thị
			...
			<TABLE width="100%" border="0" cellpadding="0" cellspacing="1" >
			  <%-- 	<tr>
					<TD align="left" colspan="4" class="legendtitle">
	    				...
	    				<textarea name="dataerror"  style="width: 100% ; font-weight:bold; border: 0px;" readonly="readonly" rows="1" ><%=tctBean.getMsg() %></textarea>
						<% tctBean.setMsg(""); %>
						</FIELDSET>
				   </TD>
				</tr>			 --%>
						...
						<TABLE width="100%" border="0" cellpadding="0" cellspacing="1" >
							<TR>
								<TD>
									...
									<% if(tctBean.GetLoaiTieuChi().equals("1")){ %> (SR) <%} 
									else if (tctBean.GetLoaiTieuChi().equals("2")){ %> (SS) <%} 
									else {%> (ASM) <%} %>
									</LEGEND>
									...
									<TABLE class="first" border="0" width="100%" cellpadding="3" cellspacing="0">
										...
										<tr>
							  				...
							  				<TD class="plainlabel">Công ty </TD>
						  	  				<TD class="plainlabel">
						  	  					<SELECT name= "ctyId" >
						  	  						<%if(rsCty!=null){ 
							  	  						while (rsCty.next()){
							  	  							if(rsCty.getString("pk_seq").equals(tctBean.GetCongty())){
							  	  								%>
							  				...
							  				<TD class="plainlabel">Hệ thống bán hàng </TD>
						  	  				...
						  	  				if(rshtbh!=null)
						  	  				{
						  	  					while(rshtbh.next())
						  	  					{
				  	  						  		if(rshtbh.getString("pk_seq").trim().equals(tctBean.gethethongBH().trim()))
					  	  							{ 
				  	  						  			htbh = rshtbh.getString("diengiai");
					  	  							}
						  	  					}
						  	  				}%>		

					  	  						<input type="text" readonly="readonly" name = "htbhId" value = "<%= htbh %>" size = 40 ></input>
						  	  				</TD>
										</TR>
										...
										<TR>
							  				<TD class="plainlabel">Tiêu chí </TD>
						  	  				<TD class="plainlabel" colspan= '3'>
						  	  					<SELECT name="tc" onChange = submitForm() >
						  	  						<OPTION value = "">&nbsp;</OPTION>
						  	  					<%try{
						  	  						while(tc.next()){
										...
										while(tct.next())
										{ 
											if(tctBean.getTieuchi().equals("2") || tctBean.getTieuchi().equals("5") || tctBean.getTieuchi().equals("6")) 
											{
										%>
												<TR>
											  				...
											  				<TD style="font-weight: bold;">Tối thiểu </TD>
														  	 ...
														  	 <TD style="font-weight: bold;">Tối đa </TD>
										  	  				 <TD>
														  	 	 ...
														  	 	 <% 
														  	 System.out.println("Loai : "+tctBean.getTieuchi()+" - htbh : "+tctBean.gethethongBH());
														  	 if(!tctBean.gethethongBH().trim().equals("100000")){
