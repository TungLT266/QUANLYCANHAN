...
public class Trahang implements ITrahang, Serializable
{
	...
	public boolean CreateCk( HttpServletRequest request )
	{
		...
		try 
		{
			...
			String query = "insert trahang(nvbh_fk, kho_fk, ngaychuyen, trangthai, npp_fk, ngaytao, nguoitao, ngaysua, nguoisua) " +
							"values('" + this.nvbhId + "', '" + this.khoId + "', '" + this.ngaychuyen + "', '1', '" + this.nppId + "', '" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "')";
				...
				if(spSOLUONG[i].trim().length() > 0)
				{
					...
					for(int j = 0; j < _solo.length; j++ )
					{
						if(_soluong[j].trim().length() > 0)
						{
							//CHECK TON KHO
							double avai = 0;
							query = "select available from NVBH_KHO_CHITIET  " +
									"where npp_fk = '" + this.nppId + "' and kho_fk = '" + this.khoId + "' and kbh_fk = '100025' and sanpham_fk = '" + spIds[i] + "' and solo = '" + _solo[j] + "' and NVBH_FK = '" + this.nvbhId + "' and available > 0 ";
							...
							if(avai < Double.parseDouble(_soluong[j].trim().replaceAll(",", "")) )
							{
								this.msg = "Tồn kho của sản phẩm ( " + spTEN[i] + " ), số lượng ( " + _soluong[j] + " ) còn tối đa ( " + avai + " ), đang booked trong đơn hàng ( " + booked + " ) ";
								db.getConnection().rollback();
								return false;
							}
							...
							query = "update NHAPP_KHO_CHITIET set available = available + '" + _soluong[j].trim().replaceAll(",", "") + "', soluong = soluong + '" + _soluong[j].trim().replaceAll(",", "") + "' " +
							...
							query = "update nhapp_kho set available = available + '" + _soluong[j].trim().replaceAll(",", "") + "', soluong = soluong + '" + _soluong[j].trim().replaceAll(",", "") + "' " +
							...
							query = "update NVBH_KHO_CHITIET set available = available - '" + _soluong[j].trim().replaceAll(",", "") + "', soluong = soluong - '" + _soluong[j].trim().replaceAll(",", "") + "' " +
									"where npp_fk = '" + this.nppId + "' and kho_fk = '" + this.khoId + "' and kbh_fk = '100025' and sanpham_fk = '" + spIds[i] + "' and solo = '" + _solo[j] + "' and NVBH_FK = '" + this.nvbhId + "' ";
							...
							query = "update NVBH_KHO set available = available - '" + _soluong[j].trim().replaceAll(",", "") + "', soluong = soluong - '" + _soluong[j].trim().replaceAll(",", "") + "' " +
	...
	public boolean ChotCk() 
	{
		try 
		{
			...
			query = "update trahang set trangthai = '1', ngaysua = '" + getDateTime() + "', nguoisua = '" + this.userId + "' where pk_seq = '" + this.id + "' ";
		...
		if(this.nvbhId.trim().length() > 0 && this.khoId.trim().length() > 0)
		{
			...
			{*/
				query = "select b.pk_seq as spId, b.ma as spMa, b.ten as spTen, isnull(c.donvi, 'NA') as donvi, a.available, " +
						" isnull(	(select sum(available) from nvbh_kho where nvbh_fk = '" + this.nvbhId + "' and sanpham_fk = b.pk_seq and kbh_fk = '100025'), 0) as avaiNVBH   " +
	...
	public void init()
	{
		...
		if(rs != null)
		{
			try
			{
				...
				query = "select sanpham_fk, solo, soluong from trahang_sanpham_chitiet where trahang_fk = '" + this.id + "'";
				rs = db.get(query);
				if(rs != null)
				{
