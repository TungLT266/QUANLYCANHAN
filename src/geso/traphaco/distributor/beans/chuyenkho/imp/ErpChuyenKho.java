...
import geso.traphaco.distributor.beans.chuyenkho.IErpChuyenKho;
...
public class ErpChuyenKho implements IErpChuyenKho
{
	...
	public boolean createNK() 
	{
		...
		try
		{
			...
			String query = " insert ERP_CHUYENKHO(SoChungTu,ngaychuyen, ghichu, trangthai, khoxuat_fk, kbh_fk, tructhuoc_fk, npp_fk, ngaytao, nguoitao, ngaysua, nguoisua," +
			               "        lenhdieudong, lddcua, lddveviec, ngaydieudong, sohopdong,ngayhopdong, nguoivanchuyen, ptvanchuyen,LoaiDonHang,loaichuyenkho) " +
						   ...
						   "        N'"+ this.lenhdieudong +"', N'"+ this.lddcua +"',N'"+ this.lddveviec +"','"+ this.ngaylenhdieudong +"',N'"+ this.sohopdong +"','"+ this.ngayhopdong +"',N'"+ this.nguoivanchuyen +"', N'"+ this.ptvanchuyen +"',4,'0')";
			...
			for(int i = 0; i < spMa.length; i++)
			{
				if(spMa[i].trim().length() > 0 && spSoluong[i].trim().length() > 0 )
				{
					...
					query = "insert ERP_CHUYENKHO_SANPHAM( chuyenkho_fk, SANPHAM_FK, soluongchuyen, dongia, dvdl_fk,loaichuyenkho ) " +
							"select '"+this.id+"', pk_seq, '" + spSoluong[i].replaceAll(",", "") + "', '" + spGianhap[i].replaceAll(",", "") + "', ISNULL( ( select pk_Seq from DONVIDOLUONG where donvi = N'" + spDonvi[i].trim() + "' ), DVDL_FK ), '1' " +
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spMa[i]) )
							{
								...
								if(!_soluongCT.equals("0"))
								{
									...
									query = "insert ERP_CHUYENKHO_SANPHAM_CHITIET( chuyenkho_fk, SANPHAM_FK, solo, ngayhethan, soluong ) " +
	...
	public boolean updateNK() 
	{
		...
		//TANG KHO NGUOC LAI
			...
			query =	" Update ERP_CHUYENKHO set SoChungTu='"+this.sochungtu+"',ngaychuyen = '" + this.ngayyeucau + "', ghichu = N'" + this.ghichu + "', " +
							" khoxuat_fk = " + this.khoXuatId + ", kbh_fk = '" + this.kbhId + "', npp_fk = '" + this.khId + "', tructhuoc_fk = '" + this.nppId + "', ngaysua = '" + getDateTime() + "', nguoisua = '" + this.userId + "', " + 
	...
	public ResultSet getSoloTheoSp(String spMa, String tongluong)
	{
		if(this.nppId.trim().length() > 0 && this.kbhId.trim().length() > 0 )
		{
			...
			String query = "select AVAILABLE + ISNULL( ( select sum(soluong) from ERP_CHUYENKHO_SANPHAM_CHITIET where chuyenkho_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  and NgayHetHan=ct.NgayHetHan ), 0 ) as AVAILABLE, NGAYHETHAN, SOLO " +
