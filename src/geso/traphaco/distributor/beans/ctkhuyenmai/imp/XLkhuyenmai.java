...
import geso.traphaco.distributor.beans.ctkhuyenmai.ISanpham;
import geso.traphaco.distributor.beans.dieukienkhuyenmai.IDieukienkhuyenmai;
import geso.traphaco.distributor.beans.dieukienkhuyenmai.imp.Dieukienkhuyenmai;
...
public class XLkhuyenmai 
{
	...
	private Hashtable<String, Float> HashA = new Hashtable<String,Float>(); //khoi tao tu don hang "abc"	
	...
	private Hashtable<String,Float> HashC = new Hashtable<String,Float>(); //dung trong truong hop Ontop 	
	...
	public XLkhuyenmai(String userId, String date, String khachhang, String isDonHangLe, String donhangId)
	{
		db = new dbutils();
		
		this.loaidonhang = isDonHangLe;
		this.idDonhang = donhangId;
		
		this.khachhang = khachhang;
		String sql = "select khachhang.kbh_fk,khachhang.npp_fk,npp.ten  from khachhang inner join nhaphanphoi npp on npp.pk_Seq=npp_fk  where khachhang.pk_Seq="+this.khachhang;
	...
	{   
		...
		/*sql = "\n select a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk,  isnull(a.diengiai, '') as diengiai,  " +
			"\n d.ngansach, trakhuyenmai.ChietKhau, a.PHANBOTHEODONHANG, ISNULL(a.soxuattoida, 0) as soxuattoida, " +
			"\n 	case a.PHANBOTHEODONHANG  " + 
			"\n 		when 0 then ISNULL( 	( select SUM(tonggiatri)    " + 
			"\n  								  from DONHANG_CTKM_TRAKM    " + 
			"\n  								  where CTKMID = d.CTKM_FK and DONHANGID in ( select PK_SEQ from DONHANG where NPP_FK = d.NPP_FK and TRANGTHAI != 2 ) and DONHANGID not in ( select DONHANG_FK from DONHANGTRAVE where DONHANG_FK is not null and NPP_FK = d.NPP_FK and TRANGTHAI in  ( 1, 3 ) )  ), 0 )  " + 
			"\n 		else ISNULL( 	( select sum( SOLUONG)    " + 
			"\n  						  from DONHANG_CTKM_TRAKM    " + 
			"\n  						  where  SPMA IS NOT NULL AND CTKMID = d.CTKM_FK and DONHANGID in ( select PK_SEQ from DONHANG where NPP_FK = d.NPP_FK and TRANGTHAI != 2 ) and DONHANGID not in ( select DONHANG_FK from DONHANGTRAVE where DONHANG_FK is not null and NPP_FK = d.NPP_FK and TRANGTHAI in  ( 1, 3 ) )  ), 0 ) end as dasudung   " + 
			"\n  ISNULL( ( select SUM(soxuat)    " + 
			"\n  		   from DONHANG_CTKM_TRAKM    " + 
			"\n  		   where CTKMID = d.CTKM_FK and DONHANGID in ( select PK_SEQ from DONHANG where NPP_FK = d.NPP_FK and TRANGTHAI != 2 and KHACHHANG_FK = '" + this.khachhang + "' ) and DONHANGID not in ( select DONHANG_FK from DONHANGTRAVE where DONHANG_FK is not null and NPP_FK = d.NPP_FK and TRANGTHAI in  ( 1, 3 ) )  ), 0 ) as soxuatDADUNG  " + 
			...
			"\n where a.loaict <> '3' and a.tungay <= '" + date + "' and '" + date + "' <= a.denngay   " +
				...
				"\n and a.pk_seq in ( select ctkm_fk from CTKHUYENMAI_LOAIKH where loaikh_fk in ( select LCH_FK from KHACHHANG where pk_seq = '" + this.khachhang + "' ) )		" +
				//"\n and a.pk_seq in ( select ctkm_fk from CTKHUYENMAI_PHAMVI where xuatkhau in ( select khongkyhopdong from KHACHHANG where pk_seq = '" + this.khachhang + "' ) )		" +
				 "\n and -- " +
	             "\n ( -- " +
	             ...
	             "\n and a.pk_seq in ( select ctkm_fk from CTKHUYENMAI_LOAIAPDUNG where LOAIAPDUNG in ( select cokhuyenmai from KHACHHANG where pk_seq = '" + this.khachhang + "' ) )		" +
		...
		sql = "select distinct a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk,  isnull(a.diengiai, '') as diengiai,   " + 
				"  	d.ngansach,  " + 
				"	a.PHANBOTHEODONHANG, ISNULL(a.soxuattoida, 0) as soxuattoida,  " + 
				"   case a.level_phanbo when 0 then   " + 
				"		CASE WHEN a.PHANBOTHEODONHANG =0 THEN ISNULL((SELECT SUM(TRAKM.SOXUAT) FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.NPP_FK=d.NPP_FK AND TRAKM.CTKMID = a.PK_SEQ AND DH.TRANGTHAI!=2 ),0)    " + 
				"		ELSE ISNULL((SELECT SUM(TRAKM.SOLUONG)FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.NPP_FK=d.NPP_FK AND TRAKM.CTKMID = a.PK_SEQ AND SPMA IS NOT NULL AND DH.TRANGTHAI !=2 ),0	)END      " + 
				"	when 1 then   " + 
				"		CASE WHEN a.PHANBOTHEODONHANG =0 THEN ISNULL((SELECT SUM(TRAKM.SOXUAT) FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.DDKD_FK=d.NVBH_FK AND TRAKM.CTKMID = a.PK_SEQ AND DH.TRANGTHAI!=2 ),0)    " + 
				"		ELSE ISNULL((SELECT SUM(TRAKM.SOLUONG)FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.DDKD_FK=d.NVBH_FK AND TRAKM.CTKMID = a.PK_SEQ AND SPMA IS NOT NULL AND DH.TRANGTHAI !=2 ),0	)END      " + 
				"	else   " + 
				"		CASE WHEN a.PHANBOTHEODONHANG =0 THEN ISNULL((SELECT SUM(TRAKM.SOXUAT) FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.KHACHHANG_FK=d.KHACHHANG_FK AND TRAKM.CTKMID = a.PK_SEQ AND DH.TRANGTHAI!=2 ),0)    " + 
				"		ELSE ISNULL((SELECT SUM(TRAKM.SOLUONG)FROM DONHANG_CTKM_TRAKM  TRAKM INNER JOIN DONHANG DH ON DH.PK_SEQ=TRAKM.DONHANGID WHERE DH.KHACHHANG_FK=d.KHACHHANG_FK AND TRAKM.CTKMID = a.PK_SEQ AND SPMA IS NOT NULL AND DH.TRANGTHAI !=2 ),0	)END      " + 
				"	END AS soxuatDADUNG  " + 
				" from ctkhuyenmai a inner join ctkm_npp b on a.pk_seq = b.ctkm_fk and b.NPP_FK = '" + this.nppId + "'   " + 
				"		inner join  phanbokhuyenmai d on b.CTKM_FK = d.CTKM_FK  " + 
				" where a.loaict <> '3' and a.tungay <= '" + date + "' and '" + date + "' <= a.denngay    " + 
				" and ( a.nhomkhnpp_fk is null or a.nhomkhnpp_fk in (select pk_seq from nhomkhachhangnpp  where pk_seq in ( select nhomkhnpp_fk from ctkm_khachhang  where khachhang_fk ='" + this.khachhang + "') ) )   " + 
				" and a.pk_seq in ( select ctkm_fk from CTKHUYENMAI_LOAIKH where loaikh_fk in ( select LCH_FK from KHACHHANG where pk_seq = '" + this.khachhang + "' ) )		 " + 
				" and a.pk_seq in ( select CTKM_FK from CTKHUYENMAI_KBH where KBH_FK in ( select kbh_fk from KHACHHANG_KENHBANHANG where khachhang_fk = '" + this.khachhang + "' ) )		 " + 
				//" and a.pk_seq in ( select ctkm_fk from CTKHUYENMAI_PHAMVI where xuatkhau in ( select khongkyhopdong from KHACHHANG where pk_seq = '" + this.khachhang + "' ) )		 " + 
				" and   " + 
				" (   " + 
				"   (select COUNT(CTKM_FK) from CTKHUYENMAI_HANGCUAHANG where CTKM_FK = a.PK_SEQ) =0   " + 
				...
				" )    " + 
				" and ( a.cttb_fk is null or a.cttb_fk in ( select cttb_fk from khachhang_anhchup where khachhang_fk = '" + this.khachhang + "' and ANHCHUP is not null and cttb_fk is not null ) )  	 " + 
				//" and '" + khachhang + "' not in ( select KHACHHANG_FK from DANGKYKM_TICHLUY_KHACHHANG )  	 " + 
				" and ( a.thuongtl_fk is null or '" + this.khachhang + "' not in ( select KHACHHANG_FK from DANGKYKM_TICHLUY dk inner join DANGKYKM_TICHLUY_KHACHHANG dk_kh on dk.PK_SEQ = dk_kh.DKKMTICHLUY_FK where TRANGTHAI = '1' and TIEUCHITL_FK = a.thuongtl_fk ) )  	 " + 
				" and ( case a.LEVEL_PHANBO when 0 then d.NPP_FK when 1 then d.NVBH_FK when 2 then d.khachhang_fk end  )    " + 
				"   = ( case a.LEVEL_PHANBO  when 0 then '" + this.nppId + "'   " + 
				" 							when 1 then ( select DDKD_FK from DONHANG where PK_SEQ = '" + this.idDonhang + "' )   " + 
				" 							when 2 then '" + this.khachhang + "' " + 
				" 							end  )    ";
		//		" order by a.isGIOHANG asc ";
		...
		if(ctkm != null)
		{
			try 
			{
				while(ctkm.next())
				{
					...
					int soxuatDADUNG = ctkm.getInt("soxuatDADUNG");
					int soxuattoida = ctkm.getInt("soxuattoida");
					int soxuatCONLAI = 100000;
					if(ctkm.getString("PHANBOTHEODONHANG").equals("1"))
					{
						soxuatCONLAI = soxuattoida - soxuatDADUNG;
					}
					...
					if( soxuatCONLAI > 0 && ngansach - dasudung > 0 )
					{
						...
						km.setSoxuatKM_TOIDA(soxuatCONLAI);
						...
						if(rs != null)
						{
							while(rs.next())
							{
								...
								Dieukienkhuyenmai dkkm = new Dieukienkhuyenmai(dkkmId, "0", 1);
	...
	private void ApOnTop() 
	{
		...
		else  //dieuchinh == false
		{
			...
			{				
				Ctkhuyenmai ct = (Ctkhuyenmai)this.ctkmOntopList.get(i);
				Hashtable<String, Float> copyHashA = copyHashtable(HashC);				
	...
	private void ApBinhThuong() 
	{
		...
		else  //dieuchinh == false
		{
			...
			{				
				Ctkhuyenmai ct = (Ctkhuyenmai)this.ctkmBTList.get(i);
				Hashtable<String, Float> copyHashA = copyHashtable(HashA);				
				Ctkhuyenmai ctkm = this.getSoxuattheoScheme(ct, copyHashA);
	...
	private void ApChietKhau() 
	{
		...
		else  //dieuchinh == false
		{
			...
			{
				Ctkhuyenmai ct = (Ctkhuyenmai)this.ctkmCKList.get(i);
				Hashtable<String, Float> hashCK = copyHashtable(HashA);	
	...
	public Ctkhuyenmai getSoxuattheoScheme(Ctkhuyenmai ctkm, Hashtable<String, Float> hashSanpham)
	{
		...
		Hashtable<String, Float> copySp = copyHashtable(hashSanpham);
...
/*					System.out.println("2222222.Thu tu Tong luong");*/
	...
	public Hashtable<String, Float> copyHashtable(Hashtable<String, Float> hash)
	{
		Hashtable<String, Float> copy = new Hashtable<String, Float>();		
	...
	private String checkDKKM_SP(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String,Float> sanpham)
	{
		int soxuatKm = 0;
		float tongtientheoDK = 0;
		Hashtable<String,Float> sp_sl = dkkm.getSanpham_Soluong();
	...
	private String checkDKKM_TongLuong(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Float> sanpham_soluong)
	{
		int soxuatKm = 0;
		double tongtientheoDK = 0;
		Hashtable<String,Float> sanpham = dkkm.getSanpham_Soluong(); //phai la nhung san pham trong dkkm truoc
		...
		if(sanpham.size() > 0)
		{
			...
			float sum = 0;
			float minSP = Integer.MAX_VALUE; //soxuat bat buoc phai co mat day du cac sanpham trong dkkm
			...
			if(sum < tongluong)
				return "0-0";
			soxuatKm = (int) min( ( sum / tongluong), minSP);
		}
		//long tongtiendh = Math.round(tongtientheoDK * 1.1);
		long tongtiendh = Math.round(tongtientheoDK);
	...
	private String checkDKKM_TongTien(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Float> sanpham_soluong)
	{
		...
		if(sanpham.size() > 0)
		{
			...
			if(Math.round(tongtiendonhang) < tongtien)
				return "0-0";
			soxuatKm = (int) min((Math.round(tongtiendonhang) / tongtien), minSP);
		}				
		//return soxuatKm;
	...
	private void dieuChinhKM(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sp_sl, Hashtable<String, Float> sp_dongia, int soXuatKM, ICtkhuyenmai ctkm)
	{
		Hashtable<String, Float> sanpham_sl = dkkm.getSanpham_Soluong();
		List<ISanpham> sanphamList = new ArrayList<ISanpham>();
			...
			{		
				...
				if(sanpham != null)
				{
					for(int i=0; i < sanpham.length; i++)
					{
						...
						{
							float slgconlai = 0;
							float soluongsudung = 0;
							
							if(dkkm.getIsThung().equals("0"))
							{
								slgconlai = (float) ( sp_sl.get(key) -  sanpham[i].getSoluongThungCan() );
								soluongsudung = (float)(sanpham[i].getSoluongThungCan());
								/*slgconlai = sp_sl.get(key) - sanpham[i].getSoluongcan();
								soluongsudung = sanpham[i].getSoluongcan();*/
							}
							else
							{
								slgconlai = (float) (sp_sl.get(key) - ( sanpham[i].getSoluongThungCan() * ( this.Hash_QuyCach.get(key)) ));
								soluongsudung = (float)(sanpham[i].getSoluongThungCan() * this.Hash_QuyCach.get(key));
							}
	...
	private void dieuChinhKM2(Dieukienkhuyenmai dkkm,  Hashtable<String, Float> hashSanpham, int soXuat)
	{
		...
		for(int i = 0; i < sanpham.size(); i++ )
		{
			...
			if(dkkm.getType() == 2 ) //mua san pham co slg bat ky trong nhom san pham...
			{
				if(dkkm.getTongluong() > 0)
				{
					float sum = dkkm.getTongluong() * soXuat;
					if(sum > sp.getSoluongcan())
						sum = sum - sp.getSoluongcan();
					else
					{						
						float slgCan = sp.getSoluongcan();
						float avai = sp.getSoluongAvaiable();
						...
						{
							float slgCanOld = sanpham.get(j).getSoluongcan();
			...
			{
				float slgCan = sp.getSoluongcan() / old;
				float sum = sp.getSoluongcan() + sp.getSoluongAvaiable();
	...
	private Sanpham[] InitSanPham(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sanpham_sl, boolean flag)
	{
		Hashtable<String, Float> sp_sl = dkkm.getSanpham_Soluong();
	...
	private Sanpham[] getSanpham_soluong(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sanpham_sl, int soxuatKm) //truong hon tong tien > 0
	{
		...
		for(int i = 0; i < sanpham.length; i++)
		{
			...
			float soluongcan = min(sanpham[i].getSoluongAvaiable(), (int)((tongtienKM - sum) / sanpham[i].getDongia()));
	...
	public Sanpham[] getSanpham_slg(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sanpham_sl, int soxuatKm) //truong hop tongluong > 0
	{
	...
	public Hashtable<String,Float> getHashA()
	{
		return this.HashA;
	}
	
	public void setHashA(Hashtable<String,Float> hash)
	{
		this.HashA = hash;
	...
	public boolean checkConfirm() 
	{
		...
		if(this.ctkmResual.size() == 1)
		{
			...
			for(int i = 0; i < trakmList.size(); i++)
			{
				...
				if( trakmList.size() >= 2 && trakm.getPheptoan() == 2 )
				{
	...
	{
		Hashtable<String, Float> spList = new Hashtable<String, Float>();
