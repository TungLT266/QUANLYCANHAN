...
public class ErpYeucauxuatkhoNpp implements IErpYeucauxuatkhoNpp
{
	...
	public void createRs() 
	{
		...
		if(this.xuatcho.equals("1"))
			query = " select pk_seq, TEN from KHACHHANG a where trangthai = '1' and NPP_FK = '" + this.nppId + "' and PK_SEQ in ( select khachhang_fk from KHACHHANG_KENHBANHANG where kbh_fk in ( select kbh_fk from NHOMKENH_KENHBANHANG where nk_fk = '100000' ) ) "
			// PHÂN QUYỀN
			+util.getPhanQuyen_TheoNhanVien("KHACHHANG", "a", "pk_seq", this.getLoainhanvien(), this.getDoituongId() );
		else if(this.xuatcho.equals("2"))
			query = " select pk_seq, TEN from KHACHHANG a where trangthai = '1' and NPP_FK = '" + this.nppId + "' and PK_SEQ in ( select khachhang_fk from KHACHHANG_KENHBANHANG where kbh_fk in ( select kbh_fk from NHOMKENH_KENHBANHANG where nk_fk = '100001' ) ) "
			// PHÂN QUYỀN
			+util.getPhanQuyen_TheoNhanVien("KHACHHANG", "a", "pk_seq", this.getLoainhanvien(), this.getDoituongId() );
		else if( this.xuatcho.equals("3") )
		...
		{
			if(this.xuatcho.equals("1") || this.xuatcho.equals("2") )
			{
				query += " union ALL select pk_seq, TEN from KHACHHANG a where trangthai = '1' and NPP_FK = '" + this.nppId + "' and PK_SEQ in ( select khachhang_fk from ERP_DONDATHANGNPP where pk_seq = '" + ddhId + "' ) "+
				// PHÂN QUYỀN
				util.getPhanQuyen_TheoNhanVien("KHACHHANG", "a", "pk_seq", this.getLoainhanvien(), this.getDoituongId() );
			}	
		}
		...
		if(this.khId.trim().length() > 0 && this.khoNhanId.trim().length() > 0 && this.xuatcho.trim().length() > 0 )
		{
			query = "select PK_SEQ, CAST(pk_seq as varchar(10)) + ' / ' + NgayDonHang as ten " +
					"from ERP_DONDATHANGNPP where TRANGTHAI = '2' and NPP_FK = '" + this.nppId + "' and KHO_FK = '" + this.khoNhanId + "' and loaidonhang = '" + this.xuatcho + "'  ";
		...
		if(this.ddhId.trim().length() > 0 && this.khoNhanId.trim().length() > 0 )
		{
			...
			query = " select ddh.*, ISNULL(xuat.soluongXUAT, 0) as xuat,   " +
					...
					" 	ISNULL( ( select AVAILABLE from NHAPP_KHO where kho_fk = '" + this.khoNhanId + "' and sanpham_fk = ddh.PK_SEQ and NPP_FK = '" + this.nppId + "' and NHOMKENH_FK = ( select nhomkenh_fk from ERP_DONDATHANGNPP where pk_seq = '" + this.ddhId + "' ) ), 0) + ISNULL(xuat.soluongXUAT, 0) as tonkho    " +
					" from   " +
					" (   " +
					...
					"				a.soluong, 1 as loai, c.SCHEME as scheme   " + 
					...
					" ddh left join    " +
					" (   " +
					" 	select b.sanpham_fk, b.LOAI, isnull(b.SCHEME, '') as SCHEME, b.soluong as soluongXUAT   " +
					" 	from ERP_YCXUATKHONPP a inner join ERP_YCXUATKHONPP_SANPHAM_THUCGIAO b on a.PK_SEQ = b.ycxk_fk  " +
					" 	where a.PK_SEQ = '" + ycxkID + "'  " +
					" )  " +
					" xuat on ddh.PK_SEQ = xuat.sanpham_fk and ddh.loai = xuat.LOAI and ddh.scheme = xuat.scheme left join    " +
					" (   " +
					" 	select b.sanpham_fk, b.LOAI, isnull(b.SCHEME, '') as SCHEME, SUM( b.soluong ) as soluongDAXUAT   " +
					" 	from ERP_YCXUATKHONPP a inner join ERP_YCXUATKHONPP_SANPHAM_THUCGIAO b on a.PK_SEQ = b.ycxk_fk  " +
	...
	public void init() 
	{
		String query =  "select ngayyeucau, isnull(ngaygiaohangGUI, '') as ngaygiaohangGUI, xuatcho, ISNULL(ghichu, '') as ghichu, " +
						"case "+
								"  when loaidonhang = '0' then NPP_DAT_FK "
								  +" when loaidonhang = '1' then KHACHHANG_FK "
								  +" when loaidonhang =  '2' then KHACHHANG_FK "
								  +" when loaidonhang =  '3' then nhanvien_fk "
								 +" end  as khId, kho_fk, trangthai, loaidonhang,  " +
						"	( select khachhangKG_FK from ERP_DondathangNPP where PK_SEQ = a.ddh_fk ) as khachhangKG_FK	" +
						"from ERP_YCXUATKHONPP a where pk_seq = '" + this.id + "'";
		...
		if(rs != null)
		{
			try 
			{
				if(rs.next())
				{
					...
					this.xuatcho = rs.getString("loaidonhang");
				...
				query = "select sanpham_fk, solo, isnull(LOAI, 0) as LOAI, soluong, ngayhethan, isnull(scheme, '') as scheme " +
						"from ERP_YCXUATKHONPP_SANPHAM_THUCGIAO_CHITIET where ycxk_fk = '" + this.id + "'";
	...
	public ResultSet getSoloTheoSp(String spIds, String tongluong, String scheme)
	{
		...
		if(this.id.trim().length() > 0)
		{	
			condition += " AND ct.SOLO in ( ";
			condition += "  select solo from ERP_HOADONNPP_SP_CHITIET where soluong > 0 and ma = ( select ma from SANPHAM where pk_seq = '" + spIds + "' ) and hoadon_fk = ( select hoadon_fk from ERP_YCXUATKHONPP where pk_seq = '" + this.id + "' ) " +
						 "	union  " +
						 "	select solo from ERP_DONDATHANGNPP_SANPHAM_CHITIET where soluong > 0 and SANPHAM_FK = '" + spIds + "' and dondathang_fk = ( select ddh_fk from ERP_YCXUATKHONPP where pk_seq = '" + this.id + "' ) ";
			condition += " ) ";
		}
		...
		{
			query = "select AVAILABLE + ISNULL( ( select sum(soluong) from ERP_YCXUATKHONPP_SANPHAM_CHITIET where ycxk_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  ), 0 ) as AVAILABLE, NGAYHETHAN, SOLO " +
				    "from NHAPP_KHO_CHITIET ct " +
				    "where KHO_FK = '" + this.khoNhanId + "' and SANPHAM_FK = '" + spIds + "' and NHOMKENH_FK = ( select nhomkenh_fk from ERP_DONDATHANGNPP where pk_seq = '" + this.ddhId + "' ) and NPP_FK = '" + this.nppId + "' " +
				   		" and ( AVAILABLE + ISNULL( ( select sum(soluong) from ERP_YCXUATKHONPP_SANPHAM_CHITIET where ycxk_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  ), 0 ) ) > 0 " +
				   		" 	" + condition +
		...
		else
		{
			query = "select AVAILABLE + ISNULL( ( select sum(soluong) from ERP_YCXUATKHONPP_SANPHAM_CHITIET where ycxk_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  ), 0 ) as AVAILABLE, NGAYHETHAN, SOLO " +
				    ...
				    "where KHACHHANG_FK = '" + this.khKGId + "' and isNPP = '0' and KHO_FK = '" + this.khoNhanId + "' and SANPHAM_FK = '" + spIds + "' and NHOMKENH_FK = ( select nhomkenh_fk from ERP_DONDATHANGNPP where pk_seq = '" + this.ddhId + "' ) and NPP_FK = '" + this.nppId + "' " +
				   		" and ( AVAILABLE + ISNULL( ( select sum(soluong) from ERP_YCXUATKHONPP_SANPHAM_CHITIET where ycxk_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  ), 0 ) ) > 0 " +
				   		" 	" + condition +
				    " order by NGAYHETHAN asc";
		}
	...
	public ResultSet getSoloTheoSpOLD(String spIds, String tongluong, String scheme)
	{
		String query = "select soluong, SOLO, NGAYHETHAN " +
					   "from ERP_DONDATHANGNPP_SANPHAM_CHITIET where dbo.trim(scheme) = N'" + scheme.trim() + "' and sanpham_fk = '" + spIds + "' and dondathang_fk = ( select ddh_fk from ERP_YCXUATKHONPP where PK_SEQ = '" + this.id + "' ) ";
		System.out.println("---LAY SO LO OLD:: " + query);
		return db.get(query);
	}
	
	public ResultSet getSoloTheoSp_Display(String spIds, String tongluong, String scheme)
	{
		String query = "select soluong, SOLO, NGAYHETHAN, 0 as AVAILABLE " +
					   "from ERP_YCXUATKHONPP_SANPHAM_THUCGIAO_CHITIET where dbo.trim(scheme) = N'" + scheme.trim() + "' and sanpham_fk = '" + spIds + "' and ycxk_fk = '" + this.id + "' ";
		System.out.println("---LAY SO LO HIEN THI:: " + query);
		return db.get(query);
	}
	...
	public boolean create() 
	{
		...
		try
		{
			...
			String query = " insert ERP_YCXUATKHONPP(NgayYeuCau, ngaygiaohangGUI, ghichu, trangthai, npp_fk, kho_fk, xuatcho, NPP_DAT_FK, KHACHHANG_FK, KBH_FK, ngaytao, nguoitao, ngaysua, nguoisua) " +
						   " select '" + this.ngayyeucau + "', '" + ngaygiaohangGui + "', N'" + this.ghichu + "', '0', '" + this.nppId + "', " + this.khoNhanId + ", '" + this.xuatcho + "', " + npp_dat_fk + ", " + khachhang_fk + ", kbh_fk, " +
			...
			msg = util.CapNhat_MaChungTu(db, this.id, "ERP_YCXUATKHONPP", "ngayyeucau");
	...
	public boolean update() 
	{
		...
		try
		{
			...
			String query = " Update ERP_YCXUATKHONPP set NgayYeuCau = '" + this.ngayyeucau + "', ngaygiaohangGUI = '" + this.ngaygiaohangGui + "', ghichu = N'" + this.ghichu + "', npp_fk = '" + this.nppId + "', kho_fk = '" + this.khoNhanId + "', " +
			...
			query = "delete ERP_YCXUATKHONPP_SANPHAM_THUCGIAO where ycxk_fk = '" + this.id + "' ";
			if(!db.update(query))
			{
				this.msg = "Không thể cập nhật ERP_YCXUATKHONPP_SANPHAM_THUCGIAO " + query;
				db.getConnection().rollback();
				return false;
			}
			
			query = "delete ERP_YCXUATKHONPP_SANPHAM_THUCGIAO_CHITIET where ycxk_fk = '" + this.id + "' ";
			if(!db.update(query))
			{
				this.msg = "Không thể cập nhật ERP_YCXUATKHONPP_SANPHAM_THUCGIAO_CHITIET " + query;
				db.getConnection().rollback();
				return false;
			}
			...
			for(int i = 0; i < spId.length; i++)
			{
				if(spId[i].trim().length() > 0 && Double.parseDouble(spSoluong[i].trim().replaceAll(",", "")) > 0 )
				{
					query = "insert ERP_YCXUATKHONPP_SANPHAM_THUCGIAO( ycxk_fk, sanpham_fk, soluong, LOAI, SCHEME ) " +
							"select '" + this.id + "', '" + spId[i] + "', '" + spSoluong[i].replaceAll(",", "") + "', '" + spLoai[i] + "', N'" + spSCheme[i] + "' ";
					...
					if(this.sanpham_soluong != null)
					{
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spId[i] + "__" + spLoai[i] + "__" + spSCheme[i].trim() ))
							{
								...
								query = "insert ERP_YCXUATKHONPP_SANPHAM_THUCGIAO_CHITIET( ycxk_fk, SANPHAM_FK, solo, ngayhethan, soluong, loai, scheme ) " +
										"select '" + this.id + "', pk_seq, N'" + _sp[3] + "', '" + _sp[4] + "', '" + _soluongCT.replaceAll(",", "") + "', '" + spLoai[i] + "', '" + spSCheme[i] + "' " +
										"from SANPHAM where PK_SEQ = '" + spId[i] + "' ";
