...
public class ErpGuiSMSTDV implements IErpGuiSMSTDV
{
	String userId;
	String id;
	...
	{
		this.tinhthanhRs = db.get("select PK_SEQ, TEN from TINHTHANH where TRANGTHAI = '1' and pk_seq != '100049' ");
		...
		if(this.xuatcho.equals("1"))
			query = " select pk_seq, TEN from KHACHHANG b where trangthai = '1' and NPP_FK = '" + this.nppId + "' and PK_SEQ in ( select khachhang_fk from KHACHHANG_KENHBANHANG where kbh_fk in ( select kbh_fk from NHOMKENH_KENHBANHANG where nk_fk = '100000' ) ) "
			+ util.getPhanQuyen_TheoNhanVien("KHACHHANG", "b", "pk_seq", this.getLoainhanvien(), this.getDoituongId() );
				...
				|| this.maFast.trim().length() > 0 || this.tenkhachhang.trim().length() > 0 || this.machungtu.trim().length() > 0 || this.nguoitao.trim().length() > 0 ) )
		{
			query = "select a.PK_SEQ, a.machungtu, ISNULL( b.maFAST, c.MaFAST ) as maKH, ISNULL( b.TEN, c.TEN ) as tenKH, a.NgayDonHang, ISNULL(d.ten, e.ten) as tinhthanh, ISNULL( SONGAYVANCHUYEN, 0 ) as songayvanchuyen "+
					 ...
					 "where a.TRANGTHAI in ( 2, 4 ) and a.npp_fk = '" + this.nppId + "' and ISNULL(d.pk_seq, e.pk_seq) != '100049' ";
		...
		else //Mặc định load ra các đơn tạo trong ngày
		{
			...
			{
				this.ddhId = "";
				query += " AND a.ngaydonhang = '" + this.ngayyeucau + "' ";
			}
			else
			{
				query += " AND ( a.ngaydonhang = '" + this.ngayyeucau + "' or a.pk_seq in ( select DDH_FK from ERP_GUISMSTDV_DDH where guisms_fk = '" + this.id + "' ) ) ";
			}
	...
	public boolean create( String[] machungtu ) 
	{
		...
		try
		{
			...
			if( this.tungay.trim().length() <= 0 )
			{
				query = "update ERP_GUISMSTDV set ngaydukienHANGDEN = CONVERT(varchar(10), DATEADD(dd, cast( ngayvanchuyen as int), ngaygiaohang), 120 ) where pk_seq = '" + this.id + "' ";
				if( !db.update(query) )
				{
				...
				rsDDH = db.get("select ngaydukienHANGDEN from ERP_GUISMSTDV where pk_seq = '" + this.id + "' ");
				if(rsDDH.next())
				{
					this.tungay = rsDDH.getString("ngaydukienHANGDEN");
				}
				rsDDH.close();
			}
	...
	public boolean update( String[] machungtu ) 
	{
		...
		try
		{
			...
			if( this.tungay.trim().length() <= 0 )
			{
				...
				ResultSet rsDDH = db.get("select ngaydukienHANGDEN from ERP_GUISMSTDV where pk_seq = '" + this.id + "' ");
				if(rsDDH.next())
				{
					this.tungay = rsDDH.getString("ngaydukienHANGDEN");
				}
