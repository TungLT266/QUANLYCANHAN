package geso.traphaco.distributor.beans.thongtinsanpham.imp;
...
public class Thongtinsanpham implements IThongtinsanpham, Serializable
{
	...
	public Thongtinsanpham(String userId)
	{	
		this.db = new dbutils();
		this.id = "";
		this.nppId = "";
...
//	   	this.msg =query;
		...
		try
		{
			String query ="select a.pk_seq as id, a.ma as masp, a.ten as tensp, b.donvikinhdoanh as dvkd, b.pk_seq as dvkdId \n" +
					", c.pk_seq as clId, c.ten as chungloai, e.pk_seq as nhId, d.donvi, e.ten as nhanhang, d.pk_seq as dvdlId \n" +
					", a.trangthai, isnull(cast(f.giabanlechuan as nvarchar(20)), 0) as giablcssss, a.type, a.DVDL_ETC_FK \n" +
					", h.TEN as nganhhang, a.spchuluc, isnull(a.spmoi, '') as spmoi \n";
			query = query + " from sanpham a \n" +
			...
			query = query  + " left join banggiablc_sanpham f on a.pk_seq = f.sanpham_fk \n" +
					"left join banggiabanlechuan g on f.bgblc_fk = g.pk_seq \n" +
					"left join NGANHHANG h on a.NGANHHANG_FK = h.PK_SEQ \n";
				     
			query = query + " where a.pk_seq = '" + this.id + "' \n";

			System.out.println("query init sanpham: \n" + query + "\n===============================");
