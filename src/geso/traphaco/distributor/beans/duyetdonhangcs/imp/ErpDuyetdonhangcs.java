package geso.traphaco.distributor.beans.duyetdonhangcs.imp;

import geso.traphaco.distributor.db.sql.dbutils;
import geso.traphaco.center.util.Utility;
import geso.traphaco.distributor.beans.dondathangotc.IErpDonhangNppOTC;
import geso.traphaco.distributor.beans.duyetdonhangcs.IErpDuyetdonhangcs;
...
public class ErpDuyetdonhangcs implements IErpDuyetdonhangcs
{
	String userId;
	String id;
	...
	public ErpDuyetdonhangcs(String id)
	{
		this.id = id;
		this.ma = "";
		this.tungay = "";
	...
	public boolean createNK() 
	{
		...
		/*
		 * if(this.ma.trim().length() <= 0) { this.msg =
		 * "Vui lòng nhập mã hợp đồng"; return false; }
		 */		
		...
		try
		{
			...
			String query = " insert ERP_DondathangNPP(ngaydonhang, ngaydenghi, loaidonhang, npp_dachot, ghichu, trangthai, dvkd_fk, kbh_fk, gsbh_fk, ddkd_fk, khachhang_fk, npp_fk, kho_fk, chietkhau, vat, hopdong_fk, ngaytao, nguoitao, ngaysua, nguoisua) " +
						   " values ( '" + this.tungay + "', '" + this.denngay + "', 0, 1, N'" + this.ghichu + "', 0, '" + this.dvkdId + "', '" + this.kbhId + "', '" + this.gsbhId + "', '" + this.ddkdId + "', '" + this.khId + "', '" + this.nppId + "', " + khonhan_fk + ", " + chietkhau + ", " + vat + ", '"+ this.ma +"', '" + this.getDateTime() + "', '" + this.userId + "', '" + this.getDateTime() + "', '" + this.userId + "' ) ";
			...
			for(int i = 0; i < spMa.length; i++)
			{
				if(spMa[i].trim().length() > 0 && spSoluong[i].trim().length() > 0 && spGianhap[i].trim().length() > 0  )
				{
					...
					if(this.ma.trim().length() >= 6 && spMa[i].trim().length() > 0)
					{
						...
						query = "select (select loaidonhang from ERP_HOPDONGNPP where PK_SEQ = "+this.ma+") as loaidh, hd.soluong - isnull(dh.daDAT, 0) as SL  " +
								"from " +
								"( " +
					...
					if(ktra)
					{
						query = "insert ERP_DONDATHANGNPP_SANPHAM( dondathang_fk, SANPHAM_FK, sanphamTEN, soluong, dongia, chietkhau, thueVAT, dvdl_fk, tungay, denngay, dongiaGOC ) " +
								...
								"					and BGMUANPP_FK in ( select top(1) PK_SEQ from BANGGIAMUANPP bg inner join BANGGIAMUANPP_NPP bg_npp on bg.PK_SEQ = bg_npp.BANGGIAMUANPP_FK where bg.TRANGTHAI = '1' and bg_npp.NPP_FK = '" + this.nppId + "' and bg.DVKD_FK = '" + this.dvkdId + "'  order by bg.TUNGAY desc ) ), 0) as giamua " +
					...
					// TĂNG BOOKED
					/*
					 * query = "Update NHAPP_KHO set AVAILABLE = AVAILABLE - '"
					 * + spSoluong[i].replaceAll(",", "") +
					 * "', booked = booked + '" + spSoluong[i].replaceAll(",",
					 * "") + "' " + "where NPP_FK = '" + this.nppId +
					 * "' and KHO_FK = '" + this.khoNhanId + "' and KBH_Fk = '"
					 * + kbh_fk + "' " +
					 * "	and SANPHAM_FK = ( select pk_seq from SANPHAM where ma = '"
					 * + spMa[i].trim() + "' ) "; if(!db.update(query)) {
			...
			//CHECK NHUNG SP CHUA THIET LAP QUY CACH VE DV CHUAN
			query =  "select khoxuat_fk as kho_fk, npp_fk, kbh_fk, sp.PK_SEQ, sp.TEN, SUM(dathang.soluong) as soluongXUAT,  " +
					...
					"			case when a.dvdl_fk IS null then a.soluong      " +
					"				 when a.dvdl_fk = b.DVDL_FK then a.soluong     " +
					"				 else  a.soluong * ( select SOLUONG1 / SOLUONG2 from QUYCACH where sanpham_fk = a.sanpham_fk and DVDL2_FK = a.dvdl_fk and dvdl1_fk = b.dvdl_fk )      " +
					"								 	 end as soluong   " +
					"	from ERP_DONDATHANGNPP_SANPHAM a inner join SANPHAM b on a.sanpham_fk = b.PK_SEQ  " +
	...
	private void initSANPHAM() 
	{
		...
		String query = "select a.SANPHAM_FK as masp,b.TEN as tensp,dv.DONVI as donvi,SUM(a.soluong) as soluong,a.GIAMUA as dongia,a.CHIETKHAU as chietkhau "
						+",isnull(a.THUEVAT,0) as thuevat, SUM( a.soluong)*GIAMUA as thanhtien "  
						+" from DONHANG_SANPHAM a, sanpham b,DONVIDOLUONG dv "
						+" where a.SANPHAM_FK = b.PK_SEQ and b.DVDL_FK = dv.PK_SEQ and DONHANG_FK ='" + this.id + "'"
						+" group by a.SANPHAM_FK,b.TEN,dv.DONVI,a.GIAMUA,a.CHIETKHAU,a.THUEVAT";
		String query1 = "";
		...
		if(spRs != null)
		{
			try 
			{
				...
				while(spRs.next())
				{
					spMA += spRs.getString("MASP") + "__";
					spTEN += spRs.getString("TENSP") + "__";
...
//					spQuyDoi +=spRs.getString("Donvi") + "__";
				}
				spRs.close();
...
//					this.spQuyDoi = spQuyDoi.split("__");
	...
	public void init() 
	{
		String query = "select ISNULL( cast(hopdong  as nvarchar), '') as mahopdong,  isnull(ngaynhap,'') as ngaydonhang, ISNULL(ghichu, '') as ghichu, kbh_fk, gsbh_fk, ddkd_fk, npp_fk, npp_fk, kho_fk, isnull(chietkhau, 0) as chietkhau, vat, " +
					" Isnull( ( select dungchungkenh from NHAPHANPHOI where pk_seq = a.npp_fk ), 0 ) as dungchungkenh, isnull(chietkhau, 0) as chietkhau,trangthai " +
						"from donhang a where pk_seq = '" + this.id + "'";
		...
		if(rs != null)
		{
			try 
			{
				if(rs.next())
				{
					...
					this.tungay = rs.getString("ngaydonhang");
	...
	private String TaoHoaDonTaiChinhNPP(dbutils db, String id, String userId, String nppId, String khachhangID, String ngaydonhang) 
	{
		...
		try
		{
			...
			// CN TẠI HÀ NỘI DÙNG MẪU 2 (HO) / CN TẠI HCM LẤY MẪU TRONG DLN KH /
			// CÒN LẠI: DÙNG MẪU 1(CN)
			if(nppId.equals("100002")) mau = "2";
			if(!nppId.equals("100002") && !nppId.equals("106210")) mau = "1";
				
			//LAY KENH CUA KHACH HANG
			query =   " insert ERP_HOADONNPP(DDKD_FK, KBH_fK, KHO_FK,nguoimua ,ngayxuatHD, trangthai, ngaytao, nguoitao, ngaysua, nguoisua, kyhieu, sohoadon, hinhthuctt , \n" +
					   ...
					   " SELECT DH.ddkd_Fk, " + 
				       " 		( select kbh_fk from KHACHHANG where pk_seq = KH.PK_SEQ ) as kbh_Fk, DH.kho_fk, N'" + nguoimua + "', '" + ngayhoadon + "', '1','" + getDateTime() + "', '" + userId + "', '" + getDateTime() + "', '" + userId + "', '" + kyhieuhoadon + "', \n" +
