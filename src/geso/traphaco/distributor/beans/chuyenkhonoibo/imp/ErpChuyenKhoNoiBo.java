...
public class ErpChuyenKhoNoiBo implements IErpChuyenKhoNoiBo
{
	String userId;
	String id;
	...
	public ErpChuyenKhoNoiBo(String id)
	{
		this.id = id;
		this.ngayyeucau = getDateTime();
		this.ghichu = "";
...
//		
		...
		String nhomkenh ="100000";
		try
		{
			
			db.getConnection().setAutoCommit(false);
			String query = " insert ERP_CHUYENKHO(SoChungTu,ngaychuyen, ghichu, trangthai, khoxuat_fk, tructhuoc_fk, npp_fk, ngaytao, nguoitao, ngaysua, nguoisua," +
			               "        lenhdieudong, lddcua, lddveviec, ngaydieudong, sohopdong,ngayhopdong, nguoivanchuyen, ptvanchuyen,LoaiDonHang,LoaiChuyenKho,khonhan_fk,ddkd_fk,nhomkenh_fk) " +
						   " values('"+this.sochungtu+"','" + this.ngayyeucau + "', N'" + this.ghichu + "', '0', '" +  this.khoXuatId + "', " + this.nppId + ", '" + this.nppId  + "', '" + getDateTime() + "', '" + this.userId + "', '" + getDateTime() + "', '" + this.userId + "'," +
						   "        N'"+ this.lenhdieudong +"', N'"+ this.lddcua +"',N'"+ this.lddveviec +"','"+ this.ngaylenhdieudong +"',N'"+ this.sohopdong +"','"+ this.ngayhopdong +"',N'"+ this.nguoivanchuyen +"', N'"+ this.ptvanchuyen +"',4, '1','100008','" + this.khId  + "', '"+nhomkenh+"')";
			...
			{
				ResultSet rsnhomkenh = db.get("select nk_fk from NHOMKENH_KENHBANHANG where kbh_FK ='"+this.kbhId+"'");
				rsnhomkenh.next();
				nhomkenh = rsnhomkenh.getString("nk_fk");
			}
			nhomkenh ="100000";
			for(int i = 0; i < spMa.length; i++)
			{
				if(spMa[i].trim().length() > 0 && spSoluong[i].trim().length() > 0 )
				{
					...
					query = "insert ERP_CHUYENKHO_SANPHAM(chuyenkho_fk, SANPHAM_FK, soluongchuyen, dongia, dvdl_fk,loaichuyenkho,nhomkenh_Fk ) " +
							"select '"+this.id+"', pk_seq, '" + spSoluong[i].replaceAll(",", "") + "', '" + spGianhap[i].replaceAll(",", "") + "', ISNULL( ( select pk_Seq from DONVIDOLUONG where donvi = N'" + spDonvi[i].trim() + "' ), DVDL_FK ),'1','"+nhomkenh+"'" +
					...
					if(this.sanpham_soluong != null)
					{
							...
							if(key.startsWith( spMa[i]) )
							{
								...
								if(!_soluongCT.equals("0"))
								{
									...
									query = "select AVAILABLE, NGAYHETHAN from NHAPP_KHO_CHITIET " +
											"where KHO_FK = '" + this.khoXuatId + "' and NPP_FK = '" + this.nppId + "' and nhomkenh_FK = '" +nhomkenh + "' and SOLO = '" + _sp[1] + "' and NGAYHETHAN = '" + _sp[2] + "' " +
									...
									query = "insert ERP_CHUYENKHO_SANPHAM_CHITIET( chuyenkho_fk, SANPHAM_FK, solo, ngayhethan, soluong,loaichuyenkho,nhomkenh_fk ) " +
											"select '"+this.id+"', pk_seq, N'" + _sp[1] + "', '" + ngayhethan + "', '" + _soluongCT.replaceAll(",", "") + "','1','"+nhomkenh+"'" +
									...
									query = "Update NHAPP_KHO_CHITIET set booked = booked + '" + _soluongCT.replaceAll(",", "") + "', AVAILABLE = AVAILABLE - '" + _soluongCT.replaceAll(",", "") + "' " +
											"where KHO_FK = '" + this.khoXuatId + "' and NPP_FK = '" + this.nppId + "' and nhomkenh_FK = '" + nhomkenh + "' and SOLO = '" + _sp[1] + "' and SANPHAM_FK = ( select pk_seq from SANPHAM where MA = '" + spMa[i] + "' )  and NgayHetHan='"+ngayhethan+"' ";
									...
									query = "Update NHAPP_KHO set booked = booked + '" + _soluongCT.replaceAll(",", "") + "', AVAILABLE = AVAILABLE - '" + _soluongCT.replaceAll(",", "") + "' " +
											"where KHO_FK = '" + this.khoXuatId + "' and NPP_FK = '" + this.nppId + "' and nhomkenh_FK = '" +nhomkenh + "' and SANPHAM_FK = ( select pk_seq from SANPHAM where MA = '" + spMa[i] + "' )  ";
						...
						{
							System.out.println("Tong totalct "+totalCT);
...
//		
		...
		//TANG KHO NGUOC LAI
			...
			query = 
					...
					" from " +
					" ( " +
					" 	select a.khoxuat_fk as kho_fk, a.tructhuoc_fk as npp_Fk, "+this.kbhId+" as kbh_fk, b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan,b.nhomkenh_fk  " +
					" 	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " +
					" 	where chuyenkho_fk = '" + this.id + "' " +
					" 	group by a.khoxuat_fk, a.tructhuoc_fk, a.kbh_fk, b.solo, b.sanpham_fk,b.NgayHetHan,b.nhomkenh_fk " +
					" ) " +
			...
			query = " update kho set kho.AVAILABLE = kho.AVAILABLE + CT.tongxuat, " +
					...
					" from " +
					" ( " +
					" 	select a.khoxuat_fk as kho_fk, a.tructhuoc_fk as npp_fk, "+this.kbhId+" as kbh_fk , b.sanpham_fk, SUM(b.soluong) as tongxuat,b.nhomkenh_fk  " +
					" 	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " +
					" 	where chuyenkho_fk = '" + this.id + "' " +
					" 	group by a.khoxuat_fk, a.tructhuoc_fk, a.kbh_fk, b.sanpham_fk,b.nhomkenh_fk " +
					" ) " +
			...
			query =	" Update ERP_CHUYENKHO set SoChungTu='"+this.sochungtu+"',ngaychuyen = '" + this.ngayyeucau + "', ghichu = N'" + this.ghichu + "', " +
							" khoxuat_fk = " + this.khoXuatId + ", kbh_fk = '" + this.kbhId + "', ddkd_fk = '" + this.khId + "', tructhuoc_fk = '" + this.nppId + "', ngaysua = '" + getDateTime() + "', nguoisua = '" + this.userId + "', " + 
							...
							" where pk_seq = '" + this.id + "' and loaichuyenkho = '1' ";
					...
					if(this.sanpham_soluong != null)
					{
						...
						while(keys.hasMoreElements())
						{
							...
							if(key.startsWith( spMa[i]) )
							{
								...
								totalCT += Double.parseDouble(_soluongCT);
								System.out.println("_soluongCT "+_soluongCT);
								if(!_soluongCT.equals("0"))
								{
									//CHECK TON KHO
									query = "select AVAILABLE, NGAYHETHAN from NHAPP_KHO_CHITIET " +
...
//			
	...
	public void createRs() 
	{
		 ...
		 */
		query = " select b.PK_SEQ, b.TEN from Daidienkinhdoanh_Npp a,Daidienkinhdoanh b where a.ddkd_fk = b.pk_seq and b.trangthai = '1' and a.npp_fk = '" + this.nppId + "'";
...
//		if(!this.dungchungkenh.equals("1"))
//		{
//			nhomkenh="(select nhomkenh_fk from ERP_CHUYENKHO where PK_SEQ= '" + this.id + "')";
//		}
//		else
//		{
//			try {
...
//				{
//					ResultSet rsnhomkenh = db.get("select nk_fk from NHOMKENH_KENHBANHANG where kbh_FK ='"+this.kbhId+"'");
//					System.out.println("nhomkenh "+"select nk_fk from NHOMKENH_KENHBANHANG where kbh_FK ='"+this.kbhId+"'");
//					rsnhomkenh.next();
//					nhomkenh = rsnhomkenh.getString("nk_fk");
//				}
...
//		}
		...
		String query =  
					...
					"                                          and NHOMKENH_FK ="+nhomkenh+"   ), 0 ) as avai " +
					...
					"where a.chuyenkho_FK = '" + this.id + "'and a.loaichuyenkho = '1'  ";
	...
	public void init() 
	{
		String query =  "select SoChungTu,ngaychuyen, ISNULL(ghichu, '') as ghichu, khoxuat_fk, kbh_fk, tructhuoc_fk,ddkd_fk, trangthai, " +
						" lenhdieudong, lddcua, lddveviec, ngaydieudong, sohopdong,ngayhopdong, nguoivanchuyen, ptvanchuyen " +
						"from ERP_CHUYENKHO where pk_seq = '" + this.id + "' and loaichuyenkho = '1' ";
		...
		if(rs != null)
		{
			try 
			{
				if(rs.next())
				{
					...
					this.khId = rs.getString("ddkd_fk");
				...
				rs.close();
				System.out.println("This.khid "+this.khId);
				//INIT SO LUONG
				query = "select sanpham_fk, (select MA from SANPHAM where pk_seq = a.sanpham_fk ) as spMA,  solo, soluong,a.NgayHetHan " +
						"from ERP_CHUYENKHO_SANPHAM_CHITIET a where chuyenkho_fk = '" + this.id + "',a.loaichuyenkho = '1' ";
				System.out.println("---INIT SP: " + query);
				rs = db.get(query);
				if(rs != null)
				{
...
//			}
			...
			String query = 
							"select AVAILABLE + ISNULL( ( select sum(soluong) from ERP_CHUYENKHO_SANPHAM_CHITIET where chuyenkho_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ct.sanpham_fk and solo = ct.solo  and NgayHetHan=ct.NgayHetHan ), 0 ) as AVAILABLE, NGAYHETHAN, SOLO " +
						   "from NHAPP_KHO_CHITIET ct " +
						   "where KHO_FK = '" + this.khoXuatId + "' and nhomkenh_FK ="+nhomkenh+"  and NPP_FK = '" + this.nppId + "' and SANPHAM_FK = ( select pk_seq from SANPHAM where ma = '" + spMa + "' ) " +
						   " and ( AVAILABLE + ISNULL( ( select sum(soluong) from ERP_CHUYENKHO_SANPHAM_CHITIET where chuyenkho_fk = '" + ( this.id.trim().length() > 0 ? this.id : "-1" ) + "' and sanpham_fk = ( select pk_seq from SANPHAM where ma = '" + spMa + "' ) and solo = ct.solo  and NgayHetHan=ct.NgayHetHan   ), 0 ) ) > 0 " +
