...
public class ErpHopdongNppList extends Phan_Trang implements IErpHopdongNppList
{
	...
	public void CreateLoaiHD()
	{
		/*String query="select distinct LoaiDonHang, (CASE LoaiDonHang " +
						...
						"ELSE N'Sở y tế' " +
						"END) AS  TenLoaiHD " +
						"from ERP_HOPDONGNPP where npp_fk = '" + this.nppId + "' ";
	...
	public void init(String search)
	{
		...
		{
			query = "select distinct a.PK_SEQ, a.trangthai, a.mahopdong, a.loaidonhang, a.tungay, a.denngay, isnull(c.ten, '') as nppTEN, b.ten as khoTEN, NV.TEN as nguoitao, b.ten as khonhan, a.NGAYSUA, a.NGAYTAO, NV2.TEN as nguoisua, isnull(a.NOTE, '') as NOTE, a.hieuluc   " +
					...
					" where a.npp_fk = '" + this.nppId + "' and hieuluc = '1' "+ 
					" 	and a.kho_fk in "+this.util.quyen_kho(this.userId);
		} 
		
		if(this.hieuLuc.trim().length() > 0 )
		{
			if( hieuLuc.equals("0") )
		...
		query += util.getPhanQuyen_TheoNhanVien("KHACHHANG", "c", "pk_seq", this.getLoainhanvien(), this.getDoituongId() );
		
		this.DondathangRs = createSplittingDataNew(this.db,50, 10, "pk_seq desc, tungay desc, trangthai asc ", query);
		
		//this.khRs = db.get("select PK_SEQ, MAFAST + ', ' + TEN as TEN from KHACHHANG where TRANGTHAI = '1' and KBH_FK in ( select kbh_fk from NHOMKENH_KENHBANHANG where nk_fk = '100000' ) and NPP_FK = '" + this.nppId + "' ");
		query = "select PK_SEQ, MAFAST + ', ' + TEN as TEN " +
				"from KHACHHANG a where TRANGTHAI = '1' AND NPP_FK = '" + this.nppId + "' AND pk_seq in ( select khachhang_fk from KHACHHANG_KENHBANHANG where kbh_fk in ( select kbh_fk from hethongbanhang_kenhbanhang where htbh_fk = '100000' ) ) ";
		...
		CreateLoaiHD();
		//System.out.println("---- KHACH HANG: " + "select PK_SEQ, MA + ' - ' + TEN as TEN from KHACHHANG where TRANGTHAI = '1' and KBH_FK in ( select kbh_fk from NHOMKENH_KENHBANHANG where nk_fk = '100000' ) and NPP_FK = '" + this.nppId + "' ");
	}
