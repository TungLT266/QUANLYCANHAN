...
{ 
	...
	public void setDdkdRs(ResultSet ddkdRs)
  {
	...
	public String getBCTheoKH() {
    	...
    	if(nvgnId.length()>0)
    	{
    		conditionDetails_ETC=" and hd.khachhang_Fk   in  (select KHACHHANG_FK from NVGN_KH where nvgn_fk='"+nvgnId+"' ) ";
    	
    		condition_DuNoDauKy+= " and dnkh.khachhang_Fk in (select KHACHHANG_FK from NVGN_KH where nvgn_fk='"+nvgnId+"' )  ";
    		
    		
    		conditionDetails_OTC+=
    			" and hd.khachhang_Fk   in  (select KHACHHANG_FK from NVGN_KH where nvgn_fk='"+nvgnId+"' ) ";
    	...
    	query = "SELECT 	lkh.ten LoaiKH,kh.DIACHI,kh.maFAST, kh.maFAST, kh.TEN,   \n" +
		    	...
		    	"		WHERE 1 =1 and tt.NGAYCHUNGTU < '" + this.tungay + "'  and tt.TRANGTHAI= 1 \n";
		    	if(this.nppId.trim().length()>0)
	...
	public String getBCTheoDoiTac() {
		...
		"	 						   )   \n"+
		"	 							bt on hd.NPP_DAT_FK = bt.NPP_DAT_FK and hd.PK_SEQ = bt.HOADON_FK \n"+ 
		"			WHERE 	1=1 and hd.NGAYXUATHD <= '"+this.tungay+"' and hd.NGAYXUATHD >= '"+this.denngay+"' and hd.NPP_DAT_FK is not null \n";
		if(this.nppId.trim().length()>0)
			query+= " and hd.NPP_FK = '"+this.nppId+"'";
		query+=
		"			GROUP BY hd.NPP_DAT_FK  \n"+
		...
		"								FROM   CANTRUCONGNO ct INNER JOIN CANTRUCONGNO_HOADON cthd on ct.PK_SEQ = cthd.CANTRUCONGNO_FK \n"+
		"								WHERE  ct.TRANGTHAI = 1  and cthd.NPP_DAT_FK is not null \n"; 
		if(this.nppId.trim().length()>0)
			query+= " 	and ct.NPP_FK = '"+this.nppId+"' \n";
		query+=
