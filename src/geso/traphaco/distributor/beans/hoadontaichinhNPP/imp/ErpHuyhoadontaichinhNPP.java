...
import geso.traphaco.erp.beans.huyhoadontaichinh.*;
...
public class ErpHuyhoadontaichinhNPP extends Phan_Trang implements IErpHuyhoadontaichinhNPP
{
	...
	public  ErpHuyhoadontaichinhNPP(){
		/*
	...
	public void init_new() {//ĐÃ CÓ ID - HÓA ĐƠN
		...
		try
		{
			...
			if(this.Id.length()>0){
				...
				query="SELECT HOADON_FK,NGAYHUY, LOAIHD  FROM ERP_HUYHOADONTAICHINH WHERE PK_SEQ="+this.Id;
				ResultSet rshd=db.get(query);
				if(rshd.next()){
					this.SoCT = rshd.getString("HOADON_FK");
					this.Ngayhuy = rshd.getString("NGAYHUY");
					this.loaiHD = rshd.getString("LOAIHD");
				}
				rshd.close();
			}
			...
			//this.khRs = db.get("SELECT PK_SEQ, TEN from KHACHHANG WHERE TRANGTHAI = '1' ORDER BY TEN");
			
			//B1.LẤY THÔNG TIN HÓA ĐƠN
/*			  query= "SELECT PK_SEQ, NGAYXUATHD,isnull(NGAYGHINHAN,'') NGAYGHINHAN, KYHIEU,SOHOADON,VAT, " +
			  		" isnull(TIENCKTHUONGMAI,0) TIENCKTHUONGMAI, \n " +
				  	 "CHIETKHAU,isnull(GHICHU,'') GHICHU , KHACHHANG_FK, ISNULL(TONGTIENBVAT, 0) TONGTIENBVAT, " +
				  	 "ISNULL(TONGTIENAVAT,0) TONGTIENAVAT,  \n " +
				  	 "HINHTHUCTT, GHICHU, isnull(KM_GHICHU,'') KM_GHICHU \n " +

				  	 "FROM   ERP_HUYHOADONTAICHINH " +
				  	 "WHERE HOADON_FK = '"+this.SoCT+"' ";*/
		
		if(!this.loaiHD.equals("6")){
			query = "  SELECT PK_SEQ, NGAYXUATHD,isnull(NGAYGHINHAN,'') NGAYGHINHAN, KYHIEU,SOHOADON, \n " +
			  		"  VAT, CHIETKHAU,isnull(GHICHU,'') GHICHU ,  \n " +
			  		
			  		"  CASE WHEN KHACHHANG_FK IS NOT NULL THEN 'KH' + CONVERT(VARCHAR, KHACHHANG_FK)  \n " +
			  		"  ELSE 'NPP' + CONVERT(VARCHAR, NPP_DAT_FK) " +
			  		"  END AS KHID,  \n " +
			  		
			  		"  ISNULL(TONGTIENAVAT,0) TONGTIENAVAT, HINHTHUCTT \n"+
			  		"  FROM ERP_HOADONNPP WHERE PK_SEQ = '"+this.SoCT+"' AND LOAIHOADON = " + this.loaiHD + " ";
			...
			if(rs!=null)
			{
				...
				}else  if(this.SoCT.length() > 0){
					this.Msg = "Không thể hủy hóa đơn, vì số chứng từ không tồn tại hoặc hóa đơn đã bị hủy rồi.";
				}
			}
			rs.close();
			...
			query=				
				...
				"	SELECT SANPHAM_FK SPID, SP.MA, SP.TEN,HD_SP.DONVITINH DONVI, HD_SP.DONVITINH MADONVI, " +
				"   ISNULL(HD_SP.DONGIA,0) as DONGIA,isnull(HD_SP.VAT,0) VAT, SOLUONG, " +
				"   (ISNULL(HD_SP.THANHTIEN, 0) - ISNULL(HD_SP.CHIETKHAU,0)) AS TIENAVAT, " +
				"	ISNULL(HD_SP.CHIETKHAU,0) CHIETKHAU, " +
		//		"isnull(TIENCK,0) TIENCK, \n"+
				"   (( SELECT ISNULL(SUM(a.SOLUONG),0) \n"+
				"	 FROM ERP_DONDATHANGNPP_SANPHAM a INNER JOIN ERP_DONDATHANGNPP b on a.DONDATHANG_FK = b.PK_SEQ \n"+ 
				"	 WHERE b.PK_SEQ IN (SELECT DDH_FK FROM ERP_HOADONNPP_DDH c WHERE c.HOADONNPP_FK = '"+this.SoCT+"') and a.SANPHAM_FK = HD_SP.SANPHAM_FK \n"+
				"	) \n"+ 
				"	- \n"+		
				"	( \n"+
				"	  SELECT ISNULL(SUM(a.SOLUONG),0) \n"+
				"	  FROM ERP_HOADONNPP_SP a " +
				"	  INNER JOIN ERP_HOADONNPP b on a.HOADON_FK = b.PK_SEQ " +
				"	  INNER JOIN ERP_HOADONNPP_DDH c on b.PK_SEQ = c.HOADONNPP_FK \n"+
				"	  WHERE b.TRANGTHAI in (0,1) and b.PK_SEQ not in ('"+this.SoCT+"') and a.SANPHAM_FK = HD_SP.SANPHAM_FK \n"+
				"		   and c.DDH_FK IN (SELECT DDH_FK FROM ERP_HOADONNPP_DDH c WHERE c.HOADONNPP_FK = '"+this.SoCT+"') \n"+
				"	)) SLGOC \n"+
				" FROM ERP_HOADONNPP_SP HD_SP " +
				" INNER JOIN SANPHAM SP on HD_SP.SANPHAM_FK = SP.PK_SEQ \n"+
				" INNER JOIN ERP_HOADONNPP HD ON HD.PK_SEQ = HD_SP.HOADON_FK \n " +
				" WHERE HOADON_FK ='"+this.SoCT+"' AND HD.LOAIHOADON = " + this.loaiHD + "  \n";
			 ...
			 try 
			    {
				    ...
				    {				    	
						...
						//	spTIENCHIETKHAU+=(rsLaySP.getDouble("TIENCK")) +"__";							
							...
							spSOLUONGGOC+=(rsLaySP.getString("SLGOC"))+"__";
						...
						//	
							...
							sp_tienck_banhang = rsLaySP.getDouble("CHIETKHAU")*soluong*dongia/100;
						...
						//	this.spMadonvi = spMADONVI.split("__");
							...
							spSOLUONGGOC = spSOLUONGGOC.substring(0, spSOLUONGGOC.length() - 2);
				...
				query = "  SELECT PK_SEQ, NGAYHOADON AS NGAYXUATHD,isnull(NGAYGHINHAN,'') NGAYGHINHAN, KYHIEUHOADON AS KYHIEU, SOHOADON, " +
		  				"  VAT*AVAT/(1.1*100) as VAT, 0 AS TIENCKTHUONGMAI, 0 AS CHIETKHAU, isnull(DIENGIAI,'') GHICHU , " +
		  				"  KHACHHANG_FK, ISNULL(AVAT,0) TONGTIENAVAT , HINHTHUCTHANHTOAN AS HINHTHUCTT \n"+
		  				"  FROM ERP_HOADONPHELIEU " +
		  				"  WHERE PK_SEQ = '" + this.SoCT + "' AND TRANGTHAI = 1 ";
				...
				//LẤY SẢN PHẨM TRÊN HÓA ĐƠN 			
		query =				
			"	SELECT '' SPID, '' AS MA, HD_SP.DIENGIAI AS TEN, DONVITINH DONVI, 0 AS MADONVI, " +
			"   ISNULL(HD_SP.DONGIA,0) as DONGIA,isnull(HD.VAT,0) VAT, SOLUONG, " +
			
		    " 	THANHTIEN AS TIENAVAT, "+
			"   CASE WHEN DONGIA <> 0 THEN 100*(DONGIA - DONGIACK)/DONGIA ELSE 100 AS CHIETKHAU, " +
			"   (DONGIA - DONGIACK)*SOLUONG AS TIENCK, " +
			
			"   SOLUONG AS SLGOC \n"+
			" 	FROM ERP_HOADONPHELIEU_SANPHAM HD_SP \n"+
		 	"	INNER JOIN ERP_HOADONPHELIEU HD ON HD.PK_SEQ = HD_SP.HOADONPHELIEU_FK \n " +

			" 	WHERE HOADONPHELIEU_FK ='"+this.SoCT+"' AND HD.TRANGTHAI = 1\n";
	...
	public void init() {//ĐÃ CÓ ID - HÓA ĐƠN
		...
		try
		{
			...
			if(!this.loaiHD.equals("6")){ // LÀ HÓA ĐƠN TÀI CHÍNH HOẶC HÓA ĐƠN KM
			//B1.LẤY THÔNG TIN HÓA ĐƠN
			  query= "SELECT PK_SEQ, NGAYXUATHD,isnull(NGAYGHINHAN,'') NGAYGHINHAN, KYHIEU,SOHOADON,VAT, " +
			  		" isnull(TIENCKTHUONGMAI,0) TIENCKTHUONGMAI, \n " +
				  	 "CHIETKHAU,isnull(GHICHU,'') GHICHU , KHACHHANG_FK, ISNULL(TONGTIENBVAT, 0) TONGTIENBVAT, " +
...
/*				  "  SELECT PK_SEQ, NGAYXUATHD,isnull(NGAYGHINHAN,'') NGAYGHINHAN, KYHIEU,SOHOADON, " +
			  		"  VAT,isnull(TIENCKTHUONGMAI,0) TIENCKTHUONGMAI ,CHIETKHAU,isnull(GHICHU,'') GHICHU , " +
			  		"  KHACHHANG_FK, ISNULL(TONGTIENAVAT,0) TONGTIENAVAT , HINHTHUCTT \n"+
			...
			query=				
				...
				"	SELECT SANPHAM_FK SPID, SP.MA, SP.TEN,HD_SP.DONVITINH DONVI, HD_SP.DVDL_FK MADONVI, " +
				"   ISNULL(HD_SP.DONGIA,0) as DONGIA,isnull(HD_SP.VAT,0) VAT, SOLUONG, " +
				"   HD_SP.TIENAVAT, isnull(HD_SP.CHIETKHAU,0) CHIETKHAU, isnull(TIENCK,0) TIENCK, \n"+
				"   (( SELECT ISNULL(SUM(a.SOLUONG),0) \n"+
				"	 FROM ERP_DONDATHANG_SP a INNER JOIN ERP_DONDATHANG b on a.DONDATHANG_FK = b.PK_SEQ \n"+ 
				"	 WHERE b.PK_SEQ IN (SELECT DDH_FK FROM ERP_HOADON_DDH c WHERE c.HOADON_FK = '"+this.SoCT+"') and a.SANPHAM_FK = HD_SP.SANPHAM_FK \n"+
				"	) \n"+ 
				"	- \n"+		
				"	( \n"+
				"	  SELECT ISNULL(SUM(a.SOLUONG),0) \n"+
				"	  FROM ERP_HOADON_SP a INNER JOIN ERP_HOADON b on a.HOADON_FK = b.PK_SEQ INNER JOIN ERP_HOADON_DDH c on b.PK_SEQ = c.HOADON_FK \n"+
				"	  WHERE b.TRANGTHAI in (0,1) and b.PK_SEQ not in ('"+this.SoCT+"') and a.SANPHAM_FK = HD_SP.SANPHAM_FK \n"+
				"		   and c.DDH_FK IN (SELECT DDH_FK FROM ERP_HOADON_DDH c WHERE c.HOADON_FK = '"+this.SoCT+"') \n"+
				"	)) SLGOC \n"+
				" FROM ERP_HOADON_SP HD_SP " +
				" INNER JOIN ERP_SANPHAM SP on HD_SP.SANPHAM_FK = SP.PK_SEQ \n"+
				" INNER JOIN ERP_HOADON HD ON HD.PK_SEQ = HD_SP.HOADON_FK \n " +
				" WHERE HOADON_FK ='"+this.SoCT+"' AND HD.LOAIHOADON = " + this.loaiHD + " AND HD.TRANGTHAI = 1 \n";
	...
	query =				
	    ...
	    " 	THANHTIEN AS TIENAVAT, "+
		"   CASE WHEN DONGIA <> 0 THEN 100*(DONGIA - DONGIACK)/DONGIA ELSE 100 END AS CHIETKHAU, " +
		"   (DONGIA - DONGIACK)*SOLUONG AS TIENCK, " +
		...
		" 	WHERE HOADONPHELIEU_FK ='"+this.SoCT+"' \n";
	...
	public ErpHuyhoadontaichinhNPP(String id)
	{
		this.db = new dbutils();
		this.TrangThai="";
		this.tungay="";
...
/*		if(id.length() > 0)
	...
	public void setDisplayHD(){
		...
		try
		{
			...
			System.out.println("INIT_THONGTINHOADON 2: "+query);
			ResultSet rs= db.get(query);
			if(rs!=null)
			{
				while(rs.next())
	...
	{	
		...
		try
		{
			...
			if(!this.loaiHD.equals("6")){
				...
				query = "insert into ERP_HUYHOADONTAICHINH \n " +
						"(NGAYHUY, HOADON_FK,	NGAYXUATHD, TRANGTHAI,	NGAYTAO, NGUOITAO,	KYHIEU,	SOHOADON, HINHTHUCTT,  \n  " +
						"NGAYSUA, NGUOISUA,	NPP_FK, KHO_FK, KBH_FK, NGUOIMUA, CHIETKHAU, TONGTIENAVAT,  TONGTIENBVAT,  \n  " +	
						"VAT, GHICHU, KHACHHANG_FK, LOAIHD,   \n " + 	
						"CONGTY_FK,	TIENTE_FK,	NGAYGHINHAN, TYGIA)  \n  " +   
			 
						"SELECT '" + this.Ngayhuy + "', PK_SEQ,	NGAYXUATHD, 0, '" + this.getdatetime() + "', " + this.UserId + ", KYHIEU,	SOHOADON, HINHTHUCTT, \n " + 
						"'" + this.getdatetime()+ "', " + this.UserId + ", NPP_FK,KHO_FK, KBH_FK, NGUOIMUA, CHIETKHAU, TONGTIENAVAT,  TONGTIENBVAT,	\n " +
						"VAT, GHICHU, KHACHHANG_FK,	LOAIHOADON,  CONGTY_FK, TIENTE_FK,	NGAYGHINHAN, TYGIA	\n " +

						"FROM ERP_HOADONNPP WHERE PK_SEQ = " + this.SoCT;
			...
			query = " INSERT INTO ERP_HUYHOADONTAICHINH_SP (HUYHOADONTAICHINH_FK, HOADON_FK, SANPHAM_FK, SOLUONG, DONGIA, " +
					" TIENBVAT, VAT, TIENAVAT, DONVITINH,	SCHEME	, "+
					...
					" CHIETKHAU, DONGIA, '', 0,	NULL, 0 " +
					" FROM ERP_HOADONNPP_SP WHERE HOADON_FK = " + this.SoCT;
			...
			query =  "insert into ERP_HUYHOADONTAICHINH \n " +
				    ...
				    " SELECT '"+this.Ngayhuy+"', PK_SEQ,	NGAYHOADON	,0,	'"+this.getdatetime()+"', "+this.UserId+", KYHIEUHOADON, SOHOADON, HINHTHUCTHANHTOAN, "+
				    " '"+this.getdatetime()+"'	,"+this.UserId+", NULL, NULL, KBH_FK, NGUOIMUAHANG, NULL, 0, AVAT, "+
				    " BVAT,	VAT, '', KHACHHANG_FK, " + this.loaiHD + ", " +
				    " CONGTY_FK, 100000, NGAYGHINHAN, '1' " +
				    " FROM ERP_HOADONPHELIEU where PK_SEQ=  " +this.SoCT;

			System.out.println(" 1. Insert Huy HD phế liệu: " + query);
			if(!db.update(query))
			{
				this.Msg = "Không thể tạo mới :  " + query ;
				db.getConnection().rollback();
			...
			query = " INSERT INTO ERP_HUYHOADONTAICHINH_SP (HUYHOADONTAICHINH_FK, HOADON_FK, SANPHAM_FK, SOLUONG, DONGIA, " +
				    ...
				    " SELECT "+newId+", HOADONPHELIEU_FK,	NULL, SOLUONG, DONGIA, THANHTIEN, " +
				    " (SELECT VAT FROM ERP_HOADONPHELIEU WHERE PK_SEQ = " + this.SoCT + "), " +
				    " THANHTIEN*(1 + 1.0*(SELECT VAT FROM ERP_HOADONPHELIEU WHERE PK_SEQ = " + this.SoCT + ")/100), DONVITINH,	''	, " +
					" CASE WHEN DONGIA <> 0 THEN 100*(DONGIA - DONGIACK)/DONGIA " +
					" ELSE 100 END AS CHIETKHAU, " +
					" DONGIA, GHICHU, 0, NULL, (DONGIA - DONGIACK)*SOLUONG " +
					" FROM ERP_HOADONPHELIEU_SANPHAM WHERE HOADONPHELIEU_FK = " + this.SoCT;
			System.out.println("2. Insert SP Huy HD phế liệu :" + query);
			if(!db.update(query))
			{
				this.Msg = "Không thể tạo mới :  " + query;
				db.getConnection().rollback();
