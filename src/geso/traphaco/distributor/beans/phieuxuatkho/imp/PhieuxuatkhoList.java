package geso.traphaco.distributor.beans.phieuxuatkho.imp;
...
public class PhieuxuatkhoList extends Phan_Trang implements IPhieuxuatkhoList, Serializable
{
	...
	public void init(String search) 
	{
		...
		if (search.length() == 0)
		{
			query = "select pxk.pk_seq as pxkId, nvgn.pk_seq as nvgnId, nvgn.ten as nvgnTen, pxk.trangthai, pxk.ngaylapphieu, pxk.ngaytao, pxk.ngaysua, nv1.ten as nguoitao, nv2.ten as nguoisua, isnull(c.maFAST,'' )as mafast, isnull(c.TEN, '') as khachhang  ";
			...
			query = query + " left join PHIEUXUATKHO_DONHANG a on pxk.PK_SEQ = a.PXK_FK left join DONHANG b on a.DONHANG_FK =b.PK_SEQ left join KHACHHANG c on b.KHACHHANG_FK = c.PK_SEQ ";
			query = query + " where pxk.npp_fk = '" + this.nppId + "' and b.kho_fk in "+util.quyen_kho(this.userId);
