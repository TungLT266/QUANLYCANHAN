package geso.traphaco.distributor.beans.banggiabandoitac.imp;
...
public class Banggiabandoitac implements IBanggiabandoitac
{
	...
	public void createRS() 
	{
		...
		this.doitacRs = db.get(query);
			...
			query = "select isnull (nsp.NSP_FK,0) as nsp_fk,a.pk_seq, a.ma, a.ten, b.donvi, " +
					"	( select top(1) bgsp.giabanleNPP from BGBANLENPP_SANPHAM bgsp inner join BANGGIABANLENPP bg on bgsp.BGBANLENPP_FK=bg.pk_seq where sanpham_fk = a.pk_seq and BGBANLENPP_FK in ( select pk_seq from BANGGIABANLENPP where npp_fk = '" + this.nppId + "'  ) order by bg.ngaybatdau DESC ) as gblc,  " +
					"	( select top(1) giabanleNPP from  BGBANLENPP_SANPHAM bgsp inner join BANGGIABANLENPP bg on bgsp.BGBANLENPP_FK=bg.pk_seq where sanpham_fk = a.pk_seq and BGBANLENPP_FK in ( select pk_seq from BANGGIABANLENPP where npp_fk = '" + this.nppId + "'  ) order by bg.ngaybatdau DESC ) as giasauCK   " +
					"\n from SANPHAM a inner join DONVIDOLUONG b on a.dvdl_fk = b.pk_seq left join NHOMSANPHAM_SANPHAM nsp on nsp.SP_FK=a.PK_SEQ " +
					"order by a.ten asc";
