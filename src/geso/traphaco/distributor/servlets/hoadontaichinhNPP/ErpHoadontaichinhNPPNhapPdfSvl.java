...
public class ErpHoadontaichinhNPPNhapPdfSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpHoadontaichinhNPPNhapPdfSvl()
	{
		super();
	}
...
/*		  String INIT_CHIETKHAU_TICHLUY =
	...
	private void CreateHd_CNHOCHIMINH_OLD(Document document,ServletOutputStream outstream, IErpHoadontaichinhNPP hdBean, String INIT_KHACHHANG, String INIT_SANPHAM_VAT,  String INIT_SANPHAM_KM, String INIT_CHIETKHAU_TICHLUY , String INIT_CHIETKHAU_KM , String INIT_CHIETKHAU_SCHEMEKM)
	{
		...
		float[] withsKM = { 0.7f*CONVERT,7.2f*CONVERT, 1.6f*CONVERT, 1.6f*CONVERT, 3.0f*CONVERT, 3.0f*CONVERT };
		...
		if(rsSP!=null){				
			while(rsSP.next()){				
        		...
        		{
        			arr = new String[] { Integer.toString(stt), tensp ,  DVT,DinhDangTraphacoERP(formatter1.format(soluong)), DinhDangTraphacoERP(formatter1.format(dongia)), DinhDangTraphacoERP(formatter1.format(thanhtien))};
        		}
        		else
        		{
        			arr = new String[] { Integer.toString(stt), tensp ,  DVT,DinhDangTraphacoERP(formatter1.format(soluong)), DinhDangTraphacoERP(formatter2.format(dongia)), DinhDangTraphacoERP(formatter1.format(thanhtien))};
        		}
		...
		if(isSpKM>0 )
		{
			...
			if(rsSP_KM!=null){		
				while(rsSP_KM.next()){
	        		...
	        		String[] arr = new String[] { Integer.toString(stt), tensp , DVT,DinhDangTraphacoERP(formatter1.format(soluong)), "-", "-"  };
	...
	{
		stt++;
		String[] arr = new String[] { "", "Tiền chiết khấu: ", "","", "",DinhDangTraphacoERP(formatter1.format(sum_tienck))};
	...
	if(schemekm > 0)
	{
		...
		String[] arr = new String[] { "", "Giảm giá theo thông báo chương trình combo Xisat XP và Mepoly", "","", "",DinhDangTraphacoERP(formatter1.format(tichluy_tienBVAT))};
	...
	if(CKKM >= 1)
	{
		...
		String[] arr = new String[] { "","Trả khuyến mại" , "","", "",DinhDangTraphacoERP(formatter1.format(tichluy_tienBVAT))};
	...
	if(cktl5>0) // CHIẾT KHẤU THEO HỢP ĐỒNG 
	{
		...
		if(rsCKTL!=null){		
			while(rsCKTL.next()){						
				 ...
				 String[] arr = new String[] { "", "Tiền chiết khấu theo HĐTT số: "+Sohopdong+" Ngày: "+nn , "","", "",DinhDangTraphacoERP(formatter1.format(tichluy_tienBVAT))};
		...
		hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(total_tienbvat)) , new Font(bf, 11, Font.NORMAL));
		...
		hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(total_thue)) , new Font(bf, 11, Font.NORMAL));
		hd.setAlignment(Element.ALIGN_RIGHT);
		cell.setVerticalAlignment(Element.ALIGN_TOP);
		cell.setFixedHeight(0.8f*CONVERT);		
		cell.setPaddingTop(-0.1f*CONVERT);
		...
		hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(total_tienavat)) , new Font(bf, 11, Font.NORMAL));
		hd.setAlignment(Element.ALIGN_RIGHT);
		cell.setVerticalAlignment(Element.ALIGN_TOP);
		cell.setFixedHeight(0.8f*CONVERT);	
		cell.setPaddingTop(-0.3f*CONVERT);
