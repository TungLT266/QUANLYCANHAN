...
public class BCDonHangBanTrongKy extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(!view.equals("TT"))
		{
			String nextJSP = "/TraphacoHYERP/pages/Distributor/BCDonHangBanTrongKy.jsp";
			response.sendRedirect(nextJSP);
		}
		else
		{
			String nextJSP = "/TraphacoHYERP/pages/Center/BCDonHangBanTrongKyNPP.jsp";
			response.sendRedirect(nextJSP);
		}
	}
	...
	protected void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (action.equals("tao")) 
		{
			try 
			{
				...
				response.setHeader("Content-Disposition", "attachment; filename=BCDonHangBanTrongKyNPP.xlsm");
	...
	private String setQueryMTV(String sql, IStockintransit obj, HttpSession session) 
	{
		...
		//PHAN QUYEN TDV
		condition += util.getPhanQuyen_TheoNhanVien("DAIDIENKINHDOANH", "ddkd", "PK_SEQ", obj.getLoainhanvien(), obj.getDoituongId() );
		...
		condition += " and a.npp_fk in ( select pk_seq from NHAPHANPHOI where congty_fk = '" + congtyId + "' or tructhuoc_fk in ( select PK_SEQ from NHAPHANPHOI where congty_fk = '" + congtyId + "' ) )  ";		
	  	...
	  	sql =  "select a.PK_SEQ as ddhId, a.trangthai, " + 
	  			 ...
	  			 "			 c.MA_FAST as masanphamOLD, c.MA as masanphamNEW, c.TEN as tensanpham, '' SOLO, b.SOLUONG, b.SOLUONGCHUAN as soluong_chuan,   " + 
				 ...
				 " 		ISNULL ( ( select TEN from KENHBANHANG where pk_seq = a.kbh_fk ), '' ) as kbh " +
	  			 ...
	  			 "		left join DAIDIENKINHDOANH ddkd on ddkd.PK_SEQ = b.DDKD_FK     " + 
	  	...
	  	{
	  			 sql += "union ALL " + 
			  			 "	select a.PK_SEQ as ddhId, a.trangthai, " + 
			  			 "			 '' as SOHOADON, a.NgayDonHang, d.TEN as tenkho,    " + 
			  			 "			 case a.LoaiDonHang when 0 then N'Bán hàng NPP' when 1 then N'ETC' when 2 then N'OTC' else N'Nội bộ' end as loaidonhang,   " + 
			  			 "			 case a.LoaiDonHang when 0 then npp.maFAST when 1 then e.maFAST when 2 then e.maFAST else nv.MA end	 as makhachhangOLD,    " + 
			  			 "			 case a.LoaiDonHang when 0 then npp.ma when 1 then e.ma when 2 then e.ma else nv.MA end	 as makhachhangNEW,    " + 
