...
public class ErpHoaDonPheLieuPdfSvl extends HttpServlet
{
	...
	{
		System.out.println(" da vao dayyyyyyyyyyyyyyyyyyyyyyyyy 2");
		...
		IErpHoadonphelieu hdBean = new ErpHoadonphelieu(id);
		...
		String task = request.getParameter("task");
			...
			String INIT_THONGTINKH =
				" select ngayhoadon, a.khachhang_fk, a.trangthai, kyhieuhoadon, sohoadon, HINHTHUCTHANHTOAN , \n"+
		  		"        '' as nguoimua,  isnull(bvat,0) as tongtienbvat,  isnull(avat,0) \n"+
		  		" as tongtienavat,  isnull(a.vat, 0) as vat,  \n"+
				" REPLACE(isnull(d.MASOTHUE,''), ' ', '') as MASOTHUE, \n"+
				" isnull(d.DIACHI,'') as DIACHI, isnull(f.SOTAIKHOAN,'') SOTAIKHOAN \n"+
				...
				"					WHERE B.PK_SEQ = (select KHACHHANG_FK from erp_hoadonphelieu where PK_SEQ = "+hdBean.getId()+"  ) \n"+
		   ...
		   String INIT_SANPHAM =	" select N''+ hdsp.diengiai TEN " +
		   							", ISNULL(donvitinh, '') DONVI, SOLUONG, DONGIA, THANHTIEN  \n"+
									" from erp_hoadonphelieu_sanpham hdsp   \n"+
									" where hoadonphelieu_fk ='"+hdBean.getId()+"' ";		   
		   
		   System.out.println("[INIT_SANPHAM_5]"+INIT_SANPHAM);
		   
			/*if(nppId.equals("106174") ) // QUAY HA DONG 
			{
				this.CreateHd_HADONG(document, outstream, hdBean);
			}
			else if(nppId.equals("106179")) //  HẢI PHÒNG  : từ 02/10 dùng mẫu mới giống Hà Nội
			{
				this.CreateHd_HAIPHONG(document, outstream, hdBean);
			}
			else if(nppId.equals("106162")) //  QUẢNG NINH
			{
				this.CreateHd_QUANGNINH(document, outstream, hdBean); 
			}
			else if(nppId.equals("106231")) //  ĐÀ NẴNG
			{
				this.CreateHd_DANANG(document, outstream, hdBean); 
			}
			else if(nppId.equals("106249")) //  QUẢNG NGÃI
			{
				this.CreateHd_QUANGNGAI(document, outstream, hdBean); 
			}
			else if(nppId.equals("106182")) //  HẢI DƯƠNG
			{
				this.CreateHd_HAIDUONG(document, outstream, hdBean);
			}
			else if(nppId.equals("106250")) //  KHÁNH HÒA 106250
			{
				this.CreateHd_KHANHHOA(document, outstream, hdBean);
			}
			else if(nppId.equals("106170"))  // THANH HOA
			{
				this.CreateHd_THANHHOA(document, outstream, hdBean);
			}
			else if(nppId.equals("106191"))  // PHUTHO
			{
				this.CreateHd_PHUTHO(document, outstream, hdBean);
			}
			else if(nppId.equals("106171"))  // NGHE AN
			{
				this.CreateHd_NGHEAN(document, outstream, hdBean);
			}
...
*/
		    ...
		    //this.CreateHd_CNHOCHIMINH_NEW(document, outstream, hdBean, INIT_THONGTINKH, INIT_SANPHAM, INIT_SANPHAM_KM, INIT_CHIETKHAU_TICHLUY);
		   
		    this.CreateHd_CNHOCHIMINH_OLD(document, outstream, hdBean, INIT_THONGTINKH, INIT_SANPHAM);
						
		
	}

	private void CreateHd_NGHEAN(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{
		   ...
		   if(rsLayKH.next())
		   {
			...
			while(rsSP.next())
			{
				...
				String[] arr = new String[] { Integer.toString(stt), rsSP.getString("MA"), rsSP.getString("TEN"), rsSP.getString("solo"),chuoi, rsSP.getString("DONVI"),
						DinhDangTraphacoERP(formatter.format(soLUONG)), DinhDangTraphacoERP(formatter4.format(dongia)) , DinhDangTraphacoERP(formatter.format(thanhtien)), DinhDangTraphacoERP(formatter.format(vat)), DinhDangTraphacoERP(formatter.format(thueGTGT)), DinhDangTraphacoERP(formatter.format(sotientt)) };
			...
			if(totalCK > 0)
			{
				...
				String [] arr5 = new String[] {Integer.toString(stt+1),"CK" ,"Chiết khấu sản phẩm " ," ","","", DinhDangTraphacoERP(formatter.format(totalCK)),DinhDangTraphacoERP(formatter.format(vatCK)),
						DinhDangTraphacoERP(formatter.format(totalCK*vatCK/100)) ,DinhDangTraphacoERP(formatter.format( totalCK + totalCK*vatCK/100))};
			...
			String[] arr = new String[] {"                             ", DinhDangTraphacoERP(formatter1.format(totalTienTruocVAT -totalCK )),"",DinhDangTraphacoERP( formatter1.format(totalThueGTGT- Math.round(totalCK*vatCK/100) )),DinhDangTraphacoERP(formatter1.format(totalSotienTT - ( totalCK + Math.round(totalCK*vatCK/100) ) )) };
	...
	private void CreateHd_PHUTHO(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{
	...
	private void CreateHd_THANHHOA(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{
	...
	private void CreateHd_HAIPHONG(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		...
		{			
			...
			"	(case when (select isnull(KHO_FK,'0') from KHACHHANG where pk_seq in (select khachhang_fk from ERP_HOADONNPP where pk_seq = '"+ hdBean.getId() +"')) = 100002 then N'Kho sỉ Nhà Bè'" +
			...
			while(rsSP.next())
			{
				...
				if(SoNgay(ngayxuathd)){
					arr = new String[] { Integer.toString(stt), rsSP.getString("MA") , rsSP.getString("TEN"), rsSP.getString("solo"), 
							...
							DinhDangTraphacoERP(formatter1.format(soLUONG)), DinhDangTraphacoERP(formatter4.format(dongia)),DinhDangTraphacoERP(formatter1.format(thanhtien)),DinhDangTraphacoERP(formatter1.format(rsSP.getDouble("thuevat"))), DinhDangTraphacoERP(formatter1.format(thueGTGT)),DinhDangTraphacoERP(formatter1.format(sotientt)) };
				}
				else{
					arr = new String[] { Integer.toString(stt), rsSP.getString("MA") , rsSP.getString("TEN"), rsSP.getString("solo"), 
						chuoi, rsSP.getString("DONVI"),
						DinhDangTraphacoERP(formatter1.format(soLUONG)), DinhDangTraphacoERP(formatter.format(dongia)),DinhDangTraphacoERP(formatter1.format(thanhtien)),DinhDangTraphacoERP(formatter1.format(rsSP.getDouble("thuevat"))), DinhDangTraphacoERP(formatter1.format(thueGTGT)),DinhDangTraphacoERP(formatter1.format(sotientt)) };
				}
		...
		if(SoNgay(ngayxuathd)){
			...
			/*truocVAT = Double.parseDouble(hdBean.getTongtienBVAT().replaceAll(",", ""));
			tienVat = hdBean.getTongVAT();
			sauVat = hdBean.getTongtienAVAT();*/
		}	
		
		String[] arr = new String[] { " ", " " , " ", " "," ", " "," "," ", DinhDangTraphacoERP(formatter1.format(truocVAT)), " " , DinhDangTraphacoERP(tienVat), DinhDangTraphacoERP(sauVat) };
	...
	private void CreateHd_KHANHHOA(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{			
			dbutils db = new dbutils();
		   ...
		   if(rsLayKH.next())
		   {
			...
			while(rsSP.next())
			{
				...
				String[] arr = new String[] { Integer.toString(stt), rsSP.getString("MA"), rsSP.getString("TEN"), rsSP.getString("solo"),
							chuoi, rsSP.getString("DONVI"),
						DinhDangTraphacoERP(formatter.format(soLUONG)), DinhDangTraphacoERP(formatter4.format(dongia)) , DinhDangTraphacoERP(formatter.format(thanhtien)), DinhDangTraphacoERP(formatter.format(rsSP.getDouble("thuevat"))), DinhDangTraphacoERP(formatter.format(thueGTGT)), DinhDangTraphacoERP(formatter.format(sotientt)) };
			...
			String[] arr = new String[] {  " " , " ", " "," ", " "," "," ", DinhDangTraphacoERP(formatter1.format(totalTienTruocVAT -totalCK )),"",DinhDangTraphacoERP( formatter1.format(totalThueGTGT- (totalCK*vatCK/100) )),DinhDangTraphacoERP(formatter1.format(Math.round(totalSotienTT - ( totalCK + totalCK*vatCK/100) ))) };
	...
	private void CreateHd_HAIDUONG(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
	...
	private void CreateHd_CNHOCHIMINH_14P(Document document,ServletOutputStream outstream, IErpHoadonphelieu hdBean) 
	{
		try
		{
	...
	private void CreateHd_CNHOCHIMINH_NEW(Document document,ServletOutputStream outstream, IErpHoadonphelieu hdBean, String INIT_KHACHHANG, String INIT_SANPHAM, String INIT_SANPHAM_KM, String INIT_CHIETKHAU_TICHLUY)
	{
		...
		{			
		   ...
		   if(rsINFO.next())
		   {
			   ...
			   loaixuathd = rsINFO.getInt("LOAIXUATHD");
		   ...
		   BaseFont bf = BaseFont.createFont("c:\\windows\\fonts\\Arial.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
			...
			float[] withsKM = { 0.5f*CONVERT,7.0f*CONVERT, 1.5f*CONVERT, 2.0f*CONVERT, 1.0f*CONVERT, 1.0f*CONVERT, 2.0f*CONVERT,2.0f*CONVERT, 2.0f*CONVERT,1.0f*CONVERT,2.5f*CONVERT, 2.5f*CONVERT };
			sanpham.setWidths(withsKM);
			
			// PHẦN 
			ResultSet rsSP = rsSP= db.get(INIT_SANPHAM);
			...
			if(rsSP!=null){				
				while(rsSP.next()){
	        		...
	        		if(loaixuathd == 0 || loaixuathd == 2 || kbh_fk.equals("100052") ) // OTC || NPP - ĐƠN GIÁ LÀM TRÒN
		        		arr = new String[] { Integer.toString(stt), tensp , solo,handung, DVT,DinhDangTraphacoERP(formatter1.format(soluong)), DinhDangTraphacoERP(formatter2.format(dongia)),
								DinhDangTraphacoERP(formatter2.format(thanhtien)),DinhDangTraphacoERP(formatter2.format(tienck)) , DinhDangTraphacoERP(formatter2.format(thuevat)) ,DinhDangTraphacoERP(formatter2.format(tienvat)), DinhDangTraphacoERP(formatter2.format(tienavat))  };
	        		else 
	        		{
		        		arr = new String[] { Integer.toString(stt), tensp , solo,handung, DVT,DinhDangTraphacoERP(formatter1.format(soluong)), DinhDangTraphacoERP(formatter1.format(dongia)),
								DinhDangTraphacoERP(formatter2.format(thanhtien)),DinhDangTraphacoERP(formatter2.format(tienck)) , DinhDangTraphacoERP(formatter2.format(thuevat)) ,DinhDangTraphacoERP(formatter2.format(tienvat)), DinhDangTraphacoERP(formatter2.format(tienavat))  };
	        		}
				...
				if(isSpKM>0)
				{
					...
					{
						cells = new PdfPCell(new Paragraph(arr_bosung[i], new Font(bf, 8, Font.BOLD)));
						cells.setVerticalAlignment(Element.ALIGN_TOP);
						cells.setHorizontalAlignment(Element.ALIGN_LEFT);
				...
				// HÀNG KHUYẾN MÃI
				ResultSet rsSP_KM = null;
					rsSP_KM= db.get(INIT_SANPHAM_KM);		
				...
				if(rsSP_KM!=null){		
					while(rsSP_KM.next()){
						...
						solo = rsSP_KM.getString("SOLO");
						handung = rsSP_KM.getString("NGAYHETHAN");
		        		...
		        		String[] arr = new String[] { Integer.toString(stt), tensp , solo,handung, DVT,DinhDangTraphacoERP(formatter1.format(soluong)), "-", "-", "-" ,"-", "X", "-"  };
							
		        		for (int m = 0; m < arr.length; m ++){// NẾU LÀ TÊN SẢN PHẨM
							if(m==1)
							{
								if(arr[m].length() <= 40)
									cells = new PdfPCell(new Paragraph(arr[m], new Font(bf, 8, Font.NORMAL)));
				...
				// IN DÒNG CHIẾT KHẤU TÍCH LŨY
				
				ResultSet rsCK_TL = db.get(INIT_CHIETKHAU_TICHLUY);	
				...
				if(rsCK_TL!=null){
					...
					while(rsCK_TL.next()){
						stt++;
						 ...
						 tichluy_vat = rsCK_TL.getDouble("tichluy_vat");
						 tichluy_tienBVAT = rsCK_TL.getDouble("tichluy_tienBVAT");
						 tichluy_tienAVAT = rsCK_TL.getDouble("tichluy_tienAVAT");
						 tichluy_tienvat = rsCK_TL.getDouble("tichluy_tienvat");
						 Sohopdong = rsCK_TL.getString("SOHOPDONG");
						 Ngayhopdong = rsCK_TL.getString("NGAYHOPDONG");
						
						 System.out.println("Ngay hợp đồng:"+Ngayhopdong);
						String[] arr = 	new String[] { "", "Tiền chiết khấu theo HĐTT số: "+Sohopdong+" Ngày: "+Ngayhopdong , "","", "","", "",
										"", DinhDangTraphacoERP(formatter2.format(tichluy_tienBVAT)),DinhDangTraphacoERP(formatter2.format(tichluy_vat)),DinhDangTraphacoERP(formatter2.format(tichluy_tienvat)),  DinhDangTraphacoERP(formatter2.format(tichluy_tienAVAT))  };
						...
						//DÒNG TỔNG CỘNG TIỀN HÀNG
						sum_tienck += tichluy_tienBVAT;
						sum_tienvat -= tichluy_tienvat;
						sum_tienavat -= tichluy_tienAVAT;
						
						//TỔNG CỘNG THUẾ SUẤT 5% - MẶC ĐỊNH TIỀN CHIẾT KHẤU LÀ 5%
						
						if(tichluy_vat == 5)
						{
							//sum_thanhtien_5 -= tichluy_tienBVAT;
							sum_tienck_5 += tichluy_tienBVAT ;
							sum_tienvat_5 -= tichluy_tienvat;
							sum_tienavat_5 -= tichluy_tienAVAT;
						}
						
						else if(tichluy_vat == 10)
						{
							sum_tienck_10 += tichluy_tienBVAT;
							sum_tienvat_10 -= tichluy_tienvat;
							sum_tienavat_10 -= tichluy_tienAVAT;
						}
		...
		String [] arr_tienhang = new String[] {"","" ,"" ,"","","", "",DinhDangTraphacoERP(formatter2.format(sum_thanhtien)),DinhDangTraphacoERP(formatter2.format(sum_tienck)), "",DinhDangTraphacoERP(formatter2.format(sum_tienvat)),DinhDangTraphacoERP(formatter2.format(sum_tienavat)) };
		String [] arr_footer_5 = new String[] {"","" ,"" ,"","","", "",DinhDangTraphacoERP(formatter2.format(sum_thanhtien_5)),DinhDangTraphacoERP(formatter2.format(sum_tienck_5)),"5",DinhDangTraphacoERP(formatter2.format(sum_tienvat_5)),DinhDangTraphacoERP(formatter2.format(sum_tienavat_5)) };
		String [] arr_footer_10 = new String[] {"","" ,"" ,"","","", "",DinhDangTraphacoERP(formatter2.format(sum_thanhtien_10)),DinhDangTraphacoERP(formatter2.format(sum_tienck_10)),"10",DinhDangTraphacoERP(formatter2.format(sum_tienvat_10)),DinhDangTraphacoERP(formatter2.format(sum_tienavat_10)) };
		String [] array = null;
		...
		for(int k =0; k<3; k++){
			...
			{
				cells = new PdfPCell(new Paragraph(array[i], new Font(bf, 8 , Font.BOLD)));
	   ...
	   //Viết hoa ký tự đầu tiên
		...
		cell = new PdfPCell();	
		hd = new Paragraph(tien +"." , new Font(bf, 9, Font.NORMAL));
		hd.setAlignment(Element.ALIGN_LEFT);
		cell.setFixedHeight(0.8f*CONVERT);
		...
		if(inBHDT>=1)
		{
			PdfPTable tablebhdt =new PdfPTable(1);
			tablebhdt.setWidthPercentage(100);			
	
			cell = new PdfPCell();	
			cell.setVerticalAlignment(Element.ALIGN_LEFT);			
			
			hd = new Paragraph("" , new Font(bf, 9, Font.NORMAL));
			hd.setAlignment(Element.ALIGN_LEFT);
			cell.setVerticalAlignment(Element.ALIGN_BOTTOM);
			cell.setFixedHeight(2.2f*CONVERT);
			...
			hd = new Paragraph("Bán hàng qua điện thoại" , new Font(bf, 9, Font.NORMAL));
			hd.setAlignment(Element.ALIGN_LEFT);
			cell.setVerticalAlignment(Element.ALIGN_BOTTOM);
			cell.setFixedHeight(0.8f*CONVERT);
			//cell.setPaddingTop(2.0f*CONVERT);
	...
	private void CreateHd_CNHOCHIMINH_OLD(Document document,ServletOutputStream outstream, IErpHoadonphelieu hdBean, String INIT_KHACHHANG, String INIT_SANPHAM )
	{
		try
		{			
			dbutils db = new dbutils();
			...
			if(rsINFO.next())
			{
			   ...
			   diachi = rsINFO.getString("DIACHI");
			   sotaikhoan = rsINFO.getString("SOTAIKHOAN");
			   HTTT = rsINFO.getString("hinhthucthanhtoan");
			   ngayxuathd = rsINFO.getString("NGAYHOADON");
			   vat = rsINFO.getInt("VAT");
			   tongtienbvat = rsINFO.getDouble("tongtienbvat");
			   tongtienavat = rsINFO.getDouble("tongtienavat");
			   tienthue = tongtienavat - tongtienbvat;
		   ...
		   document.open() ;
				...
				int stt = 0;
				String tensp = "", solo = "", handung = "", DVT = "" ;
				double soluong = 0, dongia = 0, thanhtien = 0, thuevat = 0, tienvat = 0, tienavat = 0, isSpKM = 0 ;
				...
				if(rsSP!=null){				
					while(rsSP.next()){				
		        		...
		        		String[] arr = new String[] { Integer.toString(stt), tensp ,  DVT,DinhDangTraphacoERP(formatter1.format(soluong)), DinhDangTraphacoERP(formatter1.format(dongia)), DinhDangTraphacoERP(formatter1.format(thanhtien))};
				...
				if(tongtienbvat<0) tongtienbvat = tongtienbvat*(-1);
				
				hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(tongtienbvat)) , new Font(bf, 11, Font.NORMAL));
				...
				hd = new Paragraph(vat +"" , new Font(bf, 11, Font.NORMAL));
				hd.setAlignment(Element.ALIGN_LEFT);
				cell.setVerticalAlignment(Element.ALIGN_TOP);
				cell.setFixedHeight(0.8f*CONVERT);	
				...
				hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(tienthue)) , new Font(bf, 11, Font.NORMAL));
				hd.setAlignment(Element.ALIGN_RIGHT);
				cell.setVerticalAlignment(Element.ALIGN_TOP);
				cell.setFixedHeight(0.8f*CONVERT);		
				cell.setPaddingTop(-0.2f*CONVERT);
				...
				hd = new Paragraph(DinhDangTraphacoERP(formatter1.format(tongtienavat)) , new Font(bf, 11, Font.NORMAL));
				hd.setAlignment(Element.ALIGN_RIGHT);
				cell.setVerticalAlignment(Element.ALIGN_TOP);
				cell.setFixedHeight(0.8f*CONVERT);	
				cell.setPaddingTop(-0.5f*CONVERT);
				...
				String sotien = formatter1.format(tongtienavat);
				System.out.println(sotien);
	...
	private void CreateHd_HAIPHONG_OLD(Document document,ServletOutputStream outstream, IErpHoadonphelieu hdBean) 
	{
...
/*			  sql = " select  TEN ,isnull(DIACHI,'') as DIACHI, isnull(MASOTHUE,'') as MASOTHUE  "+
	...
	private void CreateHd_HADONG(Document document,ServletOutputStream outstream, IErpHoadonphelieu hdBean) 
	{
		...
		/*	sql = " select  TEN ,DIACHI, MASOTHUE  "+
		   ...
		   if(rsLayKH.next())
		   {
			...
			while(rsSP.next())
			{
				...
				if(SoNgay(ngayxuathd)){
					arr = new String[] { Integer.toString(stt), rsSP.getString("MA"), rsSP.getString("TEN"), rsSP.getString("solo"),chuoi, rsSP.getString("DONVI"),
							DinhDangTraphacoERP(formatter1.format(soLUONG)), DinhDangTraphacoERP(formatter.format(dongia)) , DinhDangTraphacoERP(formatter1.format(thanhtien)), DinhDangTraphacoERP(formatter1.format(vat)), DinhDangTraphacoERP(formatter1.format(thueGTGT)), DinhDangTraphacoERP(formatter1.format(sotientt)) };
				}
				else{
					arr = new String[] { Integer.toString(stt), rsSP.getString("MA"), rsSP.getString("TEN"), rsSP.getString("solo"),chuoi, rsSP.getString("DONVI"),
						DinhDangTraphacoERP(formatter1.format(soLUONG)), DinhDangTraphacoERP(formatter.format(dongia)) , DinhDangTraphacoERP(formatter1.format(thanhtien)), DinhDangTraphacoERP(formatter1.format(vat)), DinhDangTraphacoERP(formatter1.format(thueGTGT)), DinhDangTraphacoERP(formatter1.format(sotientt+ thueGTGT)) };
				}
			...
			if(totalCK > 0)
			{
				stt= stt-1;
				//CHIẾT KHẤU DÒNG HÀNG (NẾU CÓ) : SUM(CHIETKHAU) CỦA TẤT CẢ CÁC SP
				String [] arr5 = new String[] {Integer.toString(stt+1),"CK" ,"Chiết khấu sản phẩm " ," ","","", DinhDangTraphacoERP(formatter1.format(totalCK)),DinhDangTraphacoERP(formatter1.format(vatCK)),
						DinhDangTraphacoERP(formatter1.format(totalCK*vatCK/100)) ,DinhDangTraphacoERP(formatter1.format( totalCK + totalCK*vatCK/100))};
				for (int j = 0; j < arr5.length; j++)
				{
					cells = new PdfPCell(new Paragraph(arr5[j], new Font(bf, 10 , Font.BOLD)));
					if(j==2)
			...
			// Tong tien thanh toan	
			String[] arr = new String[] {"                             ", DinhDangTraphacoERP(formatter1.format(totalTienTruocVAT - totalCK)),"",DinhDangTraphacoERP(formatter1.format(totalThueGTGT - (totalCK*vatCK/100) )),DinhDangTraphacoERP(formatter1.format(totalTienTruocVAT + totalThueGTGT - totalCK  - (totalCK*vatCK/100) )) };
				for (int j = 0; j < arr.length; j++)
				{
					cells = new PdfPCell(new Paragraph(arr[j], new Font(bf, 10, Font.BOLDITALIC)));
					if (j == 0)
	...
	private void CreateHd_QUANGNINH(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{			
			dbutils db = new dbutils();
	...
	private void CreateHd_DANANG(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{			
			dbutils db = new dbutils();
		...
		sanpham.setWidths(withsKM);
			...
			while(rsSP.next())
			{
				...
				String[] arr = new String[] { Integer.toString(stt), rsSP.getString("TEN"), rsSP.getString("SoLo"), 
						chuoi, rsSP.getString("DONVI"),
						DinhDangTraphacoERP(formatter1.format(soLUONG)), DinhDangTraphacoERP(formatter.format(dongia)),
						DinhDangTraphacoERP(formatter1.format(thanhtien)),DinhDangTraphacoERP(formatter1.format(rsSP.getDouble("thuevat"))),
						DinhDangTraphacoERP(formatter1.format(thueGTGT)),DinhDangTraphacoERP(formatter1.format(sotientt)) };
		...
		// DONG TRONG
			...
			String[] arr = new String[] { " ", " " , " ", " "," ", " "," ", DinhDangTraphacoERP(formatter1.format(totalTienTruocVAT -totalCK )),
					                      " ",DinhDangTraphacoERP( formatter1.format(totalThueGTGT- (totalCK*vatCK/100) )),DinhDangTraphacoERP(formatter1.format(Math.round(totalSotienTT - ( totalCK + totalCK*vatCK/100) ))) };
	...
	private void CreateHd(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{
	 ...
	 * @throws IOException
	 */
	private void CreateHd_QUANGNGAI(Document document, ServletOutputStream outstream, IErpHoadonphelieu hdBean) throws IOException
	{
		try
		{			
			dbutils db = new dbutils();
...
/*	public static void main(String[] arg)
	{
		...
		System.out.println(hd.DinhDangTraphacoERP("12,000.56"));
	}*/
