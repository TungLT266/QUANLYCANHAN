...
public class ErpHoadonchuyenkhoNPPPdfSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpHoadonchuyenkhoNPPPdfSvl()
	{
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (querystring.indexOf("pdf") > 0)
		{
			...
			response.setHeader("Content-Disposition", " inline; filename=ErpHoadonchuyenkhoNPPPdf.pdf");
			...
			String INIT_THONGTINKH =
				"select isnull(c.ChietKhau,0) as ptchietkhau,a.kho_fk,dondathangnpp_fk, a.nhomkenh_fk, a.npp_fk, RIGHT(ngayxuatHD, 8) ngayxuatHD, ISNULL(a.ghichu, '') as GHICHU2, \n"+
				"  		a.khachhang_fk, a.npp_dat_fk, a.trangthai, kyhieu, sohoadon, hinhthuctt , \n"+
				"  		(case when nguoimua is null and a.khachhang_fk is not null then isnull(d.TENNGUOIMUA,'')  \n" +   
				...
				"		(case a.loaixuatHD when 0 then g.DONVI else (case when LEN(isnull(a.TENXUATHD,''))>0 then a.TENXUATHD else isnull(d.DONVI,'') end) end ) as donvi,  \n"+
				"		(case a.loaixuatHD when 0 then REPLACE(isnull(g.MASOTHUE,''), ' ', '') else REPLACE(isnull(d.MASOTHUE,''),' ', '') end ) as MASOTHUE, \n"+
				"		(case a.loaixuatHD when 0 then isnull(g.DIACHI,'') else isnull(d.DIACHI,'') end ) as DIACHI, (select count(distinct(VAT)) from ERP_HOADONNPP_SP hd where hd.HOADON_FK = a.PK_SEQ and LEN(isnull(hd.scheme,'')) <= 0 ) countVAT, \n"+
				...
				"		isnull(f.inBHDT,0) inBHDT, a.NGAYXUATHD, a.LOAIXUATHD, c.KBH_FK , (SELECT COUNT(HOADON_FK) FROM ERP_HOADONNPP_CHIETKHAU PP WHERE PP.HOADON_FK = A.PK_SEQ AND PP.VAT = 5 ) CKTL5,  \n"+
				...
				"					WHERE B.PK_SEQ = (select KHACHHANG_FK from ERP_HOADONNPP where PK_SEQ = "+hdBean.getId()+"  ) AND A.ACTIVE = 1 \n"+
				"					ORDER BY A.PK_SEQ desc \n"+
				"				  ) d on a.khachhang_fk = d.pk_seq \n"+
				"		left join NHAPHANPHOI e on a.npp_dat_fk = e.pk_seq \n"+
				"		left join ( SELECT top(1) NPP_FK  pk_seq, A.TENNGUOIMUA, A.DONVI, A.DIACHI, A.MASOTHUE, '' CHUCUAHIEU, B.TEN \n"+
				"		FROM NPP_THONGTINHOADON A INNER JOIN NHAPHANPHOI B ON A.NPP_FK = B.PK_SEQ \n"+
				"		WHERE B.PK_SEQ = (select NPP_DAT_FK from ERP_HOADONNPP where PK_SEQ = "+hdBean.getId()+"   ) AND A.ACTIVE = 1 \n"+
				"		ORDER BY A.PK_SEQ desc \n"+
				"	  ) g on a.npp_dat_fk = g.pk_seq \n"+
...
/*		  String INIT_SANPHAM =
		  ...
		  String INIT_SANPHAM =		  
				   "select  hoadon_fk, MA, TEN,SOLO, DONVI, SOLUONG, ROUND(ISNULL(DONGIA, DONGIA_CHUAN),0) DONGIA, round(SOLUONG*ROUND(ISNULL(DONGIA, DONGIA_CHUAN),0),0) THANHTIEN , THUEVAT, \n"+
				   "		round( ( round(SOLUONG*ROUND(ISNULL(DONGIA, DONGIA_CHUAN),0),0) - round(isnull(CHIETKHAU,0),0) ) * THUEVAT /100,0) TIENVAT, "+
				   ...
				   " 		RIGHT(CAST(DATEPART(YY, NGAYHETHAN ) AS NVARCHAR(50)),2) NGAYHETHAN \n"+
				   "from ERP_HOADONNPP_SP_CHITIET \n"+ 
				   "WHERE hoadon_fk = '"+ hdBean.getId()  +"' AND isnull(SOLUONG,0) > 0 ";
		  		  
		   System.out.println(INIT_SANPHAM);
...
*/
	...
	private void CreateHd_CNHOCHIMINH_NEW(Document document,ServletOutputStream outstream, IErpHoadontaichinhNPP hdBean, String INIT_KHACHHANG, String INIT_SANPHAM, String INIT_SANPHAM_KM, String INIT_CHIETKHAU_TICHLUY)
	{
	...
	private void CreateHd_CNHOCHIMINH_OLD(Document document,ServletOutputStream outstream, IErpHoadontaichinhNPP hdBean, String INIT_KHACHHANG, String INIT_SANPHAM )
	{
		try
		{			
			dbutils db = new dbutils();
			...
			if(rsINFO.next())
			{
			   ...
			   inBHDT =  rsINFO.getInt("inBHDT");
			   ngayxuathd = rsINFO.getString("NGAYXUATHD");
			   ghichu = rsINFO.getString("GHICHU2");
			}
			rsINFO.close();	
		   ...
		   document.open() ;
			...
			if(rsSP!=null){				
				while(rsSP.next()){				
					stt++;
					tensp = rsSP.getString("TEN")+", Số lô: "+rsSP.getString("SOLO");
					DVT = rsSP.getString("DONVI");
					soluong = rsSP.getDouble("SOLUONG");
					dongia = rsSP.getDouble("DONGIA");
					thanhtien = rsSP.getDouble("THANHTIEN");
			...
			{
				String[] arr = new String[] { "", "Lý do: "+ ghichu , "" , "" , "" , ""};
			...
			hd = new Paragraph("0" , new Font(bf, 11, Font.NORMAL));
			hd.setAlignment(Element.ALIGN_LEFT);
			cell.setVerticalAlignment(Element.ALIGN_TOP);
			cell.setFixedHeight(0.8f*CONVERT);	
