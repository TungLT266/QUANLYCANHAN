...
@WebServlet("/ErpChuyenKhoNoiBoSvl")
public class ErpChuyenKhoNoiBoSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;
	
	public ErpChuyenKhoNoiBoSvl()
	{
		super();
...
//		
	...
	private String Chot(String id, String userId)
	{
		...
		try
		{
			...
			" 	where chuyenkho_fk = '" + id + "' and a.loaichuyenkho = '1' " + 
			...
			if (dungchungKENH.equals("1"))
				sqlKENH = " 100000 as nhomkenh_fk "; // LAY KENH OTC
			else
				sqlKENH = " b.nhomkenh_fk ";
			
			// GIAM KHO XUAT
//			query = 
//					" update NHAPP_KHO_CHITIET set kho.SOLUONG = kho.SOLUONG - CT.tongxuat, " + 
//					" 			   kho.BOOKED = kho.BOOKED - CT.tongxuat " + " from " + 
//					" ( " + 
//					" 	select a.khoxuat_fk as kho_fk, a.tructhuoc_fk as npp_fk, " + sqlKENH + ", b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan  " + 
//					" 	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " + 
//					" 	where chuyenkho_fk = '" + id + "' " + 
//					" 	group by a.khoxuat_fk, a.tructhuoc_fk, b.nhomkenh_fk, b.solo, b.sanpham_fk,b.NgayHetHan " + " ) " + 
//					" CT inner join NHAPP_KHO_CHITIET kho on CT.kho_fk = kho.KHO_FK  " + 
//					" 	and CT.sanpham_fk = kho.SANPHAM_FK and CT.solo = kho.SOLO and CT.NPP_FK = kho.npp_fk and CT.nhomkenh_FK = kho.nhomkenh_fk and kho.NgayHetHan=ct.NgayHetHan  ";
//			
//			
//			
//			System.out.println("---TANG KHO NGUOC LAI: " + query);
...
//				" 	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " + 
//				" 	where chuyenkho_fk = '" + id + "' " + 
//				" 	group by a.khoxuat_fk, a.tructhuoc_fk, b.nhomkenh_fk, b.sanpham_fk " + " ) " + 
//				" CT inner join NHAPP_KHO kho on CT.kho_fk = kho.KHO_FK  " + 
...
//			}
			...
			query= 
					" select a.Ngaychuyen,a.khoxuat_fk as kho_fk, a.tructhuoc_fk as npp_fk, " + sqlKENH + " , b.sanpham_fk, b.solo, SUM(b.soluong) as tongxuat,b.NgayHetHan  \n"+
					"	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk 	\n"+
					" 	where chuyenkho_fk = '" + id + "' " + 
					"	group by a.khoxuat_fk, a.tructhuoc_fk, b.nhomkenh_fk, b.solo, b.sanpham_fk,b.NgayHetHan ";
			ResultSet rs1 = db.get(query);
			{
				while(rs1.next())
				{
					String khoxuat_fk=  rs1.getString("kho_fk");// kho chuyen
					String ddkd_fk = rs1.getString("ddkd_fk");
					String npp_fk = rs1.getString("npp_fk");
					String nhomkenhID = rs1.getString("nhomkenh_fk");
					String spID = rs1.getString("sanpham_fk");
					String solo = rs1.getString("solo");
					double dongia = rs1.getDouble("dongia");
					String ngayhethan = rs1.getString("ngayhethan");
					double tongnhan = rs1.getDouble("tongnhan");
					String ngaychuyen=rs1.getString("Ngaychuyen");

						tongnhan=(-1)*tongnhan;
						
						msg=util_kho.Update_NPP_Kho_Sp_Chitiet(ngaychuyen, "Chuyển kho TDV", db, khoxuat_fk, spID, npp_fk, nhomkenhID, solo, "NULL", ngayhethan, "NULL", tongnhan, tongnhan, 0.0, dongia);
						...
						msg=util_kho.Update_NPP_Kho_Sp(ngaychuyen, "Chuyển kho TDV", db, khoxuat_fk, spID, npp_fk, nhomkenhID, tongnhan, tongnhan, 0.0, dongia);
						if(msg.length()>0 )
						{
			...
			query = " 	select a.Ngaychuyen,a.khoxuat_fk as kho_fk,c.ngaysanxuat,c.dongia, a.npp_fk,a.ddkd_fk,b.nhomkenh_fk, b.sanpham_fk, b.solo, b.ngayhethan, SUM(b.soluong) as tongnhan  " +
					" 	from ERP_CHUYENKHO a inner join ERP_CHUYENKHO_SANPHAM_CHITIET b on a.pk_seq = b.chuyenkho_fk " +
					"   inner join ERP_CHUYENKHO_SANPHAM c on a.pk_seq = c.chuyenkho_fk " +
					" 	where chuyenkho_fk = '" + id + "' " +
					" 	group by a.khoxuat_fk, a.npp_fk,a.ddkd_fk, b.solo,b.nhomkenh_fk, b.ngayhethan, b.sanpham_fk ";
			System.out.println("---Chuyen sang kho DDKD: " + query);
			rs = db.get(query);
			String nppID = "";
			String ddkdID = "";
			{
				while(rs.next())
				{
					String khoID = "100008";  // kho chuyen
					ddkdID = rs.getString("ddkd_fk");
...
//					query = " select count(*) as count from NHAPP_KHO_DDKD_CHITIET " +
//							"where npp_fk = '"+nppID+"' and  DDKD_FK = '" + ddkdID + "' and nhomkenh_fk = '" + nhomkenhID + "' and KHO_FK = '" + khoID + "' and SANPHAM_FK = '" + spID + "' and SOLO = '" + solo + "' and NgayHetHan='"+ngayhethan+"' ";
//					int count = 0;
//			
//					ResultSet rsCHECK = db.get(query);
//					System.out.println("RSCheck "+query);
//					if(rsCHECK.next())
//					{
...
//					if(count > 0)
//					{
//						query = "Update NHAPP_KHO_DDKD_CHITIET set soluong  = soluong + '" + tongnhan + "', available = available + '" + tongnhan + "' " +
//								"where npp_fk = '"+nppID+"' and  ddkd_FK = '" + ddkdID + "' and nhomkenh_FK = '" + nhomkenhID + "' and KHO_FK = '" + khoID + "' and SANPHAM_FK = '" + spID + "' and SOLO = '" + solo + "' and NgayHetHan='"+ngayhethan+"' ";
...
//					else
//					{
//						query = "insert NHAPP_KHO_DDKD_CHITIET(KHO_FK,npp_fk, ddkd_FK, SANPHAM_FK, SOLUONG, BOOKED, AVAILABLE, nhomkenh_FK, SOLO, NGAYHETHAN) " +
//								"values('" + khoID + "','" + nppID + "','" + ddkdID + "', '" + spID + "', '" + tongnhan + "', '0', '" + tongnhan + "', '" + nhomkenhID + "', '" + solo + "', '" + ngayhethan + "')";
//					
//					}
					msg=util_kho.Update_NPP_Kho_Sp_Chitiet_DDKD(ngaychuyen, "Chuyển kho TVD", db, khoID, spID, nppID, nhomkenhID, ddkdID, solo, "NULL", ngayhethan, "NULL", tongnhan, 0.0, tongnhan, dongia);
					System.out.println("---CAP NHAT KHO NHAPP_KHO_DDKD_CHITIET: " + query);
					if(msg.length()>0 )
					{
...
//					query = " select count(*) as count from NHAPP_KHO_DDKD " +
//							"where npp_fk = '"+nppID+"' and  DDKD_FK = '" + ddkdID + "' and nhomkenh_fk = '" + nhomkenhID + "' and KHO_FK = '" + khoID + "' and SANPHAM_FK = '" + spID +"' ";
//					 count = 0;
...
//					if(count > 0)
//					{
//						query = "Update NHAPP_KHO_DDKD set soluong  = soluong + '" + tongnhan + "', available = available + '" + tongnhan + "' " +
//								"where npp_fk = '"+nppID+"' and  ddkd_FK = '" + ddkdID + "' and nhomkenh_FK = '" + nhomkenhID + "' and KHO_FK = '" + khoID + "' and SANPHAM_FK = '" + spID + "'  ";
						
//					}
//					else
//					{
//						query = "insert NHAPP_KHO_DDKD(KHO_FK,npp_fk,ddkd_FK, SANPHAM_FK, SOLUONG, BOOKED, AVAILABLE, nhomkenh_FK ) " +
//								"values('" + khoID + "', '" + nppID + "','" + ddkdID + "', '" + spID + "', '" + tongnhan + "', '0', '" + tongnhan + "', '" + nhomkenhID + "')";
					
//					}
					msg=util_kho.Update_NPP_Kho_Sp_DDKD(ngaychuyen, "Chuyển kho TDV", db, khoID, spID, nppID, nhomkenhID, tongnhan, 0.0, tongnhan, dongia, ddkdID);
					System.out.println("---CAP NHAT KHO NHAPP_KHO_DDKD: " + query);
					if(msg.length()>0)
					{
...
//			if (db.updateReturnInt(query) <= 0)
//			{
//				msg = "Không thể cập nhật NHAPP_KHO_DDKD " + query;
//				System.out.println(msg);
//				db.getConnection().rollback();
//				return msg;
//			}
			...
			query = "update ERP_CHUYENKHO set trangthai = '1',Modified_Date=getdate(),NguoiSua='"+userId+"', loaichuyenkho = '1' where pk_seq = '" + id + "'  ";
			if (db.updateReturnInt(query) != 1)
			{
				msg = "Phiếu chuyển kho này đã chốt rồi !" + query;
				db.getConnection().rollback();
...
//			}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		{
			IErpChuyenKhoNoiBo lsxBean = new ErpChuyenKhoNoiBo();
	...
	private String getSearchQuery(HttpServletRequest request, IErpChuyenKhonoiboList obj)
	{
		String query = "select a.PK_SEQ, a.trangthai, isnull( a.sochungtu,'')as SOCHUNGTU ,a.ngaychuyen, a.ghichu as lydo, NV.TEN as nguoitao, b.ten as khoxuat,c.ten as nppTEN, a.NGAYSUA, a.NGAYTAO, NV2.TEN as nguoisua,isnull(c.ten,'') as tentdv   " + "from ERP_CHUYENKHO a inner join KHO b on a.khoxuat_fk = b.pk_seq inner join DAIDIENKINHDOANH c on a.npp_fk = c.pk_seq  " + "inner join NHANVIEN nv on a.NGUOITAO = nv.PK_SEQ   " + "inner join NHANVIEN nv2 on a.NGUOISUA = nv2.PK_SEQ where a.pk_seq > 0 ";
