package geso.traphaco.distributor.servlets.tratichluy;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(!cutil.check(userId, userTen, sum)){
	    	...
	    	if(action.equals("duyet"))
	    		response.sendRedirect("/TraphacoHYERP/pages/Distributor/TraTichLuyUpdate.jsp");
	    	else if(action.equals("dislay")){
	    		response.sendRedirect("/TraphacoHYERP/pages/Distributor/TraTichLuyDislay.jsp");
	    	}
	    }
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(!util.check(userId, userTen, sum)){
			...
			if ((contentType != null) && (contentType.indexOf("multipart/form-data") >= 0))
			{
				...
				if( id == null )
					response.sendRedirect("/TraphacoHYERP/pages/Distributor/TraTichLuyNew.jsp");
				else
					response.sendRedirect("/TraphacoHYERP/pages/Distributor/TraTichLuyUpdate.jsp");
			} 
			else
			{
				...
				String[] khDoanhsoDAT = request.getParameterValues("khDoanhsoDAT");
				obj.setKhDoanhsoDAT(khDoanhsoDAT);
				
				String[] khHinhthuctra = request.getParameterValues("khHinhthuctra");
				obj.setKhHinhthuctra(khHinhthuctra);
				
				if( khMa != null  )
				{
					Hashtable<String, String> dsdat_upload = new Hashtable<String, String>();
					for(int i = 0; i < khMa.length; i++ )
					{
						dsdat_upload.put(khMa[i], khDoanhsoDAT[i]);
					}
					obj.setDoanhsoDat_Upload(dsdat_upload);
				}
				...
				else if(action.equals("save"))
				{
					if( id.trim().length() > 0 )
					{
						if(obj.LuuDuyetTra()){
							...
							String nextJSP = "/TraphacoHYERP/pages/Distributor/TraTichLuy.jsp";
	...
	private String readExcel(String fileName, IDangkykhuyenmaitichluy obj) 
	{
		try
		{
			...
			for (int i = 4; i < sodong; i++)
			{
				...
				String _khDoanhsoDAT = "0";
				String _khHinhthuctra = getValue(sheet, 8, i).trim().replaceAll(",", "").length() <= 0 ? " " : getValue(sheet, 8, i).trim();
				...
				khDoanhsoDAT += _khDoanhsoDAT + "__";
				khHinhthuctra += _khHinhthuctra + "__";
			...
			if(khMA.trim().length() > 0)
			{
				...
				obj.setKhDoanhsoDAT(khDoanhsoDAT.split("__"));
				
				khHinhthuctra = khHinhthuctra.substring(0, khHinhthuctra.length() - 2);
				obj.setKhHinhthuctra(khHinhthuctra.split("__"));
			}
		} 
		catch (Exception e)
		{
