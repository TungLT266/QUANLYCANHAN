package geso.traphaco.distributor.servlets.inchietkhautruckthang;


import geso.traphaco.center.db.sql.dbutils;
import geso.traphaco.distributor.beans.inchietkhautruckthang.imp.InKhauTruCKThang;
import geso.traphaco.distributor.beans.inchietkhautruckthang.IInKhauTruCKThang;
      ...
      protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
			{
				...
				if(!type.equals("BANGKE_BBBT"))
				{
					if(!type.equals("BTCN"))
					{
						...
						if(!view.equals("TT"))
						{
							String	nextJSP = "/TraphacoHYERP/pages/Distributor/InChietKhauTruCKThang.jsp";
							response.sendRedirect(nextJSP);
						}
						else
						{
							String	nextJSP = "/TraphacoHYERP/pages/Center/InChietKhauTruCKThang.jsp";
							response.sendRedirect(nextJSP);
						}
					}
					else
				...
				else
				{
					...
					String	nextJSP = "/TraphacoHYERP/pages/Distributor/BangKeBuTruCK.jsp";
					response.sendRedirect(nextJSP);	
				}
			}
					...
					if(khIds != null)
					{
						...
						for(int i = 0; i < khIds.length; i++)
						{
							...
							/*if( tongTHUVEThangtruoc > 0 )*/
							{
								...
								// BÊN A: CHI NHÁNH CÔNG TY CP TRAPHACO TẠI ...
				...
				{	
					...
					if(khId.trim().length() > 0 )
					{
						...
						}else 
						{
							...
							int i=0;
							query=
									"select thanhtoan as ck,round((thanhtoan/6.0)*100,0) as DoanhThu,6 as pt_chietkhau,DATEPART(MONTH,DATEADD(month,-1,isnull(NgayThang+'-01','"+ngayxuatHD+"'))) as Thang  " +
											"	from DUYETTRAKHUYENMAI_DONHANG where donhang_fk='"+dhId+"' and tichluyQUY=0 and ROUND(thanhtoan,0)>0 order by NgayThang asc ";
							rs=db.get(query);
							while(rs.next())
							{
								pt_chietkhau = rs.getDouble("pt_chietkhau");
								totalCK =totalCK+rs.getDouble("ck");
