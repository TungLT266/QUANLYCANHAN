...
public class ErpDoiquycachList extends Phan_Trang implements IErpDoiquycachList
{
	...
	public void init(String search)
	{
		...
		this.nhanvienRs= db.get(" select PK_SEQ, ten from NHANVIEN WHERE PK_SEQ IN (SELECT distinct(NGUOITAO) FROM ERP_CHUYENKHO ) ");
        this.nhanvien2Rs= db.get(" select PK_SEQ, ten from NHANVIEN WHERE PK_SEQ IN (SELECT distinct(NGUOITAO) FROM ERP_CHUYENKHO ) ");
		
		if(search.length() > 0)
			query = search;
		else
		{
			query = 	" SELECT A.PK_SEQ, A.TRANGTHAI, A.ngaydoi, KHOTT.TEN as khochuyen, KHOTT2.TEN as khonhan, " +
