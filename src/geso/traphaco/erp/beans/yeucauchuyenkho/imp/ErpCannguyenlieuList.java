...
public class ErpCannguyenlieuList extends Phan_Trang implements IErpCannguyenlieuList
{
	...
	public void init(String search)
	{
		...
		this.nhanvienRs= db.get(" select PK_SEQ, ten from NHANVIEN WHERE PK_SEQ IN (SELECT distinct(NGUOITAO) FROM ERP_CANNGUYENLIEU ) ");
        this.nhanvien2Rs= db.get(" select PK_SEQ, ten from NHANVIEN WHERE PK_SEQ IN (SELECT distinct(NGUOITAO) FROM ERP_CANNGUYENLIEU ) ");
		
		if(search.length() > 0)
			query = search;
		else
		{
			query = 	" SELECT A.PK_SEQ, A.TRANGTHAI, A.ngaycan, KHOTT.TEN as khoTen, SP.TEN as spTEN, A.SOLO, " +
		...
		//Phân quyền kho
		query += " and A.khott_fk in " + util.quyen_khott(this.userId) ;
