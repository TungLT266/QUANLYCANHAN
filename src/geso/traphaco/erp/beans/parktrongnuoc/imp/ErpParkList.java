...
public class ErpParkList extends Phan_Trang implements IErpParkList
{
	...
	String ID,NCC, NCCID,LOAIHANG,NGAYGHINHAN,USERID,TRANGTHAI,MSG ,NGUOITAO , SOHOADON, sonhanhang, congtyId, nppdangnhap, loaihd,ngaytao;
	int DUYET;
	String donMuaHang;
...
//        			String thangNV = psktRs.getString("ngayghinhan").substring(5, 7);
                	...
                	if(sotienBVAT>0)
                	{
                		...
                		laytk +=
                   		 " SELECT N'NỢ' NO_CO, '" + id + "' PK_SEQ, '"+TAIKHOANNO_TIENHANG+"' SOHIEUTAIKHOAN, "+sotienBVAT+" SOTIEN, '"+NO_TENDOITUONG+"' DOITUONG, '' TRUNGTAMCHIPHI, '' TRUNGTAMDOANHTHU, "+i+" STT, 1 SAPXEP \n"+
                   		 " from ERP_PARK a  \n"+
                   		 " WHERE a.PK_SEQ = '"+id+"'" +
                   		 
                   		 " UNION ALL \n"+
                   		 
                   		 " SELECT N'CÓ' NO_CO, '" + id + "' PK_SEQ, (SELECT SOHIEUTAIKHOAN FROM ERP_TAIKHOANKT WHERE PK_SEQ = '"+TAIKHOANCO_TIENHANG+"' ) SOHIEUTAIKHOAN, "+sotienBVAT+" SOTIEN, '"+CO_TENDOITUONG+"' DOITUONG, '' TRUNGTAMCHIPHI, '' TRUNGTAMDOANHTHU, "+i+" STT, 2 SAPXEP \n"+
                   		 " from ERP_PARK a \n"+
                   		 " WHERE a.PK_SEQ = '"+id+"'" ;
                		i++;
                	}
                	...
                	if(VAT>0)
                	{
                		...
                		if(laytk.trim().length()>0) laytk+= " UNION ALL ";
                		laytk +=
                   		 " SELECT N'NỢ' NO_CO, '" + id + "' PK_SEQ, (SELECT SOHIEUTAIKHOAN FROM ERP_TAIKHOANKT WHERE PK_SEQ ='"+TAIKHOANNO_VAT+"' ) SOHIEUTAIKHOAN, "+VAT+" SOTIEN, '' DOITUONG, '' TRUNGTAMCHIPHI, '' TRUNGTAMDOANHTHU, "+i+" STT, 1 SAPXEP \n"+
                   		 " from ERP_PARK a  \n"+
                   		 " WHERE a.PK_SEQ = '"+id+"'" +
                   		 
                   		 " UNION ALL \n"+
                   		 
                   		 " SELECT N'CÓ' NO_CO, '" + id + "' PK_SEQ, (SELECT SOHIEUTAIKHOAN FROM ERP_TAIKHOANKT WHERE PK_SEQ = '"+TAIKHOANCO_VAT+"' ) SOHIEUTAIKHOAN, "+VAT+" SOTIEN, '"+CO_TENDOITUONG+"' DOITUONG, '' TRUNGTAMCHIPHI, '' TRUNGTAMDOANHTHU, "+i+" STT, 2 SAPXEP \n"+
                   		 " from ERP_PARK a \n"+
                   		 " WHERE a.PK_SEQ = '"+id+"'" ;
                		i++;
                	}
	...
	public void init(String query) 
	{
		String sql = "";
		this.getNppInfo();
		System.out.println("hoa don vd");
		if(query.length() <= 0)
		{
			
			sql =   " SELECT distinct A.PK_SEQ , A.DIENGIAI, A.NGAYGHINHAN, NCC.TEN AS TENNHACUNGCAP, A.TRANGTHAI ,     	NV.TEN AS TENNV,A.NGAYSUA,A.NGAYTAO,NV.PK_SEQ AS MANV,NV2.TEN AS \n" +  
					...
					"        HDNCC. PK_SEQ HDNCCID, HDNCC.KHOTONTRU_FK,\n"+
					...
					" left join ERP_HOADONNCC_DONMUAHANG HDNCC_DMH on HDNCC.pk_seq = HDNCC_DMH.HOADONNCC_FK" +
					" WHERE 1=1 ";
			...
			if (this.NCCID.length() > 0)
				sql += " and a.ncc_fk = '" + this.NCCID + "'";

			if (this.LOAIHANG.length() > 0)
				sql += " and a.loaihanghoa_fk = '" + this.LOAIHANG + "'";
			if (this.ID.length() > 0)
				sql += " and cast(a.PREFIX as varchar(10))+ cast(a.pk_seq as varchar(10))  like '%" + this.ID + "%'";
			...
			if (this.NGUOITAO.length() > 0)
				sql += " and NV.TEN like N'%" + this.NGUOITAO + "%'";

			if (sonhanhang.length() > 0) {
				sql += "  and  HDNCC.pk_seq in ( select a.pk_seq from ERP_HOADONNCC a inner join ERP_HOADONNCC_DONMUAHANG  b on b.hoadonncc_fk= a.pk_seq"
						+ "	inner join ERP_NHAPKHONHAPKHAU c on b.MUAHANG_FK = c.PK_SEQ " + "  where c.PK_SEQ = N'"
						+ sonhanhang + "' and a.CONGTY_FK = " + this.congtyId + " ) ";
			}
			
			if(donMuaHang.length() >0){
				sql += " and HDNCC_DMH.muahang_fk in( select PK_SEQ from ERP_MUAHANG where sopo like '%"+donMuaHang+"%') ";
			}
			
			if(this.ngaytao.length()>0){
				sql += " and a.ngaytao='"+this.ngaytao+"' ";
			}
			...
			if(tungay.length()>0)
			{
				sql+=" and a.ngayghinhan>='"+this.tungay+"'";
			}
			if(denngay.length()>0)
			{
			...
			if(khoId.length()>0){
				sql+=" and HDNCC.KHOTONTRU_FK='"+this.khoId+"'";
			}
			
			if(this.ThanhphamId.length()>0){
				sql+=" and  A.DIENGIAI like N'%"+this.ThanhphamId+"%'";
			}
			
			if(this.soloId.length()>0){
				sql+=" and  A.DIENGIAI like N'%"+this.soloId+"%'";
			}
			...
			sql+= " AND HDNCC.CONGTY_FK ="+this.congtyId+" AND A.LOAIHD = 1 ";
		}
		
		else
			sql = query;		
		
		System.out.println("hoa don trong nuoc "+sql);
		
		//Cat chuoi diengiai lay thanhpham Trang Be
		String RsThanhPham=
 "   select ltrim(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(diengiai)))-len(SUBSTRING(diengiai,CHARINDEX(N'- Số lô',diengiai),len(diengiai)))))as diengiai_chuoi \n"+ 
 "    from ERP_PARK where ltrim(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(diengiai)))-len(SUBSTRING(diengiai,CHARINDEX(N'- Số lô',diengiai),len(diengiai)))))!='' \n"+ 
 "    group by ltrim(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(SUBSTRING(diengiai,CHARINDEX(N'Sản phẩm :',diengiai),len(diengiai)))-len(SUBSTRING(diengiai,CHARINDEX(N'- Số lô',diengiai),len(diengiai)))))";
		System.out.println("thnhpham : "+ RsThanhPham);
		this.Thanhpham=db.get(RsThanhPham);
		
		
		//cat chuoi Solo
		String RsSolo=" select ltrim(SUBSTRING(diengiai,CHARINDEX(N'Số lô: ',diengiai),len(diengiai)))as diengiai \n"+ 
		 "    from ERP_PARK where ltrim(SUBSTRING(diengiai,CHARINDEX(N'Số lô: ',diengiai),len(diengiai)))!='' \n"+ 
		 "    group by ltrim(SUBSTRING(diengiai,CHARINDEX(N'Số lô: ',diengiai),len(diengiai)))";
		System.out.println("Solo : "+ RsSolo);
		this.Solo=db.get(RsSolo);
		
		this.Kho = db.get(" select PK_SEQ, MA+'-'+TEN as TEN from ERP_KHOTT where trangthai ='1' ");
		// Phân quyền nhân viên theo phòng ban, mặc định admin được nhìn thấy
		
		if( !this.USERID.equals("100002") && util.GetquyenNew("ErpParkHoadontrongnuocSvl","",USERID)[9]==0){
			sql += " and NV.PHONGBAN_FK in ( select  " +
					" distinct PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.USERID+" )";
		}
		
		if( !this.USERID.equals("100002") && checkTruongPhong() == false && util.GetquyenNew("ErpParkHoadontrongnuocSvl","",USERID)[9]==0){
			...
			if(rs!= null)
			{
				...
				while(rs.next())
				{
					...
					// INIT
						...
						ht.setloaihanghoaId(rs.getString("LOAIHH"));
						ht.setGhichu(rs.getString("DIENGIAI"));
	...
	public String checkSoHoaDon(String parkId) {
		...
		String query = "\n 	SELECT count(hd.pk_seq) dem FROM ERP_HOADONNCC hd inner join ERP_PARK p on hd.PARK_FK = p.PK_SEQ" +
					   ...
					   "\n	WHERE A.PARK_fk = "+parkId+" AND B.TRANGTHAI NOT IN (4) )"+
					   "\n	and hd.sotienAVAT = (SELECT a.sotienAVAT FROM ERP_HOADONNCC A INNER JOIN ERP_PARK B ON A.PARK_FK = B.PK_SEQ "+ 
					   "\n	WHERE A.PARK_fk = "+parkId+" AND B.TRANGTHAI NOT IN (4) )"+
					   "\n  and hd.trangthai not in (4) ";
