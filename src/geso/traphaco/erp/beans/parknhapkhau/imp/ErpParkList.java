...
import geso.traphaco.erp.beans.parknhapkhau.IErpParkList;
...
public class ErpParkList extends Phan_Trang implements IErpParkList
{
	...
	String ID,NCC, NCCID,LOAIHANG,NGAYGHINHAN,USERID,TRANGTHAI,MSG ,NGUOITAO , SOHOADON, sonhanhang, congtyId, nppdangnhap, loaihd;
	int DUYET;
...
//        			String thangNV = psktRs.getString("ngayghinhan").substring(5, 7);
	...
	public void init(String query) 
	{
		...
		if(query.length() <= 0)
		{
			...
			sql =   " SELECT  DISTINCT  A.PK_SEQ , A.NGAYGHINHAN, NCC.TEN AS TENNHACUNGCAP, A.TRANGTHAI ,     	NV.TEN AS TENNV,A.NGAYSUA,A.NGAYTAO,NV.PK_SEQ AS MANV,NV2.TEN AS \n" +  
			...
			if(sonhanhang.length() > 0)
			{
				sql += "  and  HDNCC.pk_seq in ( select a.pk_seq from ERP_HOADONNCC a inner join ERP_HOADONNCC_DONMUAHANG  b on b.hoadonncc_fk= a.pk_seq" +
						"	inner join ERP_NHAPKHONHAPKHAU c on b.MUAHANG_FK = c.PK_SEQ " +
						 "  where c.PK_SEQ = N'"+sonhanhang+"' and a.CONGTY_FK = "+this.congtyId+" ) ";			
			}
			
			if(donMuaHang.length() >0){
				sql += " and HDNCC_DMH.muahang_fk in( select PK_SEQ from ERP_MUAHANG where sopo in( " +
				" select  mh.SOPO  from ERP_MUAHANG  mh " +
		 		 " inner join ERP_DUYETMUAHANG dmh on mh.PK_SEQ = dmh.MUAHANG_FK " +
		 		 " where  (mh.SOPO like ('%"+this.donMuaHang+"%'))"+
						")) ";
			}
		...
		sql+=   " AND HDNCC.PK_SEQ IN ( " +
				" SELECT A.HOADONNCC_FK FROM ERP_HOADONNCC_DONMUAHANG  A " +
				" INNER JOIN ERP_MUAHANG B ON A.MUAHANG_FK=B.pk_seq AND B.LOAI='0'  ) ";
				
				
		System.out.println("userId"+ this.USERID);
		
		// Phân quyền nhân viên theo phòng ban, mặc định admin được nhìn thấy
		if( !this.USERID.equals("100002") && !isUserDuyet() && util.GetquyenNew("ErpParkHoadonnhapkhauSvl", "",USERID)[9]==0){
			sql += " and NV.PHONGBAN_FK in ( select  " +
					" distinct PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.USERID+" )";
		}
		
		if(!this.USERID.equals("100002") && checkTruongPhong() == false && !isUserDuyet() && util.GetquyenNew("ErpParkHoadonnhapkhauSvl", "",USERID)[9]==0){
			sql += " and a.NGUOITAO = "+ this.USERID;
		}
		
		
		System.out.println("QUERY LOC : "+sql);
	...
	public String Unchot(String parkId, String userId) {
		...
		if(dem>0)
			...
			if(db.updateReturnInt("update ERP_PARK set trangthai = '0', nguoisua = "+userId+" where pk_seq = '" + parkId + "' and trangthai=1")!=1){
				msg = "Vui lòng kiểm tra trạng thái, Bạn không thể bỏ chốt!";
			}
