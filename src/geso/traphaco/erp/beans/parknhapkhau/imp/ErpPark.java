package geso.traphaco.erp.beans.parknhapkhau.imp;
...
import geso.traphaco.erp.beans.parknhapkhau.IErpHoadon;
import geso.traphaco.erp.beans.parknhapkhau.IErpHoadonSp;
import geso.traphaco.erp.beans.parknhapkhau.IErpPark;
...
public class ErpPark implements IErpPark
{
	...
	List<IErpChuyenkho> phieuCKList;
	int isDisplay;
	//Kiem tra xem nguoi dung co vao chuc nang "Duyet hoa don ncc" va xem hoa don
	int duyet;
	String ChenhlechHd;
...
//		String today = sdf.format(date);
		...
		this.getNppInfo();
		this.ttRs = this.db.get("SELECT PK_SEQ AS TTID, MA + ' - ' + TEN AS TIENTE FROM ERP_TIENTE ");
		...
		this.nccRs = db.get("select PK_SEQ, MA + ', ' + TEN as TEN from ERP_NHACUNGCAP WHERE CONGTY_FK = "+this.CtyId + " and ISNULL (TAIKHOAN_FK,100098)='100098' ");
		...
		if(this.NCCID.length() > 0)
		{
			...
			if(this.poId.trim().length() >0){
				...
				query = " select pk_seq as TTID, MA AS TIENTE from ERP_TIENTE  where trangthai = '1' " +
						" AND PK_SEQ in  (select TIENTE_FK from ERP_MUAHANG where PK_SEQ IN ( "+this.poId+"))";
			...
			{				
				if(this.ID.trim().length() == 0){
					query = " select mh.PK_SEQ, mh.SOPO +'-' + mh.NGAYMUA +'-' + isnull(tt.MA,'') +'- HD:' +isnull(mh.SOHOPDONG,'') as TEN from ERP_MUAHANG  mh " +
				 		 " inner join ERP_DUYETMUAHANG dmh on mh.PK_SEQ = dmh.MUAHANG_FK " +
				 		 " left join ERP_TIENTE tt on tt.PK_SEQ = mh.TIENTE_FK "+
				 		 
				 		 "where mh.LOAI = 0 and mh.TRANGTHAI =1 and dmh.QUYETDINH = 1 and " +
				...
				if(this.ID.trim().length() > 0 && this.poId.length() >0){
				 ...
				 System.out.println("câu lấy đơn nhập khẩu"+ query);
			}
			
			// load lên sp
			if( this.ID.trim().length() <= 0 && this.poId.trim().length() > 0 && ( this.spList==null || this.spList.size() <=0))
			{
				String	query1 = " SELECT   mh.PK_SEQ as MUAHANGID, sp.PK_SEQ as SPID, sp.MA as MASP, sp.TEN +'-' +isnull(' maquette: ' +m.MA, '') as TENSP, " +
						        ...
						        // cho nay
						        " isnull((select SUM(SOLUONG) from ERP_HOADONNCC_DONMUAHANG a inner join ERP_HOADONNCC b on b.pk_seq=a.HOADONNCC_FK   where b.trangthai not in (4) and a.MUAHANG_FK = mh.PK_SEQ and a.SANPHAM_FK = mhsp.SANPHAM_FK "
						        + " and a.DONGIA = mhsp.dongia " +
						        "	group by a.MUAHANG_FK, a.SANPHAM_FK, a.dongia),0) as danhan" +
						        ...
						        " WHERE mh.PK_SEQ in (  "+ this.poId +")  " +
						        		" ORDER BY MH.PK_SEQ, MHSP.PK_SEQ  ";
					...
					if(rs!=null){
						...
						while(rs.next()){
							...
							if(rs.getInt("dungsai") == 0){
								sp.setSoluongMaxNhan(formatter.format(rs.getDouble("SOLUONG")));
							} else {
								sp.setSoluongMaxNhan(formatter.format(rs.getDouble("SOLUONG") * rs.getDouble("dungsai")));
							}
							
							sp.setDongiaVND("0");
							sp.setThanhtienVND("0");
			...
			if( this.ID.trim().length() > 0 && this.poId.trim().length() > 0 && ( this.spList==null || this.spList.size() <=0))
			{
				//Chi hien thi (trang display) nhung san pham co so hoa don > 0
				String tempQuery = "";
				if (isDisplay == 1) {
					tempQuery = "\n and isnull(( select SUM(a.SOLUONG) from ERP_HOADONNCC_DONMUAHANG  a "+
							...
							" and a.MUAHANG_FK = mh.PK_SEQ and a.dongia= mhsp.DONGIA),0) > 0 \n";
				} 
				   String query1 = " SELECT  mhsp.pk_Seq as sott , mh.PK_SEQ as MUAHANGID, sp.PK_SEQ as SPID, sp.MA as MASP, sp.TEN +'-' +isnull(' maquette: ' +m.MA, '') as TENSP, " +
						        " mhsp.DONVI, mhsp.SOLUONG, '' as NGAYNHAN, mhsp.thuexuat as vat, mhsp.dongia, isnull(mh.DUNGSAI,0) as dungsai," +
						       
						        " isnull(hansudung, 0) hansudung, m.PK_SEQ as idmarquette ,  " +
						        " isnull(( select SUM(a.SOLUONG) from ERP_HOADONNCC_DONMUAHANG  a "+
								" where a.HOADONNCC_FK = "+this.hoadonNCC +" and a.SANPHAM_FK = mhsp.SANPHAM_FK " +
								" and a.MUAHANG_FK = mh.PK_SEQ and a.dongia= mhsp.DONGIA),0) as soluongtong, "+
					...
					if(rs!=null){
						...
						while(rs.next()){
							...
							//cho nay
							sp.setSoluongnhan(formatter.format(rs.getDouble("soluongtong")));
...
//						String tonggiatri = pnkRs.getString("tonggiatri");
	...
	public void init() 
	{
		...
		query ="  select NGAYGHINHAN, p.NCC_FK, PO_NHAPKHAU, p.TIENTE_FK, isnull(hoadon.TIGIA,'') as TIGIA, p.LOAIHD, ISNULL(MAUSOHOADON, '') AS MAUSOHOADON, " +
		       "  kyhieu, sohoadon, ngayhoadon ,sotienAVAT,vat,sotienBVAT,hoadon.PK_SEQ as hoadonncc, isnull(KHOBIETTRU_FK,0) as KHOBIETTRU_FK," +
		       "  isnull(KHOTONTRU_FK,0) as KHOTONTRU_FK, hoadon.sotienAVAT_VND,ISNULL(P.DIENGIAI,'') AS DIENGIAI,p.trangthai ,ISNULL( hoadon.CHENHLECHHD,0) CHENHLECHHD" +
		...
		if(rs!=null){
			if(rs.next()){
				...
				this.tongVND = rs.getDouble("sotienAVAT_VND");
		...
		System.out.println("tong tien nt :"+this.tong);
		this.createRs();
		...
		if( this.hoadonNCC.trim().length() >0){
			...
			query = "  select dh.sanpham_fk, isnull(sum(dh.soluongdat),0) as sl, dh.MUAHANG_FK , isnull(dh.dongia,0) as dongia ," +
					...
					"  order by SANPHAM_FK, dh.MUAHANG_FK";
			...
			query = " select dh.muahang_fk, dh.sanpham_fk, dh.soluong,isnull(NGAYSANXUAT,'') as nsx, " +
					...
					" isnull(NGAYHETHAN,'') as ngayhethan, dh.SOLO, isnull(dh.DONGIA, 0) as DONGIA,isnull(dh.MARRQUET,'') as  MARRQUET ,isnull(nsx.ten,'') as TENNSX, dh.NSX_FK from ERP_HOADONNCC_DONMUAHANG dh " +
					"	left join ERP_NHASANXUAT nsx on nsx.pk_seq = dh.NSX_FK \n" +
			...
			if(rs!=null){
				while(rs.next()){
					...
					ct.setNSXTen(rs.getString("TENNSX"));
					ct.setNSXid(rs.getString("NSX_FK")==null?"":rs.getString("NSX_FK"));
					listct.add(ct);
			...
			for(int i=0; i< listct.size(); i++){
				...
				for(int j=0; j< this.spList.size(); j++){
					...
					if(temp.getMa().equals(sp.getId())   && temp.getMuahang_fk().equals(sp.getMuahang_fk()) 
						...
						sp.setSoluongnhan(DinhDang.dinhdangkho(Double.parseDouble(soluongtong.get(i))));
						Double tt = Double.parseDouble(soluongtong.get(i)) * Double.parseDouble(sp.getDongia().replaceAll(",", ""));
...
//			 String today = sdf.format(date);
		...
		try 
		{
			...
			query = " insert ERP_PARK(NGAYGHINHAN, NCC_FK, NGAYTAO, NGAYSUA, NGUOITAO" +
					", NGUOISUA, TRANGTHAI, CONGTY_FK, PO_NHAPKHAU, TIENTE_FK" +
					", TIGIA, LOAIHD, NPP_FK,DIENGIAI) " +
					" values('"+ this.NGAYGHINHAN + "', '" + this.NCCID + "', '" + ngaytao + "', '" + ngaytao + "','" + this.USERID + "', '" + this.USERID + "' ,0, " +
					this.CtyId+", '" + this.poId + "', " + this.ttId + ", " + this.tigia + ", "+this.loaidonmhId+"" +
					", "+this.nppdangnhap+",N'"+this.dienGiaiCT+"' )";
			...
			query = " insert ERP_HOADONNCC(mauhoadon, mausohoadon, kyhieu, sohoadon, ngayhoadon ,sotienAVAT," +
					" sotienchietkhau,vat,sotienBVAT,park_fk, NGAYTAO, NGAYSUA, NGUOITAO, NGUOISUA," +
					" TRANGTHAI, NGAYDENHANTT, TINHTHUENHAPKHAU,CONGTY_FK, NPP_FK, LOAIHD,KHOBIETTRU_FK, KHOTONTRU_FK, TIGIA,sotienAVAT_VND , CHENHLECHHD) " +
					" values('', '"+this.mauSoHoaDon+"', '" + this.kyHieuHoaDon.toUpperCase() + "', '" + this.soHoaDon + "', '" + 
					...
					this.CtyId+", "+this.nppdangnhap+", "+this.loaidonmhId+","+this.idKhoBietTru+","
					+this.idKhoTonTru+","+this.tigia+","+this.tongVND+","+this.ChenhlechHd+")";
				...
				for(int i=0; i < this.spList.size(); i++ ){
					...
					for(int j = 0; j < sanpham.getSpDetail().size(); j++){	
						...
						* Double.parseDouble(sanpham.getDongia().replaceAll(",", "")) * Double.parseDouble(sanpham.getVat())/100;
						...
						thanhtien = Math.round(Double.parseDouble(sp.getSoluong().replaceAll(",", "")) 
								* Double.parseDouble(sanpham.getDongia().replaceAll(",", "")));
						
						thanhtienVND = Math.round(Double.parseDouble(sp.getSoluong().replaceAll(",", "")) 
								* Double.parseDouble(sanpham.getDongiaVND().replaceAll(",", "")));
						
						
						if(j < (sanpham.getSpDetail().size()-1)){
							pbThanhTienVND_temp += thanhtienVND;
							pbThanhTienVATVND_temp +=  tienvatVND;
						}
						String marq =sp.getMarrquet();
						if(marq.trim().length()>0)
						{
							query = "SELECT count(*) as dem FROM MARQUETTE where ma = N'"+marq+"' AND SANPHAM_FK = "+sanpham.getId();
						...
						query = " insert ERP_HOADONNCC_DONMUAHANG(hoadonncc_fk, muahang_fk, sanpham_fk, " +
								...
								" SOLO, NGAYSANXUAT, NGAYHETHAN, thanhtienVIET, thanhtienVATVIET, DONGIAVIET,MARRQUET , NSX_FK ) " +
								...
								+sp.getNgayHethan()+"',"+thanhtienVND+",0,"+sanpham.getDongiaVND()+",'"+sp.getMarrquet()+"',"+(sp.getNSXid().trim().length()>0?sp.getNSXid():"NULL")+")";
						
						// phân bổ lại lô cuối cùng để cộng phần lẻ VAT và phần lẻ thành tiền VIỆT
						if(j == (sanpham.getSpDetail().size()-1)){
							long pbThanhTienVND = Math.round(Double.parseDouble(sanpham.getThanhtienVND().replaceAll(",", "")));
							...
							+sp.getSoluong().replaceAll(",", "")+"', '"+sanpham.getNgaynhandukien()+"', "
							+sanpham.getVat()+", "+ tienvat +", '"+sp.getSolo()+"', '"+sp.getNgaySx()+"', '"
							+sp.getNgayHethan()+"',"+last_item_thanhtien +",0,"+sanpham.getDongiaVND()+",'"+sp.getMarrquet()+"',"+(sp.getNSXid().trim().length()>0?sp.getNSXid():"NULL")+")";
...
//			 String today = sdf.format(date);
		...
		try 
		{
			...
			query = " update ERP_HOADONNCC set kyhieu =?, sohoadon= ?, ngayhoadon = ?, sotienAVAT =?," +
					...
					" ,NGAYDENHANTT= ?, TINHTHUENHAPKHAU= ?, LOAIHD= ?, KHOBIETTRU_FK = ?, KHOTONTRU_FK = ?, TIGIA = ?" +
					", sotienAVAT_VND ="+ this.tongVND +", mausohoadon = ?,  CHENHLECHHD=? " +
					" where PARK_FK =  ?";
			...
			pre.setDouble(16, Double.parseDouble(this.tigia.replaceAll(",", "")));
			pre.setString(17, this.mauSoHoaDon);
			pre.setString(18, this.ChenhlechHd);
			pre.setString(19, this.ID);
				...
				for(int i=0; i < this.spList.size(); i++ ){
					...
					for(int j = 0; j < sanpham.getSpDetail().size(); j++){	
						...
						* Double.parseDouble(sanpham.getDongia().replaceAll(",", ""));
						...
						thanhtienVND = Double.parseDouble(sp.getSoluong().replaceAll(",", "")) 
						* Double.parseDouble(sanpham.getDongiaVND().replaceAll(",", ""));
						...
						// phân bổ lại lô cuối cùng để cộng phần lẻ VAT và phần lẻ thành tiền VIỆT
						if(j == (sanpham.getSpDetail().size()-1)){
							long pbThanhTienVND = Long.parseLong(sanpham.getThanhtienVND().replaceAll(",", ""));
...
*/
	...
	{
		if (this.tigia.length() == 0) {
			String query = "SELECT TIGIAQUYDOI FROM ERP_TIGIA WHERE TUNGAY <= '" + this.NGAYGHINHAN + "' " +
							"AND DENNGAY >= '" + this.NGAYGHINHAN + "' AND TIENTE_FK = " + this.ttId + "";
			System.out.println(query);
			ResultSet rs = this.db.get(query);
			try {
				if (rs != null) {
	...
	public double getTongVND() {
		return tongVND;
	}

	public void setTongVND(double tongVND) {
