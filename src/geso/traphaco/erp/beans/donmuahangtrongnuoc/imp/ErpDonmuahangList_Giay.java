...
public class ErpDonmuahangList_Giay extends Phan_Trang implements IErpDonmuahangList_Giay 
{
	...
	private static final String ID_PHONGKEHOACH = "100043";
	String congtyId;
	String userId;
	String tungay;
	String denngay;
	...
	public void init(String search)
	{
		...
		if(sothamchieu.trim().length() > 0)
			timkiem += " and a.SOTHAMCHIEU like '%" + sothamchieu + "%'";
		
		if(tungay.length() > 0)
			timkiem += " and a.ngaymua >= '" + tungay + "'";
		
		if(denngay.length() > 0)
			timkiem += " and a.ngaymua <= '" + denngay + "'";
		
		if(dvthId.length() > 0)
			timkiem += " and a.DONVITHUCHIEN_FK = '" + dvthId + "'";
		
		if(this.loaihanghoa.length() > 0)
			timkiem += " and a.LOAIHANGHOA_FK = '" + this.loaihanghoa + "'";
		
		if(this.nccTen.length() > 0)
			timkiem += " and (dbo.ftBoDau(c.ma) like '%" + util.replaceAEIOU(this.nccTen) + "%' or dbo.ftBoDau(c.ten) like N'%" + util.replaceAEIOU(this.nccTen) + "%') ";
		
		if(this.nguotaoIds.trim().length() > 0)
			timkiem += " and a.nguoitao = '" + this.nguotaoIds.trim() + "' ";
		...
		if( this.sanphamct != "")
		{
			timkiem += " and a.pk_seq in (select muahang_fk from ERP_MUAHANG_SP where sanpham_fk="+this.sanphamct+") ";
		}
			
		if(sodonmuahang.length() > 0)
		{
			timkiem += " and a.soPO like '%" + sodonmuahang + "%' ";
		}
		
		if(trangthai.trim().length() > 0)
		{
			if(trangthai.equals("0")){//chưa duyệt
				timkiem += " and a.trangthai = 0 ";
			}	
			else if(trangthai.equals("-1")){//đã duyệt
				timkiem += " and a.trangthai = 1 ";
			
			}
			else if(trangthai.equals("-2")){//Đã có hóa đơn
				timkiem += " and (select count(*) from ERP_HOADONNCC_PHIEUNHAP where MUAHANG_FK = a.pk_seq ) >0 ";
			}
			else if(trangthai.equals("5")){//Chưa chốt
				timkiem += " and a.trangthai = 0 and isnull(a.DACHOT, 0) = 0 ";
			}
			else if(trangthai.equals("2"))//hoàn tất
			{
				timkiem += " and a.trangthai = 2 ";
			}
			else if(trangthai.equals("3"))//đã xóa
			{
				timkiem += " and a.trangthai = 3 ";
			}
			else{// đã hủy
				timkiem += "and a.trangthai = '"+ trangthai +"' and (select count(*) from ERP_HOADONNCC_PHIEUNHAP where MUAHANG_FK = a.pk_seq ) <=0";
			}
		}
		
		if( loaiDonMuaHang != "")
		{
			timkiem += "and ISGIACONG = '" + loaiDonMuaHang + "'";
		}

		
		if(mactsp.trim().length() > 0)
		{
			timkiem +=" and a.pk_seq in (" +
		...
		{
			timkiem +=" and a.pk_seq in (" +
					"     select distinct muahang_fk from erp_muahang_sp where sanpham_fk in ( select distinct pk_seq from erp_Sanpham where ten like N'%" + tensanpham + "%' ) " +
					" ) ";
		}*/
		...
		System.out.println();
		System.out.println("Vuna_searchQuery: " + timkiem);
		System.out.println();		


		

		query = returnQuery(sql,"0");

		System.out.println("lay quyen menu "+util.GetquyenNew("ErpDonmuahangtrongnuocList_GiaySvl", "&loai="+loai,userId)[9]);
		// Phân quyền nhân viên theo phòng ban, mặc định admin được nhìn thấy
		if( !this.userId.equals("100002")  && util.GetquyenNew("ErpDonmuahangtrongnuocList_GiaySvl", "&loai="+loai,userId)[9]==0){
		...
		// checkUserPhongKeHoach
		if(checkUserPhongBan(ID_PHONGKEHOACH)){
		 
		}else{
 
			
			
			// kiem tra co phai phong ke hoach khong?
			// PHONGKEHOACH: 100043 thi bo don hang dc thay het.
			
			
			if(!this.userId.equals("100002") && checkTruongPhong() == false && util.GetquyenNew("ErpDonmuahangtrongnuocList_GiaySvl", "&loai="+loai,userId)[9]==0){
	...
	// dung de union lai loai gia cong
	private String returnQuery(String sql, String isGiaCong){
		

		String query = " select ISNULL(A.HOANTAT_NHANHANG,0) AS HOANTAT_NHANHANG ,"
				+" (SELECT TOP 1 ISNULL(SP.MA,ISNULL(TS.MA,NCP.TEN)) \n"
				+"  FROM ERP_MUAHANG_SP MH_SP  \n"
				+" LEFT JOIN ERP_SANPHAM SP ON SP.PK_SEQ = MH_SP.SANPHAM_FK AND A.LOAIHANGHOA_FK = 0 \n"
				+" LEFT JOIN ERP_TAISANCODINH TS ON TS.PK_SEQ = MH_SP.TAISAN_FK AND A.LOAIHANGHOA_FK = 1  \n"
				+" LEFT JOIN ERP_NHOMCHIPHI NCP ON NCP.PK_SEQ = MH_SP.CHIPHI_FK AND A.LOAIHANGHOA_FK = 2  \n"
				+"  WHERE A.PK_SEQ = MH_SP.MUAHANG_FK  )+ ' - ' + isnull( GHICHUGC,'') as SANPHAMTEN \n"+
				" , a.PK_SEQ as dmhId,ISNULL(( select SUM(soluong) from ERP_MUAHANG_SP where MUAHANG_FK = a.PK_SEQ ), 0) AS tongluong, a.NGAYMUA , \n " +
				" isnull((select TEN from ERP_DONVITHUCHIEN where PK_SEQ = a.DONVITHUCHIEN_FK), '') as ten, isnull(c.TEN, '') as nccTen, c.MA as nccMa,  \n " +
				" SOPO +' '+isnull( GHICHUGC,'') as SOCHUNGTU,  \n " +
	...
	private boolean checkUserPhongBan(String phongbanId){
		try{
			String sql = " select PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.userId;
			ResultSet rs = this.db.get(sql);
			String phongban = "";
			if(rs!=null){
				if(rs.next()){
					phongban = rs.getString("PHONGBAN_FK");
				}
				rs.close();
			}
			if(phongban.equals(phongbanId)){
				return true;
			}
			return false;
