...
import geso.traphaco.erp.beans.donmuahangtrongnuoc.IErpDonmuahangTSCC;
...
public class ErpDonmuahangTSCC implements IErpDonmuahangTSCC {
	...
	private void createSanpham() {
		...
		// " +
				...
				+ "	PhanTramThue, ThueNhapKhau, DONGIAVIET, SUM(THANHTIENVIET) AS THANHTIENVIET, DONVI, KHOTT_FK, DUNGSAI, MUANGUYENLIEUDUKIEN_FK, CCDC_FK, TENHD, HOADONNCC_FK, thuexuat, vat \n"
				...
				+ "	PhanTramThue, ThueNhapKhau, DONGIAVIET, DONVI, KHOTT_FK, DUNGSAI, MUANGUYENLIEUDUKIEN_FK, CCDC_FK, TENHD, HOADONNCC_FK, thuexuat, vat \n"
		...
		try {
			...
			for (int i = 0; i < this.spList.size(); i++) {
				...
				for (int j = 0; j < nn.size(); j++) {
					...
					double thanhtien = Double.parseDouble(sp.getDongia()) * soluong;
					long thanhtienviet = Math.round(dongiaviet * soluong);
					query = " insert into ERP_MUAHANG_SP(muahang_fk, sanpham_fk, taisan_fk, ccdc_fk, chiphi_fk, diengiai, donvi, soluong, dongia, soluong_new, dongia_new, thuexuat, vat, thanhtien, dongiaviet, thanhtienviet, ngaynhan, khott_fk, nhomkenh_fk, dungsai, PhanTramThue, ThueNhapKhau,Muanguyenlieudukien_fk, TENHD) "
