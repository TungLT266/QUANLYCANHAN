...
public class ErpDonmuahang implements IErpDonmuahang
{
	...
	public void createRs()
	{
		String sql=" select pk_seq, ten from ERP_DONVITHUCHIEN where trangthai = '1' ";
				        //"and congty_fk = '" + this.congtyId + "' and pk_seq in "+util.quyen_donvithuchien(this.userId);
		...
		ResultSet kho = db.get("select PK_SEQ, TEN from KHO ");
		this.khoList.clear();
		if (kho != null)
		{
			try
			{
				while (kho.next())
				{
					this.khoList.add(new Kho(kho.getString("pk_seq"), kho.getString("ten")));
				}
				kho.close();
			}
			catch (SQLException e)
	...
	public boolean createDmh()
	{
		...
		try
		{
			...
			query = " SELECT DUYETCHIPHI.CHUCDANH_FK, DUYETCHIPHI.QUYETDINH, DUYETCHIPHI.THUTU \n" +
					...
					"      INNER JOIN ERP_CHINHSACHDUYET CSD ON CSD.DONVITHUCHIEN_FK = MUAHANG.DONVITHUCHIEN_FK \n" +
					"	   INNER JOIN ERP_CHINHSACHDUYET_CHIPHI DUYETCHIPHI ON  CSD.PK_SEQ = DUYETCHIPHI.CHINHSACHDUYET_FK \n" +
