...
public class ErpNhanhangList_Giay extends Phan_Trang implements IErpNhanhangList_Giay
{
	...
	public void init(String search)
	{
		...
		String query =" SELECT DISTINCT a.* from ( "+
				" SELECT distinct a.PK_SEQ as nhId, ncc.Ten as nccTen, \n"+
				...
				" INNER JOIN ERP_HOADONNCC_DONMUAHANG hdsp on f.pk_seq= hdsp.HOADONNCC_FK \n" +
		...
		if(this.soPO.length() > 0)
		{
			...
			query += " and  mh.SOPO like N'%" + this.soPO + "%' ";
		}
		...
		if(sohoadon.trim().length() > 0)
		{
			...
			//query += " and  mh.SOPO like N'%" + this.sohoadon + "%' ";
		}
		...
		// PhÃ¢n quyá»�n nhÃ¢n viÃªn theo phÃ²ng ban, máº·c Ä‘á»‹nh admin Ä‘Æ°á»£c nhÃ¬n tháº¥y
		if( !this.userId.equals("100002")  && util.GetquyenNew("ErpNhanhangtrongnuoc_GiaySvl", "&loai="+loaimh,userId)[9]==0){
		...
		// Láº¥y thÃ´ng tin hÃ³a Ä‘Æ¡n => Ä‘Æ¡n hÃ ng náº±m chá»�
		getHoaDonNamCho();
		
	}
	// táº¡o ra 1 danh sÃ¡ch cÃ¡c hÃ³a Ä‘Æ¡n Ä‘ang náº±m chá»�
	private void getHoaDonNamCho() {
		String sql = " SELECT isnull(A.TOOLTIP,'') TOOLTIP, A.PK_SEQ , A.NGAYGHINHAN, NCC.TEN AS TENNHACUNGCAP, A.TRANGTHAI ,     	NV.TEN AS TENNV,A.NGAYSUA,A.NGAYTAO,NV.PK_SEQ AS MANV,NV2.TEN AS \n"
		...
		if( !this.userId.equals("100002") && util.GetquyenNew("ErpNhanhangtrongnuoc_GiaySvl", "&loai="+loaimh,userId)[9]==0){
			...
			// loáº¡i 2 lÃ  khÃ´ng kiá»ƒm Ä‘á»‹nh
		...
		System.out.println("hÃ³a Ä‘Æ¡n náº±m chá»�:"+ sql);
	}
