package geso.traphaco.erp.beans.dieuchuyentaisan.imp;
...
public class Erp_DieuChuyenTaiSan extends Phan_Trang implements IErp_DieuChuyenTaiSan
{
	/**
	 * 
	 */
	...
	public Erp_DieuChuyenTaiSan( String id )
	{
		this.Id = id;
		this.Ma = "";
		this.Diengiai = "";
	...
	public ResultSet getRsCdts()
	{
		...
		String query = "";
				...
				query = 	"SELECT PK_SEQ AS NCPID, TEN +' ' + DIENGIAI AS DIENGIAI,0 AS PHANTRAM " +
							"FROM ERP_NHOMCHIPHI WHERE TRANGTHAI = 1 AND CONGTY_FK = "+this.ctyId+
//							" AND NPP_FK = "+this.Npp_Id+" "+
//							" AND (("+this.Dvkd_ID+" = (SELECT PK_SEQ FROM DONVIKINHDOANH WHERE DONVIKINHDOANH='CHG' AND CONGTY_FK="+this.ctyId+")) OR ( DVKD_FK in (select pk_seq from donvikinhdoanh where pk_seq =" + this.Dvkd_ID +" or (DONVIKINHDOANH='CHG' and CONGTY_FK="+this.ctyId +" )))  ) " +
							" ORDER BY PK_SEQ ";
	...
	public boolean themmoiMa(HttpServletRequest request)
	{
		...
		try {
			...
			String query="INSERT INTO ERP_DIEUCHUYENTAISAN (TAISAN_FK,NGAYCHUNGTU,SOCHUNGTU,TRANGTHAI,THANG,NAM,NGUOITAO,NGUOISUA,NGAYTAO,NGAYSUA,TTCP_FK,TTCP_FK_OLD,DIENGIAI) " +
			...
			for (int i=0;i<phanBoList.size();i++)
			{
					...
					query = "INSERT INTO ERP_DIEUCHUYENTAISAN_KHOANMUCCHIPHI(DIEUCHUYENTAISAN_FK,TAISAN_FK,KHOANMUCCHIPHI_FK, PHANTRAM) VALUES ("+this.Id+"," + this.tscdId + ", " + phanBo.getKhoanMucId() + ", " + phanBo.getPhanTram() + " )";
	...
	public boolean UpdateMa(HttpServletRequest request)
	{
		...
		/*	float totalPt=0;
			...
			String query="Update ERP_DIEUCHUYENTAISAN SET TAISAN_FK='"+this.tscdId+"' , SOCHUNGTU='"+this.sochungtu+"',NGAYCHUNGTU='"+this.ngaychungtu+"',NGAYSUA='"+getDateTime()+"',NGUOISUA="+this.userId+" ," +
			...
			query="DELETE ERP_DIEUCHUYENTAISAN_KHOANMUCCHIPHI "+
...
*/			this.db.getConnection().commit();
	...
	public void init(String sql)
	{
		...
		try {
		String query="SELECT dc.TAISAN_FK,dc.NGAYCHUNGTU,dc.SOCHUNGTU," +
		...
		System.out.println("QUERY :"+query);
			...
			query="SELECT KHOANMUCCHIPHI_FK, PHANTRAM FROM ERP_DIEUCHUYENTAISAN_KHOANMUCCHIPHI WHERE DIEUCHUYENTAISAN_FK = "+this.Id+"";
	...
	/*	
		...
		*/
		...
		if(this.tscdId.length()>0)
		{
		query=" select DVTH_FK,dvth.TEN  as TEN from ERP_TAISANCODINH tscd \n" + 
