...
import geso.traphaco.erp.beans.lapngansach.ILapngansachTSList;
...
public class LapngansachTSList implements ILapngansachTSList {
	String Id;
	String userId;
	String ngay;
	String ctyId;
	ResultSet ctylist, lnsRs, rs, dvkdRs, nslist;
	...
	public ResultSet getLapngansachTSRs(){
		return this.db.get("SELECT PK_SEQ AS LNSID, NAM, DIENGIAI FROM ERP_LAPNGANSACH WHERE LOAI = '2' ORDER BY NAM DESC");
	}
	...
	public void createBudget(){
		...
		query = 	"INSERT INTO ERP_LAPNGANSACH(DIENGIAI, CONGTY_FK,  DVKD_FK, NAM, NGAYTAO, NGUOITAO, LOAI, HIEULUC) " +
					"VALUES(N'" + this.diengiai + "', '" + this.ctyId + "', " + this.dvkdId + ", " + this.nam + ",'" + this.getDateTime() + "','" + this.userId + "', '2', '"+ this.hieuluc +" ')";
	...
	public void Copy(){
		...
		query = 	"INSERT INTO ERP_LAPNGANSACH(DIENGIAI, CONGTY_FK,  DVKD_FK, NAM, NGAYTAO, NGUOITAO, LOAI, HIEULUC) " +
					"SELECT N'Vui lòng cập nhật diễn giai', CONGTY_FK,  DVKD_FK, NAM, '" + this.getDateTime() + "', " + this.userId + ", '2', '0' " +
		...
		try{
			...
			query = "INSERT INTO ERP_LAPNGANSACH_TAISAN(LAPNGANSACH_FK, DONVITHUCHIEN_FK, DIENGIAI, CONGDUNG_FK, SOLUONG, DONGIA, THANHTIEN, NGUOITAO, NGAYTAO, SOTHANGKH, THUCCHI, " +
					"T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12 ) " +
					"( " +
					"	SELECT " + Id + ", DONVITHUCHIEN_FK, DIENGIAI, CONGDUNG_FK, SOLUONG, DONGIA, THANHTIEN, NGUOITAO, NGAYTAO, SOTHANGKH, THUCCHI, " +
					"T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12  " +
	...
	public void initUpdate(){
		String query = 	"SELECT LNS.PK_SEQ AS NSID, CTY.PK_SEQ AS CTYID, CTY.TEN AS CTY, LNS.DIENGIAI, LNS.NAM,  DVKD.DONVIKINHDOANH AS DVKD, LNS.TRANGTHAI, NV.TEN AS NGUOITAO, LNS.NGAYTAO " +
						"FROM ERP_LAPNGANSACH LNS " +
						"INNER JOIN ERP_CONGTY CTY ON CTY.PK_SEQ = LNS.CONGTY_FK " +
						"INNER JOIN DONVIKINHDOANH DVKD ON DVKD.PK_SEQ = LNS.DVKD_FK " +
						"INNER JOIN NHANVIEN NV ON NV.PK_SEQ = LNS.NGUOITAO " +
	...
	public void Delete(String Id){
			this.db.update("DELETE FROM ERP_LAPNGANSACH_TAISAN WHERE LAPNGANSACH_FK = '" + Id + "' ");
