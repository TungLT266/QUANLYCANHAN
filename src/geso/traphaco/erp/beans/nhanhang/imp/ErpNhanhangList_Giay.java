...
public class ErpNhanhangList_Giay extends Phan_Trang implements IErpNhanhangList_Giay
{
	...
	public void init(String search)
	{
		...
		if(this.nguotaoIds.trim().length() > 0)
			query += " AND a.nguoitao = '" + this.nguotaoIds+ "' ";
		...
		if(this.soPO.length() > 0)
		{
			...
			query += "  AND ( c.sopo like '%" + this.soPO.trim() + "%' ) ";
		}
		...
		{
			query += " and (  ( ( c.pk_seq is not null ) and ( c.NHACUNGCAP_FK in ( select pk_seq from ERP_NHACUNGCAP where ma like N'%" + this.nccTen + "%' or ten like N'%" +this.nccTen + "%' ) ) )              " +
		...
		if(this.mactSp.trim().length() > 0)
		{
			query +=" and a.pk_seq in (" +
					"     select distinct nhanhang_fk from erp_nhanhang_sanpham where sanpham_fk in ( select distinct pk_seq from SANPHAM where pk_seq =" + this.mactSp + ") " +
					" ) ";
		}
		...
		if(!this.userId.equals("100002") && util.GetquyenNew("ErpNhanhang_GiaySvl", "&loai=2",userId)[9]==0){
			/*query += " and a.DONVITHUCHIEN_FK in " +util.quyen_donvithuchien(this.userId);*/
			
			/*query += " and d.PHONGBAN_FK in ( select  " +
			" distinct PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.userId+" )";*/
			
			query += " and a.khonhan_FK in " +util.quyen_kho(this.userId);
		...
		String sql = 
				...
				+ " TENNVS,NV2.PK_SEQ AS MANVS, A.PREFIX, HOANTAT.SODEM AS DAHOANTAT, CASE WHEN THANHTOAN.PARK_FK IS NULL THEN 0 ELSE 1 END AS DACOTHANHTOAN, \n"
				+ " ISNULL(HDNCC.SOHOADON,'') AS SOHOADON, NCC.PK_SEQ NCCID, HDNCC. PK_SEQ HDNCCID, isnull(A.HOANTAT_NHANHANG,0) DANHANHANG, A.LOAIHD, \n"
				...
				+ "   					 	INNER JOIN ERP_HOADONNCC HD ON HD.PK_SEQ = TT.HOADON_FK AND TT.LOAIHD = 2 \n"
		...
		sql += "\n WHERE HDNCC.CONGTY_FK =" + this.congtyId + " AND A.LOAIHD = 2 " +
			   ...
			   " AND HDNCC.PK_SEQ IN (SELECT A.HOADONNCC_FK  FROM ERP_HOADONNCC_DONMUAHANG A  " +
			   " INNER JOIN ERP_MUAHANG MH ON MH.PK_SEQ= A.MUAHANG_FK  WHERE MH.LOAI=2 AND LOAIHANGHOA_FK=0  )";
		
		
		/*if( !this.userId.equals("100002")){
			sql += "\n and NV.PHONGBAN_FK in ( select  " +
						" distinct PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.userId+")";
			}*/
		sql+="\n order by A.PK_SEQ";
