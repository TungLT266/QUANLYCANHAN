package geso.traphaco.erp.beans.danhgialaitaisan.imp;
...
public class Erp_DanhGiaLaiTaiSan extends Phan_Trang implements IErp_DanhGiaLaiTaiSan
{
	/**
	 * 
	 */
	...
	public Erp_DanhGiaLaiTaiSan( String id )
	{
		this.Id = id;
		this.sodieuchuyen="";
		this.tungay="";
	...
	public boolean themmoiMa(HttpServletRequest request)
	{
		...
		try {
			...
			{
				this.Msg= "Vui lòng chọn tài sản cố định!";
			}
			...
			String query="INSERT INTO ERP_DANHGIALAITAISAN (TAISAN_FK,NGAYCHUNGTU,SOCHUNGTU,TRANGTHAI,THANG,NAM,NGUOITAO,NGUOISUA,NGAYTAO,NGAYSUA,SOTHANGKHAUHAO,SOTHANGKHAUHAOMOI,DIEUCHINHSOTHANGKHAUHAO,NGUYENGIA,NGUYENGIAMOI,DIEUCHINHNGUYENGIA,DIENGIAI) " +
						"VALUES ('"+this.tscdId+"','"+this.ngaychungtu+"','"+this.sochungtu+"','0','"+thang+"','"+nam+"','"+this.userId+"','"+this.userId+"','"+this.getDateTime()+"','"+this.getDateTime()+"',"+this.sothangkh+","+this.sothangkhmoi+","+this.dieuchinhsothangkh+"," +
	...
	public boolean UpdateMa(HttpServletRequest request)
	{
		...
		try {
			...
			String query="Update ERP_DANHGIALAITAISAN SET TAISAN_FK='"+this.tscdId+"', SOCHUNGTU='"+this.sochungtu+"',NGAYCHUNGTU='"+this.ngaychungtu+"',NGAYSUA='"+getDateTime()+"',NGUOISUA="+this.userId+" " +
					...
					" , DIEUCHINHNGUYENGIA = "+this.dieuchinhnguyengia.replaceAll(",", "")+",DIENGIAI=N'"+this.dienGiaiCT+"'  WHERE PK_SEQ = "+this.Id;
			if(!this.db.update(query))
			{
	...
	public void init(String sql)
	{
		...
		try {
		String query="SELECT TAISAN_FK,NGAYCHUNGTU,SOCHUNGTU,TRANGTHAI,THANG,NAM,SOTHANGKHAUHAO,SOTHANGKHAUHAOMOI," +
				" DIEUCHINHSOTHANGKHAUHAO,NGUYENGIA,NGUYENGIAMOI,DIEUCHINHNGUYENGIA,ISNULL(DIENGIAI,'') AS DIENGIAI FROM ERP_DANHGIALAITAISAN WHERE PK_SEQ ="+this.Id ;
		ResultSet dcRs= db.get(query);
		System.out.println("QUERY :"+query);
		
			while(dcRs.next())
			{
				this.tscdId=dcRs.getString("TAISAN_FK");
	...
	public void createRs()
	{
		...
		query=" SELECT PK_SEQ,MA + '-' + DIENGIAI as TEN FROM ERP_TAISANCODINH WHERE TRANGTHAI=1";
		...
		if(this.tscdId.length()>0)
		{
			...
			query=" select sothangkh, thanhtien from ERP_TAISANCODINH where pk_seq = "+this.tscdId ;
			ResultSet rs= db.get(query);
			 ...
			 if(!this.dieuchinhnguyengia.equals("")||!this.dieuchinhsothangkh.equals(""))
			 {
				    ...
				    if(this.dieuchinhsothangkh.equals(""))
				 {
				    ...
				    query = 	" SELECT TS.THANHTIEN AS THANHTIEN , ISNULL(TS.SOTHANGKH,0) - ISNULL(TS.SOTHANGDAKHAUHAO,0)  SOTHANGKH , ISNULL(THANGDAKHAUHAO.THANGTHU, 0)  AS THANGDAKHAUHAO, \n" +
					...
					"	FROM ERP_KHAUHAOTAISAN \n" +
					"	WHERE TAISAN_FK = '" +  this.tscdId + "' AND KHAUHAO > 0 \n" +
					"	GROUP BY TAISAN_FK \n" +
			   ...
			   int sothangKH = Integer.parseInt(rs.getString("sothangKH")) + Integer.parseInt(this.dieuchinhsothangkh);
	...
	private boolean Tinhkhauhao_Update(int sothangKH, int thangdaKH,  int sothangdaKH, double thanhtien, double tiendaKH){
		...
		try{
			...
			query="Select thangthu + isnull(ts.sothangdakhauhao,0) as thang,kh.KHAUHAO, kh.KHAUHAOLUYKE,kh.GIATRICONLAI FROM ERP_KHAUHAOTAISAN kh  inner join erp_taisancodinh ts on " +
				  " ts.pk_seq = kh.taisan_fk where TAISAN_FK ="+this.tscdId ;
			ResultSet rs=db.get(query);
			while(rs.next())
			{
				IKhauHaoDuKien khauhao = new KhauHaoDuKien();
				khauhao.setTaisanid(this.tscdId);
