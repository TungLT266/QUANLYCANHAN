...
public class ErpThutienCK implements IErpThutienCK {
	...
	public ErpThutienCK() {
		...
		String ngay=Utility.getCurrentDate();
		this.soChungTu_Chu = "GBC"+ngay.substring(5,7)+ ngay.substring(0, 4);
		this.soChungTu_So = geso.traphaco.center.util.Utility.getSoChungTuMax(db, "Erp_ThuTien", "100001");
	...
	public boolean createTTHD() {
		...
		try {
			...
			if (this.sotaikhoan.length() > 0) {
				...
				if (this.ndId.equals("100004") || khachhang_fk.trim().length() == 0)
					khachhang_fk = null;
				else
					this.bangkeId = null;
				...
				if(this.isNPP.trim().length()<=0)
					this.isNPP = "null";
				System.out.println("Chi nhánh ID"+ this.nppChinhanhId);
				query = "Insert ERP_THUTIEN(KBH_FK, NGUOINOPTIEN, DIACHI, NHOMKHTT_FK,  CHIETKHAUNT, CHIETKHAU, TIGIA, SOTIENTTNT, THUDUOCNT, PHINGANHANGNT, THUDUOC, PHINGANHANG, CHENHLECH, NGAYCHUNGTU, NGAYGHISO, TRANGTHAI, KHACHHANG_FK, HTTT_FK, \n"+
						"NGANHANG_FK, CHINHANH_FK, SOTAIKHOAN, NOIDUNGTT_FK, GHICHU, SOTIENTT, TIENTE_FK, SOCHUNGTU, NGAYTAO, NGUOITAO, NGAYSUA, NGUOISUA, NCC_FK, NHANVIEN_FK, CTKEMTHEO, ISKTTDUYET, CONGTY_FK, BANGKE_FK, PREFIX, isNPP, NPPCN_FK , ISThuhoCN, soChungTu_So,soChungTu_Chu) \n"+
						...
						this.sotientt.replaceAll(",", "") + "', '" + this.tienteId + "', '" + this.sochungtu.toUpperCase() + "', '" +
						ngaytao + "', '" + this.userId + "', '" + ngaytao + "', '" + this.userId + "', " + this.nccId +
						", " + this.nvtuId + ", N'" + this.chungtukemtheo + "', 0, " + this.ctyId + ", " +
						this.bangkeId + ", 'BC', "+this.isNPP+", "+( this.nppChinhanhId ==""?null:this.nppChinhanhId ) +", "+ (this.isthuhoCN ==""?null:this.isthuhoCN ) +", '" + this.soChungTu_So + "','" + this.soChungTu_Chu + "')";
				
				System.out.println(query);
			} else {
				System.out.println("khachhang_fk: '" + khachhang_fk + "'");
				if (this.ndId.equals("100004") || khachhang_fk.trim().length() == 0)
					khachhang_fk = null;
				else
					this.bangkeId = null;

				query = "Insert ERP_THUTIEN(KBH_FK, NGUOINOPTIEN, DIACHI, NHOMKHTT_FK\n" +
						", CHIETKHAUNT, CHIETKHAU, TIGIA, SOTIENTTNT\n" +
						", THUDUOCNT, PHINGANHANGNT, THUDUOC, PHINGANHANG\n" +
						", CHENHLECH, NGAYCHUNGTU, NGAYGHISO, TRANGTHAI\n"+
						", KHACHHANG_FK, HTTT_FK, NGANHANG_FK, CHINHANH_FK\n" +
						", SOTAIKHOAN, NOIDUNGTT_FK, GHICHU, SOTIENTT\n" +
						", TIENTE_FK, SOCHUNGTU, NGAYTAO, NGUOITAO\n" +
						", NGAYSUA, NGUOISUA, NCC_FK, NHANVIEN_FK\n" +
						", CTKEMTHEO, ISKTTDUYET,CONGTY_FK, BANGKE_FK\n" +
						", PREFIX, isNPP, NPPCN_FK , ISThuhoCN, soChungTu_So,soChungTu_Chu) \n"+
						"values( " + this.kbhId + ", N'" + this.nguoinoptien + "', N'" + this.diachi + "' , \n"+ this.nhomkhttId + "\n" +
						", " + this.chietkhauNT.replace(",", "") + ","+ this.chietkhau.replace(",", "") + " ," + this.tigia.replaceAll(",", "") + ", "+ this.sotienttNT.replaceAll(",", "") + "\n" +
						", " + this.thuduocNT.replaceAll(",", "") + ", "+ this.cpnganhangNT.replaceAll(",", "") + ", " + this.thuduoc + ", " + this.cpnganhang + "\n" +
						", " + this.chenhlech + ", '" + this.ngaychungtu + "', '" + this.ngayghiso + "', '0'\n" +
						", " + khachhang_fk + ", '" + this.htttId + "', NULL, NULL\n" +
						", '', " + " '" + this.ndId + "', N'" + this.noidungtt + "', '" + this.sotientt.replaceAll(",", "") + "'\n" +
						", '" + this.tienteId + "', '"+
						this.sochungtu.toUpperCase() + "', '" + ngaytao + "', '" + this.userId + "', '" + ngaytao + "', '"+
						this.userId + "', " + this.nccId + ", " + this.nvtuId + ",N'" + this.chungtukemtheo + "', 0, "+
						this.ctyId + ", " + this.bangkeId + ", 'BC', "+this.isNPP+", "+( this.nppChinhanhId ==""?null:this.nppChinhanhId ) +", "+ (this.isthuhoCN ==""?null:this.isthuhoCN ) +", " + this.soChungTu_So + ",'"+this.soChungTu_Chu+"')";
...
//								hoadon.setLoaihd("");
							...
							if (hoadon.getLoaihd().equals("7"))
								{
									thanhtoan = Double.parseDouble(thanhtoan)*(-1)+"";
									avat = Double.parseDouble(avat)*(-1)+"";
									conlai = Double.parseDouble(conlai)*(-1)+"";
								}
	...
	public boolean BochotTTHD(String userId) {
		try {
			...
			/*NẾU PHIẾU NỘP TIỀN Ở TRẠNG THÁI "CHƯA THU TIỀN" (TRẠNG THÁI HỦY) DƯỚI DMS
			 * THÌ CÓ THỂ MỞ CHỐT PHIẾU THU TIỀN TRÊN ERP ĐỒNG THỜI XÓA PHIẾU NỘP TIỀN
			 */
			String query=" SELECT count(*) as soDong FROM " + geso.traphaco.center.util.Utility.prefixDMS + "ERP_THUTIENNPP_NOPTIEN TTNPP_NT where EXISTS (SELECT TRANGTHAI FROM " + geso.traphaco.center.util.Utility.prefixDMS + "ERP_THUTIENNPP WHERE TRANGTHAI IN (0, 1) AND PK_SEQ = TTNPP_NT.THUTIENNPP_FK)\r\n" + 
					" AND NOPTIEN_FK in (SELECT PK_SEQ FROM " + geso.traphaco.center.util.Utility.prefixDMS + "NOPTIEN WHERE THUTIENHO_FK="+this.id+")";
		   ...
		   query = "update ERP_THUTIEN set TRANGTHAI = '0', NGUOISUA = '" + userId + "', NGAYSUA = '" + ngaysua	+ "' where PK_SEQ = '" + this.id + "'";
			...
			query = "select  TT.NPPCN_FK,tt.noidungtt_fk,ISNULL(TT.ISTHUHOCN,1) AS ISTHUHOCN \n" +
	...
	/*	if ((this.ndId.equals("100000")) && this.kbhId.trim().length() <= 0) {
		...
		try {
			...
			if(khachhang_fk.trim().length() <= 0) khachhang_fk = "NULL";
			query = "update ERP_THUTIEN set NGUOINOPTIEN = N'" + this.nguoinoptien + "', DIACHI = N'" + this.diachi +
					...
					"', SOCHUNGTU = '" + this.sochungtu.toUpperCase() + "', TIENTE_FK = '" + this.tienteId + "', "+
...
*/							
	...
	public boolean chotTTHD(String userId) {
		try {
			String ngaysua = Utility.getCurrentDate();
			/*String ngaysua = this.getNgaychungtu();*/
			db.getConnection().setAutoCommit(false);
...
//			String loaiChungTu=" N'Thu tiền theo hóa đơn' , N'Thu tiền KH trả trước' , N'Thu tiền KH quầy', N'Thu khác' , N'Thu tiền_Thu hồi tạm ứng' ";
			...
			if (db.updateReturnInt(query)!=1) {
				this.msg = "CTTHD1.1 Khong the chot ERP_THUTIEN: " + query;
				db.getConnection().rollback();
				return false;
			}
			...
			if (this.xoakhtratruocId.trim().length() > 0) {
				...
				if (!db.update(query)) {
					this.msg = "CTTHD1.2 Khong the chot ERP_XOANOKH: " + query;
					db.getConnection().rollback();
					return false;
				}
			}
...
//			}
			...
			// nếu thu tiền quầy 100006, và isnpp=4 thì lấy tài khoản 338888, đối tựng khác
			query = "select  tt.ngayghiso, tt.tiente_fk,isnull(tt.tigia,1) as tigia, tt.sotienTT, isnull(tt.thuduoc, 0) as thuduoc,isnull(tt.thuduocnt,0) as thuduocnt\n" +
					", 0 as chietkhau, \n"+
					" isnull(tt.phinganhang, 0) as phinganhang, isnull(tt.chenhlech, 0) as chenhlech, tt.dinhkhoanco, tt.noidungtt_fk , \n"+
					" isNull(tt.khachhang_fk, npp.pk_seq) as khachhang_fk, tt.httt_fk, tt.nganhang_fk, tt.chinhanh_fk, kh.maFast as maKH, tt.noidungtt_fk , \n"+
					...
					" case when tt.noidungtt_fk = '100006' \n" +
					"				then  " +
					"						CASE WHEN ISNPP=4 THEN (SELECT PK_SEQ \n" +
					"						FROM ERP_TAIKHOANKT \n" +
					/*"						WHERE trangThai = 1 and SOHIEUTAIKHOAN = npp.taiKhoan_FK AND CONGTY_FK = 100000)\n" + */
					"						WHERE trangThai = 1 and SOHIEUTAIKHOAN = '33888000'    AND CONGTY_FK = 100000 AND NPP_FK=1)\n" +
					"  						ELSE (SELECT PK_SEQ \n" +
					"						FROM ERP_TAIKHOANKT \n" +
					/*"						WHERE trangThai = 1 and SOHIEUTAIKHOAN = npp.taiKhoan_FK AND CONGTY_FK = 100000)\n" + */
					"						WHERE trangThai = 1 and SOHIEUTAIKHOAN = KH.taiKhoan_FK AND CONGTY_FK = 100000) END \n" +
					"      when isnull(tt.ISTHUHOCN,0)= 1 then \n" +
					"                     ( SELECT PK_SEQ FROM ERP_TAIKHOANKT WHERE SOHIEUTAIKHOAN = isNull(isNull((select npp.taiKhoan_FK from nhaPhanPhoi npp where npp.pk_seq = tt.NPPCN_FK), (select npp.taiKhoan_FK from nhaPhanPhoi npp where npp.pk_seq = tt.KHACHHANG_FK)),'13111000') AND CONGTY_FK = "+this.ctyId+"   ) \n" +
					"	   else isNull(\n" +
					"	   					case when tt.ISNPP = '1' then (SELECT PK_SEQ  \n" +
					"	   					FROM ERP_TAIKHOANKT  \n" +
					"	  					WHERE trangThai = 1 and SOHIEUTAIKHOAN = nppkh.taikhoan_fk AND CONGTY_FK = 100000)  \n" +
					"						else (SELECT PK_SEQ  \n" +
					"						FROM ERP_TAIKHOANKT \n" +
					"						WHERE trangThai = 1 and SOHIEUTAIKHOAN = kh.taikhoan_fk AND CONGTY_FK = 100000)  end \n" +
					"							, \n" +
					"								isnull((SELECT PK_SEQ FROM ERP_TAIKHOANKT\n" + 
					"								WHERE trangThai = 1 and SOHIEUTAIKHOAN = npp.taiKhoan_FK AND CONGTY_FK = 100000)\n" +
					"								, (SELECT PK_SEQ FROM ERP_TAIKHOANKT \n" +
					"								WHERE trangThai = 1 and SOHIEUTAIKHOAN = nppkh.taiKhoan_FK AND CONGTY_FK = 100000)\n" +
					"								)\n" +
					"						) \n" +
					"end as taikhoanCO_KH_SoHieu\n"+
					...
					" (select PK_SEQ FROM ERP_TAIKHOANKT WHERE SOHIEUTAIKHOAN = '71110000' AND CONGTY_FK = "	+ this.ctyId + " ) taikhoanCO_chietkhau\n" +
					", isnull(tt.SOCHUNGTU,'') MACHUNGTU, (tt.soChungTu_Chu + tt.soChungTu_So) sct\n" +
//					", isnull(tt.isNPP, 0) isNPP\n" +
					", TT.NPPCN_FK, isnull(tt.ISThuhoCN, 0) ISThuhoCN  \n" +
					", case when tt.isnpp=4 then 4 when tt.ISNPP = 1 or tt.NPPCN_FK is not null  and tt.ISTHUHOCN=1 then 1 else 0 end as isNPP,isnull(tt.ghichu,'') as ghichu \n"+ 				
					"from erp_thutien tt  \n" +
					"left join  " + geso.traphaco.center.util.Utility.prefixDMS + "KHACHHANG kh on tt.khachhang_fk = kh.pk_seq \n"+
					"left join  " + geso.traphaco.center.util.Utility.prefixDMS + "nhaPhanPhoi npp on tt.NPPCN_FK = npp.pk_seq \n" +
					"left join  " + geso.traphaco.center.util.Utility.prefixDMS + "nhaPhanPhoi nppkh on tt.KHACHHANG_FK = nppkh.pk_seq \n"+
					...
					if (psktRs != null) {
						while (psktRs.next()) {
							...
							{
								msg = "CTTHD1.5.1 Vui lòng chọn khách hàng với phiếu thu hộ chi nhánh";
								db.getConnection().rollback();
								return false;
							}
...
//							String isNPP_No = "";
//							String isNPP_Co = psktRs.getString("isNPP");
							if (hinhthuctt.equals("100001")) // thanh toan NGANHANG// (CHUYEN KHOAN)
							{
								if (noidungthutien.equals("100006")){
									//Thu tiền chuyển khoản quầy - nên đối tượng là mã quầy , không quan tâm mã khách hàng
									...
									doituong_CO = "Khách hàng";
									if (khachhang_fk == null || khachhang_fk.trim().length() == 0)
							...
							if(noidungthutien.equals("100006")){
								...
								if (taikhoanCO_SoHieu.trim().length() <= 0|| taikhoanNO_SoHieu.trim().length() <= 0) {
									msg = "CTTHD1.6 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
									db.getConnection().rollback();
									return false;
								}
								...
								if(tiente_fk.equals("100000")){
									...
									this.msg = util.Update_TaiKhoan_Vat_DienGiai_Quay( db, thang, nam, psktRs.getString("ngayghiso"), psktRs.getString("ngayghiso"), loaict, this.id, taikhoanNO_SoHieu, taikhoanCO_SoHieu, "", 
									Double.toString(tonggiatri), Double.toString(tonggiatri), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "", "1", Double.toString(tonggiatri), Double.toString(tonggiatri), "Thu tiền - Thực thu", "0" , dienGiai , machungtu, isnpp,quay_fk);
								
								}else{
									
									this.msg = util.Update_TaiKhoan_Vat_DienGiai_Quay( db, thang, nam, psktRs.getString("ngayghiso"), psktRs.getString("ngayghiso"), loaict, this.id, taikhoanNO_SoHieu, taikhoanCO_SoHieu, "", 
											Double.toString(tonggiatri), psktRs.getString("THUDUOCNT"), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "",  psktRs.getString("TIGIA"), Double.toString(tonggiatri), Double.toString(tonggiatri), "Thu tiền - Thực thu", "0" , dienGiai , machungtu, isnpp,quay_fk);
								
								}
							}
							
							// THU TIEN HÓA ĐƠN && KHACH HANG TRA TRUOC (KO THU HỘ)
							if (noidungthutien.equals("100000") || noidungthutien.equals("100001")) 
							{
								// GHI NHAN SO TIEN THU DUOC
								if (taikhoanCO_SoHieu.trim().length() <= 0|| taikhoanNO_SoHieu.trim().length() <= 0) {
									msg = "CTTHD1.7 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
									db.getConnection().rollback();
									return false;
								}
								...
								if(!ISThuhoCN.equals("1")){ 
									...
									{	
										if (taikhoanNO_CK_SoHieu.trim().length() <= 0 || taikhoanCO_SoHieu.trim().length() <= 0) {
											msg = "CTTHD1.8 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
											db.getConnection().rollback();
											return false;
										}
										...
										Double.toString(tienchenhlech), Double.toString(tienchenhlech), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "", "1", Double.toString(tienchenhlech), Double.toString(tienchenhlech), "Thu tiền - Chiết khấu", "0" , dienGiai , machungtu, isnpp);
									...
									if (phinganhang > 0) {
										...
										if (taikhoanCO_SoHieu.trim().length() <= 0|| taikhoanNO_SoHieu.trim().length() <= 0) {
											msg = "CTTHD1.9 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
											db.getConnection().rollback();
											return false;
										}
										...
										this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, psktRs.getString("ngayghiso"), psktRs.getString("ngayghiso"), loaict, this.id, taikhoanNO_SoHieu, taikhoanCO_SoHieu, "", 
										Double.toString(phinganhang), Double.toString(phinganhang), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "", "1", Double.toString(phinganhang), Double.toString(phinganhang), "Phí ngân hàng", "0" , dienGiai , machungtu, isnpp);
									...
									if (chenhlech != 0) {
										...
										if (taikhoanCO_SoHieu.trim().length() <= 0|| taikhoanNO_SoHieu.trim().length() <= 0) {
											msg = "CTTHD1.10 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
											db.getConnection().rollback();
											return false;
										}
										...
										this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, psktRs.getString("ngayghiso"), psktRs.getString("ngayghiso"), loaict, this.id, taikhoanNO_SoHieu, taikhoanCO_SoHieu, "", 
										Double.toString(chenhlech), Double.toString(chenhlech), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "", "1", Double.toString(chenhlech), Double.toString(chenhlech), "Thu tiền - Chênh lệch", "0" , dienGiai , machungtu, isnpp);
								...
								// hang)
									...
									if (chietkhau > 0) {
										...
										if (taikhoanCO_SoHieu.trim().length() <= 0
											...
											msg = "CTTHD1.11 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
											db.getConnection().rollback();
											return false;
										}
										loaict = "Thu tiền theo hóa đơn";
										...
										if(psktRs.getString("GHICHU").length() >0)
											dienGiai = loaict + " - " + psktRs.getString("GHICHU");
										this.msg = util.Update_TaiKhoan_Vat_DienGiai( db, thang, nam, psktRs.getString("ngayghiso"), psktRs.getString("ngayghiso"), loaict, this.id, taikhoanNO_SoHieu, taikhoanCO_SoHieu, "", 
										Double.toString(chietkhau), Double.toString(chietkhau), doituong_NO, madoituong_NO, doituong_CO, madoituong_CO, "0", "", "", tiente_fk, "", "1", Double.toString(chietkhau), Double.toString(chietkhau), "Chiết khấu thanh toán", "0" , dienGiai , machungtu, isnpp);
								...
								if(noidungthutien.equals("100001") && ISThuhoCN.equals("1"))// THU HỘ CHI NHÁNH
								{
									String sql = " SELECT (SELECT PK_SEQ FROM ERP_TAIKHOANKT WHERE NPP_FK = tt.NPPCN_FK AND SOHIEUTAIKHOAN = '33610000') TK_336, \n" +
									 			 ...
									 			 " AND SOHIEUTAIKHOAN = NPP.TAIKHOAN_FK) TK_136, \n"+	
												 ...
												 " FROM ERP_THUTIEN tt LEFT JOIN " + geso.traphaco.center.util.Utility.prefixDMS + "NHAPHANPHOI NPP ON NPP.PK_SEQ = TT.NPPCN_FK "
												 + " WHERE TT.PK_SEQ = "+this.id;
...
//											}
									...
									while (rsDKC.next()) {
										...
										if (sotienco > 0) {
											if (taikhoanCO.trim().length() <= 0	|| taikhoanNO.trim().length() <= 0) {
												msg = "CTTHD1.13 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
												db.getConnection().rollback();
												return false;
											}
										...
										if (phinganhang > 0 && i == 0) {
											...
											if (taikhoanCO.trim().length() <= 0 || taikhoanNO.trim().length() <= 0) {
												msg = "CTTHD1.14 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
												db.getConnection().rollback();
												return false;
											}
							...
							} else if (noidungthutien.equals("100003"))// THU HOI	// TAM UNG
							{
								...
								if (tonggiatri > 0) {
									if (taikhoanCO_SoHieu.trim().length() <= 0 || taikhoanNO_SoHieu.trim().length() <= 0) 
									{
										msg = "CTTHD1.15 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
										db.getConnection().rollback();
										return false;
									}
								...
								if (phinganhang > 0) {
									...
									if (taikhoanCO_SoHieu.trim().length() <= 0 || taikhoanNO_SoHieu.trim().length() <= 0) {
										msg = "CTTHD1.16 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
										db.getConnection().rollback();
										return false;
									}
								...
								if (chenhlech != 0) {
									...
									chenhlech = Math.abs(chenhlech);
									if (taikhoanCO_SoHieu.trim().length() <= 0	|| taikhoanNO_SoHieu.trim().length() <= 0) {
										msg = "CTTHD1.17 Lỗi xác định tài khoản kế toán. Vui lòng kiểm tra lại thông tin dữ liệu nền trước khi chốt.";
										db.getConnection().rollback();
										return false;
									}
			...
			// NẾU NỘI DUNG THU TIỀN LÀ KH TRẢ TRƯỚC CHO CHI NHÁNH THÌ TỰ ĐỘNG TẠO PHIẾU NỘP TIỀN XUỐNG DƯỚI CHI NHÁNH TƯƠNG ỨNG
				//// TUH TIỀN QUẦY MÀ CHỌN ĐỐI TƯỢNG KHÁC LÀM KH ĐẠI DIỆN THÌ TẠO PHIẾU NỘP CÓ ĐỐI TƯỢNG KHÁC
				if((noidungthutien.equals("100001")||(noidungthutien.equals("100006")))&&NPPCN_FK.trim().length()>0)
				{
					String doiTuongKhacId="";
					String khachHangId="";
					if(noidungthutien.equals("100006"))
					{
						ISThuhoCN="0";
						...
						{
							khachHangId="NULL";
							doiTuongKhacId=khachhang_fk;
						}else
						{
							khachHangId=khachhang_fk;
							doiTuongKhacId="NULL";;
						}
					}else
					{
					...
					//đối tượng của chi nhánh bao giờ cũng là khách hàng
						...
						//sua vi dang debub
						query =" INSERT  " + geso.traphaco.center.util.Utility.prefixDMS + "NOPTIEN ( KHACHHANG_FK,DOITUONGKHAC_FK, SOTIEN, TRANGTHAI, NGUOITAO, NGUOISUA, NGAYTAO, NGAYSUA, NPP_FK, NGAYNOP , THUTIENHO_FK ,IS_THUHO_HD_CN) "+
							   " VALUES ("+khachHangId+","+doiTuongKhacId+", "+tonggiatri+", 1, 100002 , 100002, '"+Utility.getCurrentDate()+"', '"+Utility.getCurrentDate()+"', "+NPPCN_FK+", '"+ngayghinhan+"', "+this.id+" ,'"+ISThuhoCN+"')";
						System.out.println("query" +query);
						if (!db.update(query)) {
							this.msg = "CTTHD1.18 Không thể tạo mới phiếu thu tiền dưới chi nhánh. Vui lòng liên hệ admin để được xử lý! ";
					...
					else // NHÀ PHÂN PHỐI
					{
						dbutils db = new dbutils();
						//sua vi dang debub
						query = " INSERT  " + geso.traphaco.center.util.Utility.prefixDMS + "NOPTIEN ( NPP_DAT_FK, SOTIEN, TRANGTHAI, NGUOITAO, NGUOISUA, NGAYTAO, NGAYSUA, NPP_FK, NGAYNOP , THUTIENHO_FK ) "+
						   		" VALUES ("+khachhang_fk+", "+tonggiatri+", 1, 100002, 100002, '"+Utility.getCurrentDate()+"', '"+Utility.getCurrentDate()+"', "+NPPCN_FK+", '"+ngayghinhan+"', "+this.id+" )";
					
						System.out.println(" INSERT  " + geso.traphaco.center.util.Utility.prefixDMS + "NOPTIEN :" +query);
						if (!db.update(query)) {
							this.msg = "CTTHD1.19 Không thể tạo mới phiếu thu tiền dưới chi nhánh. Vui lòng liên hệ admin để được xử lý! ";
							System.out.println("[ErpThutien.createTTHD] error message:" + this.msg);
							this.db.getConnection().rollback();
							return false;
						}
	...
	public void init() {
		...
		///KhachHang_ThuTien_ERP TRONG BẢNG NÀY CÓ LOẠI =4, DÙNG CHO TRƯỜNG HỢP THU TIỀN QUẦY CHỌN MÃ KHÁCH HÀNG ĐẠI DIỆN LÀ 1 ĐỐI TƯỢNG KHÁC
		...
		String query = " 	select 	tt.pk_seq, tt.ngaychungtu, tt.ngayghiso, tt.trangthai,  tt.khachhang_fk as khachhang_fk, \n"+
					   ...
					   " 		xoakhtratruoc_fk , NHOMKHTT_FK, isnull(tt.ctkemtheo,'') chungtukemtheo, tt.BANGKE_FK , isnull(tt.isNPP,0) isNPP, isnull(convert(nvarchar, tt.NPPCN_FK), '')  NPPCN_FK , isnull(tt.ISThuhoCN, 0) ISThuhoCN, " +
					  ...
					  "		where LOAI = 1  "+
					  "		union all   "+
				      "	    select loai,  substring (pk_seq,0,7) + ' -- 4' as pk_Seq,ten,ma,npp_fk from KhachHang_ThuTien_ERP   \n"+
					  "		where LOAI = 4  "+
		...
		if (rs != null) {
			try {
				while (rs.next()) {
					...
					System.out.println("soChungTu_So: " + soChungTu_So);
					...
					System.out.println("this.isthuhoCN" +this.isthuhoCN);
					this.isNPP = rs.getString("isNPP");
					System.out.println("nppid :"+this.nppId);
					System.out.println("ISNPP :"+this.isNPP);
	...
	private void createDoiTuongDinhKhoanCo() {
		...
		if (this.id.trim().length() > 0 && this.ndId.equals("100002")) {
			String command = "SELECT distinct ttct.TAIKHOAN_FK, ttct.DOITUONGDINHKHOAN, ttct.DOITUONG_FK, ttct.SOTIEN, ttct.ISNPP,ISNULL(ttct.SOTIENNT,'0') as sotiennt  "+ 
...
//						String is_NPP = rsDKC.getString("isNPP");
						...
						String sotienNT = formatter.format(rsDKC.getDouble("SOTIENNT"));
	...
	public void initDisplay() {
		...
		String query =" SELECT tt.pk_seq, tt.ngaychungtu,isnull(tt.sochungtu_chu,'')sochungtu_chu, isnull(tt.sochungtu_so,'')sochungtu_so, tt.ngayghiso, tt.trangthai, tt.khachhang_fk as khachhang_fk, \n"+
					  ...
					  " ISNULL(tt.SOTIENTTNT, 0) AS SOTIENTTNT, ISNULL(tt.TIGIA, 0) AS TIGIA, tt.KBH_FK, \n"+
					  " isnull(tt.chietkhau,0) as chietkhau,isnull(tt.chietkhauNT,0) as chietkhauNT, ISNULL(NGUOINOPTIEN,'') as NGUOINOPTIEN, ISNULL(tt.DIACHI,'') as DIACHI , \n"+
					  " xoakhtratruoc_fk , NHOMKHTT_FK, isnull(tt.ctkemtheo,'') chungtukemtheo, doituong, nhomkenh_fk, tt.BANGKE_FK, isnull(convert(nvarchar, tt.NPPCN_FK), '') NPPCN_FK , ISNULL(TT.isNPP, 0) isNPP , isnull(tt.ISThuhoCN, 0) ISThuhoCN, \n"+
						" case when tt.noidungtt_fk=100006 then isnull(khachhang_thutien_quay.ma,'') +' -- ' + isnull(khachhang_thutien_quay.ten,'') \n"+
						" when tt.NPPCN_FK is null then isnull(npp.maFast,'') +' -- ' + isnull(npp.ten,'')  " +
						" else isnull(khachhang_thutien.ma,'') +' -- ' + isnull(khachhang_thutien.ten,'') end  as nppTen  \n"+
				 		" from ERP_THUTIEN tt \n" + 
		...
		if (rs != null) {
			try {
				while (rs.next()) {
					...
					if(this.nppId.trim().length()>0)
						this.nppIdgoc = this.nppId  + " -- "+this.isNPP;
					this.kh_npp_Ten = rs.getString("nppTen")==null?"":rs.getString("nppTen");
	...
	public void taoMoiSoChungTu(){
		if(this.ngaychungtu.length() > 0 ){
			this.soChungTu_Chu = "GBC"+this.ngaychungtu.substring(5,7)+ this.ngaychungtu.substring(0, 4);
			this.soChungTu_So = geso.traphaco.center.util.Utility.generataSoChungTu(db, this.soChungTu_Chu,
					geso.traphaco.center.util.Utility.tblThuTien_CK ,
					geso.traphaco.center.util.Utility.httt_CK, this.id, this.ngaychungtu);
			this.sochungtu = soChungTu_Chu + soChungTu_So;
		}
	}
...
//					this.nppRs = db.getScrol( KH_CN );
			...
			if(this.nppChinhanhId.trim().length() > 0)
			{
				...
				///// '2017-06-21' yêu cầu thu tiền khách hàng quầy có thể chọn 1 đối tượng khác làm đại diện.....DUNGBHN
				query =
					"select SUBSTRING (kh.pk_seq, 0, 7) + ' -- 1' as pk_Seq, kh.tenLoai + ' -- ' + kh.ma + ' -- ' + kh.ten as nppTen from [KhachHang_ThuTien_ERP] kh\n" + 
					"where LOAI = 1 and npp_fk = " + this.nppChinhanhId  + " \n" + 
				    " union all \n" + 
					"select SUBSTRING (kh.pk_seq, 0, 7) + ' -- 4' as pk_Seq, kh.tenLoai + ' -- ' + kh.ma + ' -- ' + kh.ten as nppTen from [KhachHang_ThuTien_ERP] kh\n" + 
					"where LOAI = 4 and ( npp_fk = " + this.nppChinhanhId +" OR NPP_FK=0 )";
				System.out.println(query);
//				this.nppRs = db.getScrol( KH_CN );
			}
		}
		/// LẤY QUERY này ra để cho vào ajax!
			...
			if (this.ndId.equals("100000")) { // THU TIEN HOA DON
				...
				query += " 		GROUP BY HOADON_FK \n"+
					...
					query += " UNION ALL \n"+
							...
							+ "AND (HINHTHUCTHANHTOAN = N'TM/CK' OR HINHTHUCTHANHTOAN = N'Chuyển khoản') \n"
				...
				query += " UNION ALL " 
						...
						+ " AND GG.TRANGTHAI = '1' "
						+ " AND GG.NGAYGHINHAN <= '" + this.ngayghiso + "' " ;
				...
				if (this.DoiTuongTamUng.equals("1")) // NHA CUNG CAP
				{
					...
					query += " SELECT '2' as LOAIHD, ISNULL(TT.TIGIA,100000) AS TIGIA, TT.PK_SEQ, 'TU' AS KYHIEU, CAST(TT.PK_SEQ  AS NVARCHAR) AS SOHOADON, TT.NGAYGHINHAN AS NGAYHOADON, \n"
							...
							+ "  (TTHD.SOTIENTT - ISNULL(DAXOATU.SOTIEN_XOATU,0) - ISNULL(DATHANHTOAN.SOTIENTT,0) - ISNULL(DACANTRU.CANTRU, 0)) \n "
...
/*							+ " (TTHD.SOTIENTT*TT.TIGIA - (ISNULL(DAXOATU.SOTIEN_XOATU,0) * ISNULL(DAXOATU.TIGIA,0)) " 
							...
							+ " - (ISNULL(DATHANHTOAN.SOTIENTT,0)* ISNULL(DATHANHTOAN.TIGIA,0)) "
							+ " - ISNULL(DACANTRU.CANTRU, 0)*ISNULL(DACANTRU.TIGIA, 0) " 
							+ "	) AS SOTIENVND, \n"
*/							
							+ " CASE WHEN "
							+ this.tienteId
							+ "= 100000 THEN  0  ELSE (TTHD.SOTIENTT - ISNULL(DAXOATU.SOTIEN_XOATU,0) - ISNULL(DATHANHTOAN.SOTIENTT,0) - ISNULL(DACANTRU.CANTRU, 0)) END AS SOTIENNT , \n"
				...
				} else // NHAN VIEN
				{
					...
					query += " SELECT 0 as LOAIHD, ISNULL(TT.TIGIA,100000) AS TIGIA, TT.PK_SEQ, 'TU' AS KYHIEU, TT.PK_SEQ AS SOHOADON, TT.NGAYGHINHAN AS NGAYHOADON, \n"
...
//		}
		...
		String sql = "select pk_seq, ma + ', ' + ten as nppTen from  " + geso.traphaco.center.util.Utility.prefixDMS + "KhachHang where trangthai = '1' and CONGTY_FK = "+ this.ctyId + " and npp_fk = " + this.nppdangnhap;

		if (this.ndId.equals("100002")) // PHẢI THU KHÁC --> LOAD NCC
		{
			sql = "select pk_seq, ma + ', ' + ten as nppTen from ERP_NHACUNGCAP where trangthai = '1' and duyet = '1' and CONGTY_FK = "	+ this.ctyId + " ";
			this.nppRs = db.get(sql);
		}
		...
		if (this.nhomkhttId.trim().length() > 1 || this.bangkeId.trim().length() > 0 || this.nppId.trim().length() > 1 || this.nccId.length() > 0 || this.nvtuId.length() > 0 && this.htttId.length() > 0 && this.hoadonList.size() <= 0) 
		{
			if (this.ndId.equals("100000")) { // THU TIEN HOA DON
				...
				} else // NHAN VIEN
				{
					...
					query += " SELECT TT_HD.LOAIHOADON AS LOAIHD, ISNULL(TT.TIGIA,100000) AS TIGIA, TT.PK_SEQ, 'TU' AS KYHIEU, TT.PK_SEQ AS SOHOADON, TT.NGAYGHINHAN AS NGAYHOADON, \n"
							
							+ " (TTHD.SOTIENTT - ISNULL(DACANTRU.CANTRU,0) - (ISNULL(DAXOATU.SOTIEN_XOATU,0) * ISNULL(DAXOATU.TIGIA,0))) AS SOTIENGOC, \n "  

							+ " CASE WHEN TT.TIENTE_FK = 100000 THEN  \n "  
							+ "	(TTHD.SOTIENTT - ISNULL(DACANTRU.CANTRU,0) - ISNULL(DAXOATU.SOTIEN_XOATU,0)  - DATHANHTOAN.SOTIENTT ) \n " 
							+ " ELSE \n " 
							+ "(TTHD.SOTIENTT*TT.TIGIA - ISNULL(DACANTRU.CANTRU,0) - (ISNULL(DAXOATU.SOTIEN_XOATU,0)* ISNULL(DAXOATU.TIGIA,0)) \n " 
							+ "- (ISNULL(DATHANHTOAN.SOTIENTT,0)* ISNULL(DATHANHTOAN.TIGIA,0)) ) END AS SOTIENVND,  \n " 
	...
	{
		/*String query = "select pk_seq from erp_thutien where pk_seq in (" +
				"100937, 100944, 100947, 100948, 100949, 100950, 100951, 100953, 100955, 100957, 100961, 100962, 100963, 100965, 100966, 100967, 100968, 100969, 100971, 100972, 100973, 100974, 100975, 100976, 100977, 100978, 100979, 100980, 100983, 100984, 100985, 100986, 100987, 100988, 100989, 100993, 100994, 100995, 100996, 100997, 100998, 101000" +
				")";
		try {
			dbutils db = new dbutils();
			ResultSet rs = db.get(query);
			if (rs != null)
			{
				while (rs.next())
				{
					...
					ErpThutienCK tt = new ErpThutienCK();
					tt.setId(id);
					tt.setCtyId("100000");
					tt.init();
					tt.chotTTHD("100819");
