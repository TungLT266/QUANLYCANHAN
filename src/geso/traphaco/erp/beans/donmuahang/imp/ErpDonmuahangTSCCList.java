...
public class ErpDonmuahangTSCCList extends Phan_Trang implements IErpDonmuahangTSCCList 
{
	...
	public void init(String search)
	{
		...
		if( !this.userId.equals("100002")  && util.GetquyenNew("ErpDonmuahangTSCCListSvl", "&loai="+loai,userId)[9]==0){
			query += " and a.DONVITHUCHIEN_FK in ( select  " +
					" distinct PHONGBAN_FK from NHANVIEN where PK_SEQ ="+this.userId+" )";
		}
		
		if( !this.userId.equals("100002") && checkTruongPhong() == false  && util.GetquyenNew("ErpDonmuahangTSCCListSvl", "&loai="+loai,userId)[9]==0){
