...
public class ErpDonmuahang_Giay implements IErpDonmuahang_Giay {
	...
	public void createRs() {
		...
		this.nccRs = db.get(" select PK_SEQ, MA +'-'+ TEN as TEN from ERP_NHACUNGCAP " +
	            ...
	            " ERP_DUYETNHACUNGCAP where TRANGTHAI = 1)  and  TAIKHOAN_FK in " +
	            " (select PK_SEQ from ERP_TAIKHOANKT where npp_fk =1 and SOHIEUTAIKHOAN = 33112000) ");
...
//		ErpDonmuahang_Giay obj = new ErpDonmuahang_Giay();;
	...
	private void createPopupNguoiDuyet(){
		String sql = " select cd.DIENGIAI, d.TRANGTHAI, cd.NHANVIEN_FK, nv.TEN " +
					 " from ERP_DUYETMUAHANG d " +
					 " inner join ERP_MUAHANG mh on mh.PK_SEQ = d.MUAHANG_FK " +
					 " left join ERP_CHUCDANH cd on cd.PK_SEQ = d.CHUCDANH_FK" +
					 " left join NHANVIEN nv on nv.PK_SEQ = cd.NHANVIEN_FK " +
	...
	private void createSanpham() {
		...
		// " +
				...
				" isnull(a.diengiai, b.ten +'-'+ m.MA )   as spTen, \n" + "  b.ten +'-'+ isnull(m.MA,'')  as spTen2, 'NA' as spNh, \n"
...
//		String loainccId = "";
		...
		if(err_same_sp.length()>0 ){
			this.msg=  "Thông tin sản phẩm bị trùng tai:"+  err_same_sp  + " vui lòng kiểm tra lại đơn hàng " ;
			return false;
		}
...
//			boolean isKhangSinh = false;
			...
			for (int i = 0; i < this.spList.size(); i++) {
				...
				double dongiaviet = Double.parseDouble(formatter_4le.format((Double.parseDouble(sp.getDongia()) * this.TyGiaQuyDoi))) ;
...
//								isNK = true;
						...
						query = " insert into ERP_MUAHANG_SP(muahang_fk, sanpham_fk, taisan_fk, ccdc_fk, chiphi_fk, diengiai, " +
								...
								" ngaynhan, khott_fk, nhomkenh_fk, dungsai, PhanTramThue, ThueNhapKhau, TENHD,IDMARQUETTE) "
								...
								+ this.dungsai + "," + ptThue + ", " + thueNK + ", N'" + sp.getTenHD() + "',"+sp.getIdmarquette()+")";
...
//			boolean isKhangSinh = false;
			...
			for (int i = 0; i < this.spList.size(); i++) {
				...
				double dongiaviet = Double.parseDouble(formatter_4le.format((Double.parseDouble(sp.getDongia()) * this.TyGiaQuyDoi))) ;
				System.out.println("don gia viet :"+dongiaviet +"ti gia quy doi : "+ this.TyGiaQuyDoi);
...
//									isNK = true;
						...
						query = " insert into ERP_MUAHANG_SP(muahang_fk, sanpham_fk, taisan_fk, ccdc_fk, chiphi_fk, diengiai, donvi, soluong, " +
								...
								" dungsai, PhanTramThue, ThueNhapKhau,Muanguyenlieudukien_fk, TENHD, IDMARQUETTE) "
