package geso.traphaco.erp.beans.bobaocaotonghop.imp;
...
public class BoBaoCaoTongHop  implements IBoBaoCaoTongHop  {
	String userId;
	String ctyId;
	String nppId;
	String ctyTen;
	...
	public void init(){
		...
		String thangNamPhatSinh = "";
		int thangdauky = 0;
		int namdauky = 0;
		String thangNamDauKy = "";
		String querySearchDauKy = "";
		String querySearchDauKyDMS = "";
		String querySearchTuNgay = "";
		String querySearchTuNgayDMS = "";
		String querySearchDenNgay = "";
		String querySearchDenNgayDMS = "";
		String querySearchCuoiKy = "";
		String querySearchCuoiKyDMS = "";
		if ((this.year.length() > 0 && this.month.length() > 0) || (this.tuNgay.length()>0 && this.denNgay.length()>0)) {
			...
			if (this.tuNgay.length() >0 &&  this.denNgay.length()>0){
				int lastmonth ;
				int lastyear ;
				lastyear = Integer.parseInt(this.tuNgay.substring(0,4)) - 1;
				...
				if (thangKS != 0 && namKS != 0){
					if((thangKS >= thangdauky && namKS == namdauky) || (namKS > namdauky) ){
				...
				thangNamDauKy = "0"+ Integer.toString(thangdauky);
				thangNamDauKy = thangNamDauKy.substring(thangNamDauKy.length() - 2);
				thangNamDauKy = Integer.toString(namdauky)+"-"+thangNamDauKy; 
				
				querySearchDauKy = "AND PS.NGAYGHINHAN >'"+thangNamDauKy+"-31"+"' AND PS.NGAYGHINHAN < '"+this.tuNgay+ "'";
				querySearchTuNgay= " AND PS.NGAYGHINHAN >= '"+this.tuNgay+"' ";
				querySearchDenNgay= " AND PS.NGAYGHINHAN<= '"+this.denNgay+"' ";
				querySearchCuoiKy =  "AND PS.NGAYGHINHAN >'"+thangNamDauKy+"-31"+"' AND PS.NGAYGHINHAN <= '"+this.denNgay+"'";
				
				querySearchDauKyDMS = "AND PS.NGAYGHINHAN >''"+thangNamDauKy+"-31"+"'' AND PS.NGAYGHINHAN < ''"+this.tuNgay+ "''";
				querySearchTuNgayDMS= " AND PS.NGAYGHINHAN >= ''"+this.tuNgay+"'' ";
				querySearchDenNgayDMS= " AND PS.NGAYGHINHAN<= ''"+this.denNgay+"'' ";
				querySearchCuoiKyDMS =  "AND PS.NGAYGHINHAN >''"+thangNamDauKy+"-31"+"'' AND PS.NGAYGHINHAN <= ''"+this.denNgay+"''";
			...
			if(this.year.length() > 0 && this.month.length() > 0){
				...
				thangNamDauKy = thangNamDauKy.substring(thangNamDauKy.length() - 2);
				thangNamDauKy = Integer.toString(namdauky)+"-"+thangNamDauKy;
				thangNamPhatSinh = "0"+ this.month;
				thangNamPhatSinh = thangNamPhatSinh.substring(thangNamPhatSinh.length()-2);
				thangNamPhatSinh = this.year+"-"+thangNamPhatSinh ;
						
				querySearchDauKy  = "AND PS.NGAYGHINHAN >'"+thangNamDauKy+"-31"+"' AND PS.NGAYGHINHAN < '"+thangNamPhatSinh +"-01'";
				querySearchTuNgay = " AND PS.NGAYGHINHAN >= '"+thangNamPhatSinh +"-01'";
				querySearchDenNgay = "  AND PS.NGAYGHINHAN<= '"+thangNamPhatSinh +"-31'";
				querySearchCuoiKy =  "AND PS.NGAYGHINHAN >'"+thangNamDauKy+"-31"+"' AND PS.NGAYGHINHAN <= '"+thangNamPhatSinh +"-31'";
				
				querySearchDauKyDMS  = "AND PS.NGAYGHINHAN >''"+thangNamDauKy+"-31"+"'' AND PS.NGAYGHINHAN < ''"+thangNamPhatSinh +"-01''";
				querySearchTuNgayDMS = " AND PS.NGAYGHINHAN >= ''"+thangNamPhatSinh +"-01''";
				querySearchDenNgayDMS = "  AND PS.NGAYGHINHAN<= ''"+thangNamPhatSinh +"-31''";
				querySearchCuoiKyDMS =  "AND PS.NGAYGHINHAN >''"+thangNamDauKy+"-31"+"'' AND PS.NGAYGHINHAN <= ''"+thangNamPhatSinh +"-31''";
			...
			sql = 	" BEGIN \n" +
					...
					"    SELECT TK.PK_SEQ AS TAIKHOANID,CONVERT(VARCHAR,TK.SOHIEUTAIKHOAN) AS SOHIEUTAIKHOAN,TK.TENTAIKHOAN  \n" +
					...
					"  LEFT JOIN (  \n" +
					 ...
					 "       END AS CO   \n" + 
					"    FROM (  \n" +
					"     SELECT KS.TAIKHOANKT_FK AS TKID,TK.LOAITKBAOCAO,TK.SOHIEUTAIKHOAN,KS.MADOITUONG,  \n" +
					"    CASE WHEN KS.DOITUONG = N'Khách hàng' AND ISNULL(KS.ISNPP,'0') = '1' THEN N'Chi nhánh/Đối tác'else  KS.DOITUONG END AS DOITUONG \n" +
					...
					"     GROUP BY KS.TAIKHOANKT_FK,TK.SOHIEUTAIKHOAN,KS.MADOITUONG,KS.DOITUONG,TK.LOAITKBAOCAO,KS.ISNPP  \n" +
					"     UNION ALL  \n" +
					"     SELECT PS.TAIKHOAN_FK AS TKID,TK.LOAITKBAOCAO,TK.SOHIEUTAIKHOAN,PS.MADOITUONG,  \n" +
					"    CASE WHEN PS.DOITUONG = N'Khách hàng' AND ISNULL(PS.ISNPP,'0') = '1' THEN N'Chi nhánh/Đối tác'else  PS.DOITUONG END AS DOITUONG \n" +
					...
					"     WHERE 1= 1 " +querySearchDauKy+ "\n " +
					"     AND TK.NPP_FK IN ("+this.nppId+") AND TK1.NPP_FK IN ("+this.nppId+")  \n" +
					"     GROUP BY PS.TAIKHOAN_FK,TK.SOHIEUTAIKHOAN,PS.MADOITUONG,PS.DOITUONG,TK.LOAITKBAOCAO,ps.isnpp  \n" +
				"     ) DAUKY_PHATSINH_THEODOITUONG  \n" +
					"    GROUP BY DAUKY_PHATSINH_THEODOITUONG.TKID,DAUKY_PHATSINH_THEODOITUONG.SOHIEUTAIKHOAN ,DAUKY_PHATSINH_THEODOITUONG.LOAITKBAOCAO \n" +
					"     ,DAUKY_PHATSINH_THEODOITUONG.MADOITUONG,DAUKY_PHATSINH_THEODOITUONG.DOITUONG  \n" +
					"    ) DAUKY_PHATSINH_THEOTAIKHOAN  \n" +
					"   GROUP BY TKID,LOAITKBAOCAO,SOHIEUTAIKHOAN  \n" +
					...
					"    INNER JOIN ERP_TAIKHOANKT TK ON TK.PK_SEQ = PS.TAIKHOAN_FK  \n" +
					"    INNER JOIN ERP_TAIKHOANKT TK1 ON TK1.PK_SEQ = PS.TAIKHOANDOIUNG_FK  \n" +
					"    WHERE 1=1 "+querySearchTuNgay + " " + querySearchDenNgay + " AND TK.NPP_FK IN ("+this.nppId+") AND TK1.NPP_FK IN ("+this.nppId+")  \n" +
					"    GROUP BY PS.TAIKHOAN_FK,TK.SOHIEUTAIKHOAN,PS.MADOITUONG,PS.DOITUONG,TK.LOAITKBAOCAO,PS.ISNPP  \n" +
					"    ) PS_THEODOITUONG  \n" +
				...
				/*"     UNION ALL  \n" +   
					...
					"     INNER JOIN ERP_TAIKHOANKT TK ON TK.PK_SEQ = PS.TAIKHOAN_FK  \n" +
					"     INNER JOIN ERP_TAIKHOANKT TK1 ON TK1.PK_SEQ = PS.TAIKHOANDOIUNG_FK  \n" +
					"     WHERE month(PS.NGAYGHINHAN) = @THANG AND YEAR(PS.NGAYGHINHAN) = @NAM AND TK.NPP_FK IN (@NPP_FK) AND TK1.NPP_FK IN (@NPP_FK)  \n" +
					...
					"    GROUP BY CUOIKY_PHATSINH_THEODOITUONG.TKID,CUOIKY_PHATSINH_THEODOITUONG.SOHIEUTAIKHOAN ,CUOIKY_PHATSINH_THEODOITUONG.LOAITKBAOCAO \n" +
					...
					" 	FROM  \n" +
					" 	( \n" +
					" 	SELECT ISNULL(SUM(CUOIKYNAY),0) AS CUOIKYNAY,ISNULL(SUM(CUOIKYTRUOC),0) CUOIKYTRUOC FROM #CANDOIKETOAN \n" +
					" 	WHERE MASO IN ('214') \n" +
					" 	)DATA  \n" +
					" 	WHERE MASO IN ('133') \n" +
					"  \n" +
					...
					"  \n" +
					" 	--MÃ SỐ 141 : NỢ TK (151, 152, 153, 155, 156, 157, 158) - MÃ SỐ 263 \n" +
					" 	INSERT INTO #CANDOIKETOAN \n" +
					...
					"  \n" +
					" 	--MÃ SỐ 319 : CÓ TK (338, 138, 3441) - CÓ TK (3387) - MÃ SỐ 337 \n" +
					" 	INSERT INTO #CANDOIKETOAN \n" +
	...
	public String getQueryHDKD(){
		...
		if(this.month.length() > 0 && this.year.length() >0){
			...
			querySearchDauKy="AND MONTH(NGAYGHINHAN) = " + thangdauky + " AND YEAR(NGAYGHINHAN) = " + namdauky + "  \n" ;
			querySearchTrongKy	= "  AND MONTH(NGAYGHINHAN) = " + month + " AND YEAR(NGAYGHINHAN) = " + year + " \n" ;
		}
		if (this.tuNgay.length() >0 && this.denNgay.length() >0){
			month = Integer.parseInt(this.tuNgay.substring(5, 7));
			year = Integer.parseInt(this.tuNgay.substring(0,4));
			lastyear = Integer.parseInt(this.tuNgay.substring(0, 4)) - 1;
			lastmonth = 0;
			...
			querySearchDauKy="AND MONTH(NGAYGHINHAN) = " + thangdauky + " AND YEAR(NGAYGHINHAN) = " + namdauky + "  \n" ;
			querySearchTrongKy="AND NGAYGHINHAN>='"+this.tuNgay+"' AND NGAYGHINHAN <='"+this.denNgay+"' ";
		}
		...
		query ="SELECT MASO,SUM(KYNAY) AS KYNAY,SUM(KYTRUOC) AS KYTRUOC  \n" + 
				 ...
				 "  "+querySearchTrongKy+"  \n" +
				 "  AND TK.NPP_FK = " + this.nppId + " AND TKDU.NPP_FK = " + this.nppId + "  \n" +
				 "  UNION ALL  \n" + 
				 "  --KY TRUOC  \n" + 
...
// Lấy kỳ này
	...
	public double getTongPhatSinh(int loai, String Sohieu, String SohieuDoiung){
		...
		for(int i = 0 ; i < Sohieutk.length ; i ++)
		{
			...
			sohieutkdms += "TK.SOHIEUTAIKHOAN like ''"+ Sohieutk[i].trim() +"%'' or ";
		}
		
		
		int index = sohieutk.lastIndexOf("or");
		sohieutk = "( " + sohieutk.substring(0, index) + ")";
		index = sohieutkdms.lastIndexOf("or");
		sohieutkdms = "( " + sohieutkdms.substring(0, index) + ")";
		System.out.println("Chuỗi số hiệu TK :" + sohieutk);
		String sohieutkdu = "";
		String sohieutkdudms ="";
		String congTy = this.getErpCongtyId();
		if (congTy.trim().endsWith(",")){
			congTy = congTy.substring(0,congTy.length() -1);
		}
		...
		if(SohieuDoiung.length() > 0){
			...
			for(int i = 0 ; i < Sohieutkdu.length ; i ++)
			{
				...
				sohieutkdudms += "TK1.SOHIEUTAIKHOAN like ''"+ Sohieutkdu[i].trim() +"%'' or ";
			}
		
			
			index = sohieutkdu.lastIndexOf("or");
			sohieutkdu ="("+ sohieutkdu.substring(0, index) + ")";
			index = sohieutkdudms.lastIndexOf("or");
			sohieutkdudms ="("+ sohieutkdudms.substring(0, index) + ")";
			System.out.println("Chuỗi số hiệu TK đối ứng : " + sohieutkdu);
			//sohieutkdu = tmp + sohieutkdu + ")) ";
		}
		if(loai == 0){
			
		//CÂU SQL KHÔNG TÍCH HỢP DMS
			/*query = "SELECT ISNULL(SUM(ISNULL(ROUND(PS.NO, 0), 0)), 0) AS GIATRI \n " +
					"FROM ERP_PHATSINHKETOAN PS \n " +
					"INNER JOIN ERP_TAIKHOANKT TK ON TK.PK_SEQ = PS.TAIKHOAN_FK  \n " +
					"WHERE MONTH(NGAYGHINHAN) = " + this.month + " and YEAR(NGAYGHINHAN) = " + this.year + "   \n " +
					"AND TK.CONGTY_FK IN (" + this.ErpCongTyId + ") AND " + sohieutk + " " ;
			if(sohieutkdu.length() > 0){
				query += "AND " + sohieutkdu + " \n ";
			}*/
		//CÂU SQL TÍCH HỢP DMS
			...
			query = "SELECT SUM(ISNULL(GIATRI,0)) AS GIATRI \n"+
					...
					"INNER JOIN ERP_TAIKHOANKT TK1 ON TK1.PK_SEQ = PS.TAIKHOANDOIUNG_FK  \n " +
					"WHERE MONTH(NGAYGHINHAN) = " + this.month + " and YEAR(NGAYGHINHAN) = " + this.year + "   \n " +
					"AND TK.NPP_FK IN (" + this.nppId + ") AND " + sohieutk + " \n" ;
			if(sohieutkdu.length() > 0){
				query += "AND " + sohieutkdu + " \n ";
			}
			...
			/*query = "SELECT ISNULL(SUM(ISNULL(ROUND(PS.CO, 0), 0)), 0) AS GIATRI \n " +
					"FROM ERP_PHATSINHKETOAN PS \n " +
					"INNER JOIN ERP_TAIKHOANKT TK ON TK.PK_SEQ = PS.TAIKHOAN_FK  \n " +
					"WHERE MONTH(NGAYGHINHAN) = " + this.month + " and YEAR(NGAYGHINHAN) = " + this.year + "   \n " +
					"AND TK.CONGTY_FK IN (" + this.ErpCongTyId + ") AND " + sohieutk + "  \n ";
	...
	public double getTongPhatSinh_Kytruoc(int loai, String Sohieu, String SohieuDoiung){
		...
		if(loai == 0){
			...
			/*query = "SELECT ISNULL(SUM(ISNULL(ROUND(PS.NO, 0), 0)), 0) AS GIATRI \n " +
					"FROM ERP_PHATSINHKETOAN PS \n " +
					"INNER JOIN ERP_TAIKHOANKT TK ON TK.PK_SEQ = PS.TAIKHOAN_FK  \n " +
					"WHERE MONTH(NGAYGHINHAN) = " + lastmonth + " and YEAR(NGAYGHINHAN) = " + lastyear + "   \n " +
					"AND TK.CONGTY_FK IN (" + this.ErpCongTyId + ") AND " + sohieutk + " " ;
			...
			query = "SELECT SUM(ISNULL(GIATRI,0)) AS GIATRI \n"+
					...
					"INNER JOIN ERP_TAIKHOANKT TK1 ON TK1.PK_SEQ = PS.TAIKHOANDOIUNG_FK  \n " +
					"WHERE MONTH(NGAYGHINHAN) = " + lastmonth + " and YEAR(NGAYGHINHAN) = " + lastyear + "   \n " +
					"AND TK.NPP_FK IN (" + congTy + ") AND " + sohieutk + " \n" ;
			if(sohieutkdu.length() > 0){
				query += "AND " + sohieutkdu + " \n ";
			}
	...
	public void createRs() {
		// TODO Auto-generated method stub
		String sql = "SELECT PK_SEQ,TEN FROM NHAPHANPHOI WHERE TRANGTHAI = 1 AND PK_SEQ = 1";
	...
	public void setTieuChiLayBC(String tieuChiLayBC) {
		this.tieuChiLayBC = tieuChiLayBC;
	}
	...
	public int[] getNamBC() {
		...
		int dem=Integer.parseInt(year)+5-2016;
		System.out.println("dem :"+dem);
		this.NamBC = new int[dem];
		for (int i=0 ;i < dem;i++)
		{
			this.NamBC[i]= 2016+i;
			System.out.println("NĂM :" +this.NamBC[i]);
		}
		return NamBC;
	}
