	...
	public void init(String query)
	{
		...
		else
		{
			sql = 	"SELECT TTDH.PK_SEQ AS ID, SP.MA + ' - ' + SP.TEN AS TENSP, ISNULL(ISMUA, 0) AS ISMUA, \n " +
					...
					"ISNULL(TTDH.LOTSIZE, 0) AS LOTSIZE, ISNULL(TTDH.CLEANUP, 0) AS CLEANUP," +
					"ISNULL(TGSX.THOIGIAN, 0) AS THOIGIANSX  \n " +
					...
					"LEFT JOIN \n " +
					"( \n " +
					"	SELECT AVG(THOIGIAN) AS THOIGIAN, TTDHID \n " +
					"	FROM( \n " +
					"		SELECT AVG(THOIGIAN) AS THOIGIAN, THONGTINDATHANG_FK AS TTDHID \n " +
					"		FROM ERP_THONGTINDATHANG_DAYTRUYENSX \n " +
					"		WHERE LOAI = 1 \n " +
					"		GROUP BY THONGTINDATHANG_FK \n " +

					"		UNION ALL \n " +
					"		SELECT MAX(THOIGIAN) AS THOIGIAN, THONGTINDATHANG_FK AS TTDHID \n " +
					"		FROM ERP_THONGTINDATHANG_DAYTRUYENSX \n " +
					"		WHERE LOAI = 2 \n " +
					"		GROUP BY THONGTINDATHANG_FK \n " +
					"	)TGSX GROUP BY TTDHID \n " +
						
					")TGSX ON TGSX.TTDHID = TTDH.PK_SEQ \n " +

					"WHERE TTDH.CONGTY_FK = '" + this.ctyId + "' AND isMUA = " + this.isMua ;
