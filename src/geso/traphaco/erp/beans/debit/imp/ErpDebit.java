...
public class ErpDebit implements IErpDebit{
	private String id;
	...
	public ErpDebit(String id, String ngayGhiNhan, String doiTuong,
	...
	public void initRsDoiTuong(){
		String query = 
				" select PK_SEQ as MA, kh.Ten as TEN, 'KH' as loai from ERP_KHACHHANG kh \n" +
	...
	public boolean create() {
		try{
			...
			// insert erp_debit
			String sql = "insert into ERP_DEBIT(DOITUONG, LOAIDOITUONG, NGAYGHINHAN, DIENGIAI, TONGTIENNGUYENTE, " +
			...
			String ckCurrent = "";
			sql = "select IDENT_CURRENT('ERP_DEBIT') as ckId";
			...
			if( this.taiKhoans !=null && this.noNguyenTes !=null && this.noVNDs !=null && this.trungTamChiPhis != null){
				for(int i=0; i< this.taiKhoans.size(); i++){
					// insert từng dòng
					if( taiKhoans.get(i).trim().length()> 0){
						sql = " insert into ERP_DEBIT_DETAILS (DEBIT_FK, TAIKHOAN_FK, DIENGIAI, NONGUYENTE, " +
								" NOVND, TRUNGTAMCHIPHI_FK, KHOANMUCCHIPHI_FK) values(?,?,?,?,?,?,?)";
	...
	public boolean update() {
		try{
			this.db.getConnection().setAutoCommit(false);
			// insert erp_debit
			String sql = " update ERP_DEBIT set DOITUONG = ?, NGAYGHINHAN =?, DIENGIAI = ?, " +
			...
			sql ="delete from ERP_DEBIT_DETAILS where DEBIT_FK = "+ this.id;
	...
	public void getView() {
		...
		try{
			...
			String sql =" select PK_SEQ, DOITUONG, LOAIDOITUONG, NGAYGHINHAN, DIENGIAI, " +
					    " TONGTIENNGUYENTE,TONGTIENVND, TIENTE_FK, TIGIA from ERP_DEBIT where PK_SEQ = ?";
			...
			sql =" select DEBIT_FK, DIENGIAI, NONGUYENTE, NOVND, KHOANMUCCHIPHI_FK, TRUNGTAMCHIPHI_FK," +
				 " TAIKHOAN_FK from ERP_DEBIT_DETAILS where DEBIT_FK= ?";
			PreparedStatement prek = this.db.getConnection().prepareStatement(sql);
			prek.setInt(1, Integer.parseInt(this.id));
