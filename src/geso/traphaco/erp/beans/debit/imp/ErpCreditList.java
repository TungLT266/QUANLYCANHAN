...
public class ErpCreditList implements IErpCreditList {
	...
	public ErpCreditList(String ngayGhiNhan, String nguoiTao, String doiTuong,
	...
	public boolean delete(String id) {
		...
		String sql ="update ERP_CREDIT set TRANGTHAI ='2' where PK_SEQ ="+ id;
	...
	public boolean chot(String id) {
		try {
			this.db.getConnection().setAutoCommit(false);
			String sql =   "  select de.PK_SEQ, credit.TIENTE_FK, de.DIENGIAI,credit.DOITUONG, NONGUYENTE, NOVND, " +
					       " de.TAIKHOAN_FK, credit.NGAYGHINHAN, " +
					       " CASE WHEN credit.LOAIDOITUONG = 1 THEN ncc.TAIKHOAN_FK  " +
					       " WHEN  credit.LOAIDOITUONG = 2 THEN kh.TaiKhoan_FK  " +
					       " END as taiKhoanDoiTuong, credit.LOAIDOITUONG, credit.TIGIA, de.SOLUONG, de.DONGIA  " +
					       " from ERP_CREDIT_DETAILS de inner join ERP_CREDIT credit on credit.PK_SEQ = de.CREDIT_FK   " +
					       " left join ERP_NHACUNGCAP ncc on ncc.PK_SEQ = credit.DOITUONG   " +
					       " left join ERP_KHACHHANG kh on kh.PK_SEQ = credit.DOITUONG  "+
						   " where CREDIT_FK = "+ id;
			...
			List<Double> donGias = new ArrayList<Double>();
			...
			if( rs !=null){
				...
				while( rs.next()){
					...
					soLuongs.add(rs.getInt("SOLUONG"));
					donGias.add(rs.getDouble("DONGIA"));
				}
				} catch ( Exception ex){
					ex.printStackTrace();
					this.db.getConnection().rollback();
			...
			for(int i=0; i< taiKhoans.size(); i++){
				...
				String loaichungtu ="credit";
				String sochungtu = soChungTus.get(i);// mã pk_seq bảng credit_details
				// tài khoản nợ là NCC, nhân viên
				String taikhoanNO_fk = taiKhoans.get(i);
				// tài khoản có là tài khoản 
				String taikhoanCO_fk = taiKhoanDoiTuongs.get(i);
				...
				String MADOITUONG_CO = doiTuongs.get(i);
				String DOITUONG_NO ="";
				String MADOITUONG_NO = "";
				
				String LOAIDOITUONG ="0";
				
				String SOLUONG = soLuongs.get(i).toString();
				String DONGIA = donGias.get(i).toString();
			...
			sql = "update ERP_CREDIT set TRANGTHAI ='1' where PK_SEQ ="+ id;
			int k = this.db.updateReturnInt(sql);
			if (k != 1) {
				this.db.getConnection().rollback();
				return false;
