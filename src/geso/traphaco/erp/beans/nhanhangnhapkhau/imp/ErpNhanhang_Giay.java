...
public class ErpNhanhang_Giay implements IErpNhanhang_Giay {
	...
	public void createRs() {
		...
		String cd ="";
		System.out.println("");
		cd = "select PK_SEQ, ma + ', ' + TEN as ten  from ERP_NHACUNGCAP where trangthai = 1 and duyet = '1' and pk_seq = '"+ this.nccId + "' ";
		this.nccRs = db	.getScrol(cd);
		System.out.println("NPP:"+cd);
		

		// lấy khu vực kho nhận 
		...
		if (this.nccId.trim().length() > 0) {
			...
			try{
				  sql = " select isnull(SOCHUNGTU,'') as SOCHUNGTU from ERP_THUENHAPKHAU A  INNER JOIN ERP_THUENHAPKHAU_HOADONNCC   B ON A.PK_SEQ=B.THUENHAPKHAU_FK " +
						" where B.HOADONNCC_FK  ="+ this.hdNccId;
				  
				  System.out.println("get hoa don to khai :"+sql);
	...
	private void xuLySoluongTungSp(ISanpham sp,double tile,String dvdl_chuan, int index){
		...
		DecimalFormat formatter_USD = new DecimalFormat("###,###.#####");
		
		DecimalFormat formatterVND = new DecimalFormat("###,###,###");
		System.out.println("Tỉ lệ : "+tile);
		
		sp.setTiLeQuyDoiDv(tile);
		//tile = 1/tile;
		double tile_new = 1/tile; 
		...
		System.out.println("So luong dat: "+sp.getSoluongdat());
		System.out.println("Tỉ lệ : "+tile_new);
		...
		System.out.println("Dong gia ba con : "+dongia);
		
		sp.setDongia(formatter_USD.format(dongia));
		
		double dongialo_=dongia* Double.parseDouble(sp.getTigiaquydoi().replaceAll(",", ""));
		
		sp.setDongiaViet(formatterVND.format( dongialo_));
		// set dvdl
		...
		for (int j = 0; j < listct.size(); j++) {
			...
			spCon.setDongiaLo(formatterVND.format(dongialo_));
			listct.set(j, spCon);
		}
	...
	public void createRs1() {
		...
		if (this.KhoNhanId.trim().length() > 0) {
			...
			// lấy khu vực kho cho phần kho nhận
		...
		if (this.hdNccId.length() > 0) {
			...
			if(this.loaimh.equals("0")){ // NHẬP KHẨU
				query1 = " SELECT distinct  MAX( hdsp.SOTT) AS SOTT ,mh.PK_SEQ, hdsp.MUAHANG_FK MUAHANG_FK, mh.SOPO SOPO,  \n"
						...
						// thêm marquette
						+ "   m.PK_SEQ as idmarquette,  sp.kiemtradinhtinh, sp.kiemtradinhluong, \n"
						...
						+ "   (mhsp.DONGIA)*isnull(p.TIGIA,1) as dongiaViet, (mhsp.DONGIA) as dongia,              \n"
			...
			System.out.println("ds sanpham nhan hang 2000000 :" + query1);
				...
				try {
					while (rs.next()) {
						...
						sp.setSoluongMaxNhan(
								formatter.format(rs.getDouble("soluonghd") - rs.getDouble("soluongdanhan")));
						sp.setSoluongDaNhan(formatter.format(rs.getDouble("soluongdanhan")));
						...
						System.out.println("don gia viet  : "+formatter.format(rs.getDouble("dongiaViet")));
							...
							if(this.loaimh.equals("0") || this.loaimh.equals("1")){
								...
								{	
									...
									command = " select SANPHAM_FK, SOLUONG,SOLO, NGAYSANXUAT, NGAYHETHAN, DONGIA, " +
									  ...
									  " inner join ERP_NHANHANG_SANPHAM nhsp on nhsp.NHANHANG_FK = nh.PK_SEQ  and nhsp.SANPHAM_FK= ct.SANPHAM_FK   "+
									  " where nh.trangthai <> 3 and ct.SANPHAM_FK = d.SANPHAM_FK and ct.SOLO = d.SOLO  " +
									  " and nh.hdNCC_fk = d.HOADONNCC_FK and d.DONGIA = nhsp.DONGIA and CT.GIATHEOLO="+sp.getDongia()+" group by ct.SANPHAM_FK, ct.SOLO), 0) as soluongmax, " +
									  " d.NGAYSANXUAT as ngaysanxuat, d.NGAYHETHAN,isnull(MARRQUET,'') as MARRQUET,NSX_FK,ISNULL(NSX.TEN,'') AS NSXTEN " +
									  " from  ERP_HOADONNCC_DONMUAHANG d  " +
									  "	LEFT JOIN ERP_NHASANXUAT NSX ON NSX.PK_SEQ = d.NSX_FK "+
									  " where  d.hoadonncc_fk = "+this.hdNccId+" and d.sanpham_fk = "+sp.getId()+
									  "and d.soluong <> 0 and d.MUAHANG_FK = "+ sp.getMuahang_fk() +" and d.DONGIA=" + sp.getDongia();
								}
								else 
								{
									...
									command = " select SANPHAM_FK, SOLUONG,SOLO, NGAYSANXUAT, NGAYHETHAN, DONGIA, " +
											  ...
											  " where nh.trangthai <> 3 and ct.SANPHAM_FK = d.SANPHAM_FK and ct.SOLO = d.SOLO  " +
											  " and nh.hdNCC_fk = d.HOADONNCC_FK and d.DONGIA = nhsp.DONGIA  and ct.giatheolo="+sp.getDongia().replaceAll(",", "")+" " +
											  " group by ct.SANPHAM_FK, ct.SOLO), 0) as soluongmax, " +
											  " d.NGAYSANXUAT as ngaysanxuat, d.NGAYHETHAN,isnull(MARRQUET,'') as MARRQUET,NSX_FK,ISNULL(NSX.TEN,'') AS NSXTEN " +
											  " from  ERP_HOADONNCC_DONMUAHANG d  " +
											  "	LEFT JOIN ERP_NHASANXUAT NSX ON NSX.PK_SEQ = d.NSX_FK "+
											  " where  d.hoadonncc_fk = "+this.hdNccId+" and d.sanpham_fk = "+sp.getId()+
											  " and d.soluong <> 0 and d.MUAHANG_FK = "+ sp.getMuahang_fk() +" and d.DONGIA=" + sp.getDongia().replaceAll(",", "");
								}
								...
								if (spConRs != null) {
									while (spConRs.next()) {
										...
										String soluong = formatter.format(spConRs.getDouble("soluongmax"));
										...
										spCon.setSothung("1");
										String nsxid =spConRs.getString("NSX_FK") ==null?"":spConRs.getString("NSX_FK");
										spCon.setNSXid(nsxid);
	...
	public void init() {
		...
		String query = "";
				...
				query =   " SELECT isnull(A.ISTUDONG,0) as  ISTUDONG   ,a.PK_SEQ as nhId, a.trangthai, a.loaihanghoa_fk, a.noidungnhan_fk, a.SOHOADON, a.NGAYNHAN, a.NGAYCHOT, a.diengiai, b.pk_seq as dvthId, b.TEN as dvthTen,   \n"
						...
						+ "        isnull( ( select isnull(NGUONGOCHH, 'TN') from ERP_MUAHANG where PK_SEQ = c.pk_seq ), 'TN') as NGUONGOCHH, hdNCC_fk,  a.muahang_fk as muahang_fk,"
						+ " isnull(a.tigia, '1') as tigia, c.sopo as sopo  \n"
						+ " , isnull(e.SOCHUNGTU,'') as SOTOKHAI,isnull(p.ngayghinhan,'') as ngayhoadon "
						+ " FROM erp_nhanhang a " +
						...
						+ " left join ERP_THUENHAPKHAU_HOADONNCC TNK_HD ON TNK_HD.HOADONNCC_FK=a.hdNCC_fk  " +
								" left join ERP_THUENHAPKHAU e on e.PK_SEQ = TNK_HD.THUENHAPKHAU_FK   \n"
		...
		if (rs != null) {
			try {
				while (rs.next()) {
					...
					System.out.println("Kho cho xu ly : "+this.khoChoXuLy);
					System.out.println("kho nhân : "+this.KhoNhanId);
					this.soToKhai = rs.getString("SOTOKHAI");
				}
				rs.close();
			} catch (Exception e) {
	...
	private void initSanPham() {
		...
		else
		{
			query = "SELECT  distinct A.SOTT , case nh.loaihanghoa_fk when 0 then A.SANPHAM_FK when 1 then tscd.PK_SEQ when 3 then ccdc.pk_seq else ncp.pk_seq end as spId, \n"
				...
				+ " a.NGAYNHANDUKIEN, a.DUNGSAI, a.DONGIA, a.SOLUONGDAT, a.SOLUONGNHAN, isnull(sp.HANSUDUNG, '0') as HanSuDung, \n"
				+ " isnull(a.DonVi, 'NA') as donvi, a.TienTe_Fk, a.TyGiaQuyDoi, a.DonGiaViet, \n"
				+ " khott.pk_seq as khottId,   khott.ten as khottTen , a.SPKIEMDINH  ,a.TILEQUYDOI_DOLUONG   \n"
				+ " FROM ERP_NHANHANG_SANPHAM a inner join ERP_NhanHang nh on a.nhanhang_fk = nh.pk_seq   \n"
		...
		if (rsSp != null) {
			...
			try {
				...
				while (rsSp.next()) {
					...
					String soPO = rsSp.getString("SOPO");
					double TILEQUYDOI_DOLUONG =rsSp.getDouble("TILEQUYDOI_DOLUONG");
					...
					if (this.loaihanghoa.equals("0")) {}
					sanpham.setTiLeQuyDoiDv(rsSp.getFloat("TILEQUYDOI_DOLUONG"));
					...
					if (this.poId.trim().length() > 0) {
						String sql = "select sum(b.SOLUONGNHAN)  as soluongDaNhan  "
								...
								+ "where b.muahang_fk = '" + muahang_fk + "' and NGAYNHANDUKIEN = '" + ngaynhandk + "' and b.DONGIA = "+dongia.replaceAll(",", "")
					...
					List<ISpDetail> spConList = getListChiTietLo(muahang_fk,spId,dongia.replace(",", "") ,TILEQUYDOI_DOLUONG);
					sanpham.setSpDetail(spConList);
					
					// Lấy số mẻ từ bảng NHẬN HÀNG ra
					List<DetailMeSp> listMeSp = getListChiTietLoMe(muahang_fk,spId,dongia.replace(",", ""),TILEQUYDOI_DOLUONG);
	...
	private List<ISpDetail> getListChiTietLo( String muahang_fk, String SanPhamId, String dongia,double TILEQUYDOI_DOLUONG) throws SQLException{
		...
		double giachuaquydoi= Double.parseDouble(formatter_Usd.format((Double.parseDouble(dongia.replaceAll(",", ""))/ TILEQUYDOI_DOLUONG))) ;
		 
		
		//DecimalFormat formatter = new DecimalFormat("###,###,###.###");
		/*String  command = " select d.SANPHAM_FK, d.SOLUONG,d.SOLO, d.NGAYSANXUAT, d.NGAYHETHAN, d.DONGIA, " +
						  ...
						  " and a.SOLO = d.SOLO and a.GIATHEOLO = "+ dongia+"),0) as soluongthung" +
						  " from  ERP_HOADONNCC_DONMUAHANG d  " +
						  " where  d.hoadonncc_fk = "+this.hdNccId+" and d.sanpham_fk = "+SanPhamId +
						  "and d.soluong <> 0 and d.MUAHANG_FK = "+ muahang_fk + " and ROUND (d.DONGIA,4)  ="+   giachuaquydoi;*/
		/*String command =" select d.SANPHAM_FK, d.SOLUONG,d.SOLO, d.NGAYSANXUAT, d.NGAYHETHAN, d.DONGIA, "+
						" d.soluong - isnull((select SUM(ct.soluong) from ERP_NHANHANG_SP_CHITIET ct "+
						" inner join ERP_NHANHANG nh on ct.NHANHANG_FK = nh.PK_SEQ  where nh.trangthai <> 3 and ct.SANPHAM_FK = d.SANPHAM_FK and ct.SOLO = d.SOLO "+
						" and nh.hdNCC_fk = d.HOADONNCC_FK and round(ct.GIATHEOLO,4) ="+giachuaquydoi+"  group by ct.SANPHAM_FK, ct.SOLO), 0) as soluongmax,  "+
						" isnull((select SUM(ct.soluong) from ERP_NHANHANG_SP_CHITIET ct  inner join ERP_NHANHANG nh on ct.NHANHANG_FK = nh.PK_SEQ  where nh.trangthai <> 3 "+
						" and ct.SANPHAM_FK = d.SANPHAM_FK and ct.SOLO = d.SOLO   and ct.MUAHANG_FK = "+muahang_fk+"  and ct.NHANHANG_FK =  "+this.id+  
						" and ROUND(ct.GIATHEOLO,4) =  "+giachuaquydoi+ 
						" group by ct.SANPHAM_FK, ct.SOLO ), 0) as soluongdanhan,  d.NGAYSANXUAT as ngaysanxuat, d.NGAYHETHAN, "+
						" isnull((select COUNT(*) from ERP_NHANHANG_SP_CHITIET a  where SANPHAM_FK = "+SanPhamId+" and NHANHANG_FK = "+this.id+" and MUAHANG_FK =  "+muahang_fk+ 
						" and a.SOLO = d.SOLO and round( a.GIATHEOLO,4) = "+giachuaquydoi+" ),0) as soluongthung "+ 
						" from  ERP_HOADONNCC_DONMUAHANG d "+  
						" where  d.hoadonncc_fk = "+this.hdNccId +" and d.sanpham_fk = "+SanPhamId+" and d.soluong <> 0 and d.MUAHANG_FK = "+muahang_fk+" and ROUND (d.DONGIA,4)  ="+   giachuaquydoi;
		*/
		
		// 7_11_2016 Thu sua round gia
		
		String command =" select ISNULL(d.MARRQUET,'') AS MARRQUET,isnull(nsx.ten,'') as nsxten,d.nsx_fk,d.SANPHAM_FK, d.SOLUONG,d.SOLO, d.NGAYSANXUAT, d.NGAYHETHAN, d.DONGIA, "+
		" d.soluong - isnull((select SUM(ct.soluong) from ERP_NHANHANG_SP_CHITIET ct "+
		" inner join ERP_NHANHANG nh on ct.NHANHANG_FK = nh.PK_SEQ  where nh.trangthai <> 3 and ct.SANPHAM_FK = d.SANPHAM_FK and RTRIM(ltrim(ct.SOLO)) =RTRIM(ltrim(d.SOLO)) "+
		" and nh.hdNCC_fk = d.HOADONNCC_FK and round(ct.GIATHEOLO,4) ="+" round(cast( "+ giachuaquydoi +" as float),4)" +"  group by ct.SANPHAM_FK, ct.SOLO), 0) as soluongmax,  "+
		" isnull((select SUM(ct.soluong) from ERP_NHANHANG_SP_CHITIET ct  inner join ERP_NHANHANG nh on ct.NHANHANG_FK = nh.PK_SEQ  where nh.trangthai <> 3 "+
		" and ct.SANPHAM_FK = d.SANPHAM_FK and RTRIM(ltrim(ct.SOLO)) =RTRIM(ltrim(d.SOLO))   and ct.MUAHANG_FK = "+muahang_fk+"  and ct.NHANHANG_FK =  "+this.id+  
		" and ROUND(ct.GIATHEOLO,4) =  "+" round(cast( "+ giachuaquydoi +" as float),4)"+ 
		" group by ct.SANPHAM_FK, ct.SOLO ), 0) as soluongdanhan,  d.NGAYSANXUAT as ngaysanxuat, d.NGAYHETHAN, "+
		" isnull((select COUNT(*) from ERP_NHANHANG_SP_CHITIET a  where SANPHAM_FK = "+SanPhamId+" and NHANHANG_FK = "+this.id+" and MUAHANG_FK =  "+muahang_fk+ 
		" and RTRIM(ltrim(a.SOLO)) =RTRIM(ltrim(d.SOLO)) and round( a.GIATHEOLO,4) = "+ " round(cast( "+ giachuaquydoi +" as float),4)"+" ),0) as soluongthung "+ 
		" from  ERP_HOADONNCC_DONMUAHANG d "+  
		 "	LEFT JOIN ERP_NHASANXUAT NSX ON NSX.PK_SEQ = 	d.NSX_FK "+  
		" where  d.hoadonncc_fk = "+this.hdNccId +" and d.sanpham_fk = "+SanPhamId+" and d.soluong <> 0 and d.MUAHANG_FK = "+muahang_fk+" and ROUND (d.DONGIA,4)  =" + "round(cast( "+ giachuaquydoi +" as float),4)" ; 

		
		
					// giá USD lẻ
		System.out.println("Khoi tao san pham con theo lô : " + command);
		ResultSet spConRs = db.get(command);
		...
		if (spConRs != null) {
			while (spConRs.next()) {
				...
				if( this.id.trim().length() >0){
					soluong = DinhDang.dinhdangkho(spConRs.getDouble("soluongdanhan"));
				}
				String ngaysanxuat = spConRs.getString("ngaysanxuat");
				String ngayhethan = spConRs.getString("ngayhethan");
				...
				spCon.setDongiaLo(DinhDang.dinhdangkho(spConRs.getDouble("dongia")));
				String nsxid = spConRs.getString("nsx_fk")==null?"":spConRs.getString("nsx_fk");
				spCon.setNSXid(nsxid);
				spCon.setNSXTen(spConRs.getString("nsxten"));
				spCon.setMarrquet(spConRs.getString("MARRQUET"));
	...
	private List<DetailMeSp> getListChiTietLoMe(String muahang_fk, String SanPhamId, String dongia,double TILEQUYDOI_DOLUONG) {

//		DecimalFormat formatter = new DecimalFormat("###,###,###");
		List<DetailMeSp> listDetailMeSp = new ArrayList<DetailMeSp>();
		
		double giachuaquydoi=(Double.parseDouble(dongia.replaceAll(",", ""))/ TILEQUYDOI_DOLUONG) ;
		try {
			
			String comand = " select  ISNULL(a.MARQ,'') AS MARRQUET,isnull(nsx.ten,'') as nsxten,a.nsx_fk,a.MATHUNG, a.MAME, a.BIN_FK, a.SOLUONG, a.SOLO,a.NGAYHETHAN, a.NGAYSANXUAT " +
					        " from ERP_NHANHANG_SP_CHITIET a "+
					        "	LEFT JOIN ERP_NHASANXUAT NSX ON NSX.PK_SEQ = 	a.NSX_FK \n"
					        + "where SANPHAM_FK = "+SanPhamId +
					        " and NHANHANG_FK = "+this.id+" and MUAHANG_FK = "+ muahang_fk +" and giatheolo ="+ dongia +  
					        "  order by a.solo, cast(a.mame as float),cast(a.mathung as float) ";
	...
	System.out.println(" qr check : "+ sql +" \n ");
			ResultSet rs = this.db.get(sql);
			boolean bien=true;
			if (rs.next()) {
				bien= false;
				...
				for (int i = 0; i < spList.size(); i++) {
													...
													// nguoi dung nhap so luong
					{
						...
						query = " insert ERP_NHANHANG_SANPHAM(NHANHANG_FK, MUAHANG_FK ,SANPHAM_FK, TAISAN_FK, CCDC_FK, " +
								...
								" DONGIA, TIENTE_FK, TYGIAQUYDOI, DONGIAVIET, SONGAYVUOTMUC, SOLUONGVUOTMUC,"
								+ " IDMARQUETTE, SPKIEMDINH,TILEQUYDOI_DOLUONG) "
						...
						for (int j = 0; j < detailList.size(); j++) {
							DetailMeSp detail = detailList.get(j);
								...
								String marq =detail.getMarrquet();
								if(marq.trim().length()>0)
								{
									query = "SELECT count(*) as dem FROM MARQUETTE where ma = N'"+marq+"'  AND SANPHAM_FK = "+sp.getId();
									ResultSet rsmaqr = db.get(query);
									int checkmaq =0;
									if(rsmaqr.next())
										checkmaq = rsmaqr.getInt("dem");
									rsmaqr.close();
									if(checkmaq ==0)
									{
										this.msg = "Mã marrquette "+marq+" không tồn tại";
										System.out.println(this.msg);
										db.getConnection().rollback();
										return false;
									}
								...
								query = " insert ERP_NHANHANG_SP_CHITIET(NHANHANG_FK, MUAHANG_FK, SANPHAM_FK, LANNHAN, SOLO,MATHUNG, " +
										" SOLUONG, NGAYSANXUAT, NGAYHETHAN, NGAYNHANDUKIEN ,BIN_FK, MAME, GIATHEOLO,NGAYNHAPKHO,ISKIEMDINH,MARQ,NSX_FK) "
										...
										+ detail.getKhuVuc() +",'" + detail.getMe() + "',"+ Double.parseDouble(sp.getDongia().replaceAll(",", ""))+",'"+this.ngaynhanhang+"','"+sp.getIsKiemDinh()+"',"
												+ "N'"+detail.getMarrquet()+"',"+(detail.getNSXid().trim().length()==0?"NULL":detail.getNSXid().trim())+")";

								tongchitiet = tongchitiet +DinhDang.dinhdangso(Double.parseDouble(detail.getSoLuong().replaceAll(",", "")));
						...
						if (this.loaihanghoa.equals("0")) {
							System.out.println("[soluongnhan] "+sp.getSoluongnhan()+", [soluongct] "+DinhDang.dinhdangso(tongchitiet));
							if (DinhDang.dinhdangso(Double.parseDouble(sp.getSoluongnhan().replaceAll(",", ""))) - +DinhDang.dinhdangso(tongchitiet) != 0) {
	...
	private boolean UpdateHoanTatNhanHang(boolean flag, String id) {
		try {
			...
			System.out.println("Cap nhat hoan tat: "+query);
			int k = this.db.updateReturnInt(query);
			if (k != 1) {
			...
			if (this.spList.size() > 0) {
				for (int i = 0; i < spList.size(); i++) {
													...
													// nguoi dung nhap so luong
					{
						...
						query = " insert ERP_NHANHANG_SANPHAM(NHANHANG_FK, MUAHANG_FK ,SANPHAM_FK, TAISAN_FK, CCDC_FK, " +
								" CHIPHI_FK, DIENGIAI, DONVI, NGAYNHANDUKIEN, KHONHAN, SOLUONGDAT, SOLUONGNHAN, DUNGSAI," +
								" DONGIA, TIENTE_FK, TYGIAQUYDOI, DONGIAVIET, SONGAYVUOTMUC, SOLUONGVUOTMUC, IDMARQUETTE,"
								+ " SPKIEMDINH,TILEQUYDOI_DOLUONG) "
								...
								+ sp.getIdmarquette()+",'"+sp.getIsKiemDinh()+"',"+sp.getTiLeQuyDoiDv()+" )";
	...
	public void init_convert(String hoadonnccId, String loaihd) {
		...
		query = " select KHOBIETTRU_FK, KHOTONTRU_FK , (select ngayghinhan from erp_park where pk_seq in (select park_fk from erp_hoadonncc where pk_seq = "+hoadonnccId+") )  as ngayghinhan from ERP_HOADONNCC where pk_seq = "+ hoadonnccId;
	...
	public void GetInfoNhanHangNgamDinh(String hoaDonid) {
		try {
			String query = " select a.ncc_fk as nccId, b.pk_seq as hoadonId, a.LOAIHD "
			...
			if (rs != null) {
				if (rs.next()) {
					...
					this.hdNccId = rs.getString("hoadonId");
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
