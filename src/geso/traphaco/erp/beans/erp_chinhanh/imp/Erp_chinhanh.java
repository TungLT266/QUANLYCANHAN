package geso.traphaco.erp.beans.erp_chinhanh.imp;
...
public class Erp_chinhanh  extends Phan_Trang implements IErp_chinhanh
{
	...
	public boolean ThemMoiMa(String congtyId) 
	{
		try{
			...
			if (!db.update(query))
				return false;
			query = "Select IDENT_CURRENT('ERP_CHINHANH') as currentId";
			...
			String result = UtilitySyn.SynData(db, "ERP_CHINHANH", "ERP_CHINHANH", "PK_SEQ", currentId, "0", false);
	...
	public boolean UpdateMa()
	{
		...
		if (db.update(query)) 
		{
			String result = UtilitySyn.SynData(db, "ERP_CHINHANH", "ERP_CHINHANH", "PK_SEQ", this.ID, "1", false);
			if (result.trim().length() > 0)
			{
	...
	public boolean Delete()
	{
		...
		//SYN QUA DMS
		String result = UtilitySyn.SynData(db, "Erp_ChiNhanh", "Erp_ChiNhanh", "PK_SEQ", this.ID, "2", true);
		if (result.trim().length() > 0)
		{
