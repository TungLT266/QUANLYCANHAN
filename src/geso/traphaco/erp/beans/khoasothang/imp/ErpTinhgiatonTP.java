...
import geso.traphaco.erp.beans.khoasothang.IErpTinhgiatonTP;

public class ErpTinhgiatonTP implements IErpTinhgiatonTP
{
	String userId;
	String type;
	String tungay;
	...
	public void createRs() 
	{
		if( this.tungay.trim().length() > 0 )
		{
			...
			String query =  " select b.ma, b.ten, c.DONVI, a.DONGIA, isnull( ( select sum( toncuoi ) from ERP_GIATHANH_GIATONKHO_CHENHLECH where thang = '" + thang + "' and nam = '" + nam + "'  and sanpham_fk = a.sanpham_fk ), 0 ) as soluong  "+
							...
							" where b.loaisanpham_fk in ( 100002 ) "+
	...
	public String tinhGiaTonkho(String tungay, String denngay)
	{
		...
		return this.TinhGiaTon_TP(tungay, denngay);
		
	}

	private String TinhGiaTon_TP(String tungay, String denngay) 
	{
		dbutils db = new dbutils();
		try
		{
			...
			//Phân bổ chi phí để lên báo cáo, bên FIT để lúc khóa sổ
			int thang = Integer.parseInt( tungay.split("-")[1] );
			int nam = Integer.parseInt( tungay.split("-")[0] );
			db.execProceduce2("TinhGiaTonKho_PhanBoChiPhi", new String[] { this.congtyId, Integer.toString(thang), Integer.toString(nam) } );
	...
	private String B1_ApGiaNhap(dbutils db, String thangKS, String namKS, String tungay, String denngay) 
	{
		...
		try
		{
			...
			query = "delete ERP_GIATHANH_GIATONKHO where congty_fk = '" + this.congtyId + "' and thang = '" + thang + "' and nam = '" + nam + "' " + 
					"		and sanpham_fk in ( select pk_seq from ERP_SANPHAM where loaisanpham_fk  in ( 100002 ) )	"	;
			
			System.out.println(":::B1.Xoa gia nhap kho cu: " + query);
			db.update(query);
			
			//Giá của thành phẩm nếu trong tháng có LSX thì lấy, không thì lấy giá tồn tháng trước, ko thì lấy giá nhập mua gần nhất
			query = " insert ERP_GIATHANH_GIATONKHO( congty_fk, thang, nam, sanpham_fk, soluong, dongia, buoc )  "+
					" 	select '" + this.congtyId + "', '" + thang + "' as thang, '" + nam + "' as nam, sanpham_fk, soluong, dongia, 11 as buoc  "+
					" 	from ERP_GIATHANH_THANHPHAM " + 
					" 	where  thang = '" + thangKS + "' and nam = '" + namKS + "'  "+
					"		and sanpham_fk in ( select pk_seq from ERP_SANPHAM where loaisanpham_fk  in ( 100002 ) )	" +
					" union all "+ //==> không tính vào do công thức bình quân đã có 
					" 	select '" + this.congtyId + "', '" + thang + "' as thang, '" + nam + "' as nam, sanpham_fk,   "+
					" 		( select sum( soluong ) from ERP_TONKHOTHANG_CHITIET where sanpham_fk = a.sanpham_fk and thang = '" + thangKS + "' and nam = '" + namKS + "' ), dongia, 1 as buoc  "+
					" 	from ERP_BANGGIA_THANHPHAM_CUOIKY a " + 
					" 	where thang = '" + thangKS + "' and nam = '" + namKS + "'  "+
					"		and sanpham_fk in ( select pk_seq from ERP_SANPHAM where loaisanpham_fk  in ( 100002 ) )	" +
					"		and sanpham_fk not in ( select sanpham_fk from ERP_GIATHANH_THANHPHAM where  thang = '" + thang + "' and nam = '" + nam + "'  )	" +
					" union all "+
					...
					" 		and b.sanpham_fk not in ( select pk_seq from ERP_GIATHANH_THANHPHAM where thang = '" + thang + "' and nam = '" + nam + "'  ) "+
			...
			System.out.println(":::Bước 1. Giá TP: " + query);
			db.update(query);
			
			//Cap nhat dieu chinh ton kho
			query = "update a set a.dongia = isnull( ( select dongia from ERP_GIATHANH_GIATONKHO where congty_fk = '" + this.congtyId + "' and thang = '" + thang + "' and nam = '" + nam + "' and buoc = 0 and sanpham_fk = a.sanpham_fk ), 0 ) " + 
	...
	private String B2_TinhGiaTonKho_BQGiaQuyen(dbutils db, String tungay, String denngay) 
	{
		...
		*/
		...
		db.execProceduce2("TinhGiaTonKho_TP", new String[] { this.congtyId, tungay, denngay, "2" } );
		return "";
	...
	private String B3_TinhGiaTonKho_ApPhieuXuat(dbutils db, String tungay, String denngay) 
	{
		...
		db.execProceduce2("TinhGiaTonKho_TP", new String[] { this.congtyId, tungay, denngay, "3" } );
		return "";
	...
	private String B4_TinhGiaTonKho_ChayLaiDinhKhoan(dbutils db, String tungay, String denngay) 
	{
		...
		db.execProceduce2("TinhGiaTonKho_TP", new String[] { this.congtyId, tungay, denngay, "4" } );
		return "";
	...
	private String B5_B6_B7_B8_B9_TinhGiaTonKho_GiaTriTonKhoCuoiKy(dbutils db, String tungay, String denngay) 
	{
		...
		try
		{
			...
			//Thực hiện cập nhật lại giá phiếu xuất bên DMS, nếu chỗ này chạy chậm quá, có thể đem qua DMS chạy
			WebService.SynDataFromERP(" TinhGiaTonKho_TP_DMS 100000, '" + tungay + "', '" + denngay + "', 4 " );
			...
			query = " insert ERP_GIATHANH_GIATONKHO_CHENHLECH( thang, nam, kho_fk, sanpham_fk, toncuoi, G1, G2, chenhlech,congty_fk )  "+
					...
					" from dbo.UFN_NXT_HO_FULL_TINHGIAVON_NVL( '" + this.nppId + "', '" + tungay + "', '" + denngay + "' ) a  "+
					" 		left join ERP_BANGGIA_THANHPHAM_CUOIKY b on a.sanpham_fk = b.SANPHAM_FK and b.THANG = '" + thang + "' and b.NAM = '" + nam + "'  and b.congty_fk = '" + this.congtyId + "' "+
					" where a.kho_fk is not null " + 
					" 	and a.sanpham_fk in ( select pk_seq from ERP_SANPHAM where loaisanpham_fk  in ( 100002 )  ) ";
	...
	private void TaoButToan_TongHop(dbutils db, String ngay, int thang, int nam) 
	{
		...
		query = "delete ERP_PHATSINHKETOAN where loaichungtu = N'Bút toán tổng hợp' and sochungtu in ( select pk_seq from ERP_BUTTOANTONGHOP where NPP_FK = '" + this.nppId + "' and DIENGIAI = N'Bút toán chạy giá vốn TP tháng " + thang + ", năm " + nam + "' ) ";
		db.update(query);
		
		query = "update ERP_BUTTOANTONGHOP set trangthai = 2 where NPP_FK = '" + this.nppId + "' and DIENGIAI = N'Bút toán chạy giá vốn TP tháng " + thang + ", năm " + nam + "' ";
		db.update(query);
		...
		query = "insert ERP_BUTTOANTONGHOP( NGAYBUTTOAN, DIENGIAI, TRANGTHAI, NPP_FK, CONGTY_FK, TIENTE_FK, TIGIA, NGAYTAO, NGUOITAO, NGAYSUA, NGUOISUA ) " +
				"select '" + denngay + "', N'Bút toán chạy giá vốn TP tháng " + thang + ", năm " + nam + "', 1, '" + this.nppId + "', '" + this.congtyId + "', 100000, 1, '" + this.getDateTime() + "', '100000', '" + this.getDateTime() + "', '100000' ";
		db.update(query);
