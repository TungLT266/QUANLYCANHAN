...
public class ErpNhomDTKhac implements IErpNhomDTKhac{
	...
	public ErpNhomDTKhac(String[] param)
	{
		this.ctyId = "";
		this.ctyTen = "";
		this.id = param[0];
	...
	public boolean saveNewNNcc(){
		...
		try 
		{
			...
			command = "insert into ErpNhomDTKhac(TEN, DIENGIAI, NGAYSUA, NGAYTAO, NGUOISUA, NGUOITAO, TRANGTHAI, CONGTY_FK) values( '"+this.ten+"', N'" + this.diengiai + "', '" + this.ngaytao + "', '" + this.ngaysua +"', '" + this.nguoitao + "', '" + this.nguoisua + "', '"+ this.trangthai +"', '"+ this.ctyId +"')";			
			...
			try 
			{
				...
				if(this.ncc != null) 
				{
					...
					while(m < size)
					{
						command = "insert into ErpNhomDTKhac_DT(DTKhac_fk, ErpNhomDTKhac_FK) values('" + nccList[m] + "', '"+ this.id + "')";
						System.out.println("[Nhomncc.saveNewNkh] query = " + command);
						if(!this.db.update(command)) {
							db.getConnection().rollback();
	...
	public boolean updateNNcc()
	 {
	  try 
	  {
	   ...
	   command ="update ErpNhomDTKhac set ten = '"+this.ten+"', diengiai = N'"+ this.diengiai +"',ngaysua ='"+ this.ngaysua +"', trangthai ='" + this.trangthai + "' where pk_seq = '" + this.id + "'";
	  ...
	  command = "delete from ErpNhomDTKhac_DT where ErpNhomDTKhac_FK = '"+ this.id + "'";
	...
	private void createNccRS(){  	
		...
		String temp = "";
		if (this.ncc != null){
			query = "select pk_seq, madoituong as ma, tendoituong as ten from ERP_DOITUONGKHAC where";
			temp =  "select pk_seq from ERP_DOITUONGKHAC where";
		...
		if (this.id.length() > 0){
			if (this.ncc == null){
				query = "select a.pk_seq, madoituong as ma, a.tendoituong as ten from ERP_DOITUONGKHAC a, ErpNhomDTKhac_DT b where a.pk_seq = b.DTKhac_FK and b.ErpNhomDTKhac_FK = '" + this.id + "'";
				this.nccSelected = this.db.get(query);	
			
				query = "select a.pk_seq, madoituong as ma, a.tendoituong as ten from ERP_DOITUONGKHAC a where  a.trangthai = '1' and a.pk_seq not in (select DTKhac_FK from ErpNhomDTKhac_DT where ErpNhomDTKhac_FK = '" + this.id + "')";
			}else{
				query = "select a.pk_seq, madoituong as ma, a.tendoituong as ten from ERP_DOITUONGKHAC a" +
				" where a.trangthai = '1' and a.pk_seq not in (select DTKhac_FK from ErpNhomDTKhac_DT " +
				"where ErpNhomDTKhac_FK = '" + this.id + "') and a.pk_seq not in(" + temp + ")";
			}
				
		}else{
				query = "select a.pk_seq, madoituong as ma, a.tendoituong as ten from ERP_DOITUONGKHAC a where  a.trangthai = '1' " ;
		...
		if(this.ctyId.length() > 0){
			query += " and a.congty_fk = "+this.ctyId;
		}
		query = query + " order by a.tendoituong ";
