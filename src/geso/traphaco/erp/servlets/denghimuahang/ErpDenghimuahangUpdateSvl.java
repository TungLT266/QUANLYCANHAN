...
public class ErpDenghimuahangUpdateSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpDenghimuahangUpdateSvl()
	{
		super();
	}
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else
		{
			...
			String duyetmh = request.getParameter("duyetmh");
			if(duyetmh == null)
				duyetmh = "";
			dmhBean.setDuyetdnmh(duyetmh);
				System.out.println("aaaaaaaaaa"+duyetmh);		
			...
			else if(task.equals("xoafilenew"))
			{
				PrintWriter out = response.getWriter();
				dbutils db = new dbutils();
				System.out.print("Xoa file");
				String query = "select congvan from erp_denghimuahang where pk_seq='" + id + "'";
				...
				{
					query = "update erp_denghimuahang set congvan='0' where pk_seq='" + id + "'";
			...
			else
			{
				...
				{
					nextJSP = "/TraphacoHYERP/pages/Erp/ErpDeNghiMuaHangDisplay.jsp";
				}
				else
				{
					nextJSP = "/TraphacoHYERP/pages/Erp/ErpDeNghiMuaHangUpdate.jsp";
				}
		...
		else
		{
			...
			for(int i = 0; i < masp.length; i++)
			{
				...
				if(((loaihh.equals("2") ||loaihh.equals("3")||loaihh.equals("1")) && tensp[i].trim().length() > 0) || masp[i].trim().length() > 0)
				{
					...
					sp.setSoluongduyet(sl[i].replaceAll(",", ""));
			...
			if (action.equals("save") )
			{
				if(ktDonvi == false){
					...
					if (id == null)
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDeNghiMuaHangNew.jsp";
					}
					else
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDeNghiMuaHangUpdate.jsp";
				...
				if (id == null) // tao moi
				{
					...
					{
						IErpDenghimuahangList obj = new ErpDenghimuahangList();
				...
				else
				{
					...
					if (!dmhBean.updateDmh())
					{
						dmhBean.createRs();
						session.setAttribute("dmhBean", dmhBean);
						String nextJSP = "/TraphacoHYERP/pages/Erp/ErpDeNghiMuaHangUpdate.jsp";
						response.sendRedirect(nextJSP);
						return;
					}
					else
					{
						...
						if(dmhBean.getDuyetdnmh().equals("1"))
						{
							...
							IErpDuyetdenghimuahang ddmhBean = new ErpDuyetdenghimuahang();
					   	    ...
					   	    String searchQuery=util.getSearchFromHM(userId,"ErpDuyetdenghimuahangSvl", session);
...
//						geso.traphaco.center.util.GiuDieuKienLoc.setParamsToOject(obj, searchQuery);
	...
	private void CreateDN(Document document, ServletOutputStream outstream, HttpServletResponse response, HttpServletRequest request, dbutils db) 
	{
		...
		try
		{
			...
			PdfPCell cell = new PdfPCell(new Paragraph("", font6green));
			cell.setPaddingLeft(5.0f);
			cell.setBorderWidth(0);
			tbHeader.addCell(cell);
			
			cell = new PdfPCell(new Paragraph("", font6green));
			cell.setPaddingLeft(5.0f);
			cell.setBorderWidth(0);
			tbHeader.addCell(cell);
	...
	private String kiemtratrungma(List <String> spId){
		String matrung="";
		List <String> trung= new ArrayList<String>();
		try {
			for (int i = 0; i < spId.size()-1; i++) {
				for (int j = i+1; j < spId.size(); j++) {
					if( ( spId.get(i).length() )>0 && (   spId.get(i).equals( spId.get(j)))){
						if(!trung.contains( spId.get(i))){
							trung.add( spId.get(i));
							matrung+= spId.get(i) +" ; ";
						}
					}
				}
