...
public class ErpDonmuahangtrongnuocList_GiaySvl extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
       
    public ErpDonmuahangtrongnuocList_GiaySvl() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    {	
	    ...
	    else if(action.equals("hoantat"))
	    	{
		    	...
		    	{
		    		dbutils db = new dbutils();
		    		db.update(" update ERP_MUAHANG set DACHOT = '1' where pk_seq = '" + dmhId + "' and trangthai=0");
	...
	private String MoChot(String dmhId) {
		...
		try{
			...
			//2. TRANGTHAI = 0 , ERP_DUYETMUAHANG set TRANGTHAI = 0
				query="update ERP_MUAHANG set trangthai = '0', DACHOT = '0' where pk_seq = '" + dmhId + "' and trangthai=1";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    else
	    {
	    	if(action.equals("view") || action.equals("next") || action.equals("prev"))
	    	{
		    	...
		    	GiuDieuKienLoc.getSetDieuKienLoc(session, this.getServletName(), Thread.currentThread().getStackTrace()[1].getMethodName(), action, userId, obj);
		    	session.setAttribute("obj", obj);
		    	response.sendRedirect("/TraphacoHYERP/pages/Erp/ErpDonMuaHangtrongnuocList.jsp");	
		    }
	    	else
	    	{
		    	obj = new ErpDonmuahangList_Giay();
...
//		if( loaiDonMuaHang != "")
//		{
//			query += "and ISGIACONG = '" + loaiDonMuaHang + "'";
//		}
//
//		
	...
	private String Delete(String dmhId,String userId)
	{
		...
		try 
		{
			...
			query = "select COUNT(*) as sodong from ERP_NHANHANG where MUAHANG_FK = '" + dmhId + "'" +
					" and PK_SEQ not in (select distinct sochungtu from erp_huychungtumuahang_chungtu where loaichungtu = N'Nhận hàng' )";
