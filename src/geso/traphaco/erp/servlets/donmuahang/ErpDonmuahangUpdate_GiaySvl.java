...
import geso.traphaco.erp.beans.donmuahang.imp.SanPhamPhanBo;
...
public class ErpDonmuahangUpdate_GiaySvl extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public ErpDonmuahangUpdate_GiaySvl() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!cutil.check(userId, userTen, sum)) {
			...
			if (task.equals("print")) {
				...
				if(task.equals("inexcel"))	// xuat exell
				{
					response.setContentType("application/xlsm");
					response.setHeader("Content-Disposition", "attachment; filename=INDONHANGTSCD.xlsm");
					try {
				...
				if (request.getQueryString().indexOf("display") >= 0) {
					nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangDisplay_Giay.jsp";
				} else {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (!cutil.check(userId, userTen, sum)) {
			...
			String[] ngayThanhToanArr = request.getParameterValues("ngayThanhToan");
			dmhBean.setNgayThanhToanArr(ngayThanhToanArr);

			String[] soTienThanhToanArr = request.getParameterValues("soTienThanhToan");
			dmhBean.setSoTienThanhToanArr(soTienThanhToanArr);

			String[] phanTramThanhToanArr = request.getParameterValues("phanTramThanhToan");
			...
			String soluonghd = util.antiSQLInspection(request.getParameter("soluonghd"));
			if (soluonghd == null)
				soluonghd = "";
			dmhBean.setSoluong(soluonghd);

			String tennhank = util.antiSQLInspection(request.getParameter("tennhank"));
			if (tennhank == null)
				tennhank = "";
			dmhBean.setTennhanhapkhau(tennhank);

			String tennhasx = util.antiSQLInspection(request.getParameter("tennhasx"));
			if (tennhasx == null)
				tennhasx = "";
			dmhBean.setTennhasanxuat(tennhasx);

			String ngayship = util.antiSQLInspection(request.getParameter("ngayship"));
			if (ngayship == null)
				ngayship = "";
			...
			String dvchiutrachnhiem = util.antiSQLInspection(request.getParameter("dvchiutrachnhiem"));
			...
			if(ngaynhandukien == null){
				ngaynhandukien = new String[idsp.length];
			}
			...
			for (int i = 0; i < masp.length; i++) {
				...
				if ((loaihh.equals("2") && tensp[i].trim().length() > 0) || masp[i].trim().length() > 0) {
					String[] ngaynhan = request.getParameterValues("sub_ngaynhan_" + i);
					String[] soluong = request.getParameterValues("sub_soluong_" + i);
					List<INgaynhan> nn = new ArrayList<INgaynhan>();
					if (ngaynhan != null)
						for (int j = 0; j < ngaynhan.length; j++) {
							if ((loai.equals("2") && ngaynhan[j].trim().length() > 0) || !loai.equals("2")) {
								nn.add(new Ngaynhan(ngaynhan[j].trim(), soluong[j].trim()));
							}
						}
					...
					if (dmhBean.getLoaihanghoa().equals("2")) {
						String[] spId = request.getParameterValues("sub_spId_" + i);
						String[] spMa = request.getParameterValues("sub_spMa_" + i);
						String[] spTen = request.getParameterValues("sub_spTen_" + i);
						String[] chon = request.getParameterValues("sub_spChon_" + i);
						...
						if (sppb != null)
							for (int k = 0; k < spId.length; k++) {
								...
								if (chon != null) {
									if (m < chon.length) {
										if (chon[m].trim().equals(spId[k].trim())) {
											System.out.println("Len..." + chon[m]);
					...
					if (donvitinh[i].trim().length() == 0 || donvitinh[i].equals("NA"))
						ktDonvi = false;
			...
			if (action.equals("save")) {
				if (ktDonvi == false) {
					...
					String nextJSP;
					if (id == null) {
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangNew_Giay.jsp";
					} else {
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangUpdate_Giay.jsp";
					}
					session.setAttribute("dmhBean", dmhBean);
				...
				if (id == null) // tao moi
				{
					...
					if (!dmhBean.updateDmh()) {
						dmhBean.createRs();
						session.setAttribute("dmhBean", dmhBean);
						String nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangUpdate_Giay.jsp";
						response.sendRedirect(nextJSP);
						return;
					} else {
						IErpDonmuahangList_Giay obj = new ErpDonmuahangList_Giay();
						...
						try {
							if (rsNcc.next()) {
								...
								if (dmhBean.getNguonGocHH().equals("TN")) {
									dmhBean.setHinhThucTT(
											rsNcc.getString("thoihanno") + " ngày kể từ ngày nhận hóa đơn");
								}
							}
							rsNcc.close();
							db.shutDown();
	...
	private void Create_PO_PDF(HttpServletResponse response, HttpServletRequest request) {
		...
		float PAGE_LEFT = 2.0f * CONVERT, PAGE_RIGHT = 1.5f * CONVERT, PAGE_TOP = 0.5f * CONVERT,
				PAGE_BOTTOM = 0.0f * CONVERT; // cm
		...
		try {
			...
			ResultSet rsCheck = db
					.get("select ISNULL(nguongocHH, 'TN') as nguongoc from ERP_MUAHANG where PK_SEQ = '" + id + "'");
			if (rsCheck != null) {
				try {
					if (rsCheck.next()) {
	...
	private boolean CreateExcel(HttpServletResponse response, HttpServletRequest request, OutputStream out) throws Exception {
		...
		try {
			...
			fstream = new FileInputStream(getServletContext().getInitParameter("pathTemplate") + "\\INDONHANGTSCD.xlsm");
			...
			String query = " select a.PK_SEQ as dmhId,isnull(b.dienthoai, 'NA') as dienthoai,isnull(b.fax, 'NA') as fax  ,isnull(b.diachi, 'NA') as diachi,isnull(a.HTTT,'') HTTT, isnull(a.NguonGocHH ,'') as NguonGocHH, isnull(a.TienTe_FK, '100000') as TienTe_FK, \n"
				...
				+ " a.DONVITHUCHIEN_FK as dvthId, a.LOAIHANGHOA_FK, a.LOAISANPHAM_FK, b.loainhacungcap_fk as nccLoai, b.pk_seq as nccId, b.TEN as nccTen, isnull(a.ETD, '') as ETD, isnull(a.ETA, '') as ETA, \n"
				...
				+ " isnull(b.noibo,0) as noibo, a.DTVT_FK, a.LOAI, ISNULL(a.ISHOPDONG,0) as ishopdong, a.KENH_FK, A.KHOANMUCCHI \n"
			...
			try {
				if (infoNcc.next()) {		
					...
					khTen=infoNcc.getString("nccTen");
					khDiachi=infoNcc.getString("diachi");
					khFax=infoNcc.getString("fax");
					khTel=infoNcc.getString("dienthoai");
			...
			System.out.println("aaaaaaaaaa"+sqlLayThongTinDonViDatHang);
			ResultSet rssqlLayThongTinDonViDatHang=db.get(sqlLayThongTinDonViDatHang);
			try {
				if(rssqlLayThongTinDonViDatHang!=null)
				{ 
					while(rssqlLayThongTinDonViDatHang.next())
					{
					donViDatHang = rssqlLayThongTinDonViDatHang.getString("TEN");
					diaChi = rssqlLayThongTinDonViDatHang.getString("DIACHI");
					dienThoai = rssqlLayThongTinDonViDatHang.getString("DIENTHOAI");
					maSoThue = rssqlLayThongTinDonViDatHang.getString("MASOTHUE");
					fax = rssqlLayThongTinDonViDatHang.getString("FAX");
					nganhang=rssqlLayThongTinDonViDatHang.getString("nganhang");
					sotaikhoan=rssqlLayThongTinDonViDatHang.getString("sotaikhoan");
					}
					rssqlLayThongTinDonViDatHang.close();
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			String sqlTongCong = "SELECT SUM(SOLUONG* DONGIA + SOLUONG* DONGIA* THUEXUAT/100) AS TONGCONG FROM ERP_MUAHANG_SP WHERE MUAHANG_FK ="+ id;
			ResultSet rsTongCong = db.get(sqlTongCong);
			try {
				if (rsTongCong.next()) {
					tongCong = rsTongCong.getLong("TONGCONG");
				}
				rsTongCong.close();
			} catch (Exception ex) {
				ex.printStackTrace();
		...
		geso.traphaco.center.beans.doctien.doctienrachu doctien = new geso.traphaco.center.beans.doctien.doctienrachu();
		String docTienRaChu = doctien.docTien(tongCong);
		docTienRaChu = docTienRaChu + " (" + tienTe + ")";
		tongCongBangChu = docTienRaChu;
			...
			ReportAPI.getCellStyle(cell, Color.BLACK, false, 11,  khTen);

			cell = cells.getCell("C14");
			ReportAPI.getCellStyle(cell,  Color.BLACK, false, 11, khDiachi);


			cell = cells.getCell("C15");
			ReportAPI.getCellStyle(cell, Color.BLACK, false, 11, khTel);

			cell = cells.getCell("F15");
			ReportAPI.getCellStyle(cell,  Color.BLACK, false, 11, khFax);
...
*/
			...
			query=" select \r\n" + 
					" ROW_NUMBER() OVER(ORDER BY a.PK_SEQ ASC) AS STT,\r\n" + 
					" \r\n" + 
					" CASE WHEN B.PK_SEQ IS NOT NULL THEN B.MA\r\n" + 
					" WHEN tscd.PK_SEQ IS NOT NULL THEN tscd.MA\r\n" + 
					" WHEN ccdc.PK_SEQ IS NOT NULL THEN ccdc.MA \r\n" + 
					" WHEN ncp.PK_SEQ IS NOT NULL THEN ncp.TEN \r\n" + 
					" END AS MASP,\r\n" + 
					" \r\n" + 
					" CASE WHEN B.PK_SEQ IS NOT NULL THEN B.TEN\r\n" + 
					" WHEN tscd.PK_SEQ IS NOT NULL THEN tscd.TEN\r\n" + 
					" WHEN ccdc.PK_SEQ IS NOT NULL THEN ccdc.DIENGIAI\r\n" + 
					" WHEN ncp.PK_SEQ IS NOT NULL THEN ncp.DIENGIAI\r\n" + 
					" END AS TENSP,'','',\r\n" + 
					" \r\n" + 
					" '' as TCKT,\r\n" + 
					" isnull(a.donvi, '') as donvi,\r\n" + 
					" a.soluong AS SOLUONG,\r\n" + 
					" isnull(a.dongia, '0') as dongia,\r\n" + 
					" a.vat AS VAT,\r\n" + 
					" isnull(a.thanhtien, '0') as thanhtien,\r\n" + 
					" '' as ngaynhan, '' as diadiemnhan\r\n" + 
					" from (   \r\n" + 
					...
					" PhanTramThue, ThueNhapKhau, DONGIAVIET, SUM(THANHTIENVIET) AS THANHTIENVIET, DONVI, KHOTT_FK, DUNGSAI, MUANGUYENLIEUDUKIEN_FK, CCDC_FK, TENHD, HOADONNCC_FK, thuexuat, vat, idmarquette, ngaynhan \r\n" + 
					" from ERP_MUAHANG_SP WHERE MUAHANG_FK = "+id+" \r\n" + 
					" GROUP BY MUAHANG_FK, SANPHAM_FK, CHIPHI_FK, TAISAN_FK, DIENGIAI, DONGIA, DONGIA_NEW, TIENTE_FK, \r\n" + 
					" PhanTramThue, ThueNhapKhau, DONGIAVIET, DONVI, KHOTT_FK, DUNGSAI, MUANGUYENLIEUDUKIEN_FK, CCDC_FK, TENHD, HOADONNCC_FK, thuexuat, vat,idmarquette, ngaynhan \r\n" + 
					") a left join \r\n" + 
					...
					" where muahang_fk = "+id+"  order by a.pk_seq";
			System.out.println("rsSOSP = " + query);
			ResultSet thongTinDatHang = db.get(query);
			int index = 23;
			try {
				...
				while(thongTinDatHang.next())
				{
					for(int i = 1; i <= socottrongSql; i ++)
					{
						...
						if(rsmd.getColumnType(i) == Types.DOUBLE || rsmd.getColumnType(i)==Types.NUMERIC || rsmd.getColumnType(i)==Types.INTEGER )
						{
							...
							cell.setStyle(style);
							ReportAPI.getCellStyle_double(cell, "Times New Roman", color_, false, 11,  thongTinDatHang.getDouble(i));

						}
						else
						{
							ReportAPI.getCellStyle(cell, "Times New Roman", color_, false, 11, thongTinDatHang.getString(i));

						}
						
						/*if(i==socottrongSql){
							tongtien=tongtien+thongTinDatHang.getDouble("thanhtien");
						}
						else
						// gop dong cho ten
						if(i==3){
				...
				for(int i = 1; i <= socottrongSql; i ++)
				{
					cell = cells.getCell(countRow,i-1);
						...
						if(i==3){
							...
							{
								ReportAPI.getCellStyle(cell, "Times New Roman", color_, false, 9, "");
							}
				...
				ReportAPI.getCellStyle(cell,  Color.BLACK, false, 11, "Tổng số tiền viết bằng chữ: " +  tongCongBangChu);
				++countRow;
	...
	private void CreatePO_VietNam(Document document, ServletOutputStream outstream, HttpServletResponse response,
		...
		try {
			...
			Image hinhanh = Image
					.getInstance(getServletContext().getInitParameter("pathPrint") + "/logoTraphacoERP.png");
			...
			if (infoNcc.next()) {
				...
				cell = new PdfPCell(new Paragraph(
						"Fax: " + infoNcc.getString("fax") + "         Tel: " + infoNcc.getString("dienthoai"),
						font10));
				cell.setPaddingLeft(5.0f);
				cell.setBorderWidth(1);
				cell.setBorderWidthTop(0);
				...
				cell = new PdfPCell(new Paragraph(
						"Ngày                 : " + getVnDateTime(infoNcc.getString("ngaymua")), font13bold));
				cell.setBorder(0);
				tbSubNcc2.addCell(cell);
			...
			float[] crSanpham = { 1.2f * CONVERT, 9.0f * CONVERT, 1.9f * CONVERT, 1.8f * CONVERT, 2.4f * CONVERT,
			...
			query = " select isnull(a.tenhd, '') as tenhd, isnull(b.ten2, b.ten) as spTen, isnull(b.DAI, 0) AS DAI, ISNULL(B.RONG, 0) AS RONG, ISNULL(B.DINHLUONG,0) AS DINHLUONG, "
					...
					+ " left join erp_nhomchiphi ncp on a.chiphi_fk = ncp.pk_seq "
					+ " left join  erp_trungtamchiphi ttcp on ncp.ttchiphi_fk = ttcp.pk_seq  "
					+ " left join DONVIDOLUONG c on b.DVDL_FK = c.PK_SEQ  " + " where a.MUAHANG_FK = '" + id + "'";
			...
			while (rsSOSP.next()) {
				...
				if (rsSOSP.getString("tenhd").trim().length() <= 0) {
					...
					if (mau == null || mau.trim().toLowerCase().indexOf("không") >= 0)
						mau = "";
					else
						mau = mau.trim();

					String nguongoc = rsSOSP.getString("nguongoc");
					if (nguongoc == null || nguongoc.trim().toLowerCase().equals("no")
					...
					int bien = (chuoi.length() / 45) + ((chuoi.trim().length() % 45) > 0 ? 1 : 0);
				...
				if (qc.length() > 0) {
					chuoi += " \n" + qc;
					int bien = (qc.length() / 45) + ((qc.trim().length() % 45) > 0 ? 1 : 0);
					soSPCount = soSPCount + bien;
				}
			}
			...
			if (ghichu.trim().length() > 0) {
				...
				for (int _i = 0; _i < ghichuArr.length; _i++) {
					if (ghichuArr[_i].trim().length() > 0) {
						...
						int bien = (ghichuArr[_i].trim().length() / 45)
								+ ((ghichuArr[_i].trim().length() % 45) > 0 ? 1 : 0);
						// _count +=
				...
				for (int i = 0; i < ghichuArr.length; i++) {
					if (ghichuArr[i].trim().length() > 0) {
						query += " union all ";
						query += " select 3000000000,' ', N'" + ghichuArr[i].trim()
				...
				while (infoSanPham.next()) {
					...
					if (infoSanPham.getString("type").equals("0")) {
						...
						if (infoSanPham.getDouble("DINHLUONG") != 0) {
							qc += formatter_2sole.format(infoSanPham.getDouble("DINHLUONG"))
						...
						if (infoSanPham.getDouble("RONG") != 0) {
							...
							qc += formatter_2sole.format(infoSanPham.getDouble("RONG"))
						...
						if (infoSanPham.getDouble("DAI") != 0) {
							...
							qc += formatter_2sole.format(infoSanPham.getDouble("dai"))
						...
						cell = new PdfPCell(new Paragraph(
								infoSanPham.getString("tenhd").trim().length() > 0 ? (spTen + " \n" + qc)
										: (spTen + (mau.length() > 0 ? " " + mau : "")
						...
						cell = new PdfPCell(
								new Paragraph(formatter_2sole.format(infoSanPham.getDouble("SoLuong")), font11));
						cell.setBorder(0);
						if (stt < soSP) {
							cell.setBorderWidthLeft(0.5f);
						}
						...
						cell = new PdfPCell(
								new Paragraph(formatter_2sole.format(infoSanPham.getDouble("DonGia")), font11));
						if (stt == soSP - 1) {
							cell.setRowspan(2);
						}
						cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
						...
						cell = new PdfPCell(
								new Paragraph(
										formatter_2sole.format(
												infoSanPham.getDouble("SoLuong") * infoSanPham.getDouble("DonGia")),
										font11));
						if (stt == soSP - 1) {
							cell.setRowspan(2);
						}
			...
			cell.addElement(new Paragraph("1.Giao hàng đúng thời gian yêu cầu, nếu có thay đổi phải báo trước ... ngày",
					font8));
		...
		/*	cell = new PdfPCell(new Paragraph("Cộng tiền hàng", font12boldItalic));
			...
			if (nguoilapId.equals("100320")) {
				footerStr = new String[] { " ", phong, "Nguyễn Hồng Nam", "Nguyễn Văn Bình" };
			} else {
				footerStr = new String[] { " ", nguoilapTen, phong, "Nguyễn Hồng Nam", "Nguyễn Văn Bình" };
			}
			int numFooterRows = 7;
			if (httt.length() > 32) {
				numFooterRows = 6;
			...
			for (int i = 0; i <= numFooterRows; i++) {
				...
				for (int j = 0; j < numcolumns; j++) {
					...
					if (i == numFooterRows) {
						...
						query = " SELECT CASE WHEN SUM(QUYETDINH) > 0 " + "		THEN  ( "
								+ "			SELECT COUNT(TRANGTHAI) - SUM(TRANGTHAI) "
								+ "			FROM ERP_DUYETMUAHANG  " + "			WHERE MUAHANG_FK = " + id
								+ " AND QUYETDINH = 1 " + "		) 	"
	...
	private void CreatePO_English(Document document, ServletOutputStream outstream, HttpServletResponse response,
			HttpServletRequest request, dbutils db) {
		HttpSession session = request.getSession();
		...
		try {
			...
			if (infoNcc.next()) {
				...
				cell = new PdfPCell(new Paragraph(
						"   SHIP TO                      : "
								+ (diadiemgiaohang.trim().length() > 0 ? diadiemgiaohang : "HoChiMinh City Port"),
						font11));
				cell.setBorder(0);
				tbSubNccInfo.addCell(cell);
			...
			float[] crSanpham = { 1.2f * CONVERT, 6.1f * CONVERT, 1.6f * CONVERT, 2.2f * CONVERT, 2.2f * CONVERT,
			...
			query = " select isnull(a.tenhd, '') as tenhd, isnull(b.TEN2, ' ') + isnull(MAU, ' ') + isnull(quycach_nguongoc, ' ') as spTen, cast( ISNULL(B.DAI, '0') as varchar(10) ) + isnull(B.DVDL_DAI, ' ') + "
					...
					+ " from ERP_MUAHANG_SP a left join ERP_SANPHAM b on a.SANPHAM_FK = b.PK_SEQ     "
					+ " left join DONVIDOLUONG c on b.DVDL_FK = c.PK_SEQ  " + " where a.MUAHANG_FK = '" + id + "' "
					+ " union all " + " select isnull(diengiai, '') as tenhd, '' as spTen, '' as sqQc, 2 as type "
					+ " from ERP_MUAHANG_CPKHAC " + " where MUAHANG_FK = " + id + " ";

			ResultSet rsSOSP = db.get(query);
			int soSPCount = 0;
			String spTen = "";
			while (rsSOSP.next()) {
				spTen = rsSOSP.getString("tenhd").trim().length() > 0 ? rsSOSP.getString("tenhd").trim()
						: rsSOSP.getString("spTen");
			...
			if (ghichu.trim().length() > 0) // Do dai 34
			{
				...
				for (int i = 0; i < ghichuArr.length; i++) {
					query += " union all ";
					query += " select ' ', '" + ghichuArr[i].replace("'", "''")
							+ "', ' ', ' ', '0', '0', '0', '', '', '', '', '0', '', '0', 5 as type ";
				}
			...
			if (infoSanPham != null) {
				...
				while (infoSanPham.next()) {
					...
					spTen = infoSanPham.getString("tenhd").trim().length() > 0 ? infoSanPham.getString("tenhd").trim()
							: infoSanPham.getString("spTen");
					...
					if (infoSanPham.getString("type").equals("0")) {
						...
						try {
							_rong = Double.parseDouble(infoSanPham.getString("RONG"));
						} catch (Exception e) {
						}
						;
						try {
							_dai = Double.parseDouble(infoSanPham.getString("DAI"));
						} catch (Exception e) {
						}
						;
						try {
							_dinhluong = Double.parseDouble(infoSanPham.getString("DINHLUONG"));
						} catch (Exception e) {
						}
						;
						...
						if (infoSanPham.getString("tenhd").trim().length() <= 0) {
							if (!infoSanPham.getString("QUYCACH_NGUONGOC").equals("NO")
									&& !infoSanPham.getString("QUYCACH_NGUONGOC").equals("Không")) {
								spTen = spTen + " " + infoSanPham.getString("QUYCACH_NGUONGOC");
							}

							if (!infoSanPham.getString("MAU").equals("NO")
									&& !infoSanPham.getString("MAU").equals("Không màu")) {
						...
						cell = new PdfPCell(new Paragraph(DinhDang.dinhdangkho(infoSanPham.getDouble("SoLuong")), font11));
						cell.setBorder(0);
						if (stt < soSP) {
							cell.setBorderWidthLeft(0.5f);
						}
						...
						cell = new PdfPCell(
								new Paragraph(formatter2_sole.format(infoSanPham.getDouble("DonGia")), font11));
						if (stt == soSP - 1) {
							cell.setRowspan(2);
						}
						cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			...
			cell.addElement(
					new Paragraph("3.  " + dungsai + "%  TOLERANT OF QUANTITIES EACH ITEM IS ACCEPTABLE.", font8));
			...
			cell = new PdfPCell(new Paragraph(DinhDang.dinhdangkho(tongluong), font12boldItalic));
			cell.setColspan(2);
			cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			tbSanPham.addCell(cell);
	...
	private String getEnDateTime(String date) {
		if (date.length() == 10) {
			...
			thang = thang.equals("01") ? "Jan"
					: thang.equals("02") ? "Feb"
							: thang.equals("03") ? "Mar"
									: thang.equals("04") ? "Apr"
											: thang.equals("05") ? "May"
													: thang.equals("06") ? "Jun" : thang.equals("07") ? "Jul"
															: thang.equals("08") ? "Aug"
																	: thang.equals("09") ? "Sep"
																			: thang.equals("10") ? "Oct"
																					: thang.equals("11") ? "Nov"
