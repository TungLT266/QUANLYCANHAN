...
public class ErpDonmuahangTSCCUpdateSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	public ErpDonmuahangTSCCUpdateSvl()
	{
		super();
	}
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else
		{
			...
			IErpDonmuahangTSCC dmhBean = new ErpDonmuahangTSCC(id);
			...
			else
			{
				...
				{
					nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangTSCCDisplay.jsp";
				}
				else
				{
					if (request.getQueryString().indexOf("hoantat") >= 0)
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpHTDonMuaHangTSCCDisplay.jsp";
					else
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangTSCCUpdate.jsp";
					}
				}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else
		{
			...
			IErpDonmuahangTSCC dmhBean;
			Utility util = new Utility();
			String id = util.antiSQLInspection(request.getParameter("id"));
			System.out.println("ID" + id);
			if (id == null)
			{
				dmhBean = new ErpDonmuahangTSCC("");
			}
			else
			{
				dmhBean = new ErpDonmuahangTSCC(id);
			}
			...
			if (NguonGocHH == null) NguonGocHH = "";
			if(dmhBean.getLoai().equals("0")) NguonGocHH = "NN";
			if(dmhBean.getLoai().equals("1")) NguonGocHH = "TN";
			...
			String[] ptThanhToanArr = request.getParameterValues("ptThanhToan");
			dmhBean.setPtThanhToanArr(ptThanhToanArr);
			...
			if (action.equals("save") )
			{
				if(ktDonvi == false){
					...
					if (id == null)
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangTSCCNew.jsp";
					}
					else
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangTSCCUpdate.jsp";
				...
				if (id == null) // tao moi
				{
					...
					{
						IErpDonmuahangTSCCList obj = new ErpDonmuahangTSCCList();
				...
				else
				{
					if (!dmhBean.updateDmh())
					{
						dmhBean.createRs();
						session.setAttribute("dmhBean", dmhBean);
						String nextJSP = "/TraphacoHYERP/pages/Erp/ErpDonMuaHangTSCCUpdate.jsp";
						response.sendRedirect(nextJSP);
						return;
					}
					else
	...
	private void CreatePO_VietNam(Document document, ServletOutputStream outstream, HttpServletResponse response, HttpServletRequest request, dbutils db) 
	{
		...
		try
		{
			...
			if(infoNcc.next())
			{
				...
				diadiemgiaohang  = diadiemgiaohang.trim().length() <= 0 ? "Kho Công ty TraphacoERP" : diadiemgiaohang.trim();
			...
			while( rsSOSP.next())
			{
			    ...
			    if(rsSOSP.getString("tenhd").trim().length() <= 0) {
			    	...
			    	if(nguongoc == null || nguongoc.trim().toLowerCase().equals("no") || nguongoc.trim().toLowerCase().indexOf("không") >= 0) 
			    		nguongoc = ""; 
			    	else nguongoc = nguongoc.trim();
			...
			if(ghichu.trim().length() > 0)
			{
				...
				for(int _i = 0; _i < ghichuArr.length; _i++) {
					if(ghichuArr[_i].trim().length() > 0) {
						...
						int bien= ( ghichuArr[_i].trim().length() / 45) + (( ghichuArr[_i].trim().length() % 45) >0?1:0);
				...
				for(int i = 0; i < ghichuArr.length; i++)
				{
					if(ghichuArr[i].trim().length() > 0) {
						query += " union all ";
						query += " select 3000000000,' ', N'" + ghichuArr[i].trim() + "', '0', '0', '0', '', '', '', '', '', '', '0', '', '0', 1 as type, 0,'','','',0,'','',0,'','','' ";
					}
				}
	...
	private void CreatePO_English(Document document, ServletOutputStream outstream, HttpServletResponse response, HttpServletRequest request, dbutils db) 
	{
		...
		{			
			...
			//NumberFormat formatter3_sole = new DecimalFormat("#,###,###.000"); 
			...
			String query = "select ma, N'NEW TOYO (VIETNAM) ALUMINIUM PAPER PACKAGING CO., LTD' as ten, N'Lot 15, 17, 19 and 21, The Industrial Zone of Linh Trung Ex-Processing Zone 2' as diachi, " +
								...
								"isnull(dienthoai, 'NA') as dienthoai, isnull(fax, 'NA') as fax " +
							"from erp_congty where pk_seq = '" + ctyId + "'";
			/*String query = "select ma, N' NEW TOYO (VIETNAM) ALUMINIUM PAPER PACKAGING CO., LTD' as ten, diachitienganh as diachi, " +
			"N' Thu Duc District, HCMC, Vietnam' as masothue, " +
			"isnull(dienthoai, 'NA') as dienthoai, isnull(fax, 'NA') as fax " +
		"from erp_congty where pk_seq = '" + ctyId + "'";*/
...
//				cell = new PdfPCell(new Paragraph("   PAYMENT TERMS   : " + "TT " + infoNcc.getString("thoihanno") + " days after B/L date", font11));
			...
			if(ghichu.trim().length() > 0) //Do dai 34
			{
				...
				{
					query += " union all ";
					query += " select ' ', '" + ghichuArr[i].replace("'", "''") + "', ' ', ' ', '0', '0', '0', '', '', '', '', '0', '', '0', 5 as type ";
				}
	...
	{
		ErpDonmuahangTSCCUpdateSvl dmh = new ErpDonmuahangTSCCUpdateSvl();
		dmh.removeFONT("");
	}
