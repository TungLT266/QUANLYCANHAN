...
import geso.traphaco.erp.beans.nhanhangtrongnuoc.imp.Sanpham;
import geso.traphaco.erp.beans.nhanhangtrongnuoc.imp.SpDetail;
...
public class ErpNhanhangtrongnuocUpdate_GiaySvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;
	
	PrintWriter out;
	
	public ErpNhanhangtrongnuocUpdate_GiaySvl()
	{
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else
		{
			...
			{
				nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocDisplay_Giay.jsp";
			}
			...
			{
				nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocUpdate_Giay.jsp";	
			}
		...
		else
		{
			...
			if (action.equals("save")){
				...
				if(changeSoMe.equals("changeSoMe")){
					...
					if (id == null)
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocNew_Giay.jsp";
					}
					else
					{
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocUpdate_Giay.jsp";
					...
					if (spMa != null)
					{
						...
						while (m < spMa.length)
						{
							...
							String[] nsxTen = request.getParameterValues(tem+ ".nsxX");
							String[] nsxId = request.getParameterValues(tem+ ".nsxidX");
							String[] marrquet = request.getParameterValues(tem+ ".marrquetX");
							
							
							// xử lý trường hợp kho mà không có khu
							if( khuvucX == null && mathungX!=null){
							...
							if(mathungX !=null){
								for(int i=0; i< mathungX.length; i++){
									...
									temp.setNsxId(nsxId[i]);
									temp.setNsxTen(nsxTen[i]);
									temp.setMarrquet(marrquet[i]);
					...
					if (id == null) // tao moi
					{
						...
						if (!nhBean.updateNhanHang()) {
							...
							session.setAttribute("nhBean", nhBean);
							String nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocUpdate_Giay.jsp";
							response.sendRedirect(nextJSP);
						} else {
							// Do co dung sai nen ko the hoan tat don mua hang
							// tai cho nay ???????
			...
			else
			{
				...
				else if(action.equals("changeKhoNhan"))
				{
					...
					}else
					{
						...
						GiuDieuKienLoc.getSetDieuKienLoc(session, this.getServletName(), Thread.currentThread().getStackTrace()[1].getMethodName(), action, userId, nhBean);
						nhBean.init();						
						nextJSP = "/TraphacoHYERP/pages/Erp/ErpNhanHangtrongnuocUpdate_Giay.jsp";
					}
