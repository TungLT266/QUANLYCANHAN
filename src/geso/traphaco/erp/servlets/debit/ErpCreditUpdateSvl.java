...
public class ErpCreditUpdateSvl extends HttpServlet {
private static final long serialVersionUID = 1L;
	
    public ErpCreditUpdateSvl() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    if(action.equals("update")){
			...
			String  nextJSP = "/TraphacoHYERP/pages/Erp/ErpCreditNew.jsp";
			...
			String  nextJSP = "/TraphacoHYERP/pages/Erp/ErpCreditDisplay.jsp";
			response.sendRedirect(nextJSP);
		   ...
		   session.setAttribute("userId", userId);
		   session.setAttribute("userTen", userTen);
		   String nextJSP = "/TraphacoHYERP/pages/Erp/ErpCredit.jsp";
		   response.sendRedirect(nextJSP);
		   ...
		   response.sendRedirect(nextJSP);
			...
			response.setHeader("Content-Disposition","attachment; filename=credit.xls");
			IErpCreditList obj = new ErpCreditList();
	...
	private boolean CreatePivotTable(OutputStream out,HttpServletResponse response, HttpServletRequest request,String id) throws IOException {
		...
		fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\credit.xls");
	...
	private boolean FillData(Workbook workbook, String id) {
		...
		String sql = " select de.TIENTE_FK, tt.MA, case when de.LOAIDOITUONG ='1' then " +
				     ...
				     " from ERP_CREDIT  de left join ERP_TIENTE tt on tt.PK_SEQ= de.TIENTE_FK  " +
		...
		sql = "  select DIENGIAI, NONGUYENTE, DONGIA, SOLUONG from ERP_CREDIT_DETAILS where CREDIT_FK = "+ id;
		...
		for( int i=0; i< dienGiais.size(); i++){
			...
			cell.setValue(noNguyenTes.get(i));
			style = cell.getStyle();
			font = style.getFont();
			style.setNumber(4);
			font.setBold(true);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String action = request.getParameter("action");		
 		...
 		if(donGias != null){
 			obj.setDonGias(convertArrayToDouble(donGias));
 		}
 		if( soLuongs !=null){
 			obj.setSoLuongs(convertArrayToInteger(soLuongs));
 		}
 		...
 		if( action.equals("new")){
 			...
 			if(!obj.create()){
 				...
 				IErpCreditList bean = new ErpCreditList();
 				bean.init("");
	...
	private List<Integer> convertArrayToInteger(String[] string){
		List<Integer> list = new ArrayList<Integer>();
		for(int i=0; i< string.length; i++){
			if(string[i].trim().length() == 0){
				list.add(new Integer("0"));
			} else {
				list.add(Integer.parseInt(string[i].replaceAll(",", "")));
			}
		}
		return list;
	}
