...
public class ErpBCCCDC extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nextJSP = "/TraphacoHYERP/pages/Erp/ErpBCCCDC.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(!cutil.check(userId, userTen, sum)){
			...
			System.out.println("nam: " + nam + ", thang new: " + thang);
			...
			if (action.equals("tao")) 
			{
				try {
					...
					{
						response.setHeader("Content-Disposition", "attachment; filename=BCTongHopCCDC_ChiPhiTraTruoc_" + getDate() + ".xlsm");
					}
					else
					{
						response.setHeader("Content-Disposition", "attachment; filename=BCTongHopCCDC_ChiPhiTraTruoc_" + getDate() + ".xlsm");
					}
					String congTyId = "100000";
	...
	private void CreateReport(OutputStream out, String congTyId, String thang, String nam)throws Exception 
	{
		try{
			...
			fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\BCCCDC.xlsm");
			...
			ResultSet rs = db.getRsByPro("GetBaoCaoCCDC", param);
			...
			while(rs.next())
			{
				...
				{
					Cell cell = cells.getCell(countRow,i-2 );
			...
			for(int i =2;i <=socottrongSql ; i ++)
			{
				CreateBorderSetting3(cells.getCell(countRow - 1,i-2));
			}
