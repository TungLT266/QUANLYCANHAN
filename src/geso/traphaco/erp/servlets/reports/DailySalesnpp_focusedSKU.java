...
public class DailySalesnpp_focusedSKU extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nextJSP = "/TraphacoHYERP/pages/Distributor/DailySalesReportDistributor_focusedSKU.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        obj.setuserTen((String) session.getAttribute("userTen")!=null? 
	   	...
	   	if(nspId.length() == 0)
	   		obj.setMsg("Vui lòng chọn 1 nhóm sản phẩm");
	   	else
		...
		if (dsc.equals("1"))
		 ...
		 if(obj.GetNhoSPId().length() > 0){
			 sql += " and NSP.PK_SEQ = '" + obj.GetNhoSPId() + "'" ;
		 }
		 
		 String nextJSP = "/TraphacoHYERP/pages/Distributor/DailySalesReportDistributor_focusedSKU.jsp";		 
		 String action=request.getParameter("action");		 
		 System.out.println("action: " + action + "; nspId: " + nspId);
		 
		 if(action.equals("tao") & nspId.length() > 0)
		 {
			try{						
	...
	private String setQuery(String sql,IStockintransit obj)
	{
		...
		query = query +	"ISNULL(GSBH.TEN, 'Chua xac dinh') AS SALESUPER, " +  	 
						...
						"INNER JOIN NHOMSANPHAM NSP ON NSP.PK_SEQ = NSPSP.NSP_FK " +
...
//		 FileInputStream fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoNPP_focusedSKU.xlsm");		
		 ...
		 FileInputStream fstream = new FileInputStream("D:\\Best Stable\\SalesUp\\WebContent\\pages\\Templates\\DoanhSoNPP_focusedSKU.xlsm");
