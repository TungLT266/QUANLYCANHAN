...
public class DailySecondarySalesTTSvl extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nextJSP = "/TraphacoHYERP/pages/Center/DailySecondarySalesReportCenter.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if (dsc.equals("1"))
			...
			obj.setdiscount("0");		
		System.out.println("Chiet Khau :"+obj.getdiscount());
		System.out.println("Thue : "+obj.getvat());
			 ...
			 thang = Integer.valueOf(obj.gettungay().substring(6,7));
			  ...
			  if(obj.getFromMonth().length() <= 0) //xem theo ngay
			  {
					...
					Distributor += "sum(ISNULL(ROUND((CAST((QC.SOLUONG2 * DH.SOLUONG) AS FLOAT) / CAST(QC.SOLUONG1 AS FLOAT)),3), 0)) AS QUANTITY " +
							...
							" group by dvkd.donvikinhdoanh, V.TEN, KV.TEN, NPP.TEN, NPP.SITECODE, NH.TEN, CL.TEN, SP.MA + '_' + SP.TEN, SP.MA, KBH.DIENGIAI, DH.NGAYNHAP ";
					
						
						Distributor += " ORDER BY DH.NGAYNHAP";
			  ...
			  else //xem theo thang
			  {
				  ...
				  SalesRep =  "select BU, region, area, isnull(salesuper, 'Chua xac dinh') as salesuper, SalesRepId, SalesRep, distcode, brand, " +
			  		"category, distributor, SKU, SKUcode, chanel, sum(piece) as piece, sum(quantity) as quantity, thang as ngay, ";
	...
	private boolean CreatePivotTable(OutputStream out, IStockintransit obj, String query, String level)throws Exception 
	{
		...
		case 0:	
			if(obj.getFromMonth().length() <= 0) //Xem theo Ngay
			{
				fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT.xlsm");
			}
			else
			{
				fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT_Thang.xlsm");
			}
//				
		...
		case 1:	
			if(obj.getFromMonth().length() <= 0) //Xem theo Ngay
			{
				fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT-DDKD.xlsm");
				}
			else
			{
				fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT-DDKD_Thang.xlsm");
				}
			workbook.open(fstream);
			workbook.setFileFormatType(FileFormatType.EXCEL2007XLSM);
		...
		case 2:
			if(obj.getFromMonth().length() <= 0) //Xem theo Ngay
			{
				fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT_Customer.xlsm");
			}
			else
			{
					fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTT_Customer_Thang.xlsm");
			}
...
//				ReportAPI.setHidden(workbook, 60);
				/*PivotTables pivotTables = worksheet.getPivotTables();
				String pos = Integer.toString(i - 1);
				int index = pivotTables.add("=AA1:AW"+ pos, "A10","PivotTable1");				
			    ...
			    selected.put("NhaPhanPhoi",5);		   
			    selected.put("NhanHang",6);
			    selected.put("ChungLoai",7);		   
			    selected.put("MaNhaPhanPhoi",8);
			    selected.put("MaDaiDienKinhDoanh",9);
			    selected.put("DaiDienKinhDoanh",10);			    
			    selected.put("TenSanPham",11);
			    selected.put("MaSanPham",12);
			    selected.put("KhachHang",13);
			    selected.put("ViTriCuaHang",14);
			    selected.put("LoaiCuaHang",15);
			    selected.put("Ngay",16);			    
			    selected.put("NhomSanPham",17);
			    selected.put("NhomKhachHang",18);
			    selected.put("SoTien",19);
			    selected.put("SoLuong(Le)",20);
			    selected.put("SoLuong(Thung)",21);
			    selected.put("SmartId", 22);	
	    		...
	    		pivotTable.getRowFields().get(2).setAutoSubtotals(true);
	    		pivotTable.getRowFields().get(3).setAutoSubtotals(true);
	    		pivotTable.getRowFields().get(4).setAutoSubtotals(false);
	    		pivotTable.getRowFields().get(5).setAutoSubtotals(true);
	    		pivotTable.getRowFields().get(6).setAutoSubtotals(false);
	    		pivotTable.getRowFields().get(7).setAutoSubtotals(false);
	    		pivotTable.getRowFields().get(10).setAutoSubtotals(true);*/
			    
//			    if(pivotTable.getDataFields().size()>0){
