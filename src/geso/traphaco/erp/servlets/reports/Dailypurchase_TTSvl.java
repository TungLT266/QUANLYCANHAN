...
public class Dailypurchase_TTSvl extends HttpServlet 
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nextJSP = "/TraphacoHYERP/pages/Center/RDailypurchase_center.jsp";
    ...
    {  		
	    try
	    {
			...
			// fstream = new FileInputStream("D:\\Provence Projects\\SalesUp\\WebContent\\pages\\Templates\\Dailypurchase_TTSvl.xlsm");
	...
	private void CreateStaticHeader(Workbook workbook, String dateFrom, String dateTo, String UserName) 
	{
		...
		cell = cells.getCell("AL1");		cell.setValue("Kho");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AM1");		cell.setValue("TinhThanh");					ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AN1");		cell.setValue("QuanHuyen");			ReportAPI.setCellHeader(cell);	
		cell = cells.getCell("AO1");		cell.setValue("NgayChungTu");			ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AP1");		cell.setValue("SoLuongLe");			ReportAPI.setCellHeader(cell);	    
		cell = cells.getCell("AQ1");		cell.setValue("SoTien");			ReportAPI.setCellHeader(cell);
	...
	private boolean CreateStaticData(Workbook workbook, IStockintransit obj) throws Exception 
	{
		...
		String sql = "select  kbh.ten as Channel, "
				...
				+ " nhsp.sanpham_fk as SKU_code, "
				+ " sp.ten as SKU,nh.ngaynhan as Purdate, "
				...
				+ " where nh.trangthai <> '0'" + " and nh.ngaynhan >= '"
				+ obj.gettungay() + "' " + " and nh.ngaynhan <= '"
				+ obj.getdenngay() + "'";
		...
		sql += " and npp.pk_seq in " + ut.quyen_npp(obj.getuserId())
		 ...
		 if(obj.getnppId().length()>0)
			 sql = sql +" and cl.pk_seq ='"+ obj.getnppId() +"'";
		...
		if (rs != null) 
		{
			...
			{// se do rong cho cac cot se dung
				...
				while (rs.next())// lap den cuoi bang du lieu
				{
					...
					cell = cells.getCell("AK" + Integer.toString(i));	cell.setValue(SKU); //10
					cell = cells.getCell("AL" + Integer.toString(i));	cell.setValue(khoTen); //11
					cell = cells.getCell("AM" + Integer.toString(i));	cell.setValue(Province);
					cell = cells.getCell("AN" + Integer.toString(i));	cell.setValue(Town);
					cell = cells.getCell("AO" + Integer.toString(i));	cell.setValue(Purdate); //14
					cell = cells.getCell("AP" + Integer.toString(i));	cell.setValue(Float.parseFloat(Piece)); //15
					cell = cells.getCell("AQ" + Integer.toString(i));	cell.setValue(Float.parseFloat(Amount));	 //16
			    ...
			    int j = pivotTables.add("=StockInTransit!AA1:AQ" + pos,"A11","PivotTable");	   
			    ...
			    pivotTable.addFieldToArea(PivotFieldType.ROW, 0);	pivotTable.getRowFields().get(0).setAutoSubtotals(true);
			 	...
			 	pivotTable.addFieldToArea(PivotFieldType.ROW, 14); 	pivotTable.getRowFields().get(5).setAutoSubtotals(true);
			 	...
			 	pivotTable.addFieldToArea(PivotFieldType.DATA, 15); 	pivotTable.getDataFields().get(0).setDisplayName("SoLuongLe");
			 	pivotTable.getDataFields().get(0).setNumber(3);
			 	 
			 	pivotTable.addFieldToArea(PivotFieldType.DATA, 16); 	pivotTable.getDataFields().get(1).setDisplayName("SoTien");
