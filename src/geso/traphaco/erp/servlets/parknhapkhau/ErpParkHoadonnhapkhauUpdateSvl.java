package geso.traphaco.erp.servlets.parknhapkhau;
...
import geso.traphaco.erp.beans.parknhapkhau.imp.ErpHoadon;
import geso.traphaco.erp.beans.parknhapkhau.imp.ErpPark;
import geso.traphaco.erp.beans.parknhapkhau.imp.ErpParkList;
import geso.traphaco.erp.beans.parknhapkhau.imp.ErpHoadonSp;
...
public class ErpParkHoadonnhapkhauUpdateSvl extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	PrintWriter out;
    public ErpParkHoadonnhapkhauUpdateSvl() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		else
		{
	        ...
	        if(request.getQueryString().indexOf("display") >= 0 ) 
	        {
	        	...
	        	pBean.init();
	        	nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhauDisplay.jsp";
	        }
	        else
	        {
		        pBean.init();
	        	nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhauUpdate.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		else
		{
		   ...
		   pBean.setPoId(poId);
		    ...
		    String tongVND = request.getParameter("tongVND");
		    temp = 0;
		    try {
		    	if(tong.trim().length() >0){
		    		temp = Double.parseDouble(tongVND.replaceAll(",", ""));
		    	}
		    
		    } catch(Exception e) {
		    	e.printStackTrace();
		    }
		    pBean.setTongVND(temp);
		    ...
		    pBean.setChenhlechHd(Double.toString(chenhlech_hd));
			...
			String[] dongiaVND = request.getParameterValues("dongiaMuaVND");
			String[] thanhTienVND = request.getParameterValues("thanhtienVND");
		   ...
		   if (muahang_fk != null )
			...
			if (spMa != null)
			{
				...
				while (m < spMa.length)
				{
					...
					if( soluongnhan[m].trim().length() >0 && Double.parseDouble(soluongnhan[m].replaceAll(",", "")) > 0)
					{
						if(soluongnhan[m].trim().length() > 0) slnhan =soluongnhan[m].replaceAll(",", "");
						
						sanpham = new Sanpham(spId[m], spMa[m], spTen[m],"", "", "", soluongdat[m].replaceAll(",", ""), spDonvi[m]);
						...
						sanpham.setDongiaVND(dongiaVND[m].replaceAll(",", ""));
						
						sanpham.setHansudung(hansudung[m]);
	
						sanpham.setthanhtien(thanhTien[m].replaceAll(",", ""));
						sanpham.setThanhtienVND(thanhTienVND[m].replaceAll(",", ""));
						System.out.println("THANH TIEN "+thanhTienVND[m]);
			...
			if(error == true){
				...
				session.setAttribute("pBean", pBean);
				String nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhauNew.jsp";
		    ...
		    if(action.equals("change")){
    		    ...
    		    pBean.init();
    		    session.setAttribute("pBean", pBean);
    			String nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhauUpdate.jsp";
    			response.sendRedirect(nextJSP);
    			return;
			...
			{	
				if(id == null) //tao moi
				{
					...
					else
					{
			    		...
			    		response.sendRedirect("/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhau.jsp");
					}
				}
				else
				{
					...
					else
					{
						...
						else
						{
						    ...
						    obj.init("");
							session.setAttribute("obj", obj);							
							String nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhau.jsp";
							response.sendRedirect(nextJSP);
		    ...
		    {		    System.out.println("vào hàm chot");
		        	...
		        	pBean.init();
		        	pBean.setDuyet(1);
		    		pBeanDuyet.setMsg(pBeanDuyet.getMsg());
		    		session.setAttribute("pBean", pBean);
					String nextJSP = "/TraphacoHYERP/pages/Erp/ErpParkHoaDonNhapKhauDisplay.jsp";
					response.sendRedirect(nextJSP);
					return;
		    }
			else
			{
				...
				}else if(action.equals("changePO"))
				{
					spList.clear();
					pBean.setSpList(spList);
				}
	...
	private void CreatePO_Training(Document document,
		...
		try {
			...
			Font font12v1 = new Font(bf, 10, Font.NORMAL);
...
//			img.setAbsolutePosition(3f * CONVERT, document.getPageSize().getHeight() - 1.2f * CONVERT);
			img.setAbsolutePosition(2.7f * CONVERT, document.getPageSize().getHeight() - 1.2f * CONVERT);
			document.add(img);
			...
			p.add(new Chunk("CÔNG TY CP TRAPHACO", font12v1)); 
			...
			1.25f * CONVERT, 1.5f * CONVERT, 1.5f * CONVERT, 2.0f * CONVERT};
					...
					if (spList.size() > 0) {
						...
						System.out.println("Xuat vat : "+tongVAT);
						//---------------------------------
						Double TienVat=0.0,ThueNK=0.0;
						String query_thue="select c.vat,c.THUENK "
									+ " from ERP_PARK a inner join ERP_HOADONNCC b on a.pk_seq = b.park_fk "
									+ " inner join ERP_THUENHAPKHAU c on c.HOADONNCC=b.pk_seq " 
									+ " where b.pk_seq='"+ pBean.getHoadonNCC()+"'";
						System.out.println("Xuat query thue: "+query_thue);
						ResultSet rs_thue = db.get(query_thue);
						if(rs_thue != null)
						{
							try{
								while(rs_thue.next())
								{
								TienVat=rs_thue.getDouble("vat");
								ThueNK=rs_thue.getDouble("THUENK");
								}
								rs_thue.close();
					...
					//	String[] spTongCong = {"Tổng cộng tiền hàng", "Chi phí", "Thuế giá trị gia tăng"};
						String[] spTongCong = {"Tổng cộng tiền hàng", "Chi phí","Tổng tiền thanh toán" ,"Thuế nhập khẩu", "Tổng cộng","Thuế GTGT hàng nhập khẩu" };
						for (int z = 0; z < spTongCong.length; z++) {
							...
							if(z==2||z==4)
								cell = new PdfPCell(new Paragraph(formatter1.format(soTien), font11_bold));
							else
