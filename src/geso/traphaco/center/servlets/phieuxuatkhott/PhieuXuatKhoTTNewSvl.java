package geso.traphaco.center.servlets.phieuxuatkhott;

import geso.traphaco.center.beans.hoadon.IHoaDon;
import geso.traphaco.center.beans.hoadon.imp.HoaDon;
import geso.traphaco.center.beans.phieuxuatkhott.IPhieuXuatKhoTT;
import geso.traphaco.center.beans.phieuxuatkhott.imp.PhieuXuatKhoTT;
		...
		session.setAttribute("rs_dondathang", rs_dondathang);
         ...
         String sql_getdvkd="select pk_seq,donvikinhdoanh as ten from donvikinhdoanh where trangthai='1' ";//lay nhung don dat hang da chot,va chua chuyen het hang sang kho khac
			...
			if(action.equals("update")){
				String 	nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTUpdate.jsp";
				response.sendRedirect(nextJSP);
			}else if(action.equals("chot")){
				String 	nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTChot.jsp";
				response.sendRedirect(nextJSP);
			}else if(action.equals("display")){
				String 	nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTDisplay.jsp";
				response.sendRedirect(nextJSP);
				...
				IPhieuXuatKhoTT   obj = new PhieuXuatKhoTT();
				  ...
				  String  nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTT.jsp";
				   response.sendRedirect(nextJSP);
			}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = request.getParameter("action");		
 		...
 		if(tenform.equals("newform")){
 			nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTNew.jsp";
 		}else{
 		   nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTUpdate.jsp";
 		}
			  ...
			  if(phieuxuat.SavePhieuXuatKho()){
				...
				if(phieuxuat.EditPhieuXuatKho()){
				  ...
				  if(phieuxuat.ChotPhieuXuatKho()){
			       ...
			       obj.init("");
				   session.setAttribute("obj", obj);
				   nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTT.jsp";
				   response.sendRedirect(nextJSP);
				   }else
				   {
					   nextJSP = "/TraphacoHYERP/pages/Center/PhieuXuatKhoTTChot.jsp";
					   session.setAttribute("phieuxuat",phieuxuat );
					   session.setAttribute("obj",hoadon);
					  response.sendRedirect(nextJSP);
				   }	
