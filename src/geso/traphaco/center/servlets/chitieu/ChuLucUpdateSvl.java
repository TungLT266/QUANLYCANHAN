...
public class ChuLucUpdateSvl extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nextJSP = "/TraphacoHYERP/pages/Center/ChuLucUpdate.jsp";
		if (querystring.indexOf("display") >= 0)
			nextJSP = "/TraphacoHYERP/pages/Center/ChuLucDisplay.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		else
		{
			...
			scheme = util.antiSQLInspection(request.getParameter("scheme"));
			if (scheme == null)
				scheme = "";
			tctskuBean.setScheme(scheme);
			...
			quy = util.antiSQLInspection(request.getParameter("quy") == null ? "" : request.getParameter("quy"));
			tctskuBean.setQuy(quy);

			apdung = util.antiSQLInspection(request.getParameter("apdung") == null ? "" : request.getParameter("apdung"));
			tctskuBean.setApdung(apdung);

			loaichitieu = util.antiSQLInspection(request.getParameter("loaichitieu") == null ? "0" : request.getParameter("loaichitieu"));
			tctskuBean.setLoaichitieu(loaichitieu);

			view = util.antiSQLInspection(request.getParameter("view") == null ? "" : request.getParameter("view"));
			tctskuBean.setView(view);

			tructhuocId = util.antiSQLInspection(request.getParameter("tructhuocId") == null ? "" : request.getParameter("tructhuocId"));
			tctskuBean.setTructhuocId(tructhuocId);

			loaichitieu = util.antiSQLInspection(request.getParameter("loaichitieu") == null ? "0" : request.getParameter("loaichitieu"));
			tctskuBean.setLoaichitieu(loaichitieu);
			
			doituong = util.antiSQLInspection(request.getParameter("doituong") == null ? "0" : request.getParameter("doituong"));
			tctskuBean.setDoituong(doituong);
			
			sonhomdat = util.antiSQLInspection(request.getParameter("sonhomdat") == null ? "" : request.getParameter("sonhomdat").replaceAll(",", ""));
			tctskuBean.setSonhomdat(sonhomdat);

			thuongSR = util.antiSQLInspection(request.getParameter("thuongSR") == null ? "" : request.getParameter("thuongSR").replaceAll(",", ""));
			tctskuBean.setThuongSR(thuongSR);
			
			thuongSS = util.antiSQLInspection(request.getParameter("thuongSS") == null ? "" : request.getParameter("thuongSS").replaceAll(",", ""));
			tctskuBean.setThuongSS(thuongSS);

			thuongASM = util.antiSQLInspection(request.getParameter("thuongASM") == null ? "" : request.getParameter("thuongASM").replaceAll(",", ""));
			tctskuBean.setThuongASM(thuongASM);
			
			thuongRSM = util.antiSQLInspection(request.getParameter("thuongRSM") == null ? "" : request.getParameter("thuongRSM").replaceAll(",", ""));
			
			tctskuBean.setThuongRSM(thuongRSM);
			
			vungId = request.getParameter("vungId");
			if (vungId == null)
				vungId = "";
			tctskuBean.setVungIds(vungId);

			kvId = request.getParameter("khuvucId");
			if (kvId == null)
				kvId = "";
			...
			action = util.antiSQLInspection(request.getParameter("action") == null ? "" : request.getParameter("action"));
		}
		...
		if (action.equals("save"))
		{
			if (id == null)
			{
				if (!tctskuBean.save())
				{
					...
					session.setAttribute("tctskuBean", tctskuBean);
					String nextJSP = "/TraphacoHYERP/pages/Center/ChuLucNew.jsp";
					response.sendRedirect(nextJSP);
				} else
				{
		...
		} else
		{
			...
			if (id == null)
			{
				nextJSP = "/TraphacoHYERP/pages/Center/ChuLucNew.jsp";
			} else
			{
				nextJSP = "/TraphacoHYERP/pages/Center/ChuLucUpdate.jsp";
			}
			response.sendRedirect(nextJSP);
		}
	}
