...
public class NganhHangSvl extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    else if (action.equals("search"))
	    {
    		...
    		response.sendRedirect("/TraphacoHYERP/pages/Center/NganhHang.jsp");	    	
	    } 
	...
	private String Delete(String id)
	{
		...
		ResultSet rs = db.get("select count(pk_seq) as num from sanpham where nganhang_fk = '" + id + "'");
	   	try{
	   	    while(rs.next())
	   		if(rs.getString("num").equals("0"))
	   		{
	   			String command = "delete from nganhhang where pk_seq ='" + id + "'";
	   			db.update(command);
	   			
	   			//SYN QUA DMS
	   			UtilitySyn.SynData(db, "NGANHHANG", "NGANHHANG", "PK_SEQ", id, "2", true);
	   			
	   			db.shutDown();
	   		}
	   		else
	   			return "Don vi do luong nay da co san pham roi, khong the xoa duoc";
	   	}
	   	catch(Exception e) 
	   	{
	   		return e.getMessage();
