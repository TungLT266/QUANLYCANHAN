...
import geso.traphaco.distributor.beans.report.imp.Report;
...
public class BCDonDatHangNPP extends HttpServlet
{
	...
	private String setQuery(HttpServletRequest request, String tungay, String denngay, String st)
	{
		...
		"		QC.SOLUONG1 AS QC1,QC.SOLUONG2 AS QC2 ,ISNULL(DDH.SOID,0) , LOAIDONHANG,KmTT  \n"+
		"	FROM \n"+     
		"	( \n"+  
		"		SELECT * FROM \n"+  
		"		DONDATHANG DDH \n"+    
		"		WHERE DDH.NGAYDAT >='"+tungay+"' AND DDH.NGAYDAT <= '"+denngay+"' AND DDH.TRANGTHAI>=1  \n"+    
		"	)DDH   \n"+
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nextJSP = "/TraphacoHYERP/pages/Center/BCDonDatHangNPP.jsp";
	...
	private void CreateHeader(Workbook workbook, Ireport obj)
	{
		...
		cell = cells.getCell("AM1");cell.setValue("So luong dat(le)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AN1");cell.setValue("So luong dat(Thung)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AO1");cell.setValue("So luong duyet(le)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AP1");cell.setValue("So luong duyet(Thung)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AQ1");cell.setValue("Tong tien");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AR1");cell.setValue("NguoiDuyet");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AS1");cell.setValue("NgayGioDuyet");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AT1");cell.setValue("TongTienDat");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AU1");cell.setValue("Ma NPP");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AV1");cell.setValue("ThongTinDonHang");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AW1");cell.setValue("ThucXuat(le)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AX1");cell.setValue("ThucXuat(Thung)");ReportAPI.setCellHeader(cell);
		cell = cells.getCell("AY1");cell.setValue("LoaiDon");ReportAPI.setCellHeader(cell);
		...
		if (rs != null)
		{
			try
			{
				...
				while (rs.next())
				{
					...
					int  LoaiDonHang = rs.getInt("LoaiDonHang");
					int LoaiKm = rs.getInt("KmTT");
					...
					cell = cells.getCell("AN" + String.valueOf(index));cell.setValue(sldat_thg);

					cell = cells.getCell("AO" + String.valueOf(index));cell.setValue(rs.getFloat("SlDuyet"));
					cell = cells.getCell("AP" + String.valueOf(index));cell.setValue(slduyet_thg);

					cell = cells.getCell("AQ" + String.valueOf(index));cell.setValue(rs.getDouble("TongTien"));
					cell = cells.getCell("AR" + String.valueOf(index));cell.setValue(rs.getString("nguoiduyet"));
					cell = cells.getCell("AS" + String.valueOf(index));cell.setValue(rs.getString("dateduyet"));
					cell = cells.getCell("AT" + String.valueOf(index));cell.setValue(rs.getDouble("TongTienDat"));
					cell = cells.getCell("AU" + String.valueOf(index));cell.setValue(rs.getString("MaNPP"));
					cell = cells.getCell("AV" + String.valueOf(index));cell.setValue(rs.getString("thongtin"));
					cell = cells.getCell("AW" + String.valueOf(index));cell.setValue(rs.getDouble("thucxuat"));
					cell = cells.getCell("AX" + String.valueOf(index));cell.setValue(thucxuat_thg);
					
					cell = cells.getCell("AY" + String.valueOf(index));cell.setValue(Loai);
