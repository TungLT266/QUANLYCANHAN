...
public class CapnhatnhanvienSvl extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String nhomquyenId = util.antiSQLInspection(request.getParameter("nhomquyenId"));
		if (nhomquyenId == null)
			nhomquyenId = "";
		obj.setNhomquyenId(nhomquyenId);
		...
		//	st += " and a.timkiem like '%' + dbo.ftBoDau(N'%" + Ten + "%') + '%' ";
			
			
			st += "  and  dbo.ftBoDau( a.ten ) like upper(N'%"+util.replaceAEIOU(Ten).trim()+"') or dbo.ftBoDau (a.DANGNHAP) like upper(N'%"+util.replaceAEIOU(Ten).trim()+"')";
		...
		if( nhomquyenId.trim().length() > 0 )
		{
			//st = st + " and a.pk_seq in ( select Nhanvien_fk from PHANQUYEN where DMQ_fk in ( select PK_SEQ from DANHMUCQUYEN where dbo.ftBoDau(ten) like N'%" + util.replaceAEIOU(tenQuyen) + "%' ) ) ";
			st = st + " and a.pk_seq in ( select Nhanvien_fk from PHANQUYEN where DMQ_fk in ( select PK_SEQ from DANHMUCQUYEN where pk_seq = '" + nhomquyenId + "' ) ) ";
		}
		
		if(phongbanId.trim().length() >0){
			st = st+ " and dvth.PK_SeQ = "+ phongbanId;
		}
		...
		else if (action.equals("toExcel"))
		{
			...
			String sql = "select a.pk_seq, a.Ten, a.dangnhap, a.email, a.dienthoai, a.trangthai, a.phanloai, b.ten as nguoitao1, c.ten as nguoisua1, a.ngaytao, a.ngaysua,  "+
						 "	 (	 "+
		...
		else  if (action.equals("view") || action.equals("next") || action.equals("prev")) 
		{
			...
			String nextJSP = "/TraphacoHYERP/pages/Center/CapNhatNhanVien.jsp";
	...
	private void ToExcel(HttpServletResponse response, String query) throws IOException
	{
		...
		try
		{
			...
			sheet.addCell(new Label(11, 4, "TÊN QUYỀN", cellFormat));
			...
			while (rs.next())
			{
				...
				label = new Label(11, j, rs.getString("quyenIds"), cformat);
				sheet.addCell(label);
