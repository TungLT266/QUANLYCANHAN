...
public class TieuchithuongTBList implements ITieuchithuongTBList 
{
	String userId;
	...
	public void init(String query)
	{
		...
		else
		{
			sql = "select a.scheme, a.pk_seq, a.thang, a.nam, a.diengiai, a.trangthai, a.ngaytao, a.ngaysua, b.ten as nguoitao, c.ten as nguoisua  " +
					"from TieuchithuongTB a inner join NHANVIEN b on a.NGUOITAO = b.pk_seq inner join NHANVIEN c on a.NGUOISUA = c.pk_seq " +
	...
	public void initReport(String query) 
	{
		...
		if(this.thang.trim().length() > 0 && this.nam.trim().length() > 0)
		{
			...
			query = "select pk_seq as schemeId, scheme as schemeTen " +
					"from CtTRungBay where ngaytrungbay >= '" + this.tungay + "' ";
			
			if(this.denngay.trim().length() > 0)
				query += " and ngaytrungbay <= '" + this.denngay + "' ";
