...
public class Hethongbanhang implements IHethongbanhang
{
	...
	public Hethongbanhang(String[] param)
	{
		this.db = new dbutils();
		this.id = param[0];
		this.hethongbanhang = param[1];
	...
	public Hethongbanhang(String id)
	{
		this.db = new dbutils();
		this.id = id;
		this.hethongbanhang = "";
	...
	public boolean CreateHtbh()
	{
		...
		String command = "insert into hethongbanhang(ten, diengiai, trangthai, ngaytao, ngaysua, nguoitao, nguoisua) values(N'" + this.hethongbanhang + "',N'" + this.diengiai + "'," + this.trangthai + ",'" + this.ngaytao + "','" + this.ngaytao + "','" + this.userId + "','" + this.userId + "')"; 
		 ...
		 * 
		 */
		command = "select IDENT_CURRENT('hethongbanhang')as id";
		...
		{
			command="select * from hethongbanhang_kenhbanhang where kbh_fk in ( " + this.kbhId + ")";
			...
			if(kt)
			{
				command="insert into hethongbanhang_kenhbanhang(kbh_fk, htbh_fk)" +
			...
			{
				this.msg="Lỗi kênh bán hàng đã thuộc hệ thống bán hàng khác ";
				this.db.update("rollback");
				return false;
			}
		}
	...
	public boolean UpdateHtbh() 
	{
		...
		String command ="update hethongbanhang set ten = N'" + this.hethongbanhang + "', diengiai = N'" + this.diengiai + "', trangthai='" + this.trangthai + "', ngaysua = '" + this.ngaysua + "', nguoisua = '" + this.userId + "' where pk_seq = '" + this.id + "'";
		...
		command="delete hethongbanhang_kenhbanhang where htbh_fk = '" + this.id +"'";
		System.out.println("xoa hethongbanhang_kenhbanhang:" + command);
		if(!this.db.update(command))
		{
			this.msg="Không thể cập nhật hethongbanhang_kenhbanhang "+command;
			this.db.update("rollback");
			return false;
		}
		if(this.kbhId.length()>0)
		{
			command="select * from hethongbanhang_kenhbanhang where kbh_fk in ( " + this.kbhId + ")";
			System.out.println("kiem tra ton tai " + command);
			ResultSet rs = db.get(command);
			boolean kt = true;
			if(rs.next())
			{
	...
	{	
        ...
        try{
        	...
        	this.hethongbanhang = rs.getString("htbhTen");
        ...
        if(this.id!=null && this.id.length()>0)
  		{
  			query="select htbh_fk, kbh_fk from hethongbanhang_kenhbanhang where  htbh_fk='"+this.id+"'";
			rs=this.db.get(query);
			System.out.println("htbh_kbh: " + query);
			try
			{
  		      	while(rs.next())
  		      	{
