package geso.traphaco.center.beans.upload.imp;
...
import geso.traphaco.center.beans.upload.IUpload;
...
/***  PHẦN THÊM BẰNG CHƯƠNG TRÌNH   ***/	
	...
	{
		String query=
		"select PK_SEQ as nppId,MA  as nppMa,Ten as nppTen from NHAPHANPHOI "+
		"where PK_SEQ "+  
	 ...
	 * Nhap Hang
	 * (non-Javadoc)
	 * @see geso.traphaco.center.beans.upload.IUpload#importTonKho()
	 */
	@Override
	public void importTonKho()
	{ 
		String query=" select distinct khoId from tonkho_tmp where khoid not in (100000,100001,100002) ";
		...
		"	where b.pk_seq  in ( select  NPP_FK from DONHANG where trangthai!=2)" ;
	...
	if(errr.length() >0)
		...
		try
		{
			...
			while(rs.next())
			{
				...
				query=
				"insert into NHAPP_KHO_CHITIET(KHO_FK,NPP_FK,SANPHAM_FK,SOLUONG,BOOKED,AVAILABLE,GIAMUA,KBH_FK,SOLO,NGAYHETHAN)"+
				" select '"+khoId+"','"+nppId+"','"+spId+"','"+soluong+"',0,"+soluong+",'"+giamua+"','"+kenhId+"','NA','2030-12-31' ";
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
			...
			/*	query=
				...
				*/
				...
				query ="insert into TONKHOTHANG_CHITIET(KHO_FK, NPP_FK,KBH_FK,SANPHAM_FK,THANG,NAM,SOLUONG,booked,available,SOLO, NGAYHETHAN)"+
						"select '"+khoId+"','"+nppId+"','"+kenhId+"','"+spId+"','"+thang+"' ,'"+nam+"', '"+soluong+"' ,0, '"+soluong+"', 'NA', '2030-12-31'  ";
				if(!db.update(query))
				{
					this.db.getConnection().rollback();
					this.msg+="Lỗi hệ thống "+query;
			...
			query= "insert into KHOASOTHANG(NPP_FK, THANGKSGANNHAT, THANGKS, NAM, NGAYTAO,NGUOITAO) "+
					"select '"+this.nppId+"','', '"+thang+"', '"+nam+"', '"+getDateTime()+"', '"+this.userId+"'  ";
