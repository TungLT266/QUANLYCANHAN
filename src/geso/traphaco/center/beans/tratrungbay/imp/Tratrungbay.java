package geso.traphaco.center.beans.tratrungbay.imp;
...
import geso.traphaco.center.beans.tratrungbay.ITratrungbay;
...
public class Tratrungbay implements ITratrungbay
{
	...
	public boolean CreateTratb(String[] masp, String[] dongia, String[] soluong) 
	{
		...
		try 
		{
			...
			{
				db.update("rollback");
				this.msg = "Khong the tao moi 'Tratrungbay', " + query;
				return false; 
			}
			if(this.type.equals("2")) //tra sanpham
			{
				...
				{
					String tratbCurrent = "";
					query = "select IDENT_CURRENT('Tratrungbay') as tratbId";
					...
					for(int i = 0; i < masp.length; i++)
					{
						if(masp[i].length() > 0)
						{
							...
							if(this.hinhthuc.equals("2"))
								soluong[i] = "null";
							if(pk_seq != "")
								query = "Insert into tratrungbay_sanpham(tratrungbay_fk, sanpham_fk, soluong, dongia) values('" + tratbCurrent + "', '" + pk_seq + "', " + soluong[i].trim() + ", '" + dongia[i] + "')";
							if(!db.update(query))
							{
								db.update("rollback");
								this.msg = "Loi khi them moi bang tratrungbay_sanpham, " + query;
								return false; 
							}
						}					
					}
	...
	public boolean UpdateTratb(String[] masp, String[] dongia, String[] soluong) 
	{
		...
		try 
		{
			...
			{
				db.update("rollback");
				this.msg = "Khong the cap nhat 'Tratrungbay', " + query;
				return false; 
			}
					...
					for(int i = 0; i < masp.length; i++)
					{
						if(masp[i].length() > 0)
						{
							...
							if(this.hinhthuc.equals("2"))
								soluong[i] = "null";
							if(pk_seq != "")
								query = "Insert into tratrungbay_sanpham(tratrungbay_fk, sanpham_fk, soluong, dongia) values('" + this.id + "', '" + pk_seq + "', " + soluong[i].trim() + ", '" + dongia[i] + "')";
							if(!db.update(query))
							{
								db.update("rollback");
								this.msg = "Loi khi them moi bang tratrungbay_sanpham, " + query;
	...
	{
		String query = "select b.pk_seq, b.MA, b.TEN, isnull(c.giabanlechuan, 0) as gblc from NHOMSANPHAM_SANPHAM a inner join SANPHAM b on a.SP_FK = b.PK_SEQ left join banggiablc_sanpham c on b.pk_seq = c.sanpham_fk ";
