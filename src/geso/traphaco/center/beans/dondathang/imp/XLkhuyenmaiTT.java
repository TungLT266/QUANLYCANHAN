...
public class XLkhuyenmaiTT 
{
	...
	{   
		...
		sql =  "select distinct a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk, isnull(a.diengiai, '') as diengiai,  " +
				"	d.ngansach, 0 as ChietKhau, a.PHANBOTHEODONHANG, ISNULL(d.SOXUATTOIDA, 0) as soxuattoida,  " +
				"	case a.PHANBOTHEODONHANG  " + 
				"		when 0 then ISNULL( 	( select SUM(tonggiatri)    " + 
				" 								  from ERP_DONDATHANG_CTKM_TRAKM    " + 
				" 								  where CTKMID = d.CTKM_FK and DONDATHANGID in ( select PK_SEQ from ERP_DONDATHANG where  TRANGTHAI != 3 )   ), 0 )  " + 
				"		else ISNULL( 	( select sum( SOLUONG)    " + 
				" 						  from ERP_DONDATHANG_CTKM_TRAKM    " + 
				" 						  where  SPMA IS NOT NULL AND CTKMID = d.CTKM_FK and DONDATHANGID in ( select PK_SEQ from ERP_DONDATHANG where TRANGTHAI != 3 )   ), 0 ) end as dasudung,   " +
				"ISNULL( 	( select SUM(soxuat)    " + 
				" 								  from ERP_DONDATHANG_CTKM_TRAKM    " + 
				" 								  where CTKMID = d.CTKM_FK and DONDATHANGID in ( select PK_SEQ from ERP_DONDATHANG where  TRANGTHAI != 3 )   ), 0 ) soxuatDADUNG " +
				"from ctkhuyenmai a  " +
						"inner join  phanbokhuyenmai d on a.PK_SEQ = d.CTKM_FK   " +
				"where a.loaict <> '3' and a.tungay <= '" + date + "' and '" + date + "' <= a.denngay   ";
	...
	private void ApOnTop() 
	{
		...
		if(this.dieuchinhKM)
		{
			...
			for(int i = 0; i < this.ctkmOntopList.size(); i++)
			{
				...
				{
					float sx = ctkm.checkCtkm(tonggiatriDh);
	...
	private void ApBinhThuong() 
	{
		...
		System.out.println("114.So CTKM binh thuong: " + this.ctkmBTList.size() + "; dieuchinh: " + this.dieuchinhKM);
		...
		if(this.dieuchinhKM)
		{
			for(int i = 0; i < this.ctkmBTList.size(); i++)
			{
				...
				System.out.println("----ctkm: " + ct.getscheme() + "; soxuat: " + ctkm.getSoxuatKM());
				if(ctkm.getSoxuatKM() > 0) //truong hop soxuat khuyenmai lon hon so xuat duoc huong toi da
				{
					if(ctkm.checkCtkm(tonggiatriDh) > 0)
					{
	...
	public Ctkhuyenmai getSoxuattheoScheme(Ctkhuyenmai ctkm, Hashtable<String, Float> hashSanpham)
	{
		...
		for(int i = 0; i < dkkmList.size(); i++)
		{
			...
			{
				flag = true;
				sl = (int) min(sl, ctkm.getSoxuatKM_TOIDA());
	...
	private String checkDKKM_TongLuong(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Float> sanpham_soluong)
	{
		...
		return Float.toString(soxuatKm) + "-" + Long.toString(tongtiendh);
	}
	...
	private String checkDKKM_TongTien(Dieukienkhuyenmai dkkm, Hashtable<String,Float> sanpham_dongia, Hashtable<String, Float> sanpham_soluong)
	{
		...
		//return soxuatKm;
		return Float.toString(soxuatKm) + "-" + Long.toString(tongtientheodk);
	}
	...
	private void dieuChinhKM(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sp_sl, Hashtable<String, Float> sp_dongia, float soXuatKM, ICtkhuyenmai ctkm)
	{
	...
	private void dieuChinhKM2(Dieukienkhuyenmai dkkm,  Hashtable<String, Float> hashSanpham, float soXuat)
	{
		float old = dkkm.getSoxuatKM();
	...
	private Sanpham[] getSanpham_soluong(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sanpham_sl, float soxuatKm) //truong hon tong tien > 0
	{
	...
	public Sanpham[] getSanpham_slg(Dieukienkhuyenmai dkkm, Hashtable<String, Float> sanpham_sl, float soxuatKm) //truong hop tongluong > 0
	{
	...
	public boolean checkConfirm() 
	{
		...
		//TAM THOI LUC NAO CUNG SANG TRANG
		//return true;
	...
	{
		Hashtable<String, Float> hashA = new Hashtable<String, Float>();
		...
		hashA.put("65020001", 60f);
		hashA.put("01.01.11.01.00.003", 50f);
		hashA.put("01.01.11.01.00.004", 150f);
