...
import geso.traphaco.distributor.beans.ctkhuyenmai.imp.Ctkhuyenmai;
import geso.traphaco.distributor.beans.ctkhuyenmai.imp.Sanpham;
...
import geso.traphaco.distributor.beans.dieukienkhuyenmai.imp.*;
import geso.traphaco.distributor.beans.trakhuyenmai.ITrakhuyenmai;
import geso.traphaco.distributor.beans.trakhuyenmai.imp.*;
...
public class XLkhuyenmaiTT_Import 
{
private dbutils db;
	...
	public XLkhuyenmaiTT_Import(String userId, String date, String nppId, String khId, String donhangId)
	{
		db = new dbutils();
		...
		this.khachhang = khId;
		String sql = "select khachhang.kbh_fk,khachhang.npp_fk,npp.ten " + 
					 " from khachhang inner join nhaphanphoi npp on npp.pk_Seq=npp_fk  where khachhang.pk_Seq="+this.khachhang;
		...
		{
			e.printStackTrace();
			System.out.println("115.Error Herer: " + e.toString());
		}
	...
	{   
		...
		sql = "select distinct a.pk_seq, a.scheme, a.tungay, a.denngay, a.loaict, a.kho_fk,  isnull(a.diengiai, '') as diengiai,   " + 
				 ...
				 "	END AS soxuatDADUNG, trakhuyenmai.ChietKhau, trakhuyenmai.tongluong   " + 
				 ...
				 "\n left join " + 
			     "\n ( " + 
			     "\n  select a.CTKHUYENMAI_FK, MAX(ISNULL(b.chietkhau, 0)) as ChietKhau, MAX( ISNULL( tongluong, 0) ) as tongluong   " + 
				 ...
				 " and ( a.thuongtl_fk is null or '" + this.khachhang + "' not in ( select KHACHHANG_FK from DANGKYKM_TICHLUY dk inner join DANGKYKM_TICHLUY_KHACHHANG dk_kh on dk.PK_SEQ = dk_kh.DKKMTICHLUY_FK where TRANGTHAI = '1' and TIEUCHITL_FK in ( 100018, 100021 ) and ngayketthuchd >= '" + date + "' ) )  	 " + 
				 ...
				 " 							when 1 then ( select DDKD_FK from ERP_DONDATHANGNPP where PK_SEQ = '" + this.idDonhang + "' )   " + 
				 " 							when 2 then '" + this.khachhang + "' " + 
				 " 							end  )    " +
				 "  order by trakhuyenmai.ChietKhau desc, trakhuyenmai.tongluong desc	";
					...
					if( soxuatCONLAI > 0 && ngansach - dasudung > 0 )
					{
						...
						if(rsTraKm != null)
						{
							...
							while(rsTraKm.next())
							{
								...
								Trakhuyenmai trakm = new Trakhuyenmai(trakmId, nppId, date, this.idDonhang, khoId);
