package geso.traphaco.center.beans.trakhuyenmai.imp;
...
import geso.traphaco.center.beans.trakhuyenmai.ITrakhuyenmai;
...
public class Trakhuyenmai implements ITrakhuyenmai
{
	...
	public boolean CreateTrakm(String[] masp, String[] soluong) 
	{
		...
		try 
		{
			...
			String trakmCurrent = "";
			query = "select SCOPE_IDENTITY() AS trakmId";
			...
			if(this.type.equals("3")) //tra sanpham
			{
				if(masp.length > 0)
				{
					for(int i = 0; i < masp.length; i++)
					{
						if(masp[i].length() > 0)
						{
							ResultSet rs = db.get("select pk_seq from erp_sanpham where ma='" + masp[i].trim() + "'");
							String pk_seq = "";
							if(rs != null) 
							{
								try
		...
		catch (Exception e) 
		{
			e.printStackTrace();
			db.update("rollback");
			this.msg = "Loi khi tao moi tra khuyen mai "+e.getMessage();
			return false;			
	...
	{		
		String query = "select a.soluong, a.dongia, b.pk_seq, b.MA, b.TEN from trakhuyenmai_sanpham a inner join erp_SANPHAM b on a.SanPham_FK = b.PK_SEQ where a.trakhuyenmai_fk = '" + this.id + "'";				
		ResultSet rs = db.get(query);
		List<ISanpham> splist = new ArrayList<ISanpham>();
		if(rs != null)
		{
