package geso.traphaco.center.beans.thongbao.imp;
...
public class Thongbao implements IThongbao
{
	...
	public void createRs() 
	{
		...
		query = "select distinct nhanvien.pk_seq, nhanvien.dangnhap, nhanvien.ten, nhanvien.dienthoai, 0 as trangthai " +
				"from nhanvien inner join  phamvihoatdong a on a.nhanvien_fk = nhanvien.pk_seq " +
		  		...
		  		" where  phanloai = 2 and nhanvien.pk_seq not in ( select nhanvien_fk from THONGBAO_NHANVIEN where thongbao_fk = '" + ( ( this.ID == null || this.ID.trim().length() <= 0 ) ? "0" : this.ID ) + "' )  ";
		...
		query = "select  distinct  a.PK_SEQ, a.maFAST as MANHANVIEN, a.TEN, a.DIACHI, isnull(a.DIENTHOAI, '') as dienthoai  " +
		...
		if(this.ddkdId.trim().length() > 0)
			query += " union select PK_SEQ, MAFAST as MANHANVIEN, TEN, DIACHI, isnull(DIENTHOAI, '') as DIENTHOAI from DAIDIENKINHDOANH where pk_seq in ( " + this.ddkdId + " ) ";
		
		System.out.println("List trình dược viên: "+query);
	...
	public boolean createTb() 
	{
		...
		{
			this.MSG = "Vui lòng chọn nhân viên hoặc trình dược viên nhận thông báo";
			return false;
		}
